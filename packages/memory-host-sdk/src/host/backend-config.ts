...
export interface ResolvedMemoryBackendConfig {
  backend: MemoryBackend;
  citations: MemoryCitationsMode;
  qmd?: ResolvedQmdConfig;
}

export interface ResolvedQmdCollection {
  name: string;
  path: string;
  pattern: string;
...
export interface ResolvedQmdUpdateConfig {
  intervalMs: number;
  debounceMs: number;
  onBoot: boolean;
...
export interface ResolvedQmdLimitsConfig {
  maxResults: number;
...
export interface ResolvedQmdSessionConfig {
  enabled: boolean;
  exportDir?: string;
  retentionDays?: number;
}

export interface ResolvedQmdMcporterConfig {
  enabled: boolean;
  serverName: string;
  startDaemon: boolean;
}

export interface ResolvedQmdConfig {
  command: string;
...
function resolveDefaultCollections(
  ...
  const entries: { path: string; pattern: string; base: string }[] = [
    { base: "memory-root", path: workspaceDir, pattern: "MEMORY.md" },
    { base: "memory-alt", path: workspaceDir, pattern: "memory.md" },
    { base: "memory-dir", path: path.join(workspaceDir, "memory"), pattern: "**/*.md" },
  ];
...
export function resolveMemoryBackendConfig(params: {
  ...
  const mergedExtraPaths = [
    ...
    .filter(Boolean);
  const dedupedExtraPaths = [...new Set(mergedExtraPaths)];
