...
describe("resolveMemoryBackendConfig", () => {
  it("defaults to builtin backend when config missing", () => {
    ...
    const resolved = resolveMemoryBackendConfig({ agentId: "main", cfg });
  ...
  it("scopes qmd collection names per agent", () => {
    const cfg = {
      agents: {
        defaults: { workspace: "/workspace/root" },
        list: [
          { default: true, id: "main", workspace: "/workspace/root" },
      ...
      memory: {
        ...
        qmd: {
          ...
          paths: [{ name: "workspace", path: "notes", pattern: "**/*.md" }],
        },
      },
    } as OpenClawConfig;
    const mainResolved = resolveMemoryBackendConfig({ agentId: "main", cfg });
    const devResolved = resolveMemoryBackendConfig({ agentId: "dev", cfg });
  ...
  it("preserves explicit custom collection names for paths outside the workspace", () => {
    const cfg = {
      ...
      memory: {
        ...
        qmd: {
          includeDefaultMemory: true,
          paths: [{ name: "notion-mirror", path: "/shared/notion-mirror", pattern: "**/*.md" }],
        },
      },
    } as OpenClawConfig;
    const mainResolved = resolveMemoryBackendConfig({ agentId: "main", cfg });
    ...
    try {
      ...
      const cfg = {
        ...
        memory: {
          ...
          qmd: {
            includeDefaultMemory: false,
            paths: [{ name: "workspace", path: workspaceAliasDir, pattern: "**/*.md" }],
          },
        },
      } as OpenClawConfig;
      const resolved = resolveMemoryBackendConfig({ agentId: "main", cfg });
      ...
      const cfg = {
        ...
        memory: {
          ...
          qmd: {
            includeDefaultMemory: false,
            paths: [
              { name: "notes", path: path.join(workspaceAliasDir, "notes"), pattern: "**/*.md" },
            ],
          },
        },
      } as OpenClawConfig;
...
describe("memorySearch.extraPaths integration", () => {
  it("maps agents.defaults.memorySearch.extraPaths to QMD collections", () => {
    ...
    const result = resolveMemoryBackendConfig({ agentId: "test-agent", cfg });
  ...
  it("merges default and per-agent memorySearch.extraPaths for QMD collections", () => {
    ...
    const result = resolveMemoryBackendConfig({ agentId: "my-agent", cfg });
    expect(result.backend).toBe("qmd");
    const customCollections = (result.qmd?.collections ?? []).filter(
      (collection) => collection.kind === "custom",
    );
  ...
  it("deduplicates identical roots shared by memory.qmd.paths and memorySearch.extraPaths", () => {
    const cfg = {
      ...
      memory: {
        backend: "qmd",
        qmd: {
          paths: [{ name: "workspace-docs", path: "docs", pattern: "**/*.md" }],
        },
      },
    } as OpenClawConfig;

    const result = resolveMemoryBackendConfig({ agentId: "main", cfg });
    const customCollections = (result.qmd?.collections ?? []).filter(
      (collection) => collection.kind === "custom",
    );
