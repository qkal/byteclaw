...
describe("runVoyageEmbeddingBatches", () => {
  ...
  const mockRequests: VoyageBatchRequest[] = [
    { body: { input: "text1" }, custom_id: "req-1" },
    { body: { input: "text2" }, custom_id: "req-2" },
  ];
  ...
  it("successfully submits batch, waits, and streams results", async () => {
    const outputLines: VoyageBatchOutputLine[] = [
      {
        custom_id: "req-1",
        response: { body: { data: [{ embedding: [0.1, 0.1] }] }, status_code: 200 },
      },
      {
        custom_id: "req-2",
        response: { body: { data: [{ embedding: [0.2, 0.2] }] }, status_code: 200 },
      },
    ];
