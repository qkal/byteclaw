...
export interface ExternalPluginCompatibility {
...
export interface ExternalPluginValidationIssue {
  fieldPath: string;
  message: string;
}

export interface ExternalCodePluginValidationResult {
...
function readOpenClawBlock(packageJson: unknown) {
  ...
  return { build, compat, install, openclaw, root };
}
