...
export function formatRelevantMemoriesContext(
  memories: { category: MemoryCategory; text: string }[],
): string {
      ...
      ({ program }) => {
        ...
        memory
          ...
          .action(async (query, opts) => {
            const vector = await embeddings.embed(query);
            const results = await db.search(vector, Number.parseInt(opts.limit), 0.3);
    ...
    if (cfg.autoRecall) {
      api.on("before_agent_start", async (event) => {
        ...
        try {
          ...
          api.logger.warn(`memory-lancedb: recall failed: ${String(error)}`);
        }
      });
    }
    ...
    if (cfg.autoCapture) {
      api.on("agent_end", async (event) => {
        ...
        try {
          ...
          api.logger.warn(`memory-lancedb: capture failed: ${String(error)}`);
        }
      });
    }
