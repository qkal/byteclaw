...
import { type QmdCollectionPatternFlag, resolveQmdCollectionPatternFlags } from "./qmd-compat.js";
...
interface SessionExporterConfig {
  dir: string;
  retentionMs?: number;
...
export class QmdMemoryManager implements MemorySearchManager {
  static async create(params: {
    ...
    const manager = new QmdMemoryManager({ agentId: params.agentId, cfg: params.cfg, resolved });
  ...
  private async initialize(mode: QmdManagerMode): Promise<void> {
    ...
    if (this.qmd.update.onBoot) {
      ...
      if (this.qmd.update.waitForBootSync) {
        await bootRun.catch((error) => {
          log.warn(`qmd boot update failed: ${String(error)}`);
        });
      } else {
        void bootRun.catch((error) => {
          log.warn(`qmd boot update failed: ${String(error)}`);
        });
      }
    }
    if (this.qmd.update.intervalMs > 0) {
      this.updateTimer = setInterval(() => {
        void this.runUpdate("interval").catch((error) => {
          log.warn(`qmd update failed (${String(error)})`);
        });
    ...
    if (this.shouldScheduleEmbedTimer()) {
      const startPeriodicEmbedTimer = () => {
        this.embedTimer = setInterval(() => {
          void this.runUpdate("embed-interval").catch((error) => {
            log.warn(`qmd embed interval update failed (${String(error)})`);
          });
  ...
  private bootstrapCollections(): void {
    ...
    for (const collection of this.qmd.collections) {
      ...
      this.collectionRoots.set(collection.name, { kind, path: collection.path });
  ...
  private async tryRebindConflictingCollection(params: {
    ...
    try {
      ...
      const removeMessage = formatErrorMessage(error);
    ...
    try {
      ...
      const retryMessage = formatErrorMessage(error);
      log.warn(
  ...
  private async addCollection(pathArg: string, name: string, pattern: string): Promise<void> {
    ...
    for (const flag of candidateFlags) {
      try {
        ...
        lastError = error;
        if (!this.isUnsupportedQmdOptionError(error) || candidateFlags.at(-1) === flag) {
          throw error;
        }
  ...
  private async rebuildManagedCollectionsForRepair(reason: string): Promise<void> {
    for (const collection of this.qmd.collections) {
      ...
      try {
        ...
        const addMessage = formatErrorMessage(error);
  ...
  async search(
    ...
    const runSearchAttempt = async (
      ...
      try {
        ...
        if (allowMissingCollectionRepair && this.isMissingCollectionSearchError(error)) {
          throw error;
        }
        if (
          !mcporterEnabled &&
          qmdSearchCommand !== "query" &&
          this.isUnsupportedQmdOptionError(error)
        ) {
          log.warn(
          ...
          try {
            ...
            log.warn(`qmd query fallback failed: ${String(error)}`);
    ...
    try {
      ...
      if (!(await this.tryRepairMissingCollectionSearch(error))) {
        throw error instanceof Error ? error : new Error(String(error));
      }
  ...
  async sync(params?: {
    ...
    if (params?.progress) {
      params.progress({ completed: 0, label: "Updating QMD index…", total: 1 });
    }
    await this.runUpdate(params?.reason ?? "manual", params?.force);
    if (params?.progress) {
      params.progress({ completed: 1, label: "QMD index updated", total: 1 });
    }
  }
  ...
  async readFile(params: {
    ...
    if (params.from !== undefined || params.lines !== undefined) {
      ...
      return { path: relPath, text: partial.text };
    }
    ...
    if (!params.from && !params.lines) {
      return { path: relPath, text: full.text };
    }
  ...
  async probeVectorAvailability(): Promise<boolean> {
    try {
      const result = await this.runQmd(["status"], {
        timeoutMs: Math.min(this.qmd.limits.timeoutMs, 5000),
      });
    ...
    const run = async () => {
      ...
      if (this.shouldRunEmbed(force)) {
        try {
          ...
          this.noteEmbedFailure(reason, error);
        }
      }
  ...
  private scheduleWatchSync(): void {
    ...
    this.watchTimer = setTimeout(() => {
      ...
      void this.sync({ reason: "watch" }).catch((error) => {
        log.warn(`qmd watch sync failed: ${String(error)}`);
      });
  ...
  private async maybeWarmSession(sessionKey?: string): Promise<void> {
    ...
    void this.sync({ reason: "session-start" }).catch((error) => {
      log.warn(`qmd session-start sync failed: ${String(error)}`);
    });
  }
  ...
  private async runQmdUpdateWithRetry(reason: string): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      try {
        ...
        if (attempt >= maxAttempts || !this.isRetryableUpdateError(error)) {
          throw error;
        }
        const delayMs = 500 * 2 ** (attempt - 1);
        log.warn(
          `qmd update retry ${attempt}/${maxAttempts - 1} after failure (${reason}): ${String(error)}`,
        );
  ...
  private async runQmdUpdateOnce(reason: string): Promise<void> {
    try {
      ...
      if (
        !(await this.tryRepairNullByteCollections(error, reason)) &&
        !(await this.tryRepairDuplicateDocumentConstraint(error, reason))
      ) {
        throw error;
      }
      await this.runQmd(["update"], {
  ...
  private async symlinkSharedModels(): Promise<void> {
    ...
    try {
      ...
      const stat = await fs.stat(defaultModelsDir).catch((error: unknown) => {
      ...
      log.warn(`failed to symlink qmd models directory: ${String(error)}`);
    }
  }
  ...
  private buildV2Searches(
    ...
    switch (searchCommand) {
      case "search": {
        ...
        return [{ query, type: "lex" }];
      }
      case "vsearch": {
        // Vector search only
        return [{ query, type: "vec" }];
      }
      case "query":
      case undefined:
      default: {
    ...
    if (!state.daemonStart) {
      state.daemonStart = (async () => {
        try {
          ...
          log.warn(`mcporter daemon start failed: ${String(error)}`);
  ...
  private async runQmdSearchViaMcporter(
    ...
    try {
      result = await this.runMcporter(
        ...
        { timeoutMs: Math.max(params.timeoutMs + 2000, 5000) },
      );
      ...
      if (useUnifiedQueryTool && this.isQueryToolNotFoundError(error)) {
  ...
  private async resolveDocLocation(
    ...
    try {
      ...
      if (this.isSqliteBusyError(error)) {
        log.debug(`qmd index is busy while resolving doc path: ${String(error)}`);
  ...
  private resolveIndexedDocLocationFromHint(
    ...
    try {
      ...
      const exactRows = db
        ...
        .all(trimmedCollection, exactPath) as { path: string }[];
      ...
      rows = db
        ...
        .all(trimmedCollection) as { path: string }[];
    } catch (error) {
      if (this.isSqliteBusyError(error)) {
        log.debug(`qmd index is busy while resolving hinted path: ${String(error)}`);
        throw this.createQmdBusyError(error);
      }
      // Hint-based lookup is best effort. Fall back to the raw hinted path when
      // The index is unavailable or still warming.
      log.debug(`qmd index hint lookup skipped: ${String(error)}`);
      return null;
    }
  ...
  private pickDocLocation(
    rows: { collection: string; path: string }[],
    hints?: { preferredCollection?: string; preferredFile?: string },
  ): { rel: string; abs: string; source: MemorySource } | null {
  ...
  private extractSnippetLines(snippet: string): { startLine: number; endLine: number } {
    ...
    return { endLine: lines, startLine: 1 };
  }
  ...
  private resolveSnippetLines(
    ...
    if (explicitStart !== undefined && explicitEnd !== undefined) {
      return explicitStart <= explicitEnd
        ? { endLine: explicitEnd, startLine: explicitStart }
        : { endLine: explicitStart, startLine: explicitEnd };
    }
    if (explicitStart !== undefined) {
      ...
      return { endLine: explicitStart, startLine: explicitStart };
    }
    if (explicitEnd !== undefined) {
      ...
      return { endLine: explicitEnd, startLine: explicitEnd };
    }
    if (headerLines) {
      return headerLines;
    }
    return { endLine: snippet.split("\n").length, startLine: 1 };
  }
  ...
  private parseSnippetHeaderLines(snippet: string): { startLine: number; endLine: number } | null {
    ...
    if (Number.isFinite(start) && Number.isFinite(count)) {
      return { endLine: start + count - 1, startLine: start };
    }
    return null;
  }
  ...
  private readCounts(): {
    ...
    sourceCounts: { source: MemorySource; files: number; chunks: number }[];
  } {
    try {
      const db = this.ensureDb();
      const rows = db
      ...
      for (const source of this.sources) {
        bySource.set(source, { chunks: 0, files: 0 });
      }
      let total = 0;
      for (const row of rows) {
        const root = this.collectionRoots.get(row.collection);
        const source = root?.kind ?? "memory";
        const entry = bySource.get(source) ?? { chunks: 0, files: 0 };
        entry.files += row.c ?? 0;
      ...
      return {
        sourceCounts: [...bySource.entries()].map(([source, value]) => ({
          chunks: value.chunks,
          files: value.files,
          source,
        })),
      ...
      log.warn(`failed to read qmd index stats: ${String(error)}`);
      return {
        sourceCounts: [...this.sources].map((source) => ({ chunks: 0, files: 0, source })),
        totalDocuments: 0,
      };
    }
  }
  ...
  private toDocLocation(
    ...
    return { abs: absPath, rel: relPath, source: root.kind };
  }
  ...
  private diversifyResultsBySource(
    ...
    const sourceOrder = [...bySource.entries()]
