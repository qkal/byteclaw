...
export abstract class MemoryManagerEmbeddingOps extends MemoryManagerSyncOps {
    ...
    for (const batch of batches) {
      ...
      for (let i = 0; i < batch.length; i += 1) {
        ...
        if (item) {
          ...
          toCache.push({ embedding, hash: item.chunk.hash });
        }
      }
      cursor += batch.length;
    }
  ...
  protected computeProviderKey(): string {
    ...
    if (!this.provider) {
      return hashText(JSON.stringify({ model: "fts-only", provider: "none" }));
    }
    if (this.providerRuntime?.cacheKeyData) {
      return hashText(JSON.stringify(this.providerRuntime.cacheKeyData));
    }
    return hashText(JSON.stringify({ model: this.provider.model, provider: this.provider.id }));
  }
  ...
  private buildBatchDebug(source: MemorySource, chunks: MemoryChunk[]) {
    return (message: string, data?: Record<string, unknown>) =>
      log.debug(
        message,
        data ? { ...data, chunks: chunks.length, source } : { chunks: chunks.length, source },
      );
  }
  ...
  private collectCachedEmbeddings(chunks: MemoryChunk[]): {
    embeddings: number[][];
    missing: { index: number; chunk: MemoryChunk }[];
  } {
  ...
  private async recordBatchFailure(params: {
    ...
    return await this.withBatchFailureLock(async () => {
      ...
      this.batchFailureLastError = nextState.lastError;
      this.batchFailureLastProvider = nextState.lastProvider;
      return { count: nextState.count, disabled: !nextState.enabled };
    });
  }
    ...
    try {
      ...
      if (this.isBatchTimeoutError(message)) {
        ...
        try {
          ...
          (error as { batchAttempts?: number }).batchAttempts = 2;
          throw error;
        }
      }
      throw error;
  ...
  private async runBatchWithFallback<T>(params: {
    ...
    try {
      ...
      const message = formatErrorMessage(error);
      const attempts = (error as { batchAttempts?: number }).batchAttempts ?? 1;
  ...
  protected async indexFile(
    ...
    if (!this.provider) {
      ...
      const content = options.content ?? (await fs.readFile(entry.absPath, "utf8"));
    ...
    if ("kind" in entry && entry.kind === "multimodal") {
      ...
      ({ structuredInputBytes } = multimodalChunk);
