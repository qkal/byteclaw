...
describe("QmdMemoryManager", () => {
    ...
    cfg = {
      agents: {
        defaults: {
          memorySearch: {
            ...
            provider: "openai",
            store: { path: path.join(workspaceDir, "index.sqlite"), vector: { enabled: false } },
            sync: { onSearch: false, onSessionStart: true, watch: false },
          },
          workspace: workspaceDir,
        },
        list: [{ default: true, id: agentId, workspace: workspaceDir }],
      ...
      memory: {
        ...
        qmd: {
          includeDefaultMemory: false,
          paths: [{ name: "workspace", path: workspaceDir, pattern: "**/*.md" }],
          update: { debounceMs: 0, interval: "0s", onBoot: false },
        },
      },
    } as OpenClawConfig;
    ...
    cfg = {
      ...
      memory: {
        ...
        qmd: {
          includeDefaultMemory: false,
          paths: [{ name: "workspace", path: workspaceDir, pattern: "**/*.md" }],
          update: { debounceMs: 60_000, interval: "0s", onBoot: true },
        },
      },
    } as OpenClawConfig;
    ...
    cfg = {
      ...
      memory: {
        ...
        qmd: {
          includeDefaultMemory: false,
          paths: [{ name: "workspace", path: workspaceDir, pattern: "**/*.md" }],
          update: { debounceMs: 60_000, interval: "5m", onBoot: true },
        },
      },
    } as OpenClawConfig;
  ...
  it("can be configured to block startup on boot update", async () => {
    ...
    const createPromise = QmdMemoryManager.create({ agentId, cfg, mode: "full", resolved });
  ...
  it("rebinds sessions collection when existing collection path targets another agent", async () => {
    ...
    cfg = {
      ...
      agents: {
        list: [
          { default: true, id: agentId, workspace: workspaceDir },
      ...
      memory: {
        ...
        qmd: {
          includeDefaultMemory: false,
          paths: [{ name: "workspace", path: devWorkspaceDir, pattern: "**/*.md" }],
    ...
    spawnMock.mockImplementation((_cmd: string, args: string[]) => {
      if (args[0] === "collection" && args[1] === "list") {
        ...
        emitAndClose(
          ...
          "stdout",
          JSON.stringify([
            { mask: "**/*.md", name: sessionCollectionName, path: wrongSessionsPath },
          ]),
        );
        return child;
      }
    ...
    const resolved = resolveMemoryBackendConfig({ agentId: devAgentId, cfg });
    const manager = trackManager(
      await QmdMemoryManager.create({
        agentId: devAgentId,
        cfg,
    ...
    const addSessions = commands.find((args) => {
      ...
      return nameIdx !== -1 && args[nameIdx + 1] === sessionCollectionName;
    });
  ...
  it("rebinds collection when qmd text output exposes a changed pattern without a path", async () => {
    ...
    const addCall = commands.find((args) => {
      ...
      const nameIdx = args.indexOf("--name");
      return nameIdx !== -1 && args[nameIdx + 1] === "workspace-main";
    });
    ...
    spawnMock.mockImplementation((_cmd: string, args: string[]) => {
      if (args[0] === "collection" && args[1] === "list") {
        ...
        emitAndClose(
          child,
          "stdout",
          JSON.stringify([{ mask: "MEMORY.md", name: "memory-root", path: differentPath }]),
        );
        return child;
      }
      if (args[0] === "collection" && args[1] === "remove") {
  ...
  it("times out qmd update during sync when configured", async () => {
    ...
    const resolved = resolveMemoryBackendConfig({ agentId, cfg });
    const createPromise = QmdMemoryManager.create({ agentId, cfg, mode: "status", resolved });
      ...
      if (args[0] === "update") {
        ...
        if (updateCalls === 1) {
          emitAndClose(
            child,
            "stderr",
            String.raw`ENOENT: no such file or directory, open '/tmp/workspace/MEMORY.md\x00'`,
            1,
          );
          return child;
        }
  ...
  it("repairs missing managed collections and retries search once", async () => {
    ...
    spawnMock.mockImplementation((_cmd: string, args: string[]) => {
      ...
      if (args[0] === "search") {
        ...
        const collection = collectionFlagIndex !== -1 ? args[collectionFlagIndex + 1] : "";
  ...
  it("resolves bare qmd command to a Windows-compatible spawn invocation", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({ bin: { qmd: "dist/cli.js" }, name: "qmd", version: "0.0.0" }),
        "utf8",
      );
  ...
  it("queues a forced sync behind an in-flight update", async () => {
    ...
    const forced = manager.sync({ force: true, reason: "manual" });
  ...
  it("honors multiple forced sync requests while forced queue is active", async () => {
    ...
    const inFlight = manager.sync({ reason: "interval" });
    const forcedOne = manager.sync({ force: true, reason: "manual" });
    ...
    const forcedTwo = manager.sync({ force: true, reason: "manual-again" });
    ...
    cfg = {
      ...
      memory: {
        ...
        qmd: {
          includeDefaultMemory: false,
          paths: [
            { name: "workspace", path: workspaceDir, pattern: "**/*.md" },
            { name: "notes", path: path.join(workspaceDir, "notes"), pattern: "**/*.md" },
          ],
          update: { debounceMs: 60_000, interval: "0s", onBoot: false },
        },
      },
  ...
  it("uses explicit external custom collection names verbatim at query time", async () => {
    ...
    cfg = {
      ...
      memory: {
        ...
        qmd: {
          includeDefaultMemory: false,
          paths: [{ name: "notion-mirror", path: sharedMirrorDir, pattern: "**/*.md" }],
          update: { debounceMs: 60_000, interval: "0s", onBoot: false },
        },
      },
    } as OpenClawConfig;
  ...
  it('uses unified v2 args when the explicit mcporter search tool override is "query"', async () => {
    ...
    spawnMock.mockImplementation((cmd: string, args: string[]) => {
      ...
      if (isMcporterCommand(cmd) && args[0] === "call") {
        expect(args[1]).toBe("qmd.query");
        const callArgs = JSON.parse(args[args.indexOf("--args") + 1]);
        expect(callArgs).toHaveProperty("searches", [{ query: "hello", type: "lex" }]);
        expect(callArgs).toHaveProperty("collections", ["workspace-main"]);
        expect(callArgs).not.toHaveProperty("query");
        expect(callArgs).not.toHaveProperty("minScore");
        expect(callArgs).not.toHaveProperty("collection");
    ...
    cfg = {
      ...
      memory: {
        ...
        qmd: {
          ...
          mcporter: { enabled: true, serverName: "qmd", startDaemon: false },
          paths: [
            { name: "workspace-a", path: path.join(workspaceDir, "notes-a"), pattern: "**/*.md" },
            { name: "workspace-b", path: path.join(workspaceDir, "notes-b"), pattern: "**/*.md" },
          ],
          searchMode: "search",
          searchTool: "query",
          update: { debounceMs: 60_000, interval: "0s", onBoot: false },
  ...
  it("resolves mcporter to a direct Windows entrypoint without enabling shell mode", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(packageDir, "package.json"),
        JSON.stringify({ bin: { mcporter: "dist/cli.js" }, name: "mcporter", version: "0.0.0" }),
        "utf8",
      );
      await fs.writeFile(scriptPath, "module.exports = {};\n", "utf8");
      process.env.PATH = `${shimDir};${previousPath ?? ""}`;
  ...
  it("serializes qmd embeds within a process before taking the shared file lock", async () => {
    ...
    const firstSync = first.manager.sync({ force: true, reason: "manual" });
    await vi.advanceTimersByTimeAsync(0);
    ...
    const secondSync = second.manager.sync({ force: true, reason: "manual" });
    await vi.advanceTimersByTimeAsync(0);
    expect(embedChildren).toHaveLength(1);
  ...
  it("runs qmd embed in search mode for forced sync", async () => {
    ...
    const { manager } = await createManager({ mode: "status" });
    await manager.sync({ force: true, reason: "manual" });
  ...
  it("blocks non-markdown or symlink reads for qmd paths", async () => {
    ...
    await fs.writeFile(textPath, "nope", "utf8");
  ...
  it("reads only requested line ranges without loading the whole file", async () => {
    ...
    await fs.writeFile(path.join(workspaceDir, "window.md"), text, "utf8");

    const { manager } = await createManager();

    const result = await manager.readFile({ from: 10, lines: 3, relPath: "window.md" });
  ...
  it("returns empty text when qmd files are missing before or during read", async () => {
    ...
    const cases = [
      ...
      },
      {
        ...
        request: { from: 2, lines: 1, relPath },
      },
    ] as const;
    ...
    for (const testCase of cases) {
      ...
      try {
        ...
        expect(result, testCase.name).toEqual({ path: testCase.expectedPath, text: "" });
      } finally {
        restoreOpen?.();
        await manager.close();
      }
  ...
  it("reuses exported session markdown files when inputs are unchanged", async () => {
    ...
    try {
      await manager.sync({ reason: "manual" });
      const firstExport = await fs.readFile(exportFile, "utf8");
      expect(firstExport).toContain("hello");

      await manager.sync({ reason: "manual" });
      const secondExport = await fs.readFile(exportFile, "utf8");
  ...
  it("treats plain-text no-results markers from stdout/stderr as empty result sets", async () => {
    const cases = [
      { name: "stdout with punctuation", payload: "No results found.", stream: "stdout" },
      { name: "stdout without punctuation", payload: "No results found\n\n", stream: "stdout" },
      { name: "stderr", payload: "No results found.\n", stream: "stderr" },
    ] as const;
      ...
      const cases: {
      ...
      }[] = [
        {
          assert: async () => {
            ...
            const content = await fs.readFile(path.join(customModelsDir, "model.bin"), "utf8");
        ...
        },
        {
          ...
          setup: async () => {
            await fs.rm(defaultModelsDir, { force: true, recursive: true });
          },
        },
      ];

      for (const testCase of cases) {
        await fs.rm(customModelsDir, { force: true, recursive: true });
        await fs.mkdir(defaultModelsDir, { recursive: true });
        await fs.writeFile(path.join(defaultModelsDir, "model.bin"), "fake-model");
        logWarnMock.mockClear();
