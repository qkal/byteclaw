...
interface MemoryEmbeddingTextPart {
  type: "text";
  text: string;
}

interface MemoryEmbeddingInlineDataPart {
  type: "inline-data";
  mimeType: string;
  data: string;
}
...
interface MemoryEmbeddingInput {
  text: string;
  parts?: (MemoryEmbeddingTextPart | MemoryEmbeddingInlineDataPart)[];
}

interface MemoryEmbeddingChunk {
  text: string;
