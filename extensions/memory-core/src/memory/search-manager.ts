...
interface MemorySearchManagerCacheStore {
...
export async function getMemorySearchManager(params: {
  ...
  try {
    ...
    const message = formatErrorMessage(error);
    return { error: message, manager: null };
  }
}
...
export async function closeAllMemorySearchManagers(): Promise<void> {
  const managers = [...QMD_MANAGER_CACHE.values()];
  QMD_MANAGER_CACHE.clear();
  for (const manager of managers) {
    try {
      await manager.close?.();
    } catch (error) {
      log.warn(`failed to close qmd memory manager: ${String(error)}`);
    }
  }
...
class FallbackMemoryManager implements MemorySearchManager {
  ...
  async probeEmbeddingAvailability(): Promise<MemoryEmbeddingProbeResult> {
    ...
    return { error: this.lastError ?? "memory embeddings unavailable", ok: false };
  }
