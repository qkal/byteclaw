...
interface StartupCronSourceRefs {
  context: Record<string, unknown>;
  deps: Record<string, unknown> | null;
}

export interface ShortTermPromotionDreamingConfig {
  enabled: boolean;
  cron: string;
  timezone?: string;
  limit: number;
...
async function migrateLegacyPhaseDreamingCronJobs(params: {
  ...
  for (const job of params.legacyJobs) {
    try {
      ...
      params.logger.warn(
        `memory-core: failed to migrate legacy phase dreaming cron job ${job.id}: ${formatErrorMessage(error)}`,
      );
    }
  }
  if (migrated > 0) {
...
export async function reconcileShortTermDreamingCronJob(params: {
  ...
  if (!params.config.enabled) {
    ...
    for (const job of managed) {
      try {
        ...
        params.logger.warn(
          `memory-core: failed to remove managed dreaming cron job ${job.id}: ${formatErrorMessage(error)}`,
        );
      }
    }
    if (removed > 0) {
  ...
  if (managed.length === 0) {
    ...
    return { removed: migratedLegacy, status: "added" };
  }
  ...
  for (const duplicate of duplicates) {
    try {
      ...
      params.logger.warn(
        `memory-core: failed to prune duplicate managed dreaming cron job ${duplicate.id}: ${formatErrorMessage(error)}`,
      );
    }
  }
...
export async function runShortTermDreamingPromotionIfTriggered(params: {
  ...
  for (const workspaceDir of workspaces) {
    try {
      ...
      failedWorkspaces += 1;
      params.logger.error(
        `memory-core: dreaming promotion failed for workspace ${workspaceDir}: ${formatErrorMessage(error)}`,
      );
    }
  }
  params.logger.info(
...
export function registerShortTermPromotionDreaming(api: OpenClawPluginApi): void {
  ...
  api.registerHook(
    ...
    async (event: unknown) => {
      try {
        ...
        api.logger.error(
          `memory-core: dreaming startup reconciliation failed: ${formatErrorMessage(error)}`,
        );
      }
    },
  ...
  api.on("before_agent_reply", async (event, ctx) => {
    try {
      ...
      api.logger.error(`memory-core: dreaming trigger failed: ${formatErrorMessage(error)}`);
      return undefined;
    }
  });
}
