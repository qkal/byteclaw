...
export interface NarrativePhaseData {
...
async function startNarrativeRunOrFallback(params: {
  ...
  try {
    ...
    if (!isRequestScopedSubagentRuntimeError(error)) {
      throw error;
    }
    try {
      ...
      params.logger.warn(
        `memory-core: narrative fallback failed for ${params.data.phase} phase (${formatFallbackWriteFailure(error)})`,
      );
    }
    return null;
  }
...
async function readDreamsFile(dreamsPath: string): Promise<string> {
  try {
    return await fs.readFile(dreamsPath, "utf8");
  } catch (error) {
    if ((error as NodeJS.ErrnoException)?.code === "ENOENT") {
      return "";
    }
...
function stripBackfillDiaryBlocks(existing: string): { updated: string; removed: number } {
  ...
  const endIdx = ensured.indexOf(DIARY_END_MARKER);
  if (startIdx === -1 || endIdx === -1 || endIdx < startIdx) {
    return { removed: 0, updated: ensured };
  }
...
async function assertSafeDreamsPath(dreamsPath: string): Promise<void> {
  const stat = await fs.lstat(dreamsPath).catch((error: NodeJS.ErrnoException) => {
...
async function writeDreamsFileAtomic(dreamsPath: string, content: string): Promise<void> {
  ...
  const existing = await fs.stat(dreamsPath).catch((error: NodeJS.ErrnoException) => {
    if (error.code === "ENOENT") {
      return null;
    }
    throw error;
  ...
  try {
    ...
    const cleanupError = await fs.rm(tempPath, { force: true }).catch((error) => error);
    if (cleanupError) {
      throw new Error(
        `Atomic DREAMS.md write failed (${formatErrorMessage(error)}); cleanup also failed (${formatErrorMessage(cleanupError)})`,
        { cause: error },
      );
    }
    throw error;
...
export async function appendNarrativeEntry(params: {
  ...
  let existing = "";
  try {
    existing = await fs.readFile(dreamsPath, "utf8");
  } catch (error) {
    if ((error as NodeJS.ErrnoException)?.code !== "ENOENT") {
      throw error;
    }
