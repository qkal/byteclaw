...
function createHarness(config: OpenClawConfig, workspaceDir?: string) {
  ...
  const beforeAgentReply = async (
    ...
    const light = resolveMemoryLightDreamingConfig({ cfg: resolvedConfig, pluginConfig });
    ...
    const rem = resolveMemoryRemDreamingConfig({ cfg: resolvedConfig, pluginConfig });
...
describe("memory-core dreaming phases", () => {
  ...
  it("checkpoints session transcript ingestion and skips unchanged transcripts", async () => {
    ...
    await fs.writeFile(
      ...
      [
        ...
        JSON.stringify({
          message: {
            content: [{ text: "Move backups to S3 Glacier.", type: "text" }],
            role: "user",
        ...
        JSON.stringify({
          message: {
            content: [{ text: "Set retention to 365 days.", type: "text" }],
            role: "assistant",
  ...
  it("redacts sensitive session content before writing session corpus", async () => {
    ...
    await fs.writeFile(
      ...
      [
        JSON.stringify({
          message: {
            content: [{ text: "OPENAI_API_KEY=sk-1234567890abcdef", type: "text" }],
            role: "user",
            timestamp: "2026-04-05T18:01:00.000Z",
          },
          type: "message",
  ...
  it("dedupes reset/deleted session archives instead of double-ingesting", async () => {
    ...
    await fs.writeFile(
      ...
      [
        JSON.stringify({
          message: {
            content: [{ text: oldMessage, type: "text" }],
            role: "user",
            timestamp: "2026-04-05T18:01:00.000Z",
          },
          type: "message",
    ...
    try {
      ...
      await fs.writeFile(resetPath, await fs.readFile(transcriptPath, "utf8"), "utf8");
      ...
      await fs.writeFile(
        ...
        [
          ...
          JSON.stringify({
            message: {
              content: [{ text: newMessage, type: "text" }],
              role: "assistant",
    ...
    for (const fileName of corpusFiles) {
      combinedCorpus += `${await fs.readFile(path.join(sessionCorpusDir, fileName), "utf8")}\n`;
    }
  ...
  it("buckets session snippets by per-message day rather than file mtime", async () => {
    ...
    await fs.writeFile(
      ...
      [
        JSON.stringify({
          message: {
            content: [
              { text: "Old planning note that should stay out of lookback.", type: "text" },
            ],
            role: "user",
        ...
        JSON.stringify({
          message: {
            content: [{ text: "Current reminder that should be in today corpus.", type: "text" }],
            role: "assistant",
            timestamp: "2026-04-05T18:02:00.000Z",
          },
          type: "message",
    ...
    const dayCorpus = await fs.readFile(path.join(corpusDir, "2026-04-05.txt"), "utf8");
  ...
  it("drains >80 unseen transcript messages across multiple unchanged sweeps", async () => {
    ...
    for (let index = 0; index < 160; index += 1) {
      lines.push(
        JSON.stringify({
          message: {
            content: [{ text: `bulk-line-${index}`, type: "text" }],
    ...
    try {
      ...
      await fs.writeFile(
        ...
        [
          JSON.stringify({
            message: {
              content: [{ text: "Retention policy stays at 365 days.", type: "text" }],
              role: "assistant",
              timestamp: "2026-04-06T01:02:00.000Z",
            },
            type: "message",
  ...
  it("ingests sessions when dreaming is enabled even if memorySearch is disabled", async () => {
    ...
    await fs.writeFile(
      ...
      [
        JSON.stringify({
          message: {
            content: [{ text: "Glacier archive migration is now complete.", type: "text" }],
            role: "user",
            timestamp: "2026-04-05T18:01:00.000Z",
          },
          type: "message",
  ...
  it("records light/rem signals that reinforce deep promotion ranking", async () => {
    ...
    const phaseSignalStore = JSON.parse(await fs.readFile(phaseSignalPath, "utf8")) as {
