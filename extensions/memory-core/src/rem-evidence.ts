...
export interface GroundedRemPreviewItem {
  text: string;
  refs: string[];
}
...
export interface GroundedRemFilePreview {
  path: string;
...
export interface GroundedRemPreviewResult {
  workspaceDir: string;
  scannedFiles: number;
...
function sectionToSnippets(section: ParsedMarkdownSection): SectionSnippet[] {
  ...
  for (const entry of section.lines) {
    ...
    seen.add(dedupeKey);
    snippets.push({ line: entry.line, text });
  }
  return snippets;
}
...
function chooseFactSnippets(
  ...
  return [...snippets]
    .map((snippet) => {
      ...
      return { score, snippet: { ...snippet, text } };
    })
    .filter(
      (entry) =>
...
function atomizeClaimText(text: string): string[] {
  ...
  const atomic = splitTopLevelClauses(normalized, ";")
    ...
    .filter(Boolean);
  return [...new Set(atomic)].slice(0, 3);
}
...
function addReflection(
  ...
  seen.add(key);
  reflections.push({ refs, text: normalized });
}
...
function previewGroundedRemForFile(params: {
  ...
  const candidates = candidateSnippets
    .toSorted((left, right) => {
      const leanRank = { likely_durable: 0, likely_situational: 2, unclear: 1 };
  ...
  const durableImplications = candidateSnippets
    ...
    .slice(0, REM_SUMMARY_MEMORY_LIMIT)
    .map((candidate) => ({ refs: candidate.refs, text: candidate.text }));
  ...
  for (const summary of factSummaries.toSorted((left, right) => right.score - left.score)) {
    ...
    facts.push({ refs: summary.refs, text: summary.text });
...
export async function previewGroundedRemMarkdown(params: {
  ...
  for (const filePath of files) {
    ...
    previews.push(previewGroundedRemForFile({ content, relPath }));
  }
  return {
    files: previews,
