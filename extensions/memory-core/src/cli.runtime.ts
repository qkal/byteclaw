...
interface MemorySourceScan {
  sources: SourceScan[];
  totalFiles: number | null;
  issues: string[];
}

interface LoadedMemoryCommandConfig {
...
function formatDreamingSummary(cfg: OpenClawConfig): string {
  ...
  const dreaming = resolveShortTermPromotionDreamingConfig({ cfg, pluginConfig });
  if (!dreaming.enabled) {
    return "off";
  }
...
function buildCliMemorySearchSessionKey(agentId: string): string {
  return buildAgentSessionKey({
    ...
    peer: { id: "memory-search", kind: "direct" },
  });
}
...
async function scanSessionFiles(agentId: string): Promise<SourceScan> {
  ...
  try {
    ...
    return { issues, source: "sessions", totalFiles: null };
  }
}
...
async function scanMemoryFiles(
  ...
  return { issues, source: "memory", totalFiles };
}
...
async function scanMemorySources(params: {
  ...
  return { issues, sources: scans, totalFiles };
}
  ...
  for (const agentId of agentIds) {
    ...
    await withMemoryManagerForAgent({
      ...
      run: async (manager) => {
            ...
            await withProgressTotals(
              ...
              async (update, progress) => {
                try {
                  ...
                  indexError = formatErrorMessage(error);
        ...
        allResults.push({ agentId, audit, embeddingProbe, indexError, repair, scan, status });
      },
    });
  }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      try {
        ...
        defaultRuntime.error(`Memory promote ranking failed: ${formatErrorMessage(error)}`);
        process.exitCode = 1;
        return;
      }
      ...
      if (opts.apply) {
        try {
          ...
          defaultRuntime.error(`Memory promote apply failed: ${formatErrorMessage(error)}`);
          process.exitCode = 1;
          return;
        }
      }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      try {
        ...
        defaultRuntime.error(`Memory promote-explain failed: ${formatErrorMessage(error)}`);
        process.exitCode = 1;
        return;
      }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      if (opts.path) {
        ...
        ({ workspaceDir } = historical);
        cleanupWorkspaceDir = historical.workspaceDir;
        ({ sourceFiles } = historical);
        groundedInputPaths = historical.workspaceSourceFiles;
        ({ importedFileCount } = historical);
        ({ importedSignalCount } = historical);
        ({ skippedPaths } = historical);
        if (sourceFiles.length === 0) {
          await fs.rm(historical.workspaceDir, { force: true, recursive: true });
          defaultRuntime.error(
      ...
      try {
        ...
        const recallEntries = (await readShortTermRecallEntries({ nowMs, workspaceDir })).filter(
        ...
        if (cleanupWorkspaceDir) {
          await fs.rm(cleanupWorkspaceDir, { force: true, recursive: true });
        }
      }
    },
  });
