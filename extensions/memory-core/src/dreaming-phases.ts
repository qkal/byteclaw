...
import { type NarrativePhaseData, generateAndAppendDreamNarrative } from "./dreaming-narrative.js";
...
interface DreamingPhaseStorageConfig {
  timezone?: string;
...
interface DailyIngestionBatch {
  day: string;
  results: MemorySearchResult[];
}

interface DailyIngestionFileState {
  mtimeMs: number;
  size: number;
}

interface DailyIngestionState {
  version: 1;
...
interface SessionIngestionFileState {
  mtimeMs: number;
  size: number;
  contentHash: string;
  lineCount: number;
...
interface SessionIngestionState {
  version: 3;
  files: Record<string, SessionIngestionFileState>;
  seenMessages: Record<string, string[]>;
}

interface SessionIngestionMessage {
  day: string;
  snippet: string;
  rendered: string;
}

interface SessionIngestionCollectionResult {
...
function normalizeSessionIngestionState(raw: unknown): SessionIngestionState {
  ...
  return { files, seenMessages, version: 3 };
}
...
async function collectSessionIngestionBatches(params: {
  ...
  if (!params.cfg) {
    return {
      ...
      nextState: { files: {}, seenMessages: {}, version: 3 },
    };
  }
  ...
  const sessionFiles: { agentId: string; absolutePath: string; sessionPath: string }[] = [];
  ...
  for (const file of sortedFiles) {
    ...
    const stat = await fs.stat(file.absolutePath).catch((error: unknown) => {
      if ((error as NodeJS.ErrnoException)?.code === "ENOENT") {
        return null;
      }
      throw error;
    ...
    for (let index = cursor; index < lines.length; index += 1) {
      ...
      bucket.push({ day, rendered, snippet });
  ...
  return {
    batches,
    changed,
    nextState: { files: nextFiles, seenMessages: trimmedSeenMessages, version: 3 },
  };
}
...
interface DailyIngestionCollectionResult {
  batches: DailyIngestionBatch[];
...
async function collectDailyIngestionBatches(params: {
  ...
  const cutoffMs = calculateLookbackCutoffMs(params.nowMs, params.lookbackDays);
  const entries = await fs.readdir(memoryDir, { withFileTypes: true }).catch((error: unknown) => {
    if ((error as NodeJS.ErrnoException)?.code === "ENOENT") {
      return [] as Dirent[];
    }
    throw error;
  ...
  const files = entries
    ...
    .map((entry) => {
      ...
      return { day, fileName: entry.name };
    })
...
export async function seedHistoricalDailyMemorySignals(params: {
  ...
  const resolved = normalizedPaths
    .map((filePath) => {
      ...
      if (!match) {
        return { day: null as string | null, filePath };
      }
      return { day: match[1] ?? null, filePath };
    })
    .toSorted((a, b) => {
      if (a.day && b.day) {
  ...
  for (const entry of valid) {
    ...
    const raw = await fs.readFile(entry.filePath, "utf8").catch((error: unknown) => {
      if ((error as NodeJS.ErrnoException)?.code === "ENOENT") {
...
async function runLightDreaming(params: {
  ...
  const entries = dedupeEntries(
    (await readShortTermRecallEntries({ nowMs, workspaceDir: params.workspaceDir }))
...
async function runPhaseIfTriggered(
  ...
  for (const workspaceDir of workspaces) {
    try {
      ...
      params.logger.error(
        `memory-core: ${params.phase} dreaming failed for workspace ${workspaceDir}: ${formatErrorMessage(error)}`,
      );
    }
  }
