...
const telemetryState = vi.hoisted(() => {
  ...
  return { counters, histograms, meter, tracer };
});
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    const service = createDiagnosticsOtelService();
    const ctx = createOtelContext(OTEL_TEST_ENDPOINT, { logs: true, metrics: true, traces: true });
    await service.start(ctx);
    ...
    registeredTransports[0]?.({
      ...
      1: "hello",
      _meta: { date: new Date(), logLevelName: "INFO" },
    });
    expect(logEmit).toHaveBeenCalled();
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    const emitCall = await emitAndCaptureLog({
      ...
      1: "auth configured",
      _meta: { date: new Date(), logLevelName: "DEBUG" },
    });
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    emitDiagnosticEvent({
      ...
      state: "waiting",
      type: "session.state", // Pragma: allowlist secret
    });
