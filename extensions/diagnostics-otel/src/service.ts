    ...
    async start(ctx) {
      ...
      if (tracesEnabled || metricsEnabled) {
        ...
        try {
          ...
          ctx.logger.error(`diagnostics-otel: failed to start SDK: ${formatError(error)}`);
          throw error;
        }
      }
      ...
      if (logsEnabled) {
        ...
        stopLogTransport = registerLogTransport((logObj) => {
          try {
            ...
            ctx.logger.error(`diagnostics-otel: log transport failed: ${formatError(error)}`);
          }
        });
      }
      ...
      unsubscribe = onDiagnosticEvent((evt: DiagnosticEventPayload) => {
        try {
          ...
          ctx.logger.error(
            `diagnostics-otel: event handler failed (${evt.type}): ${formatError(error)}`,
          );
        }
      });
