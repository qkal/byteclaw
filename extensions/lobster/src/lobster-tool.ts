...
interface LobsterToolOptions {
  runner?: LobsterRunner;
  taskFlow?: BoundTaskFlow;
}

interface ManagedFlowRunParams {
  controllerId: string;
  goal: string;
  currentStep?: string;
  waitingStep?: string;
...
interface ManagedFlowResumeParams {
  flowId: string;
...
interface ManagedFlowSuccessResult {
  ok: true;
  envelope: unknown;
  flow: unknown;
  mutation: unknown;
...
export function createLobsterTool(api: OpenClawPluginApi, options?: LobsterToolOptions) {
  ...
  return {
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      return {
        content: [{ text: JSON.stringify(envelope, null, 2), type: "text" }],
        details: envelope,
      };
    },
