...
export function buildQaBusSnapshot(params: {
  ...
  return {
    conversations: [...params.conversations.values()].map((conversation) => ({
      ...conversation,
    })),
    cursor: params.cursor,
    events: params.events.map((event) => cloneEvent(event)),
    messages: [...params.messages.values()].map((message) => cloneMessage(message)),
    threads: [...params.threads.values()].map((thread) => ({ ...thread })),
  };
}
