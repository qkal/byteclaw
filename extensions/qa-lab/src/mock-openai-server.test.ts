...
describe("qa mock openai server", () => {
  ...
  it("prefers path-like refs over generic quoted keys in prompts", async () => {
    ...
    expect(response.status).toBe(200);
    const body = await response.text();
    expect(body).toContain(String.raw`"arguments":"{\"path\":\"QA_KICKOFF_TASK.md\"}"`);
  ...
  it("drives the Lobster Invaders write flow and memory recall responses", async () => {
    ...
    expect((await requests.json()) as { model?: string }[]).toMatchObject([
      { model: "gpt-5.4" },
      { model: "gpt-5.4-alt" },
    ]);
  });
  ...
  it("requests non-threaded subagent handoff for QA channel runs", async () => {
    ...
    expect(body).toContain(String.raw`\"label\":\"qa-sidecar\"`);
    expect(body).toContain(String.raw`\"thread\":false`);
  });
  ...
  it("supports advanced QA memory and subagent recovery prompts", async () => {
    ...
    expect(spawnBody).toContain(String.raw`\"label\":\"qa-fanout-alpha\"`);
    ...
    expect(secondSpawnBody).toContain(String.raw`\"label\":\"qa-fanout-beta\"`);
