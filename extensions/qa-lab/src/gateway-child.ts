...
import { type QaThinkingLevel, buildQaGatewayConfig } from "./qa-gateway-config.js";
...
function resolveQaLiveProviderConfigPath(env: NodeJS.ProcessEnv = process.env) {
  ...
  return explicit
    ? { explicit: true, path: resolveQaUserPath(explicit, env) }
    : { explicit: false, path: path.join(os.homedir(), ".openclaw", "openclaw.json") };
}
...
async function createQaBundledPluginsDir(params: {
  ...
  if (
    ...
    await fs.rm(stagedRoot, { force: true, recursive: true });
...
export async function startQaGatewayChild(params: {
  ...
  try {
    ...
    if (!keepTemp && stagedBundledPluginsRoot) {
      await fs.rm(stagedBundledPluginsRoot, { force: true, recursive: true }).catch(() => {});
    }
    throw error;
  }
    ...
    async stop(opts?: { keepTemp?: boolean }) {
      ...
      if (!(opts?.keepTemp ?? keepTemp)) {
        ...
        if (stagedBundledPluginsRoot) {
          await fs.rm(stagedBundledPluginsRoot, { force: true, recursive: true });
        }
      }
    },
    tempRoot,
