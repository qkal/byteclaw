...
import { type QaProviderMode, isQaFastModeModelRef } from "./model-selection.js";
...
export interface QaCharacterModelOptions {
  thinkingDefault?: QaThinkingLevel;
  fastMode?: boolean;
}

export interface QaCharacterEvalRun {
  model: string;
...
export interface QaCharacterEvalJudgment {
  model: string;
  rank: number;
  score: number;
  summary: string;
...
export interface QaCharacterEvalResult {
  outputDir: string;
  reportPath: string;
  summaryPath: string;
...
export interface QaCharacterEvalJudgeResult {
  model: string;
  thinkingDefault: QaThinkingLevel;
  fastMode: boolean;
  blindModels: boolean;
...
export interface QaCharacterEvalParams {
  repoRoot?: string;
  outputDir?: string;
  models: string[];
  scenarioId?: string;
...
function normalizeJudgment(value: unknown, allowedModels: Set<string>): QaCharacterEvalJudgment[] {
  ...
  return rankings
    .map((entry): QaCharacterEvalJudgment | null => {
      ...
      return { model, rank, score, strengths, summary, weaknesses };
    })
