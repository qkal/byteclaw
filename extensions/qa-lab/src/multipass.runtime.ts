...
export interface QaMultipassPlan {
  repoRoot: string;
  outputDir: string;
  reportPath: string;
  summaryPath: string;
...
export interface QaMultipassRunResult {
  outputDir: string;
  reportPath: string;
  summaryPath: string;
  hostLogPath: string;
...
interface RenderGuestScriptOptions {
...
function resolveLiveProviderConfigPath(env: NodeJS.ProcessEnv = process.env) {
  ...
  return explicit
    ? { explicit: true, path: resolveUserPath(explicit, env) }
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    await fs.promises.rm(hostTransferDirPath, { force: true, recursive: true });
    if (launched) {
      try {
