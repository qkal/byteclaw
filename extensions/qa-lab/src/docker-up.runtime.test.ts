...
describe("runQaDockerUp", () => {
        ...
        },
        {
          ...
          async runCommand(command, args, cwd) {
            ...
            if (args.join(" ").includes("ps --format json openclaw-qa-gateway")) {
              return { stderr: "", stdout: '{"Health":"healthy","State":"running"}\n' };
            }
  ...
  it("falls back to free host ports when defaults are already occupied", async () => {
    ...
    const gatewayPortReservation = await occupyPortOrAcceptExisting(18_789);
    const qaLabPortReservation = await occupyPortOrAcceptExisting(43_124);
        ...
        },
        {
          ...
          async runCommand(command, args, cwd) {
            ...
            if (joined.includes("ps -q openclaw-qa-gateway")) {
              return { stderr: "", stdout: "gateway-container\n" };
            }
            if (command === "docker" && args[0] === "inspect") {
              return { stderr: "", stdout: "192.168.165.4\n" };
            }
            return { stderr: "", stdout: "" };
          },
          sleepImpl: vi.fn(async () => {}),
