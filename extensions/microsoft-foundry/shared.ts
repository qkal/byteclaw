...
export interface FoundrySelection {
  endpoint: string;
  modelId: string;
...
export interface FoundryDeploymentConfigInput {
  name: string;
  modelName?: string;
  api?: FoundryProviderApi;
}
...
interface FoundryModelCompat {
  supportsStore?: boolean;
  maxTokensField: "max_completion_tokens" | "max_tokens";
}

interface FoundryAuthProfileConfig {
  provider: string;
...
export function buildFoundryProviderConfig(
  ...
  const deployments = options?.deployments?.length
    ? options.deployments
    : [{ modelName: modelNameHint ?? undefined, name: modelId }];
