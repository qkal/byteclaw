...
function collectDiscordAccountScopes(
  ...
  scopes.push({ account: discord, prefix: "channels.discord" });
  ...
  for (const key of Object.keys(accounts)) {
    ...
    if (account) {
      scopes.push({ account, prefix: `channels.discord.accounts.${key}` });
    }
  }
  return scopes;
}
...
function collectDiscordIdLists(
  ...
  if (dm) {
    refs.push({ holder: dm, key: "allowFrom", pathLabel: `${prefix}.dm.allowFrom` });
    refs.push({ holder: dm, key: "groupChannels", pathLabel: `${prefix}.dm.groupChannels` });
  }
  ...
  for (const guildId of Object.keys(guilds)) {
    ...
    refs.push({ holder: guild, key: "users", pathLabel: `${prefix}.guilds.${guildId}.users` });
    refs.push({ holder: guild, key: "roles", pathLabel: `${prefix}.guilds.${guildId}.roles` });
...
export function maybeRepairDiscordNumericIds(
  ...
  if (changes.length === 0) {
    return {
      changes: [],
      config: cfg,
      warnings: collectDiscordNumericIdWarnings({ doctorFixCommand, hits }),
    };
  }
  return {
    changes,
...
function collectDiscordMutableAllowlistWarnings(cfg: OpenClawConfig): string[] {
  const hits: { path: string; entry: string }[] = [];
  const addHits = (pathLabel: string, list: unknown) => {
    ...
    for (const entry of list) {
      ...
      hits.push({ entry: text, path: pathLabel });
    }
  };
