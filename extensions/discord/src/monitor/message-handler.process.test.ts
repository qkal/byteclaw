...
describe("processDiscordMessage draft streaming", () => {
  ...
  async function createBlockModeContext() {
    return await createBaseContext({
      cfg: {
        channels: {
          discord: {
            draftChunk: { breakPreference: "newline", maxChars: 5, minChars: 1 },
          },
        },
  ...
  it("finalizes via preview edit when final fits one chunk", async () => {
    await runSingleChunkFinalScenario({ maxLinesPerMessage: 5, streamMode: "partial" });
    expectSinglePreviewEdit();
  });

  it("accepts streaming=true alias for partial preview mode", async () => {
    await runSingleChunkFinalScenario({ maxLinesPerMessage: 5, streaming: true });
    expectSinglePreviewEdit();
  });
  ...
  it("falls back to standard send when final needs multiple chunks", async () => {
    await runSingleChunkFinalScenario({ maxLinesPerMessage: 1, streamMode: "partial" });
  ...
  it("falls back to standard delivery for explicit reply-tag finals", async () => {
    ...
    const ctx = await createBaseContext({
      discordConfig: { maxLinesPerMessage: 5, streamMode: "partial" },
    });
  ...
  it("suppresses reasoning payload delivery to Discord", async () => {
    mockDispatchSingleBlockReply({ isReasoning: true, text: "thinking..." });
