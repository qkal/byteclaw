...
const mockClient = {
  rest: { get: getMock, patch: patchMock, post: postMock },
...
describe("maybeCreateDiscordAutoThread autoArchiveDuration", () => {
  it("uses configured autoArchiveDuration", async () => {
    ...
    await maybeCreateDiscordAutoThread(
      createBaseParams({
        channelConfig: { allowed: true, autoArchiveDuration: "10080", autoThread: true },
      }),
    );
    expect(postMock).toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({ body: expect.objectContaining({ auto_archive_duration: 10_080 }) }),
    );
  });
  ...
  it("accepts numeric autoArchiveDuration", async () => {
    ...
    await maybeCreateDiscordAutoThread(
      createBaseParams({
        channelConfig: { allowed: true, autoArchiveDuration: 4320, autoThread: true },
      }),
    );
    expect(postMock).toHaveBeenCalledWith(
      expect.any(String),
