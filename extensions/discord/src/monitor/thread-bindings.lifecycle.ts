import { type AcpSessionStoreEntry, readAcpSessionEntry } from "openclaw/plugin-sdk/acp-runtime";
...
export interface AcpThreadBindingReconciliationResult {
  checked: number;
  removed: number;
...
export async function reconcileAcpThreadBindingsOnStartup(params: {
  ...
  for (const binding of acpBindings) {
    ...
    probeTargets.push({ binding, session, sessionKey });
  }
