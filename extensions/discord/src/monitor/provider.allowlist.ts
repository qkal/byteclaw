...
function formatResolutionLogDetails(base: string, details: (string | undefined)[]): string {
...
async function resolveGuildEntriesByChannelAllowlist(params: {
  ...
  try {
    ...
    params.runtime.log?.(
      `discord channel resolve failed; using config entries. ${formatErrorMessage(error)}`,
    );
    return params.guildEntries;
  }
}
...
async function resolveAllowFromByUserAllowlist(params: {
  ...
  try {
    ...
    params.runtime.log?.(
      `discord user resolve failed; using config entries. ${formatErrorMessage(error)}`,
    );
    return params.allowFrom;
  }
}
...
async function resolveGuildEntriesByUserAllowlist(params: {
  ...
  try {
    ...
    params.runtime.log?.(
      `discord channel user resolve failed; using config entries. ${formatErrorMessage(error)}`,
    );
    return params.guildEntries;
  }
}
