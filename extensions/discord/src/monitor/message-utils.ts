...
import { type APIAttachment, type APIStickerItem, StickerFormatType } from "discord-api-types/v10";
import { type FetchLike, fetchRemoteMedia } from "openclaw/plugin-sdk/media-runtime";
...
function mergeHostnameList(...lists: (string[] | undefined)[]): string[] | undefined {
  const merged = lists
    .flatMap((list) => list ?? [])
...
interface DiscordMediaResolveOptions {
  fetchImpl?: FetchLike;
...
interface DiscordSnapshotAuthor {
  id?: string | null;
...
interface DiscordSnapshotMessage {
  content?: string | null;
  embeds?: { description?: string | null; title?: string | null }[] | null;
...
export async function resolveDiscordChannelInfo(
  ...
  try {
    ...
    logVerbose(`discord: failed to fetch channel ${channelId}: ${String(error)}`);
    DISCORD_CHANNEL_INFO_CACHE.set(channelId, {
      expiresAt: Date.now() + DISCORD_CHANNEL_INFO_NEGATIVE_CACHE_TTL_MS,
      value: null,
    });
...
async function appendResolvedMediaFromAttachments(params: {
  ...
  for (const attachment of attachments) {
    try {
      ...
      logVerbose(`${params.errorPrefix} ${id}: ${String(error)}`);
...
interface DiscordStickerAssetCandidate {
  url: string;
  fileName: string;
}
