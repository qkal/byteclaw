...
export async function maybeSendBindingMessage(params: {
  ...
  if (params.preferWebhook !== false && record.webhookId && record.webhookToken) {
    try {
      ...
      logVerbose(`discord thread binding webhook send failed: ${summarizeDiscordError(error)}`);
    }
  }
  try {
    ...
    logVerbose(`discord thread binding fallback send failed: ${summarizeDiscordError(error)}`);
  }
}
...
export async function createWebhookForChannel(params: {
  ...
  try {
    ...
    logVerbose(
      `discord thread binding webhook create failed for ${params.channelId}: ${summarizeDiscordError(error)}`,
    );
    return {};
  }
}
...
export async function resolveChannelIdForBinding(params: {
  ...
  try {
    ...
    logVerbose(
      `discord thread binding channel resolve failed for ${params.threadId}: ${summarizeDiscordError(error)}`,
    );
    return null;
  }
}
...
export async function createThreadForBinding(params: {
  ...
  try {
    ...
    logVerbose(
      `discord thread binding auto-thread create failed for ${params.channelId}: ${summarizeDiscordError(error)}`,
    );
    return null;
  }
}
