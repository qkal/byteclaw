...
const DISCORD_ACP_STATUS_PROBE_TIMEOUT_MS = 8000;
...
function classifyAcpStatusProbeError(params: {
  ...
  if (params.isAcpRuntimeError(params.error) && params.error.code === "ACP_SESSION_INIT_FAILED") {
    return { reason: "session-init-failed", status: "stale" };
  }

  const message = formatErrorMessage(params.error);
  if (isLegacyMissingSessionError(message)) {
    return { reason: "session-missing", status: "stale" };
  }

  return params.isStaleRunning
    ? { reason: "status-error-running-stale", status: "stale" }
    : { reason: "status-error", status: "uncertain" };
}
...
async function probeDiscordAcpBindingHealth(params: {
  ...
  if (result.kind === "timeout") {
    return isStaleRunning
      ? { reason: "status-timeout-running-stale", status: "stale" }
      : { reason: "status-timeout", status: "uncertain" };
  }
  ...
  if (result.status.state === "error") {
    ...
    return { reason: "status-error-state", status: "uncertain" };
  }
...
async function deployDiscordCommands(params: {
  ...
  restClient.put = async (path: string, data?: unknown, query?: unknown) => {
    ...
    try {
      ...
      attachDiscordDeployRequestBody(error, body);
      const details = formatDiscordDeployErrorDetails(error);
      params.runtime.error?.(
        `discord startup [${accountId}] deploy-rest:put:error ${Math.max(0, Date.now() - startupStartedAt)}ms path=${path} requestMs=${Date.now() - startedAt} error=${formatErrorMessage(error)}${details}`,
      );
      throw error;
    }
  };
  try {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      try {
        ...
        const retryAfterMs = Math.max(0, Math.ceil(error.retryAfter * 1000));
        ...
        if ((shouldLogVerboseForTesting ?? shouldLogVerbose)()) {
          params.runtime.log?.(
            `discord startup [${accountId}] deploy-retry ${Math.max(0, Date.now() - startupStartedAt)}ms attempt=${attempt}/${maxAttempts - 1} retryAfterMs=${retryAfterMs} scope=${error.scope ?? "unknown"} code=${error.discordCode ?? "unknown"}`,
          );
        }
    ...
    const details = formatDiscordDeployErrorDetails(error);
    params.runtime.error?.(
      danger(`discord: failed to deploy native commands: ${formatErrorMessage(error)}${details}`),
    );
  } finally {
    if (restClient.options) {
...
interface DiscordDeployErrorLike {
  status?: unknown;
  discordCode?: unknown;
  rawBody?: unknown;
...
function stringifyDiscordDeployField(value: unknown): string {
  ...
  try {
    ...
    return inspect(value, { breakLength: 120, depth: 2 });
  }
}
...
function formatDiscordDeployErrorDetails(err: unknown): string {
  ...
  if (rawBody !== undefined) {
    ...
    try {
      ...
      bodyText =
        typeof rawBody === "string" ? rawBody : inspect(rawBody, { breakLength: 120, depth: 3 });
    }
    if (bodyText) {
      const maxLen = 800;
...
export async function monitorDiscordProvider(opts: MonitorDiscordOpts = {}) {
  ...
  ({ guildEntries } = allowlistResolved);
  ({ allowFrom } = allowlistResolved);
  ...
  if (nativeEnabled && nativeSkillsEnabled && commandSpecs.length > maxDiscordCommands) {
    ...
    commandSpecs = (listNativeCommandSpecsForConfigForTesting ?? listNativeCommandSpecsForConfig)(
      cfg,
      { provider: "discord", skillCommands: [] },
    );
    commandSpecs = appendPluginCommandSpecs({ commandSpecs, runtime });
    runtime.log?.(
      warn(
  ...
  try {
    ...
    if (!lifecycleStarted) {
      try {
        ...
        runtime.error?.(
          danger(`discord: failed to disconnect gateway during startup cleanup: ${String(error)}`),
        );
      }
    }
...
async function clearDiscordNativeCommands(params: {
  ...
  try {
    ...
    params.runtime.error?.(danger(`discord: failed to clear native commands: ${String(error)}`));
  }
}
