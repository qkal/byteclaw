...
describe("monitorDiscordProvider", () => {
  interface ReconcileHealthProbeParams {
    cfg: OpenClawConfig;
    accountId: string;
    sessionKey: string;
    binding: unknown;
  ...
  interface ReconcileStartupParams {
    cfg: OpenClawConfig;
    healthProbe?: (
  ...
  it("disconnects the shared gateway and suppresses late gateway errors when startup fails before lifecycle begins", async () => {
    ...
    const gateway = { disconnect, emitter, isConnected: false };
  ...
  it("wires exec approval button context from the resolved Discord account config", async () => {
    ...
    const execApprovalsConfig = { approvers: ["123"], enabled: true };
    resolveDiscordAccountMock.mockReturnValue({
      accountId: "default",
      config: {
        agentComponents: { enabled: false },
  ...
  it("captures gateway errors emitted before lifecycle wait starts", async () => {
    const emitter = new EventEmitter();
    const drained: { message: string; type: string }[] = [];
    clientGetPluginMock.mockImplementation((name: string) =>
      name === "gateway" ? { disconnect: vi.fn(), emitter } : undefined,
    );
  ...
  it("continues startup when Discord daily slash-command create quota is exhausted", async () => {
    ...
    rateLimitError.discordCode = 30_034;
