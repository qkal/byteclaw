...
const DISCORD_GATEWAY_STARTUP_DISCONNECT_DRAIN_TIMEOUT_MS = 5000;
const DISCORD_GATEWAY_STARTUP_TERMINATE_CLOSE_TIMEOUT_MS = 1000;
...
export async function runDiscordGatewayLifecycle(params: {
  ...
  try {
    ...
    if (!sawDisallowedIntents && !params.isDisallowedIntentsError(error)) {
      throw error;
    }
  } finally {
    lifecycleStopping = true;
