...
import { type APIAttachment, type APIStickerItem, Routes } from "discord-api-types/v10";
import {
  type OpenClawConfig,
  type ReplyToMode,
...
export interface DiscordThreadStarter {
  text: string;
  author: string;
  authorId?: string;
  authorName?: string;
...
interface DiscordThreadParentInfo {
  id?: string;
  name?: string;
  type?: ChannelType;
}

interface DiscordThreadStarterRestEmbed {
  title?: string | null;
  description?: string | null;
}

interface DiscordThreadStarterRestSnapshotMessage {
  content?: string | null;
...
interface DiscordThreadStarterRestAuthor {
  id?: string | null;
  username?: string | null;
  discriminator?: string | null;
}

interface DiscordThreadStarterRestMember {
  nick?: string | null;
  displayName?: string | null;
  roles?: string[];
}

interface DiscordThreadStarterRestMessage {
  content?: string | null;
  embeds?: DiscordThreadStarterRestEmbed[] | null;
  message_snapshots?: { message?: DiscordThreadStarterRestSnapshotMessage | null }[] | null;
...
interface DiscordThreadStarterCacheEntry {
...
function setCachedThreadStarter(key: string, value: DiscordThreadStarter, now: number): void {
  ...
  DISCORD_THREAD_STARTER_CACHE.delete(key);
  DISCORD_THREAD_STARTER_CACHE.set(key, { updatedAt: now, value });
...
interface DiscordReplyDeliveryPlan {
  deliverTarget: string;
  replyTarget: string;
  replyReference: ReturnType<typeof createReplyReferencePlanner>;
}

export interface DiscordAutoThreadContext {
...
export function resolveDiscordAutoThreadContext(params: {
  ...
  const threadSessionKey = buildAgentSessionKey({
    ...
    peer: { id: createdThreadId, kind: "channel" },
  });
  const parentSessionKey = buildAgentSessionKey({
    agentId: params.agentId,
    channel: params.channel,
    peer: { id: messageChannelId, kind: "channel" },
  });
...
interface MaybeCreateDiscordAutoThreadParams {
  client: Client;
  message: DiscordMessageEvent["message"];
  messageChannelId?: string;
  channel?: string;
...
export async function resolveDiscordAutoThreadReplyPlan(
  ...
  const autoThreadContext = params.isGuildMessage
    ...
    : null;
  return { ...deliveryPlan, autoThreadContext, createdThreadId };
}
...
export async function maybeCreateDiscordAutoThread(
  ...
  try {
    ...
    logVerbose(
      `discord: autoThread creation failed for ${messageChannelId}/${params.message.id}: ${String(error)}`,
    );
...
function resolveDiscordThreadTitleModelRef(params: {
  ...
  const parentSessionKey = buildAgentSessionKey({
    agentId: params.agentId,
    channel,
    peer: { id: params.messageChannelId, kind: "channel" },
  });
...
async function maybeRenameDiscordAutoThread(params: {
  ...
  try {
    ...
    logVerbose(`discord: autoThread rename failed for ${params.threadId}: ${String(error)}`);
  }
}
...
export function resolveDiscordReplyDeliveryPlan(params: {
  ...
  return { deliverTarget, replyReference, replyTarget };
}
