...
export interface DiscordModelPickerState {
...
export interface DiscordModelPickerProviderItem {
  id: string;
  count: number;
}

export interface DiscordModelPickerPage<T> {
  items: T[];
  page: number;
  pageSize: number;
  totalPages: number;
...
interface DiscordModelPickerButtonOptions {
  label: string;
  customId: string;
  style?: ButtonStyle;
  disabled?: boolean;
...
interface DiscordModelPickerCurrentModelRef {
  provider: string;
  model: string;
}
...
interface DiscordModelPickerRenderShellParams {
...
export interface DiscordModelPickerRenderedView {
  layout: DiscordModelPickerLayout;
  content?: string;
  components: TopLevelComponents[];
}

export interface DiscordModelPickerProviderViewParams {
  command: DiscordModelPickerCommandContext;
  userId: string;
...
export interface DiscordModelPickerModelViewParams {
  command: DiscordModelPickerCommandContext;
  userId: string;
  data: ModelsProviderData;
  provider: string;
...
function buildRenderedShell(
  ...
  const containerComponents: (TextDisplay | Separator | DiscordModelPickerRow)[] = [
...
function buildModelRows(params: {
  ...
  return { buttonRow: new Row(buttonRowItems), rows };
}
...
export interface DiscordModelPickerRecentsViewParams {
  command: DiscordModelPickerCommandContext;
  userId: string;
  data: ModelsProviderData;
  quickModels: string[];
