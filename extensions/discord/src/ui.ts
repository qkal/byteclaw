...
interface ResolveDiscordAccentColorParams {
  cfg: OpenClawConfig;
...
export function resolveDiscordAccentColor(params: ResolveDiscordAccentColorParams): string {
  const account = inspectDiscordAccount({ accountId: params.accountId, cfg: params.cfg });
...
export class DiscordUiContainer extends Container {
  constructor(params: {
    ...
    const accentColor =
      accentOverride ?? resolveDiscordAccentColor({ accountId: params.accountId, cfg: params.cfg });
