...
describe("sendMessageDiscord", () => {
  it("creates a thread", async () => {
    ...
    await createThreadDiscord("chan1", { messageId: "m1", name: "thread" }, { rest, token: "t" });
  ...
  it("creates media threads with provided content", async () => {
    ...
    await createThreadDiscord(
      "chan1",
      { content: "initial forum post", name: "thread" },
      { rest, token: "t" },
    );
    expect(postMock).toHaveBeenCalledWith(
      Routes.threads("chan1"),
  ...
  it("passes applied_tags for forum threads", async () => {
    ...
    await createThreadDiscord(
      "chan1",
      { appliedTags: ["tag1", "tag2"], name: "tagged post" },
      { rest, token: "t" },
    );
    expect(postMock).toHaveBeenCalledWith(
      Routes.threads("chan1"),
  ...
  it("sends initial message for non-forum threads with content", async () => {
    ...
    await createThreadDiscord(
      "chan1",
      { content: "Hello thread!", name: "thread" },
      { rest, token: "t" },
    );
  ...
  it("sends initial message for message-attached threads with content", async () => {
    ...
    await createThreadDiscord(
      "chan1",
      { content: "Discussion here", messageId: "m1", name: "thread" },
      { rest, token: "t" },
    );
  ...
  it("times out a member", async () => {
    ...
    await timeoutMemberDiscord(
      { durationMinutes: 10, guildId: "g1", userId: "u1" },
      { rest, token: "t" },
    );
  ...
  it("adds and removes roles", async () => {
    ...
    await addRoleDiscord({ guildId: "g1", roleId: "r1", userId: "u1" }, { rest, token: "t" });
    await removeRoleDiscord({ guildId: "g1", roleId: "r1", userId: "u1" }, { rest, token: "t" });
  ...
  it("bans a member", async () => {
    ...
    await banMemberDiscord(
      { deleteMessageDays: 2, guildId: "g1", userId: "u1" },
      { rest, token: "t" },
    );
...
describe("retry rate limits", () => {
  ...
  it("retries on Discord rate limits", async () => {
    ...
    postMock
      ...
      .mockResolvedValueOnce({ channel_id: "789", id: "msg1" });
  ...
  it("retries media upload without duplicating overflow text", async () => {
    ...
    postMock
      .mockRejectedValueOnce(rateLimitError)
      .mockResolvedValueOnce({ channel_id: "789", id: "msg1" })
      .mockResolvedValueOnce({ channel_id: "789", id: "msg2" });
