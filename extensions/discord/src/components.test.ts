...
describe("discord components", () => {
  it("builds v2 containers with modal trigger", () => {
    const spec = readDiscordComponentSpec({
      ...
      modal: {
        ...
        fields: [{ label: "Requester", type: "text" }],
        title: "Details",
      },
      text: "Choose a path",
    });
  ...
  it("requires options for modal select fields", () => {
    expect(() =>
      readDiscordComponentSpec({
        modal: {
          fields: [{ label: "Priority", type: "select" }],
          title: "Details",
        },
      }),
    ).toThrow("options");
  ...
  it("requires attachment references for file blocks", () => {
    expect(() =>
      readDiscordComponentSpec({
        blocks: [{ file: "https://example.com/report.pdf", type: "file" }],
      }),
    ).toThrow("attachment://");
    expect(() =>
      readDiscordComponentSpec({
        blocks: [{ file: "attachment://", type: "file" }],
      }),
    ).toThrow("filename");
  });
});
...
describe("discord component registry", () => {
  ...
  it("registers and consumes component entries", () => {
    ...
    const entry = resolveDiscordComponentEntry({ consume: false, id: "btn_1" });
    expect(entry?.messageId).toBe("msg_1");

    const modal = resolveDiscordModalEntry({ consume: false, id: "mdl_1" });
  ...
  it("shares registry state across duplicate module instances", async () => {
    ...
    expect(second.resolveDiscordComponentEntry({ consume: false, id: "btn_shared" })).toMatchObject(
      {
        id: "btn_shared",
        label: "Shared",
      },
