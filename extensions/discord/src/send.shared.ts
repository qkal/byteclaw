...
export type DiscordSendEmbeds = (APIEmbed | Embed)[];
...
async function sendDiscordText(
  ...
  const chunks = buildDiscordTextChunks(text, { chunkMode, maxLinesPerMessage });
...
async function sendDiscordMedia(
  ...
  const resolvedFileName =
    ...
    "upload";
  const chunks = text ? buildDiscordTextChunks(text, { chunkMode, maxLinesPerMessage }) : [];
