...
export interface VoiceReceiveRecoveryState {
  decryptFailureCount: number;
  lastDecryptFailureAt: number;
  decryptRecoveryInFlight: boolean;
}

export interface VoiceReceiveErrorAnalysis {
  message: string;
  isAbortLike: boolean;
...
interface DavePassthroughTarget {
  guildId: string;
  channelId: string;
  connection: {
    state: {
...
export function enableDaveReceivePassthrough(params: {
  ...
  try {
    ...
    onWarn(
      `discord voice: failed to enable DAVE passthrough guild=${target.guildId} channel=${target.channelId} reason=${reason}: ${formatErrorMessage(error)}`,
    );
    return false;
  }
}
