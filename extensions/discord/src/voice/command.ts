...
interface VoiceCommandChannelOverride {
  id: string;
  name?: string;
  parentId?: string;
}

interface VoiceCommandRuntimeContext {
  guildId: string;
...
async function authorizeVoiceCommand(
  ...
  if (!access.ok) {
    return { message: access.message, ok: false };
  }

  return { guildId: interaction.guild.id, ok: true };
}
...
export function createDiscordVoiceCommand(params: VoiceCommandContext): CommandWithSubcommands {
  ...
  class JoinCommand extends Command {
    ...
    async run(interaction: CommandInteraction) {
      ...
      const result = await manager.join({ channelId: channel.id, guildId });
