...
describe("sendMessageDiscord", () => {
  ...
  async function sendChunkedReplyAndCollectBodies(params: { text: string; mediaUrl?: string }) {
    ...
    postMock.mockResolvedValue({ channel_id: "789", id: "msg1" });
  ...
  function setupForumSend(secondResponse: { id: string; channel_id: string }) {
    ...
    postMock
      .mockResolvedValueOnce({
        id: "thread1",
        message: { channel_id: "thread1", id: "starter1" },
      })
      .mockResolvedValueOnce(secondResponse);
    return { postMock, rest };
  }
  ...
  it("sends basic channel messages", async () => {
    ...
    expect(res).toEqual({ channelId: "789", messageId: "msg1" });
  ...
  it("auto-creates a forum thread when target is a Forum channel", async () => {
    ...
    expect(res).toEqual({ channelId: "thread1", messageId: "starter1" });
  ...
  it("posts media as a follow-up message in forum channels", async () => {
    const { rest, postMock } = setupForumSend({ channel_id: "thread1", id: "media1" });
  ...
  it("chunks long forum posts into follow-up messages", async () => {
    const { rest, postMock } = setupForumSend({ channel_id: "thread1", id: "msg2" });
  ...
  it("starts DM when recipient is a user", async () => {
    ...
    postMock
      ...
      .mockResolvedValueOnce({ channel_id: "chan1", id: "msg1" });
  ...
  it("uploads media attachments", async () => {
    const { rest, postMock } = makeDiscordRest();
    postMock.mockResolvedValue({ channel_id: "789", id: "msg" });
...
describe("fetchReactionsDiscord", () => {
  ...
  it("returns reactions with users", async () => {
    ...
    getMock
      .mockResolvedValueOnce({
        reactions: [
          { count: 2, emoji: { id: null, name: "✅" } },
          { count: 1, emoji: { id: "123", name: "party_blob" } },
        ],
      })
      .mockResolvedValueOnce([{ discriminator: "0001", id: "u1", username: "alpha" }])
    ...
    expect(res).toEqual([
      {
        ...
        users: [{ id: "u1", tag: "alpha#0001", username: "alpha" }],
      },
      {
        count: 1,
        emoji: { id: "123", name: "party_blob", raw: "party_blob:123" },
        users: [{ id: "u2", tag: "beta", username: "beta" }],
      },
    ]);
  });
});
...
describe("readMessagesDiscord", () => {
  ...
  it("passes query params as an object", async () => {
    const { rest, getMock } = makeDiscordRest();
    getMock.mockResolvedValue([]);
    await readMessagesDiscord("chan1", { before: "10", limit: 5 }, { rest, token: "t" });
    const call = getMock.mock.calls[0];
    const options = call?.[1] as Record<string, unknown>;
    expect(options).toEqual({ before: "10", limit: 5 });
  });
});
...
describe("searchMessagesDiscord", () => {
  ...
  it("uses URLSearchParams for search", async () => {
    const { rest, getMock } = makeDiscordRest();
    getMock.mockResolvedValue({ messages: [], total_results: 0 });
    await searchMessagesDiscord(
      { content: "hello", guildId: "g1", limit: 5 },
      { rest, token: "t" },
    );
    const call = getMock.mock.calls[0];
