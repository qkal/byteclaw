...
function createDiscordApproverDmTargetResolver(configOverride?: DiscordExecApprovalConfig | null) {
  return createChannelApproverDmTargetResolver({
    ...
    resolveApprovers: ({ cfg, accountId }) =>
      getDiscordExecApprovalApprovers({ accountId, cfg, configOverride }),
    shouldHandleRequest: ({ cfg, accountId, request }) =>
      shouldHandleDiscordApprovalRequest({
        accountId,
        cfg,
...
export function createDiscordApprovalCapability(configOverride?: DiscordExecApprovalConfig | null) {
  return createApproverRestrictedNativeApprovalCapability({
    ...
    hasApprovers: ({ cfg, accountId }) =>
      getDiscordExecApprovalApprovers({ accountId, cfg, configOverride }).length > 0,
    isExecAuthorizedSender: ({ cfg, accountId, senderId }) =>
      isDiscordExecApprovalApprover({ accountId, cfg, configOverride, senderId }),
    isNativeDeliveryEnabled: ({ cfg, accountId }) =>
      isDiscordExecApprovalClientEnabled({ accountId, cfg, configOverride }),
    ...
    resolveNativeDeliveryMode: ({ cfg, accountId }) =>
      ...
      resolveDiscordAccount({ accountId, cfg }).config.execApprovals?.target ??
      "dm",
