...
export interface MattermostOpaqueTargetResolution {
  kind: "user" | "channel";
  id: string;
  to: string;
}
...
export async function resolveMattermostOpaqueTarget(params: {
  ...
  const account =
    params.cfg && (!params.token || !params.baseUrl)
      ? resolveMattermostAccount({ accountId: params.accountId, cfg: params.cfg })
      : null;
  ...
  if (cached === true) {
    return { id: input, kind: "user", to: `user:${input}` };
  }
  if (cached === false) {
    return { id: input, kind: "channel", to: `channel:${input}` };
  }
