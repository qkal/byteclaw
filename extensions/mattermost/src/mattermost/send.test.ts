...
vi.mock("../runtime.js", () => ({
  getMattermostRuntime: () => ({
    ...
    logging: {
      getChildLogger: () => ({ debug: vi.fn(), error: vi.fn(), info: vi.fn(), warn: vi.fn() }),
...
describe("parseMattermostTarget", () => {
  it("parses channel: prefix with valid ID as channel id", () => {
    ...
    expect(target).toEqual({ id: "dthcxgoxhifn3pwh65cut3ud3w", kind: "channel" });
  });
  ...
  it("is case-insensitive for prefixes", () => {
    ...
    expect(parseMattermostTarget("User:XYZ")).toEqual({ id: "XYZ", kind: "user" });
    expect(parseMattermostTarget("Mattermost:QRS")).toEqual({ id: "QRS", kind: "user" });
  });
});
