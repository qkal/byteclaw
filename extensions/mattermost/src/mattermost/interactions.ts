...
import { type MattermostClient, type MattermostPost, updateMattermostPost } from "./client.js";
import { type OpenClawConfig, isTrustedProxyAddress, resolveClientIp } from "./runtime-api.js";
 ...
 */
export interface MattermostInteractionPayload {
  user_id: string;
  user_name?: string;
  channel_id: string;
  team_id?: string;
...
export interface MattermostInteractionResponse {
  update?: {
    message: string;
...
export interface MattermostInteractiveButtonInput {
  id?: string;
...
export function computeInteractionCallbackUrl(
  ...
  const port = typeof cfg?.gateway?.port === "number" ? cfg.gateway.port : 18_789;
  let host =
...
export function createMattermostInteractionHandler(params: {
  ...
  return async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      log?.(`mattermost interaction: failed to parse body: ${String(error)}`);
      res.statusCode = 400;
      res.setHeader("Content-Type", "application/json");
    ...
    try {
      ...
      const postAttachments = Array.isArray(originalPost?.props?.attachments)
        ? (originalPost.props.attachments as {
      ...
      log?.(`mattermost interaction: failed to validate post ${payload.post_id}: ${String(error)}`);
      res.statusCode = 500;
      res.setHeader("Content-Type", "application/json");
    ...
    if (params.authorizeButtonClick) {
      try {
        ...
        log?.(`mattermost interaction: authorization failed: ${String(error)}`);
        res.statusCode = 500;
        res.setHeader("Content-Type", "application/json");
    ...
    if (params.handleInteraction) {
      try {
        ...
        log?.(`mattermost interaction: custom handler failed: ${String(error)}`);
        res.statusCode = 500;
        res.setHeader("Content-Type", "application/json");
