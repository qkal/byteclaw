...
export interface MattermostWebSocketLike {
...
interface CreateMattermostConnectOnceOpts {
  wsUrl: string;
  botToken: string;
      ...
      return await new Promise<void>((resolve, reject) => {
        ...
        const runHealthCheck = async () => {
          ...
          try {
            ...
            opts.runtime.error?.(`${label}: ${String(error)}`);
          } finally {
        ...
        ws.on("message", async (data) => {
          ...
          if (payload.event === "reaction_added" || payload.event === "reaction_removed") {
            ...
            try {
              ...
              opts.runtime.error?.(`mattermost reaction handler failed: ${String(error)}`);
            }
            return;
          }
          ...
          try {
            ...
            opts.runtime.error?.(`mattermost handler failed: ${String(error)}`);
          }
        });
