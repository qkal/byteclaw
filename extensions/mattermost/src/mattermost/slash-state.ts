...
import { type MattermostRegisteredCommand, resolveSlashCommandConfig } from "./slash-commands.js";
import { createSlashCommandHttpHandler } from "./slash-http.js";

// ─── Per-account state ───────────────────────────────────────────────────────

export interface SlashCommandAccountState {
...
export function resolveSlashHandlerForToken(token: string): {
  ...
  if (matches.length === 1) {
    return { accountIds: [matches[0].accountId], handler: matches[0].handler, kind: "single" };
  }
