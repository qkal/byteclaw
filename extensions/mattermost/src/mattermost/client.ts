...
export function createMattermostClient(params: {
  ...
  const guardedFetchImpl: MattermostFetch = async (input, init) => {
    ...
    try {
      ...
      return new Response(bodyBytes, { headers: response.headers, status: response.status });
    } finally {
      await release();
    }
  };
  ...
  return { apiBaseUrl, baseUrl, fetchImpl, request, token };
}
...
export interface CreateDmChannelRetryOptions {
