...
export interface IrcGroupMatch {
  allowed: boolean;
...
export interface IrcGroupAccessGate {
  allowed: boolean;
  reason: string;
}
...
export function resolveIrcMentionGate(params: {
  ...
  if (!params.isGroup) {
    return { reason: "direct", shouldSkip: false };
  }
  if (!params.requireMention) {
    return { reason: "mention-not-required", shouldSkip: false };
  }
  if (params.wasMentioned) {
    return { reason: "mentioned", shouldSkip: false };
  }
  if (params.hasControlCommand && params.allowTextCommands && params.commandAuthorized) {
    return { reason: "authorized-command", shouldSkip: false };
  }
  return { reason: "missing-mention", shouldSkip: true };
}
