...
export interface SlackActionClientOpts {
  accountId?: string;
  token?: string;
  client?: WebClient;
}
...
interface SlackFileInfoSummary {
  id?: string;
  name?: string;
  mimetype?: string;
  url_private?: string;
...
interface SlackFileThreadShare {
  channelId: string;
  ts?: string;
  threadTs?: string;
}
...
function collectSlackShareMaps(file: SlackFileInfoSummary): Record<string, unknown>[] {
...
function collectSlackThreadShares(
  ...
  for (const shareMap of collectSlackShareMaps(file)) {
    ...
    for (const rawEntry of rawEntries) {
      ...
      matches.push({ channelId, threadTs, ts });
    }
  }
  return matches;
}
...
export async function downloadSlackFile(
  ...
  if (hasSlackScopeMismatch({ channelId: opts.channelId, file, threadId: opts.threadId })) {
    return null;
  }
