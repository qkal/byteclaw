...
export function registerSlackMessageEvents(params: {
  ...
  const handleIncomingMessageEvent = async ({ event, body }: { event: unknown; body: unknown }) => {
    try {
      ...
      ctx.runtime.error?.(danger(`slack handler failed: ${String(error)}`));
    }
  };
  ...
  ctx.app.event("message", async ({ event, body }: SlackEventMiddlewareArgs<"message">) => {
    await handleIncomingMessageEvent({ body, event });
  });
  ...
  ctx.app.event("app_mention", async ({ event, body }: SlackEventMiddlewareArgs<"app_mention">) => {
    try {
      ...
      ctx.runtime.error?.(danger(`slack mention handler failed: ${String(error)}`));
    }
  });
}
