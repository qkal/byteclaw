...
function looksLikeHtmlBuffer(buffer: Buffer): boolean {
  const head = normalizeLowercaseStringOrEmpty(
    buffer.subarray(0, 512).toString("utf8").replace(/^\s+/, ""),
  );
  return head.startsWith("<!doctype html") || head.startsWith("<html");
}

export interface SlackMediaResult {
  path: string;
  contentType?: string;
  placeholder: string;
}
...
export async function resolveSlackAttachmentContent(params: {
  ...
  return { media: allMedia, text: combinedText };
}

export interface SlackThreadStarter {
  text: string;
  userId?: string;
  botId?: string;
  ts?: string;
...
interface SlackThreadStarterCacheEntry {
...
interface SlackRepliesPageMessage {
  text?: string;
  user?: string;
  bot_id?: string;
  ts?: string;
