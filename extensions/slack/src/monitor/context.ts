...
export function createSlackMonitorContext(params: {
  ...
  allowNameMatching: boolean;
  groupDmEnabled: boolean;
  groupDmChannels: (string | number)[] | undefined;
  defaultRequireMention?: boolean;
  ...
  const seenMessages = createDedupeCache({ maxSize: 500, ttlMs: 60_000 });
  ...
  const resolveSlackSystemEventSessionKey = (p: {
    ...
    return resolveSessionKey(
      params.sessionScope,
      { ChatType: chatType, From: from, Provider: "slack" },
      params.mainKey,
    );
  };
  ...
  const resolveChannelName = async (channelId: string) => {
    ...
    try {
      ...
      const entry = { name, purpose, topic, type };
  ...
  const setSlackThreadStatus = async (p: {
    ...
    try {
      ...
      logVerbose(`slack status update failed for channel ${p.channelId}: ${String(error)}`);
    }
  };
