...
import { type SlackChannelConfigResolved, resolveSlackChannelConfig } from "./channel-config.js";
...
function buildSlackArgMenuConfirm(params: { command: string; arg: string }) {
  ...
  return {
    confirm: { text: "Run command", type: "plain_text" },
    deny: { text: "Cancel", type: "plain_text" },
    text: {
      text: `Run */${command}* with *${arg}* set to this value?`,
      type: "mrkdwn",
    },
    title: { text: "Confirm selection", type: "plain_text" },
  };
}
...
function buildSlackArgMenuOptions(choices: EncodedMenuChoice[]) {
  return choices.map((choice) => ({
    text: { text: choice.label.slice(0, 75), type: "plain_text" },
    value: choice.value,
  }));
}
...
function buildSlackCommandArgMenuBlocks(params: {
      ...
      : encodedChoices.length <= SLACK_COMMAND_ARG_BUTTON_ROW_SIZE || !canUseStaticSelect
        ? chunkItems(encodedChoices, SLACK_COMMAND_ARG_BUTTON_ROW_SIZE).map((choices) => ({
            elements: choices.map((choice) => ({
              action_id: SLACK_COMMAND_ARG_ACTION_ID,
              confirm: buildSlackArgMenuConfirm({ command: params.command, arg: params.arg }),
              text: { text: choice.label, type: "plain_text" },
              type: "button",
              value: choice.value,
            })),
            type: "actions",
  ...
  return [
    {
      text: { text: headerText, type: "plain_text" },
      type: "header",
    },
    {
      text: { text: sectionText, type: "mrkdwn" },
      type: "section",
    },
    {
      elements: [{ text: contextText, type: "mrkdwn" }],
      type: "context",
    },
    ...rows,
  ];
...
export async function registerSlackMonitorSlashCommands(params: {
  ...
  const handleSlashCommand = async (p: {
    ...
    try {
      ...
      commandAuthorized = resolveCommandAuthorizedFromAuthorizers({
        authorizers: [{ allowed: ownerAllowed, configured: effectiveAllowFromLower.length > 0 }],
      ...
      if (isRoomish) {
        commandAuthorized = resolveCommandAuthorizedFromAuthorizers({
          authorizers: [
            { allowed: ownerAllowed, configured: effectiveAllowFromLower.length > 0 },
            { allowed: channelUserAllowed, configured: channelUsersAllowlistConfigured },
          ],
          modeWhenAccessGroupsOff: "configured",
          useAccessGroups: ctx.useAccessGroups,
        });
      ...
      runtime.error?.(danger(`slack slash handler failed: ${String(error)}`));
      await respond({
        response_type: "ephemeral",
  ...
  let nativeCommands: { name: string }[] = [];
  ...
  try {
    ...
    logVerbose(
      `slack: external arg-menu registration failed, falling back to static menus: ${String(error)}`,
    );
  }
