...
async function resolveThreadTsFromHistory(params: {
  ...
  try {
    ...
    })) as { messages?: { ts?: string; thread_ts?: string }[] };
    const message =
    ...
    if (shouldLogVerbose()) {
      logVerbose(
        `slack inbound: failed to resolve thread_ts via conversations.history for channel=${params.channelId} ts=${params.messageTs}: ${String(error)}`,
      );
    }
    return undefined;
  }
