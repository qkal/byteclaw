...
interface ResolvedAllowFromLists {
  allowFrom: string[];
  allowFromLower: string[];
}

interface SlackAllowFromCacheState {
...
export interface SlackSystemEventAuthResult {
  allowed: boolean;
  reason?:
    | "missing-sender"
    | "sender-mismatch"
...
export async function authorizeSlackSystemEventSender(params: {
  ...
  if (channelType === "im") {
    if (!params.ctx.dmEnabled || params.ctx.dmPolicy === "disabled") {
      return { allowed: false, channelName, channelType, reason: "dm-disabled" };
    }
