...
function createSlackEvent(params: { type: "message" | "app_mention"; ts: string; text: string }) {
  return { channel: "C1", text: params.text, ts: params.ts, type: params.type } as never;
}

async function sendMessageEvent(handler: ReturnType<typeof createTestHandler>, ts: string) {
  await handler(createSlackEvent({ text: "hello", ts, type: "message" }), { source: "message" });
}

async function sendMentionEvent(handler: ReturnType<typeof createTestHandler>, ts: string) {
  await handler(createSlackEvent({ text: "<@U_BOT> hello", ts, type: "app_mention" }), {
    source: "app_mention",
    wasMentioned: true,
  });
}
...
async function createInFlightMessageScenario(ts: string) {
  ...
  const messagePending = handler(createSlackEvent({ text: "hello", ts, type: "message" }), {
    source: "message",
  });
