...
import { type SlackChannelResolution, resolveSlackChannelAllowlist } from "../resolve-channels.js";
import { type SlackUserResolution, resolveSlackUserAllowlist } from "../resolve-users.js";
...
interface SlackBoltResolvedExports {
  App: SlackAppConstructor;
  HTTPReceiver: SlackHttpReceiverConstructor;
}
interface SlackSocketShutdownClient {
...
export async function monitorSlackProvider(opts: MonitorSlackOpts = {}) {
  ...
  const handleSlackMessage = createSlackMessageHandler({ account, ctx, trackEvent });
  if (
  ...
  registerSlackMonitorEvents({ account, ctx, handleSlackMessage, trackEvent });
  await registerSlackMonitorSlashCommands({ account, ctx });
    ...
    void (async () => {
      ...
      if (channelsConfig && Object.keys(channelsConfig).length > 0) {
        try {
          ...
          runtime.log?.(`slack channel resolve failed; using config entries. ${String(error)}`);
        }
      }
      ...
      if (allowEntries.length > 0) {
        try {
          ...
          allowFrom = mergeAllowlist({ additions, existing: allowFrom });
          ctx.allowFrom = normalizeAllowList(allowFrom);
          summarizeMapping("slack users", mapping, unresolved, runtime);
        } catch (error) {
          runtime.log?.(`slack user resolve failed; using config entries. ${String(error)}`);
        }
      }
      ...
      if (channelsConfig && Object.keys(channelsConfig).length > 0) {
        ...
        if (userEntries.size > 0) {
          try {
            ...
            runtime.log?.(
              `slack channel user resolve failed; using config entries. ${String(error)}`,
            );
          }
        }
      }
      ...
      while (!opts.abortSignal?.aborted) {
        try {
          ...
          if (isNonRecoverableSlackAuthError(error)) {
            runtime.error?.(
              `slack socket mode failed to start due to non-recoverable auth error — skipping channel (${formatUnknownError(error)})`,
            );
            throw error;
          }
          reconnectAttempts += 1;
          ...
          runtime.error?.(
            `slack socket mode failed to start. retry ${reconnectAttempts}/${SLACK_SOCKET_RECONNECT_POLICY.maxAttempts || "∞"} in ${Math.round(delayMs / 1000)}s (${formatUnknownError(error)})`,
          );
          try {
