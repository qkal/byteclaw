...
describe("handleSlackAction", () => {
  ...
  async function sendSecondMessageAndExpectNoThread(params: {
    ...
    await handleSlackAction(
      { action: "sendMessage", content: "Second", to: "channel:C123" },
      params.cfg,
      params.context,
    );
  ...
  async function resolveReadToken(cfg: OpenClawConfig): Promise<string | undefined> {
    ...
    readSlackMessages.mockResolvedValueOnce({ hasMore: false, messages: [] });
  ...
  async function resolveSendToken(cfg: OpenClawConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
    await handleSlackAction({ action: "sendMessage", content: "Hello", to: "channel:C1" }, cfg);
  ...
  it.each([
    { channelId: "C1", name: "raw channel id" },
    { channelId: "channel:C1", name: "channel: prefixed id" },
  ...
  it.each([
    {
      blocks: JSON.stringify([
        { text: { text: "*Deploy* status", type: "mrkdwn" }, type: "section" },
      ]),
      expectedBlocks: [{ text: { text: "*Deploy* status", type: "mrkdwn" }, type: "section" }],
      name: "JSON blocks",
    },
    {
      blocks: [{ type: "divider" }],
  ...
  it.each([
    ...
    { blocks: "[]", expectedError: /at least one block/i, name: "empty blocks arrays" },
  ...
  it.each([
    ...
    {
      blocks: [{ text: { text: "updated", type: "mrkdwn" }, type: "section" }],
      expectedBlocks: [{ text: { text: "updated", type: "mrkdwn" }, type: "section" }],
      name: "array blocks",
    },
  ...
  it("replyToMode=first threads first message then stops", async () => {
    ...
    await handleSlackAction(
      { action: "sendMessage", content: "First", to: "channel:C123" },
      cfg,
      context,
    );
  ...
  it("replyToMode=first without hasRepliedRef does not thread", async () => {
    await handleSlackAction(
      { action: "sendMessage", content: "No ref", to: "channel:C123" },
      slackConfig(),
      {
        currentChannelId: "C123",
        currentThreadTs: "1111111111.111111",
  ...
  it("does not auto-inject threadTs when replyToMode=off", async () => {
    await handleSlackAction(
      { action: "sendMessage", content: "No thread", to: "channel:C123" },
      slackConfig(),
      {
        currentChannelId: "C123",
        currentThreadTs: "1111111111.111111",
  ...
  it("does not auto-inject threadTs when sending to different channel", async () => {
    await handleSlackAction(
      { action: "sendMessage", content: "Other channel", to: "channel:C999" },
      slackConfig(),
      {
        currentChannelId: "C123",
        currentThreadTs: "1111111111.111111",
  ...
  it("handles channel target without prefix when replyToMode=all", async () => {
    await handleSlackAction(
      { action: "sendMessage", content: "Bare target", to: "C123" },
      slackConfig(),
      {
        currentChannelId: "C123",
        currentThreadTs: "1111111111.111111",
  ...
  it("adds normalized timestamps to readMessages payloads", async () => {
    readSlackMessages.mockResolvedValueOnce({
      hasMore: false,
      messages: [{ text: "hi", ts: "1712345678.123456" }],
    });
  ...
  it("adds normalized timestamps to pin payloads", async () => {
    listSlackPins.mockResolvedValueOnce([{ message: { text: "pin", ts: "1712345678.123456" } }]);
