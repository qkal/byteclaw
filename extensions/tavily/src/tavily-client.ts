...
export async function runTavilyExtract(
  ...
  const rawResults = Array.isArray(payload.results) ? payload.results : [];
  const results = rawResults.map((r: Record<string, unknown>) => ((Object.assign({url:typeof r.url===`string`?r.url:``,rawContent:typeof r.raw_content===`string`?wrapExternalContent(r.raw_content,{source:`web_fetch`,includeWarning:false}):``}, typeof r.content===`string`?{content:wrapExternalContent(r.content,{source:`web_fetch`,includeWarning:false})}:{}, Array.isArray(r.images)?{images:(r.images as string[]).map(img=>wrapExternalContent(String(img),{source:`web_fetch`,includeWarning:false}))}:{}))));
