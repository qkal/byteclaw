...
export interface MemoryWikiImportedSourceStateEntry {
...
interface MemoryWikiImportedSourceState {
  version: 1;
...
export async function readMemoryWikiSourceSyncState(
  ...
  const raw = await fs.readFile(statePath, "utf8").catch((error: unknown) => {
