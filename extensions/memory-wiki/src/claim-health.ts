...
export interface WikiClaimContradictionCluster {
  key: string;
  label: string;
  entries: WikiClaimHealth[];
}

export interface WikiPageContradictionCluster {
  key: string;
  label: string;
  entries: {
    pagePath: string;
...
function resolveLatestTimestamp(candidates: (string | undefined)[]): string | undefined {
...
export function assessPageFreshness(page: WikiPageSummary, now?: Date): WikiFreshness {
  return buildFreshnessFromTimestamp({ now, timestamp: page.updatedAt });
}
...
export function assessClaimFreshness(params: {
  ...
  return buildFreshnessFromTimestamp({ now: params.now, timestamp: latestTimestamp });
}
...
export function buildWikiClaimHealth(params: {
  ...
  return {
    ...
    freshness: assessClaimFreshness({ claim: params.claim, now: params.now, page: params.page }),
  };
}

export function collectWikiClaimHealth(pages: WikiPageSummary[], now?: Date): WikiClaimHealth[] {
  return pages.flatMap((page) =>
    page.claims.map((claim, index) => buildWikiClaimHealth({ claim, index, now, page })),
  );
}
