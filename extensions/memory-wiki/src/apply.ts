...
import { type CompileMemoryWikiResult, compileMemoryWikiVault } from "./compile.js";
...
export interface CreateSynthesisMemoryWikiMutation {
  op: "create_synthesis";
  title: string;
  body: string;
  sourceIds: string[];
...
export interface UpdateMetadataMemoryWikiMutation {
  op: "update_metadata";
  lookup: string;
  sourceIds?: string[];
  claims?: WikiClaim[];
...
export interface ApplyMemoryWikiMutationResult {
  changed: boolean;
...
async function applyCreateSynthesisMutation(params: {
  ...
  return { changed, pageId, pagePath };
}
