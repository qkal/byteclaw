...
interface QuerySearchOverrides {
...
async function readQueryableWikiPagesByPaths(
  ...
  const pages = await Promise.all(
    files.map(async (relativePath) => {
      ...
      const summary = toWikiPageSummary({ absolutePath, raw, relativePath });
