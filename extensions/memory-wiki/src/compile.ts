...
const COMPILE_PAGE_GROUPS: { kind: WikiPageKind; dir: string; heading: string }[] = [
  { dir: "sources", heading: "Sources", kind: "source" },
  { dir: "entities", heading: "Entities", kind: "entity" },
  { dir: "concepts", heading: "Concepts", kind: "concept" },
  { dir: "syntheses", heading: "Syntheses", kind: "synthesis" },
  { dir: "reports", heading: "Reports", kind: "report" },
];
const AGENT_DIGEST_PATH = ".openclaw-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".openclaw-wiki/cache/claims.jsonl";

interface DashboardPageDefinition {
  id: string;
  title: string;
  relativePath: string;
  buildBody: (params: {
...
const DASHBOARD_PAGES: DashboardPageDefinition[] = [
  ...
  },
  {
    buildBody: ({ config, pages, now }) => {
      ...
      const claimClusters = buildClaimContradictionClusters({ now, pages });
...
export interface CompileMemoryWikiResult {
  vaultRoot: string;
...
export interface RefreshMemoryWikiIndexesResult {
  refreshed: boolean;
...
async function readPageSummaries(rootDir: string): Promise<WikiPageSummary[]> {
  ...
  const pages = await Promise.all(
    filePaths.map(async (relativePath) => {
      ...
      return toWikiPageSummary({ absolutePath, raw, relativePath });
    }),
  );
...
interface AgentDigestClaimHealthSummary {
...
interface AgentDigestContradictionCluster {
  key: string;
  label: string;
...
function sortClaims(page: WikiPageSummary): WikiClaim[] {
  return [...page.claims].toSorted((left, right) => {
    ...
    const leftFreshness = rankFreshnessLevel(assessClaimFreshness({ claim: left, page }).level);
    const rightFreshness = rankFreshnessLevel(assessClaimFreshness({ claim: right, page }).level);
    ...
    .map((page) => {
      ...
      return {
        ...
        topClaims: sortClaims(page)
          .slice(0, 5)
          .map((claim) => {
            const freshness = assessClaimFreshness({ claim, page });
            return {
...
export async function compileMemoryWikiVault(
  ...
  const dashboardUpdatedFiles = await refreshDashboardPages({ config, pages, rootDir });
  ...
  if (
    await writeManagedMarkdownFile({
      body: buildRootIndexBody({ config, counts, pages }),
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        body: buildDirectoryIndexBody({ config, group, pages }),
