...
interface TelegramPollingSessionOpts {
  token: string;
...
export class TelegramPollingSession {
  ...
  async #createPollingBot(): Promise<TelegramBot | undefined> {
    ...
    try {
      ...
      await this.#waitBeforeRetryOnRecoverableSetupError(error, "Telegram setup network error");
  ...
  async #confirmPersistedOffset(bot: TelegramBot): Promise<void> {
    ...
    try {
      await bot.api.getUpdates({ limit: 1, offset: lastUpdateId + 1, timeout: 0 });
    } catch {
  ...
  async #runPollingCycle(bot: TelegramBot): Promise<"continue" | "exit"> {
    ...
    bot.api.config.use(async (prev, method, payload, signal) => {
      ...
      try {
        ...
        lastGetUpdatesError = formatErrorMessage(error);
        throw error;
      } finally {
    ...
    try {
      ...
      const isConflict = isGetUpdatesConflict(error);
      if (isConflict) {
