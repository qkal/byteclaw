...
import { type ApiClientOptions, Bot, apiThrottler, sequentialize } from "./bot.runtime.js";
...
export function createTelegramBot(opts: TelegramBotOptions): TelegramBotInstance {
  ...
  bot.catch((error) => {
    runtime.error?.(danger(`telegram bot error: ${formatUncaughtError(error)}`));
  });
  ...
  bot.use(async (ctx, next) => {
    if (shouldLogVerbose()) {
      try {
        ...
        rawUpdateLogger.debug(`telegram update log failed: ${String(error)}`);
      }
    }
    await next();
  });
  ...
  const resolveGroupActivation = (params: {
    ...
    try {
      ...
      logVerbose(`Failed to load session for activation check: ${String(error)}`);
    }
    return undefined;
  };
