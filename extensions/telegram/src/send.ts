...
import { type RetryConfig, createTelegramRetryRunner } from "openclaw/plugin-sdk/retry-runtime";
...
interface TelegramThreadScopedParams {
...
interface TelegramReactionOpts {
  cfg?: ReturnType<typeof loadConfig>;
  token?: string;
  accountId?: string;
  api?: TelegramApiOverride;
...
function createTelegramRequestWithDiag(params: {
  ...
  return <T>(
    ...
    return call.catch((error) => {
      logHttpError(label ?? "request", error);
      throw error;
    });
  };
}
...
>(
  ...
  try {
    ...
    if (!hasMessageThreadIdParam(params) || !isTelegramThreadNotFoundError(error)) {
      throw error;
    }
    if (verbose) {
      sendLogger.warn(
        `telegram ${label} failed with message_thread_id, retrying without thread: ${formatErrorMessage(error)}`,
      );
    }
...
function createRequestWithChatNotFound(params: {
  ...
  return async <T>(fn: () => Promise<T>, label: string) =>
    params.requestWithDiag(fn, label).catch((error) => {
      throw wrapTelegramChatNotFoundError(error, {
...
export async function sendMessageTelegram(
  ...
  const renderHtmlText = (value: string) => renderTelegramHtmlText(value, { tableMode, textMode });
  ...
  const sendTelegramTextChunks = async (
    ...
    return { chatId: lastChatId, messageId: lastMessageId };
  };
  ...
  if (mediaUrl) {
    ...
    if (needsSeparateText && followUpText) {
      if (textMode === "html") {
        ...
        return { chatId: resolvedChatId, messageId: textResult.messageId };
      }
      const textResult = await sendTelegramTextChunks(
        [{ htmlText: renderHtmlText(followUpText), plainText: followUpText }],
        "text follow-up send",
      );
      return { chatId: resolvedChatId, messageId: textResult.messageId };
    }

    return { chatId: resolvedChatId, messageId: String(mediaMessageId) };
  }
  ...
  if (textMode === "html") {
    ...
    textResult = await sendTelegramTextChunks(
      [{ htmlText: renderHtmlText(text), plainText: opts.plainText ?? text }],
      "text send",
    );
  }
  recordChannelActivity({
...
export async function reactMessageTelegram(
  ...
  const reactions: ReactionType[] =
    remove || !trimmedEmoji
      ? []
      : [{ emoji: trimmedEmoji as ReactionTypeEmoji["emoji"], type: "emoji" }];
...
export async function pinMessageTelegram(
  ...
  return { chatId, messageId: String(messageId), ok: true };
}
...
interface TelegramEditReplyMarkupOpts {
  token?: string;
  accountId?: string;
  verbose?: boolean;
  api?: TelegramApiOverride;
...
export async function editMessageTelegram(
  ...
  const htmlText = renderTelegramHtmlText(text, { tableMode, textMode });
...
export async function sendStickerTelegram(
  ...
  return { chatId: resolvedChatId, messageId: String(messageId) };
}

interface TelegramPollOpts {
  cfg?: ReturnType<typeof loadConfig>;
  token?: string;
  accountId?: string;
  verbose?: boolean;
...
export async function sendPollTelegram(
  ...
  return { chatId: resolvedChatId, messageId: String(messageId), pollId };
}
...
interface TelegramCreateForumTopicOpts {
  cfg?: ReturnType<typeof loadConfig>;
  token?: string;
  accountId?: string;
  api?: TelegramApiOverride;
...
export interface TelegramCreateForumTopicResult {
  topicId: number;
  name: string;
  chatId: string;
}
