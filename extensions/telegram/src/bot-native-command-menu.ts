...
import { TELEGRAM_COMMAND_NAME_PATTERN, normalizeTelegramCommandName } from "./command-config.js";
...
interface TelegramPluginCommandSpec {
  name: unknown;
  description: unknown;
}
...
async function readCachedCommandHash(
  ...
  try {
    return (await fs.readFile(resolveCommandHashPath(accountId, botIdentity), "utf8")).trim();
  } catch {
    return null;
  }
}
  ...
  const sync = async () => {
    ...
    while (retryCommands.length > 0) {
      try {
        ...
        if (!isBotCommandsTooMuchError(error)) {
          throw error;
        }
