...
export interface TelegramInboundBodyResult {
  bodyText: string;
  rawBody: string;
  historyKey?: string;
...
export async function resolveTelegramInboundBody(params: {
  ...
  const commandGate = resolveControlCommandGate({
    ...
    authorizers: [{ allowed: senderAllowedForCommands, configured: allowForCommands.hasEntries }],
  ...
  const stickerSupportsVision = msg.sticker
    ? await resolveStickerVisionSupport({ agentId: routeAgentId, cfg })
    : false;
  ...
  if (needsPreflightTranscription) {
    try {
      ...
      logVerbose(`telegram: audio preflight transcription failed: ${String(error)}`);
    }
  }
