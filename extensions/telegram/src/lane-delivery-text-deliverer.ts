...
interface CreateLaneTextDelivererParams {
...
interface DeliverLaneTextParams {
  laneName: LaneName;
  text: string;
...
interface TryUpdatePreviewParams {
  lane: DraftLaneState;
  laneName: LaneName;
  text: string;
  previewButtons?: TelegramInlineButtons;
...
interface ConsumeArchivedAnswerPreviewParams {
  lane: DraftLaneState;
  text: string;
  payload: ReplyPayload;
  previewButtons?: TelegramInlineButtons;
    ...
    try {
      ...
      if (args.context === "final") {
        if (args.finalTextAlreadyLanded) {
          params.log(
            `telegram: ${args.laneName} preview final edit failed after stop flush; keeping existing preview (${String(error)})`,
          );
          params.markDelivered();
          return "retained";
        }
        if (isSafeToRetrySendError(error)) {
          params.log(
            `telegram: ${args.laneName} preview final edit failed before reaching Telegram; falling back to standard send (${String(error)})`,
          );
          return "fallback";
        }
        if (isMissingPreviewMessageError(error)) {
          if (args.retainAlternatePreviewOnMissingTarget) {
            params.log(
              `telegram: ${args.laneName} preview final edit target missing; keeping alternate preview without fallback (${String(error)})`,
            );
            params.markDelivered();
            return "retained";
          }
          params.log(
            `telegram: ${args.laneName} preview final edit target missing with no alternate preview; falling back to standard send (${String(error)})`,
          );
          return "fallback";
        }
        if (isRecoverableTelegramNetworkError(error, { allowMessageMatch: true })) {
          params.log(
            `telegram: ${args.laneName} preview final edit may have landed despite network error; keeping existing preview (${String(error)})`,
          );
          params.markDelivered();
          return "retained";
        }
        if (isTelegramClientRejection(error)) {
          params.log(
            `telegram: ${args.laneName} preview final edit rejected by Telegram (client error); falling back to standard send (${String(error)})`,
          );
          return "fallback";
        }
        // Default: ambiguous error — prefer incomplete over duplicate
        params.log(
          `telegram: ${args.laneName} preview final edit failed with ambiguous error; keeping existing preview to avoid duplicate (${String(error)})`,
        );
        params.markDelivered();
        return "retained";
      }
      params.log(
        `telegram: ${args.laneName} preview ${args.context} edit failed; falling back to standard send (${String(error)})`,
      );
      return "fallback";
    }
  };
