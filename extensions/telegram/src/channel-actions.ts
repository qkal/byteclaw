...
function resolveTelegramActionDiscovery(cfg: Parameters<typeof listEnabledTelegramAccounts>[0]) {
  ...
  const buttonsEnabled = accounts.some((account) =>
    isTelegramInlineButtonsEnabled({ accountId: account.accountId, cfg }),
  );
  return {
    buttonsEnabled,
...
function resolveScopedTelegramActionDiscovery(params: {
  ...
  const account = resolveTelegramAccount({ accountId: params.accountId, cfg: params.cfg });
...
>[0]): ChannelMessageToolDiscovery {
  const discovery = resolveScopedTelegramActionDiscovery({ accountId, cfg });
  if (!discovery) {
    return {
      actions: [],
      capabilities: [],
