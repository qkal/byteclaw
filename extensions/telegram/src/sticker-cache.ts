...
function loadCache(): StickerCache {
  ...
  if (!data || typeof data !== "object") {
    return { stickers: {}, version: CACHE_VERSION };
  }
...
export function searchStickers(query: string, limit = 10): CachedSticker[] {
  ...
  const results: { sticker: CachedSticker; score: number }[] = [];
...
export async function describeStickerImage(params: DescribeStickerParams): Promise<string | null> {
  ...
  try {
    ...
    if (supportsVision) {
      activeModel = { model: defaultModel.model, provider: defaultModel.provider };
    }
  } catch {
  ...
  const hasProviderKey = async (provider: string) => {
    try {
      await resolveApiKeyForProvider({ agentDir, cfg, provider });
      return true;
    } catch {
      return false;
    }
  ...
  if (!resolved) {
    for (const provider of autoProviders) {
      ...
      if (entry) {
        resolved = { model: entry.id, provider };
        break;
      }
    }
  }
  ...
  try {
    ...
    logVerbose(`telegram: failed to describe sticker: ${String(error)}`);
    return null;
  }
}
