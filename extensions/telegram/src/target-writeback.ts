...
export async function maybePersistResolvedTelegramTarget(params: {
  ...
  try {
    ...
    if (params.verbose) {
      writebackLogger.warn(`failed to persist Telegram defaultTo target ${raw}: ${String(error)}`);
    }
  }
  ...
  try {
    ...
    if (params.verbose) {
      writebackLogger.warn(`failed to persist Telegram cron target ${raw}: ${String(error)}`);
    }
  }
}
