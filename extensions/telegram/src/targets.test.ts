...
describe("telegram allow-from helpers", () => {
  it("normalizes tg/telegram prefixes", () => {
    const cases = [
      { expected: "123", value: " TG:123 " },
      { expected: "@someone", value: "telegram:@someone" },
    ] as const;
  ...
  it("accepts signed numeric IDs", () => {
    const cases = [
      { expected: true, value: "123456789" },
      { expected: true, value: "-1001234567890" },
      { expected: false, value: "@someone" },
      { expected: false, value: "12 34" },
    ] as const;
    for (const testCase of cases) {
