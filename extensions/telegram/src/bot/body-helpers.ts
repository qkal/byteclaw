...
export interface TelegramPrimaryMedia {
  placeholder: string;
...
export function resolveTelegramPrimaryMedia(
  ...
  if (photo) {
    return { fileRef: photo, placeholder: "<media:image>" };
  }
  if (msg.video) {
    return { fileRef: msg.video, placeholder: "<media:video>" };
  }
  if (msg.video_note) {
    return { fileRef: msg.video_note, placeholder: "<media:video>" };
  }
  if (msg.audio) {
    return { fileRef: msg.audio, placeholder: "<media:audio>" };
  }
  if (msg.voice) {
    return { fileRef: msg.voice, placeholder: "<media:audio>" };
  }
  if (msg.document) {
    return { fileRef: msg.document, placeholder: "<media:document>" };
  }
  if (msg.sticker) {
    return { fileRef: msg.sticker, placeholder: "<media:sticker>" };
  }
  return undefined;
}
...
interface TelegramTextLinkEntity {
  type: string;
  offset: number;
  length: number;
  url?: string;
...
export interface TelegramForwardedContext {
  from: string;
  date?: number;
  fromType: string;
  fromId?: string;
...
function normalizeForwardedUserLabel(user: User) {
  ...
  return { display, id, name: name || undefined, username };
}
...
function normalizeForwardedChatLabel(chat: Chat, fallbackKind: "chat" | "channel") {
  ...
  return { display, id, title, username };
}
