...
function buildTelegramMediaSsrfPolicy(apiRoot?: string, dangerouslyAllowPrivateNetwork?: boolean) {
  ...
  if (apiRoot) {
    try {
      ...
      logVerbose(`telegram: invalid apiRoot URL "${apiRoot}": ${String(error)}`);
    }
  }
  return {
...
async function resolveTelegramFileWithRetry(
  ...
  try {
    ...
    logVerbose(`telegram: getFile failed after retries: ${String(error)}`);
    return null;
  }
}
  ...
  if (trustedLocalFile) {
    ...
    try {
      ...
      throw new MediaFetchError(
        "fetch_failed",
        `Failed to read local Telegram Bot API media from ${params.filePath}: ${formatErrorMessage(error)}`,
        { cause: error },
      );
    }
...
> {
  ...
  try {
    ...
    logVerbose(`telegram: failed to process sticker: ${String(error)}`);
    return null;
  }
}
...
export async function resolveMedia(params: {
  ...
  return { contentType: saved.contentType, path: saved.path, placeholder };
}
