...
interface StoredTelegramBindingState {
  version: number;
  bindings: TelegramThreadBindingRecord[];
}

export interface TelegramThreadBindingManager {
  accountId: string;
...
interface TelegramThreadBindingsState {
...
function persistBindingsSafely(params: {
  ...
  void enqueuePersistBindings(params).catch((error) => {
    logVerbose(
      `telegram thread bindings persist failed (${params.accountId}, ${params.reason}): ${String(error)}`,
    );
  });
}
    ...
    bind: async (input) => {
      ...
      if (placement === "child") {
        ...
        try {
          ...
          logVerbose(
            `telegram: child thread-binding failed for ${chatId}: ${formatErrorMessage(error)}`,
          );
          return null;
        }
      } else {
