...
interface TelegramNativeReplyChannelData {
  buttons?: TelegramInlineButtons;
  pin?: boolean;
}

interface TelegramCommandAuthResult {
  chatId: number;
  isGroup: boolean;
  isForum: boolean;
...
export interface RegisterTelegramNativeCommandsParams {
  bot: Bot;
  cfg: OpenClawConfig;
  runtime: RuntimeEnv;
  accountId: string;
...
async function resolveTelegramCommandAuth(params: {
  ...
  const commandAuthorized = commandsAllowFromConfigured
    ...
    : resolveCommandAuthorizedFromAuthorizers({
        authorizers: [
          { allowed: senderAllowed, configured: dmAllow.hasEntries },
          ...(isGroup
            ? [{ allowed: groupSenderAllowed, configured: effectiveGroupAllow.hasEntries }]
            : []),
        ],
...
export const registerTelegramNativeCommands = ({
  ...
  const boundRoute =
    nativeEnabled && nativeSkillsEnabled
      ? resolveAgentRoute({ accountId, cfg, channel: "telegram" })
      : null;
  ...
  const allCommandsFull: { command: string; description: string }[] = [
    ...nativeCommands
      .map((command) => {
  ...
  const resolveCommandRuntimeContext = async (params: {
    ...
    return { chatId, chunkMode, mediaLocalRoots, route, tableMode, threadSpec };
  };
