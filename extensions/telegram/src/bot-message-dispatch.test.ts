...
const buildModelsProviderData = vi.hoisted(() =>
  vi.fn(async () => ({
    ...
    providers: [],
    resolvedDefault: { model: "gpt-test", provider: "openai" },
  })),
);
...
describe("dispatchTelegramMessage draft streaming", () => {
  ...
  function createContext(overrides?: Partial<TelegramMessageContext>): TelegramMessageContext {
    const base = {
      ...
      route: { accountId: "default", agentId: "default" },
  ...
  it("does not inject approval buttons in local dispatch once the monitor owns approvals", async () => {
    ...
    const deliveredPayload = (deliverReplies.mock.calls[0]?.[0] as { replies?: unknown[] })
  ...
  it("does not overwrite finalized preview when additional final payloads are sent", async () => {
    ...
    deliverReplies.mockResolvedValue({ delivered: true });
    editMessageTelegram.mockResolvedValue({ chatId: "123", messageId: "999", ok: true });
  ...
  it("keeps streamed preview visible when final text regresses after a tool warning", async () => {
    ...
    dispatchReplyWithBufferedBlockDispatcher.mockImplementation(
      async ({ dispatcherOptions, replyOptions }) => {
        ...
        await dispatcherOptions.deliver(
          { isError: true, text: "⚠️ Recovered tool error details" },
          { kind: "tool" },
        );
  ...
  it("materializes boundary preview and keeps it when no matching final arrives", async () => {
    ...
    const bot = createBot();
    await dispatchWithContext({ bot, context: createContext(), streamMode: "partial" });
  ...
  it("waits for queued boundary rotation before final lane delivery", async () => {
    ...
    deliverReplies.mockResolvedValue({ delivered: true });
    editMessageTelegram.mockResolvedValue({ chatId: "123", messageId: "1001", ok: true });
  ...
  it("still finalizes the active preview after an archived final edit is retained", async () => {
    ...
    editMessageTelegram
      ...
      .mockResolvedValueOnce({ chatId: "123", messageId: "1002", ok: true });
  ...
  it("clears the active preview when a later final falls back after archived retain", async () => {
    ...
    const finalTextSentViaDeliverReplies = deliverReplies.mock.calls.some((call: unknown[]) =>
      (call[0] as { replies?: { text?: string }[] })?.replies?.some(
  ...
  it.each(["partial", "block"] as const)(
    ...
    async (streamMode) => {
      ...
      await dispatchWithContext({ bot, context: createContext(), streamMode });
  ...
  it.each(["partial", "block"] as const)(
    ...
    async (streamMode) => {
      ...
      deliverReplies.mockResolvedValue({ delivered: true });
      editMessageTelegram.mockResolvedValue({ chatId: "123", messageId: "111", ok: true });
  ...
  it("does not edit preview message when final payload is an error", async () => {
    ...
    dispatchReplyWithBufferedBlockDispatcher.mockImplementation(
      async ({ dispatcherOptions, replyOptions }) => {
        ...
        await dispatcherOptions.deliver(
          { isError: true, text: "⚠️ 🛠️ Exec: cat /nonexistent failed: No such file" },
          { kind: "final" },
        );
        return { queuedFinal: true };
      },
  ...
  it("clears preview for error-only finals", async () => {
    ...
    createTelegramDraftStream.mockReturnValue(draftStream);
    dispatchReplyWithBufferedBlockDispatcher.mockImplementation(async ({ dispatcherOptions }) => {
      await dispatcherOptions.deliver({ isError: true, text: "tool failed" }, { kind: "final" });
      await dispatcherOptions.deliver({ isError: true, text: "another error" }, { kind: "final" });
      return { queuedFinal: true };
    });
    deliverReplies.mockResolvedValue({ delivered: true });
  ...
  it("handles error block + response final — error delivered, response finalizes preview", async () => {
    ...
    dispatchReplyWithBufferedBlockDispatcher.mockImplementation(
      async ({ dispatcherOptions, replyOptions }) => {
        ...
        await dispatcherOptions.deliver(
          { isError: true, text: "⚠️ exec failed" },
          { kind: "block" },
        );
        await dispatcherOptions.deliver(
