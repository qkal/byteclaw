...
export function parseZalouserMessageSidFull(
  ...
  return { cliMsgId: cliMsgIdPart, msgId: msgIdPart };
}
...
export function resolveZalouserReactionMessageIds(params: {
  ...
  if (explicitMessageId && explicitCliMsgId) {
    return { cliMsgId: explicitCliMsgId, msgId: explicitMessageId };
  }
  ...
  if (explicitMessageId && !explicitCliMsgId) {
    return { cliMsgId: currentRaw, msgId: explicitMessageId };
  }
  if (!explicitMessageId && explicitCliMsgId) {
    return { cliMsgId: explicitCliMsgId, msgId: currentRaw };
  }
  return { cliMsgId: currentRaw, msgId: currentRaw };
}
