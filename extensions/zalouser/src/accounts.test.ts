...
describe("zalouser account resolution", () => {
  ...
  it("resolves sync account by merging base + account config", () => {
    ...
    const resolved = resolveZalouserAccountSync({ accountId: "work", cfg });
  ...
  it("defaults group policy to allowlist when unset", () => {
    ...
    const resolved = resolveZalouserAccountSync({ accountId: "default", cfg });
  ...
  it("resolves profile precedence correctly", () => {
    ...
    expect(resolveZalouserAccountSync({ accountId: "work", cfg }).profile).toBe("zalo-env");

    delete process.env.ZALOUSER_PROFILE;
    process.env.ZCA_PROFILE = "zca-env";
    expect(resolveZalouserAccountSync({ accountId: "work", cfg }).profile).toBe("zca-env");

    delete process.env.ZCA_PROFILE;
    expect(resolveZalouserAccountSync({ accountId: "work", cfg }).profile).toBe("work");
  });
  ...
  it("uses explicit profile from config over env fallback", () => {
    ...
    expect(resolveZalouserAccountSync({ accountId: "work", cfg }).profile).toBe("explicit-profile");
  });
  ...
  it("checks authentication during async account resolution", async () => {
    ...
    const resolved = await resolveZalouserAccount({ accountId: "default", cfg });
