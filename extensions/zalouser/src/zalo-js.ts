...
interface StoredZaloCredentials {
  imei: string;
...
function extractSendMessageId(result: unknown): string | undefined {
  ...
  const payload = result as {
    ...
    attachment?: { msgId?: string | number }[];
  };
...
function resolveUploadedVoiceAsset(
  ...
  for (const item of uploaded) {
    ...
    if (fileType === "others" || fileType === "video") {
      return { fileName: normalizeOptionalString(item.fileName), fileUrl };
    }
  }
  return undefined;
}
...
function touchCredentials(profile: string): void {
  ...
  fs.writeFileSync(resolveCredentialsPath(profile), JSON.stringify(next, null, 2), "utf8");
}
...
export async function sendZaloTextMessage(
  ...
  if (!trimmedThreadId) {
    return { error: "No threadId provided", ok: false };
  }
  ...
  try {
    if (options.mediaUrl?.trim()) {
      ...
      return { messageId: extractSendMessageId(response), ok: true };
    }
...
export async function sendZaloReaction(params: {
  ...
  if (!threadId || !msgId || !cliMsgId) {
    return { error: "threadId, msgId, and cliMsgId are required", ok: false };
  }
  try {
    const api = await ensureApi(profile);
    const type = params.isGroup ? ThreadType.Group : ThreadType.User;
    const icon = params.remove
      ? { icon: "", rType: -1, source: 6 }
...
export async function sendZaloLink(
  ...
  try {
    ...
    return { messageId: String(response.msgId), ok: true };
  } catch (error) {
    return { error: toErrorMessage(error), ok: false };
  }
}
...
export async function resolveZaloGroupsByEntries(params: {
  profile?: string | null;
  entries: string[];
}): Promise<{ input: string; resolved: boolean; id?: string }[]> {
  ...
  return params.entries.map((input) => {
    ...
    if (/^\d+$/.test(trimmed)) {
      return { id: trimmed, input, resolved: true };
    }
    const candidates = byName.get(normalizeLowercaseStringOrEmpty(trimmed)) ?? [];
    const match = candidates[0];
    return match ? { id: match.groupId, input, resolved: true } : { input, resolved: false };
  });
}
...
export async function resolveZaloAllowFromEntries(params: {
  profile?: string | null;
  entries: string[];
}): Promise<{ input: string; resolved: boolean; id?: string; note?: string }[]> {
