...
function normalizeNotifyState(raw: unknown): NotifyStateFile {
  ...
  return { notifiedRequestIds, subscribers };
}
  ...
  api.registerService({
    ...
    start: async (ctx) => {
      ...
      await tick().catch((error) => {
        api.logger.warn(
          `device-pair: initial notify poll failed: ${String((error as Error)?.message ?? error)}`,
        );
      });
      ...
      notifyInterval = setInterval(() => {
        tick().catch((error) => {
          api.logger.warn(
            `device-pair: notify poll failed: ${String((error as Error)?.message ?? error)}`,
          );
        });
