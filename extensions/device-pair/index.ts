...
interface DevicePairPluginConfig {
  publicUrl?: string;
}
...
function resolveAuthLabel(cfg: OpenClawPluginApi["config"]): ResolveAuthLabelResult {
  ...
  if (mode === "token" || mode === "password") {
    return resolveRequiredAuthLabel(mode, { password, token });
  }
  if (token) {
    return { label: "token" };
  }
...
function pickFirstDefined(candidates: unknown[]): string | null {
...
async function resolveGatewayUrl(api: OpenClawPluginApi): Promise<ResolveUrlResult> {
  ...
  if (remoteUrl) {
    ...
    if (url) {
      return { source: "gateway.remote.url", url };
    }
  }
