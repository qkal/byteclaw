...
const IAM_TOKEN_TTL_MS = 3_600_000; // Refresh every 1 hour (tokens valid up to 12h)
...
export async function generateBearerTokenFromIam(params: {
  ...
  try {
    ...
    const token = await getTokenProvider({
      ...
      region: params.region, // 2 hours
    })();
    iamTokenCache.set(params.region, { expiresAt: now + IAM_TOKEN_TTL_MS, token });
    return token;
  } catch (error) {
...
export async function discoverMantleModels(params: {
  ...
  try {
    ...
    discoveryCache.set(cacheKey, { fetchedAt: now(), models });
    return models;
  } catch (error) {
...
export async function resolveImplicitMantleProvider(params: {
  ...
  log.debug?.("Mantle provider resolved", { modelCount: models.length, region });
