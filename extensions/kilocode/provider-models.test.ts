...
import { KILOCODE_MODELS_URL, discoverKilocodeModels } from "./provider-models.js";
...
describe("discoverKilocodeModels", () => {
  ...
  it("static catalog has correct defaults for kilo/auto", async () => {
    ...
    expect(auto?.maxTokens).toBe(128_000);
    expect(auto?.cost).toEqual({ cacheRead: 0, cacheWrite: 0, input: 0, output: 0 });
  });
});
