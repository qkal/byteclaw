...
describe("extractGeminiCliCredentials", () => {
  ...
  function makeFakeLayout() {
    ...
    return { binDir, geminiPath, oauth2Path, resolvedPath };
  }
  ...
  it("extracts credentials from oauth2.js in known path", async () => {
    installGeminiLayout({ oauth2Content: FAKE_OAUTH2_CONTENT, oauth2Exists: true });
  ...
  it("extracts credentials when PATH entry is an npm global shim", async () => {
    installNpmShimLayout({ oauth2Content: FAKE_OAUTH2_CONTENT, oauth2Exists: true });
  ...
  it("returns null when oauth2.js lacks credentials", async () => {
    installGeminiLayout({ oauth2Content: "// no credentials here", oauth2Exists: true });
