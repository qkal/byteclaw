...
import { createChatForHandle, resolveChatGuidForTarget, sendMessageBlueBubbles } from "./send.js";
import {
...
import { type BlueBubblesSendTarget, _setFetchGuardForTesting } from "./types.js";
...
describe("send", () => {
  describe("resolveChatGuidForTarget", () => {
    const resolveHandleTargetGuid = async (data: Record<string, unknown>[]) => {
      mockFetch.mockResolvedValueOnce({
        json: () => Promise.resolve({ data }),
        ok: true,
      });
    ...
    it("queries chats to resolve chat_id target", async () => {
      mockFetch.mockResolvedValueOnce({
        json: () =>
          Promise.resolve({
            data: [
              { guid: "iMessage;-;chat123", id: 123, participants: [] },
              { guid: "iMessage;-;chat456", id: 456, participants: [] },
            ],
          }),
        ok: true,
      });

      const target: BlueBubblesSendTarget = { chatId: 456, kind: "chat_id" };
      const result = await resolveChatGuidForTarget({
        baseUrl: "http://localhost:1234",
        password: "test",
        target,
    ...
    it("returns null when chat not found", async () => {
      ...
      const target: BlueBubblesSendTarget = { chatId: 999, kind: "chat_id" };
      const result = await resolveChatGuidForTarget({
        baseUrl: "http://localhost:1234",
        password: "test",
        target,
    ...
    it("handles API error gracefully", async () => {
      ...
      const target: BlueBubblesSendTarget = { chatId: 123, kind: "chat_id" };
      const result = await resolveChatGuidForTarget({
        baseUrl: "http://localhost:1234",
        password: "test",
        target,
    ...
    it("paginates through chats to find match", async () => {
      mockFetch
        ...
        .mockResolvedValueOnce({
          json: () =>
            Promise.resolve({
              data: [{ guid: "found-chat", id: 555, participants: [] }],
            }),
          ok: true,
        });

      const target: BlueBubblesSendTarget = { chatId: 555, kind: "chat_id" };
      const result = await resolveChatGuidForTarget({
        baseUrl: "http://localhost:1234",
        password: "test",
        target,
    ...
    it("extracts guid from various response formats", async () => {
      ...
      const target: BlueBubblesSendTarget = { chatId: 100, kind: "chat_id" };
      const result = await resolveChatGuidForTarget({
        baseUrl: "http://localhost:1234",
        password: "test",
        target,
