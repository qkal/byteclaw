...
export async function fetchBlueBubblesServerInfo(params: {
  ...
  const url = buildBlueBubblesApiUrl({ baseUrl, password, path: "/api/v1/server/info" });
  try {
    ...
    if (data) {
      serverInfoCache.set(cacheKey, { expires: Date.now() + CACHE_TTL_MS, info: data });
...
export async function probeBlueBubbles(params: {
  ...
  const password = normalizeSecretInputString(params.password);
  if (!baseUrl) {
    return { error: "serverUrl not configured", ok: false };
  }
  if (!password) {
    return { error: "password not configured", ok: false };
  }
  const probeSsrfPolicy = params.allowPrivateNetwork ? { allowPrivateNetwork: true } : {};
  const url = buildBlueBubblesApiUrl({ baseUrl, password, path: "/api/v1/ping" });
  try {
    const res = await blueBubblesFetchWithTimeout(
      url,
      { method: "GET" },
    ...
    if (!res.ok) {
      return { error: `HTTP ${res.status}`, ok: false, status: res.status };
    }
