...
interface ContactNameCacheEntry {
  name?: string;
  expiresAt: number;
}
...
interface ParticipantContactNameDeps {
...
interface ResolvedParticipantContactNameDeps {
  platform: NodeJS.Platform;
  now: () => number;
  resolvePhoneNames?: ResolvePhoneNamesFn;
  homeDir?: string;
...
async function queryContactsDatabase(
  ...
  phoneKeys: string[],
  deps: ResolvedParticipantContactNameDeps,
): Promise<{ phoneKey: string; name: string }[]> {
...
`;
  ...
  const rows: { phoneKey: string; name: string }[] = [];
...
async function resolvePhoneNamesFromMacOsContacts(
  ...
  for (const dbPath of databases) {
    let rows: { phoneKey: string; name: string }[] = [];
    try {
