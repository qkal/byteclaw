...
export async function handleBlueBubblesWebhookRequest(
  ...
  return await withResolvedWebhookRequestPipeline({
    ...
    handle: async ({ path, targets }) => {
      ...
      if (reaction) {
        processReaction(reaction, target).catch((error) => {
          target.runtime.error?.(
            `[${target.account.accountId}] BlueBubbles reaction failed: ${String(error)}`,
          );
        });
      } else if (message) {
        ...
        debouncer.enqueue({ message, target }).catch((error) => {
          target.runtime.error?.(
            `[${target.account.accountId}] BlueBubbles webhook failed: ${String(error)}`,
          );
        });
      }
