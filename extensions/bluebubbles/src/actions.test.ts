...
describe("bluebubblesMessageActions", () => {
  ...
  describe("describeMessageTool", () => {
    ...
    it("honors account-scoped action gates during discovery", () => {
      ...
      expect(describeMessageTool({ accountId: "default", cfg })?.actions).not.toContain("react");
      expect(describeMessageTool({ accountId: "work", cfg })?.actions).toContain("react");
    });
  ...
  describe("handleAction", () => {
    ...
    it("throws a private-api error for private-only actions when disabled", async () => {
      ...
      await expect(
        callHandleAction({
          ...
          action: "react",
          cfg,
          params: { chatGuid: "iMessage;-;+15551234567", emoji: "❤️", messageId: "msg-123" },
        }),
    ...
    it("uses toolContext currentChannelId when no explicit target is provided", async () => {
      ...
      expect(resolveChatGuidForTarget).toHaveBeenCalledWith(
        expect.objectContaining({
          target: { chatGuid: "iMessage;-;+15550001111", kind: "chat_guid" },
        }),
      );
      expect(sendBlueBubblesReaction).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it("accepts message param for edit action", async () => {
      ...
      await callHandleAction({
        ...
        action: "edit",
        cfg,
        params: { message: "updated", messageId: "msg-123" },
      });

      expect(editBlueBubblesMessage).toHaveBeenCalledWith(
        "msg-123",
        "updated",
        expect.objectContaining({ accountId: undefined, cfg }),
      );
    });
    ...
    it("accepts message/target aliases for sendWithEffect", async () => {
      ...
      expect(result).toMatchObject({
        details: { effect: "invisible ink", messageId: "msg-123", ok: true },
      });
    });
    ...
    it("sets group icon successfully with chatGuid and buffer", async () => {
      ...
      expect(result).toMatchObject({
        details: { chatGuid: "iMessage;-;chat-guid", iconSet: true, ok: true },
      });
    });
