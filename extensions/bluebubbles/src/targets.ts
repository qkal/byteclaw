...
const SERVICE_PREFIXES: { prefix: string; service: BlueBubblesService }[] = [
...
function parseRawChatIdentifierTarget(
  ...
  if (/^chat\d+$/i.test(trimmed)) {
    return { chatIdentifier: trimmed, kind: "chat_identifier" };
  }
...
export function parseBlueBubblesTarget(raw: string): BlueBubblesTarget {
  ...
  const groupTarget = parseGroupTarget({ lower, requireValue: true, trimmed });
  if (groupTarget) {
    return groupTarget;
  }
  ...
  if (rawChatGuid) {
    return { chatGuid: rawChatGuid, kind: "chat_guid" };
  }
  ...
  return { kind: "handle", service: "auto", to: trimmed };
}
...
export function parseBlueBubblesAllowTarget(raw: string): BlueBubblesAllowTarget {
  ...
  const groupTarget = parseGroupTarget({ lower, requireValue: false, trimmed });
  if (groupTarget) {
    return groupTarget;
  }
  ...
  return { handle: normalizeBlueBubblesHandle(trimmed), kind: "handle" };
}
