...
export async function downloadImageFeishu(params: {
  ...
  const { client } = createConfiguredFeishuMediaClient({ accountId, cfg });
...
export async function downloadMessageResourceFeishu(params: {
  ...
  const response = await client.im.messageResource.get({
    params: { type },
    path: { file_key: normalizedFileKey, message_id: messageId },
  });
...
export async function sendMediaFeishu(params: {
  ...
  const routing = resolveFeishuOutboundMediaKind({ contentType, fileName: name });

  if (routing.msgType === "image") {
    const { imageKey } = await uploadImageFeishu({ accountId, cfg, image: buffer });
    return sendImageFeishu({ accountId, cfg, imageKey, replyInThread, replyToMessageId, to });
  } else {
