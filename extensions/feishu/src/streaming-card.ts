...
export class FeishuStreamingSession {
  ...
  async start(
    ...
    const elements: Record<string, unknown>[] = [
      { content: "⏳ Thinking...", element_id: "content", tag: "markdown" },
    ];
    if (options?.note) {
    ...
    const { response: createRes, release: releaseCreate } = await fetchWithSsrFGuard({
      ...
      init: {
        body: JSON.stringify({ data: JSON.stringify(cardJson), type: "card_json" }),
        headers: {
    ...
    const cardContent = JSON.stringify({ data: { card_id: cardId }, type: "card" });
  ...
  private async updateNoteContent(note: string): Promise<void> {
    ...
    await fetchWithSsrFGuard({
      ...
      .catch((error) => this.log?.(`Note update failed: ${String(error)}`));
  }
  ...
  async close(finalText?: string, options?: { note?: string }): Promise<void> {
    ...
    await fetchWithSsrFGuard({
      ...
      .catch((error) => this.log?.(`Close failed: ${String(error)}`));
