...
interface ResolveReactionSyntheticEventParams {
  cfg: ClawdbotConfig;
  accountId: string;
...
export async function resolveReactionSyntheticEvent(
  ...
  const reactedMsg = await raceWithTimeoutAndAbort(fetchMessage({ accountId, cfg, messageId }), {
...
interface RegisterEventHandlersContext {
  cfg: ClawdbotConfig;
  accountId: string;
  runtime?: RuntimeEnv;
  ...
  const runFeishuHandler = async (params: { task: () => Promise<void>; errorMessage: string }) => {
    if (fireAndForget) {
      void params.task().catch((error) => {
        error(`${params.errorMessage}: ${String(error)}`);
      });
      return;
    }
    try {
  ...
  const recordSuppressedMessageIds = async (
    ...
    for (const messageId of suppressedIds) {
      try {
        ...
        error(
          `feishu[${accountId}]: failed to record merged dedupe id ${messageId}: ${String(error)}`,
        );
      }
    }
  };
  ...
  eventDispatcher.register({
    "application.bot.menu_v6": async (data) => {
      try {
        ...
        if (fireAndForget) {
          promise.catch((error) => {
            error(`feishu[${accountId}]: error handling bot menu event: ${String(error)}`);
          });
          return;
        }
        await promise;
    ...
    "card.action.trigger": async (data: unknown) => {
      try {
        ...
        if (fireAndForget) {
          promise.catch((error) => {
            error(`feishu[${accountId}]: error handling card action: ${String(error)}`);
          });
        } else {
          await promise;
        }
    ...
    "im.chat.member.bot.added_v1": async (data) => {
      try {
        ...
        error(`feishu[${accountId}]: error handling bot added event: ${String(error)}`);
      }
    },
    "im.chat.member.bot.deleted_v1": async (data) => {
      try {
        ...
        error(`feishu[${accountId}]: error handling bot removed event: ${String(error)}`);
      }
    },
    ...
    "im.message.receive_v1": async (data) => {
      ...
      if (fireAndForget) {
        void processMessage().catch((error) => {
          ...
          error(`feishu[${accountId}]: error handling message: ${String(error)}`);
        });
        return;
      }
      try {
...
export interface MonitorSingleAccountParams {
  cfg: ClawdbotConfig;
...
export async function monitorSingleAccount(params: MonitorSingleAccountParams): Promise<void> {
  ...
  const botIdentity =
    botOpenIdSource.kind === "prefetched"
      ? { botName: botOpenIdSource.botName, botOpenId: botOpenIdSource.botOpenId }
      : await fetchBotIdentityForMonitor(account, { abortSignal, runtime });
  const { botOpenId } = applyBotIdentityState(accountId, botIdentity);
  log(`feishu[${accountId}]: bot open_id resolved: ${botOpenId ?? "unknown"}`);

  if (!botOpenId && !abortSignal?.aborted) {
    startBotIdentityRecovery({ abortSignal, account, accountId, runtime });
  }
  ...
  try {
    ...
    if (connectionMode === "webhook") {
      return await monitorWebhook({ abortSignal, account, accountId, eventDispatcher, runtime });
    }
    return await monitorWebSocket({ abortSignal, account, accountId, eventDispatcher, runtime });
  } finally {
