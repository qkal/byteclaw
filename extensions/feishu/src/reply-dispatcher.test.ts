...
interface StreamingSessionStub {
  active: boolean;
...
describe("createFeishuReplyDispatcher streaming behavior", () => {
  ...
  function createRuntimeLogger() {
    return { error: vi.fn(), log: vi.fn() } as never;
  }
  ...
  it("uses streaming session for auto mode markdown payloads", async () => {
    ...
    expect(streamingInstances[0].start).toHaveBeenCalledWith(
      ...
      "chat_id",
      expect.objectContaining({
        header: { template: "blue", title: "agent" },
        note: "Agent: agent",
  ...
  it("suppresses duplicate final text while still sending media", async () => {
    ...
    await options.deliver(
      { mediaUrl: "https://example.com/a.png", text: "plain final" },
      { kind: "final" },
    );
  ...
  it("falls back to legacy mediaUrl when mediaUrls is an empty array", async () => {
    const { options } = createDispatcherHarness();
    await options.deliver(
      { mediaUrl: "https://example.com/a.png", mediaUrls: [], text: "caption" },
      { kind: "final" },
    );
  ...
  it("recovers streaming after start() throws (HTTP 400)", async () => {
    ...
    try {
      createFeishuReplyDispatcher({
        ...
        cfg: {} as never,
        chatId: "oc_chat",
        runtime: { error: errorMock, log: vi.fn() } as never,
      });
