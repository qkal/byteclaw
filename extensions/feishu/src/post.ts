...
interface PostParseResult {
  textContent: string;
  imageKeys: string[];
  mediaKeys: { fileKey: string; fileName?: string }[];
...
function escapeMarkdownText(text: string): string {
  return text.replace(MARKDOWN_SPECIAL_CHARS, String.raw`\$1`);
}
...
export function parsePostContent(content: string): PostParseResult {
  try {
    ...
    const mediaKeys: { fileKey: string; fileName?: string }[] = [];
    ...
    return { imageKeys: [], mediaKeys: [], mentionedOpenIds: [], textContent: FALLBACK_POST_TEXT };
  }
}
