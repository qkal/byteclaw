...
export async function insertTableRow(
  ...
  return { block: res.data?.block, success: true };
}
...
export async function deleteTableRows(
  ...
  const res = await client.docx.documentBlock.patch({
    data: { delete_table_rows: { row_end_index: rowStart + rowCount, row_start_index: rowStart } },
    path: { block_id: blockId, document_id: docToken },
  });
  if (res.code !== 0) {
    throw new Error(res.msg);
  }
  return { block: res.data?.block, rows_deleted: rowCount, success: true };
}
...
export async function deleteTableColumns(
  ...
  return { block: res.data?.block, columns_deleted: columnCount, success: true };
}
