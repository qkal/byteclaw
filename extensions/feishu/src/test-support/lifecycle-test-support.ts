...
interface InboundDebouncerParams<T> {
  onFlush?: (items: T[]) => Promise<void>;
  onError?: (err: unknown, items: T[]) => void;
}
type UnknownMock = Mock<(...args: unknown[]) => unknown>;
type AsyncUnknownMock = Mock<(...args: unknown[]) => Promise<unknown>>;
interface FeishuDispatchReplyCounts {
  final: number;
  block?: number;
  tool?: number;
}
...
interface FeishuDispatchReplyDispatcher {
...
>;
interface FeishuLifecycleReplyDispatcher {
  dispatcher: {
