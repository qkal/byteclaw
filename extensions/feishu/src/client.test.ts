...
describe("createFeishuClient HTTP timeout", () => {
  ...
  it("passes a custom httpInstance with default timeout to Lark.Client", () => {
    createFeishuClient({ accountId: "timeout-test", appId: "app_1", appSecret: "secret_1" }); // Pragma: allowlist secret

    expect(readCallOptions(clientCtorMock).httpInstance).toBeDefined();
  });

  it("injects default timeout into HTTP request options", async () => {
    createFeishuClient({ accountId: "timeout-inject", appId: "app_2", appSecret: "secret_2" }); // Pragma: allowlist secret
    ...
    expect(mockBaseHttpInstance.post).toHaveBeenCalledWith(
      "https://example.com/api",
      { data: 1 },
      expect.objectContaining({ headers: { "X-Custom": "yes" }, timeout: FEISHU_HTTP_TIMEOUT_MS }),
    );
  });

  it("allows explicit timeout override per-request", async () => {
    createFeishuClient({ accountId: "timeout-override", appId: "app_3", appSecret: "secret_3" }); // Pragma: allowlist secret

    const httpInstance = getLastClientHttpInstance();

    expect(httpInstance).toBeDefined();
    await httpInstance?.get("https://example.com/api", { timeout: 5000 });
