...
const WITHDRAWN_REPLY_ERROR_CODES = new Set([230_011, 231_003]);
...
interface FeishuCreateMessageClient {
  im: {
    message: {
      reply: (opts: {
...
interface FeishuGetMessageResponse {
  code?: number;
  msg?: string;
...
function parseFeishuMessageItem(
  ...
  return {
    ...
    contentType: msgType,
    createTime: item.create_time ? Number.parseInt(String(item.create_time), 10) : undefined,
...
export interface FeishuThreadMessageInfo {
  messageId: string;
  senderId?: string;
  senderType?: string;
  content: string;
...
export async function sendMessageFeishu(
  ...
  const { client, receiveId, receiveIdType } = resolveFeishuSendTarget({ accountId, cfg, to });
  ...
  const directParams = { content, msgType, receiveId, receiveIdType };
...
export async function sendCardFeishu(params: SendFeishuCardParams): Promise<FeishuSendResult> {
  ...
  const directParams = { content, msgType: "interactive", receiveId, receiveIdType };
  return sendReplyOrFallbackDirect(client, {
    content,
...
export async function editMessageFeishu(params: {
  ...
  if (card) {
    ...
    return { contentType: "interactive", messageId };
  }
  ...
  return { contentType: "post", messageId };
}
...
export function buildStructuredCard(
  ...
  const elements: Record<string, unknown>[] = [{ content: text, tag: "markdown" }];
  if (options?.note) {
    elements.push({ tag: "hr" });
    elements.push({ content: `<font color='grey'>${options.note}</font>`, tag: "markdown" });
  }
  ...
  if (options?.header) {
    card.header = {
      ...
      title: { content: options.header.title, tag: "plain_text" },
    };
  }
  return card;
}
...
export async function sendStructuredCardFeishu(params: {
  ...
  return sendCardFeishu({ accountId, card, cfg, replyInThread, replyToMessageId, to });
}
