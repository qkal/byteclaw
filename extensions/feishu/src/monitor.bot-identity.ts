...
import { type FeishuMonitorBotIdentity, fetchBotIdentityForMonitor } from "./monitor.startup.js";
...
async function retryBotIdentityProbe(
  ...
  for (let i = 0; i < BOT_IDENTITY_RETRY_DELAYS_MS.length; i += 1) {
    ...
    const identity = await fetchBotIdentityForMonitor(account, { abortSignal, runtime });
