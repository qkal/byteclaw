...
import { type CommentFileType, normalizeCommentFileType } from "./comment-target.js";
import type { ResolvedFeishuAccount } from "./types.js";

const FEISHU_COMMENT_VERIFY_TIMEOUT_MS = 3000;
const FEISHU_COMMENT_REPLY_PAGE_SIZE = 100;
const FEISHU_COMMENT_REPLY_PAGE_LIMIT = 5;
const FEISHU_COMMENT_REPLY_MISS_RETRY_DELAY_MS = 1000;
const FEISHU_COMMENT_REPLY_MISS_RETRY_LIMIT = 6;

interface FeishuDriveCommentUserId {
  open_id?: string;
  user_id?: string;
  union_id?: string;
}

export interface FeishuDriveCommentNoticeEvent {
  comment_id?: string;
  event_id?: string;
...
interface ResolveDriveCommentEventParams {
  cfg: ClawdbotConfig;
  accountId: string;
...
export interface ResolvedDriveCommentEventTurn {
  eventId: string;
  messageId: string;
  commentId: string;
  replyId?: string;
...
interface FeishuOpenApiResponse<T> {
  code?: number;
  log_id?: string;
  msg?: string;
  data?: T;
...
interface FeishuDriveCommentReply {
  reply_id?: string;
  content?: {
    elements?: unknown[];
  };
...
interface FeishuDriveCommentCard {
  comment_id?: string;
  is_whole?: boolean;
  quote?: string;
  reply_list?: {
