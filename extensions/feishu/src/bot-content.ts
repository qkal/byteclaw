...
export interface ResolvedFeishuGroupSession {
  peerId: string;
...
export function normalizeMentions(
  ...
  const escaped = (value: string) => value.replace(/[.*+?^${}()|[\]\\]/g, String.raw`\$&`);
...
export function parseMediaKeys(
  ...
  try {
    ...
    switch (messageType) {
      case "image": {
        return { fileName: parsed.file_name, imageKey };
      }
      case "file":
      case "audio":
      case "sticker": {
      ...
      case "video":
      case "media": {
        return { fileKey, fileName: parsed.file_name, imageKey };
      }
      default: {
        return {};
      }
...
export async function resolveFeishuMediaList(params: {
  ...
  if (messageType === "post") {
    ...
    for (const imageKey of imageKeys) {
      try {
        ...
        log?.(`feishu: failed to download embedded image ${imageKey}: ${String(error)}`);
      }
    }
    ...
    for (const media of mediaKeys) {
      try {
        ...
        log?.(`feishu: failed to download embedded media ${media.fileKey}: ${String(error)}`);
      }
    }
    return out;
  }
  ...
  try {
    ...
    log?.(`feishu: failed to download ${messageType} media: ${String(error)}`);
  }
  return out;
}
