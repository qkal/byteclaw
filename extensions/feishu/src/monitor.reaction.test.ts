...
import { type FeishuReactionCreatedEvent, resolveReactionSyntheticEvent } from "./monitor.js";
...
describe("Feishu inbound debounce regressions", () => {
  ...
  it("keeps bot mention when per-message mention keys collide across non-forward messages", async () => {
    ...
    await enqueueDebouncedMessage(
      onMessage,
      createTextEvent({
        mentions: [createMention({ name: "user-a", openId: "ou_user_a" })],
        messageId: "om_1",
        text: "first",
      }),
    );
    await enqueueDebouncedMessage(
      onMessage,
      createTextEvent({
        mentions: [createMention({ name: "bot", openId: "ou_bot" })],
        messageId: "om_2",
        text: "@bot second",
      }),
    );
  ...
  it("does not synthesize mention-forward intent across separate messages", async () => {
    ...
    await enqueueDebouncedMessage(
      onMessage,
      createTextEvent({
        mentions: [createMention({ name: "alice", openId: "ou_alice" })],
