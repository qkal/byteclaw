 ...
 */
const FEISHU_BACKOFF_CODES = new Set([99_991_400, 99_991_403, 429]);
...
export async function addTypingIndicator(params: {
  ...
  try {
    ...
    if (getFeishuRuntime().logging.shouldLogVerbose()) {
      runtime?.log?.(`[feishu] failed to add typing indicator: ${String(error)}`);
    }
    return { messageId, reactionId: null };
  }
}
...
export async function removeTypingIndicator(params: {
  ...
  try {
    ...
    if (getFeishuRuntime().logging.shouldLogVerbose()) {
      runtime?.log?.(`[feishu] failed to remove typing indicator: ${String(error)}`);
    }
  }
}
