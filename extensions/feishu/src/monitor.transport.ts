...
export interface MonitorTransportParams {
  ...
  return new Promise((resolve, reject) => {
    ...
    const cleanup = () => {
      ...
      try {
        ...
        error(`feishu[${accountId}]: error closing WebSocket client: ${String(error)}`);
      } finally {
  ...
  server.on("request", (req, res) => {
    ...
    void (async () => {
      try {
        ...
        error(`feishu[${accountId}]: webhook handler error: ${String(error)}`);
        if (!res.headersSent) {
