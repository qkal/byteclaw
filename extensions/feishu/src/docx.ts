...
async function convertMarkdown(client: Lark.Client, markdown: string) {
  const res = await client.docx.document.convert({
    data: { content: markdown, content_type: "markdown" },
  });
  if (res.code !== 0) {
...
async function insertBlocksWithDescendant(
  ...
  const res = await client.docx.documentBlockDescendant.create({
    ...
    path: { block_id: parentBlockId, document_id: docToken },
  });
...
async function clearDocumentContent(client: Lark.Client, docToken: string) {
  ...
  if (childIds.length > 0) {
    const res = await client.docx.documentBlockChildren.batchDelete({
      data: { end_index: childIds.length, start_index: 0 },
      path: { block_id: docToken, document_id: docToken },
    });
    if (res.code !== 0) {
      throw new Error(res.msg);
    }
...
async function downloadImage(url: string, maxBytes: number): Promise<Buffer> {
  const fetched = await getFeishuRuntime().channel.media.fetchRemoteMedia({ maxBytes, url });
...
async function uploadImageBlock(
  ...
  const insertRes = await client.docx.documentBlockChildren.create({
    ...
    path: { block_id: parentBlockId ?? docToken, document_id: docToken },
  });
  if (insertRes.code !== 0) {
  ...
  const patchRes = await client.docx.documentBlock.patch({
    ...
    path: { block_id: imageBlockId, document_id: docToken },
  });
  if (patchRes.code !== 0) {
...
async function uploadFileBlock(
  ...
  const childrenRes = await client.docx.documentBlockChildren.get({
    path: { block_id: parentId, document_id: docToken },
  });
  ...
  if (placeholderIdx !== -1) {
    const deleteRes = await client.docx.documentBlockChildren.batchDelete({
      data: { end_index: placeholderIdx + 1, start_index: placeholderIdx },
      path: { block_id: parentId, document_id: docToken },
    });
    if (deleteRes.code !== 0) {
  ...
  if (shouldGrantToRequester) {
    if (!requesterOpenId) {
      ...
      try {
        ...
        requesterPermissionError = formatErrorMessage(error);
      }
    }
  }
...
async function writeDoc(
  ...
  if (blocks.length === 0) {
    return { blocks_added: 0, blocks_deleted: deleted, images_processed: 0, success: true };
  }
...
async function insertDoc(
  ...
  const blockInfo = await client.docx.documentBlock.get({
    path: { block_id: afterBlockId, document_id: docToken },
  });
  if (blockInfo.code !== 0) {
...
async function writeTableCells(
  ...
  const tableRes = await client.docx.documentBlock.get({
    path: { block_id: tableBlockId, document_id: docToken },
  });
  if (tableRes.code !== 0) {
  ...
  for (let r = 0; r < writeRows; r++) {
    ...
    for (let c = 0; c < writeCols; c++) {
      ...
      const childrenRes = await client.docx.documentBlockChildren.get({
        path: { block_id: cellId, document_id: docToken },
      });
      if (childrenRes.code !== 0) {
        throw new Error(childrenRes.msg);
      }
      ...
      if (existingChildren.length > 0) {
        const delRes = await client.docx.documentBlockChildren.batchDelete({
          data: { end_index: existingChildren.length, start_index: 0 },
          path: { block_id: cellId, document_id: docToken },
        });
        if (delRes.code !== 0) {
...
async function updateBlock(
  ...
  return { block_id: blockId, success: true };
}
...
async function deleteBlock(client: Lark.Client, docToken: string, blockId: string) {
  ...
  const res = await client.docx.documentBlockChildren.batchDelete({
    data: { end_index: index + 1, start_index: index },
    path: { block_id: parentId, document_id: docToken },
  });
  if (res.code !== 0) {
    throw new Error(res.msg);
  ...
  return { deleted_block_id: blockId, success: true };
}
...
export function registerFeishuDocTools(api: OpenClawPluginApi) {
  ...
  const getMediaMaxBytes = (
    ...
    (resolveFeishuToolAccount({ api, defaultAccountId, executeParams: params }).config
      ?.mediaMaxMb ?? 30) *
    1024 *
    1024;
