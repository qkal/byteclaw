...
export interface FeishuPermissionError {
  code: number;
  message: string;
  grantUrl?: string;
}

interface SenderNameResult {
  name?: string;
...
export async function resolveFeishuSenderName(params: {
  ...
  try {
    ...
    if (name) {
      senderNameCache.set(normalizedSenderId, { expireAt: now + SENDER_NAME_TTL_MS, name });
      return { name };
    }
    return {};
  } catch (error) {
    ...
    log(`feishu: failed to resolve sender name for ${normalizedSenderId}: ${String(error)}`);
    return {};
  }
}
