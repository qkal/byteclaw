...
import { type CommentFileType, parseFeishuCommentTarget } from "./comment-target.js";
...
interface FeishuExplorerRootFolderMetaResponse {
  code: number;
  msg?: string;
  data?: {
    token?: string;
...
interface FeishuDriveApiResponse<T> {
  code: number;
  log_id?: string;
  msg?: string;
  data?: T;
...
interface FeishuDriveToolContext {
  deliveryContext?: {
    channel?: string;
    to?: string;
  };
...
function isReplyNotAllowedError(error: unknown): boolean {
  ...
  return error.feishuCode === 1_069_302;
}
