...
import { type SignalDaemonHandle, formatSignalDaemonExit, spawnSignalDaemon } from "./daemon.js";
...
export interface MonitorSignalOpts {
  runtime?: RuntimeEnv;
...
function mergeAbortSignals(
  ...
  if (!a && !b) {
    return { dispose: () => {}, signal: undefined };
  }
  if (!a) {
    return { dispose: () => {}, signal: b };
  }
  if (!b) {
    return { dispose: () => {}, signal: a };
  }
...
function normalizeAllowList(raw?: (string | number)[]): string[] {
...
function resolveSignalReactionTargets(reaction: SignalReactionMessage): SignalReactionTarget[] {
  ...
  if (uuid) {
    targets.push({ display: `uuid:${uuid}`, id: uuid, kind: "uuid" });
  }
  const author = reaction.targetAuthor?.trim();
  if (author) {
    const normalized = normalizeE164(author);
    targets.push({ display: normalized, id: normalized, kind: "phone" });
  }
  return targets;
}
...
export async function monitorSignalProvider(opts: MonitorSignalOpts = {}): Promise<void> {
  ...
  const startupTimeoutMs = Math.min(
    120_000,
    Math.max(1000, opts.startupTimeoutMs ?? accountInfo.config.startupTimeoutMs ?? 30_000),
  );
    ...
    await runSignalSseLoop({
      ...
      onEvent: (event) => {
        void handleEvent(event).catch((error) => {
          runtime.error?.(`event handler failed: ${String(error)}`);
        });
      },
