...
const SAMPLE_ASSETS: ReleaseAsset[] = [
  ...
  { browser_download_url: "https://example.com/jvm.tar.gz", name: "signal-cli-0.13.14.tar.gz" },
  {
...
describe("extractSignalCliArchive", () => {
  ...
  it("extracts zip archives", async () => {
    await withArchiveWorkspace(async (workDir) => {
      ...
      const extracted = await fs.readFile(path.join(extractDir, "root", "signal-cli"), "utf8");
  ...
  it("extracts tar.gz archives", async () => {
    await withArchiveWorkspace(async (workDir) => {
      ...
      await fs.writeFile(path.join(rootDir, "signal-cli"), "bin", "utf8");
