...
describe("signalMessageActions", () => {
  ...
  it("honors account-scoped reaction gates during discovery", () => {
    ...
    expect(
      signalMessageActions.describeMessageTool?.({ accountId: "default", cfg })?.actions,
    ).toEqual(["send"]);
    expect(signalMessageActions.describeMessageTool?.({ accountId: "work", cfg })?.actions).toEqual(
      ["send", "react"],
    );
  });
  ...
  it("blocks reactions when the action gate is disabled", async () => {
    ...
    await expect(
      signalMessageActions.handleAction?.({
        ...
        cfg,
        channel: "signal",
        params: { emoji: "✅", messageId: "123", to: "+15550001111" },
      }),
  ...
  it("rejects invalid reaction inputs before dispatch", async () => {
    ...
    await expect(
      signalMessageActions.handleAction?.({
        ...
        cfg,
        channel: "signal",
        params: { emoji: "✅", messageId: "123", to: "signal:group:group-id" },
      }),
