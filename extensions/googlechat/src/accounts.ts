...
export interface ResolvedGoogleChatAccount {
  accountId: string;
  name?: string;
  enabled: boolean;
...
export function resolveGoogleChatAccount(params: {
  ...
  const credentials = resolveCredentialsFromConfig({ account: merged, accountId });
...
export function listEnabledGoogleChatAccounts(cfg: OpenClawConfig): ResolvedGoogleChatAccount[] {
  return listGoogleChatAccountIds(cfg)
    .map((accountId) => resolveGoogleChatAccount({ accountId, cfg }))
