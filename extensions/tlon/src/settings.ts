...
export interface TlonSettingsStore {
...
export interface TlonSettingsState {
  ...
  const notify = () => {
    for (const listener of listeners) {
      try {
        ...
        logger?.error?.(`[settings] Listener error: ${String(error)}`);
      }
    }
  };
  ...
  return {
    ...
    async load(): Promise<TlonSettingsStore> {
      try {
        ...
        logger?.log?.(`[settings] No settings found (using defaults): ${String(error)}`);
        state.current = {};
        state.loaded = true;
        return state.current;
      }
