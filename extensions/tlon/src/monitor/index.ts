...
export interface MonitorTlonOpts {
  runtime?: RuntimeEnv;
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
    ...
    for (const { key, fileValue, settingsValue } of migrations) {
      if (shouldMigrateTlonSetting(fileValue, settingsValue)) {
        try {
          ...
          runtime.log?.(`[tlon] Failed to migrate ${key}: ${String(error)}`);
        }
      }
    }
  }
  ...
  try {
    ...
    runtime.log?.(`[tlon] Settings store not available, using file config: ${String(error)}`);
  }
  ...
  const processMessage = async (params: {
    ...
    if (isGroup && groupChannel && isSummarizationRequest(messageText)) {
      try {
        ...
        if (isGroup && groupChannel) {
          ...
          await sendDm({ api, fromShip: botShipName, text: errorMsg, toShip: senderShip });
        }
        return;
      }
    }
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          sendDm({
            ...
            runtime.error?.(`[tlon] Failed to send security warning to owner: ${error}`),
          );
        }
      }
    ...
    if (shouldComputeAuth) {
      ...
      commandAuthorized = core.channel.commands.resolveCommandAuthorizedFromAuthorizers({
        authorizers: [{ allowed: senderIsOwner, configured: Boolean(effectiveOwnerShip) }],
        useAccessGroups,
      });
    ...
    const ctxPayload = core.channel.reply.finalizeInboundContext({
      ...
      ...(parentId && { ReplyToId: String(parentId), ThreadId: String(parentId) }),
    });
        ...
        for (const invite of event as DmInvite[]) {
          ...
          if (isOwner(ship)) {
            try {
              ...
              runtime.error?.(`[tlon] Failed to auto-accept DM from owner: ${String(error)}`);
            }
            continue;
          }
          ...
          if (effectiveAutoAcceptDmInvites && isDmAllowed(ship, effectiveDmAllowlist)) {
            try {
              ...
              runtime.error?.(`[tlon] Failed to auto-accept DM from ${ship}: ${String(error)}`);
            }
            continue;
          }
  ...
  try {
    ...
    try {
      ...
      runtime.log?.(`[tlon] Settings subscription not available: ${String(error)}`);
    }
                  ...
                  if (!watchedChannels.has(channelNest)) {
                    ...
                    if (effectiveAutoAcceptGroupInvites) {
                      try {
                        ...
                        runtime.error?.(
                          `[tlon] Failed to persist channel to settings: ${String(error)}`,
                        );
                      }
                    }
                  }
    ...
    // Always subscribe so we can hot-reload the setting via settings store
    {
        ...
        for (const [groupFlag, foreign] of Object.entries(foreigns)) {
          ...
          if (isOwner(inviterShip)) {
            try {
              ...
              runtime.error?.(`[tlon] Failed to accept group invite from owner: ${String(error)}`);
            }
            continue;
          }
          ...
          try {
            ...
            runtime.error?.(`[tlon] Failed to auto-accept group ${groupFlag}: ${String(error)}`);
          }
        }
      };
      ...
      try {
        ...
        runtime.log?.(`[tlon] Foreigns subscription failed: ${String(error)}`);
      }
    }
