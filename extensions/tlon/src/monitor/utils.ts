  ...
  for (const verse of content) {
    if (verse?.block?.cite && typeof verse.block.cite === "object") {
      ...
      if (cite.chan && typeof cite.chan === "object") {
        ...
        cites.push({ group: cite.group, type: "group" });
      } else if (cite.desk && typeof cite.desk === "object") {
        cites.push({ flag: cite.desk.flag, type: "desk", where: cite.desk.where });
...
export function isBotMentioned(
  ...
  const escapedShip = normalizedBotShip.replace(/[.*+?^${}()|[\]\\]/g, String.raw`\$&`);
  ...
  if (nickname) {
    const escapedNickname = nickname.replace(/[.*+?^${}()|[\]\\]/g, String.raw`\$&`);
