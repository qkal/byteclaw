...
import { type ParsedCite, asRecord, extractCites, extractMessageText } from "./utils.js";

interface TlonScryApi {
...
export function createTlonCitationResolver(params: { api: TlonScryApi; runtime: RuntimeEnv }) {
  ...
  const resolveCiteContent = async (cite: ParsedCite): Promise<string | null> => {
    ...
    try {
      ...
      runtime.log?.(`[tlon] Failed to fetch cited post: ${String(error)}`);
      return null;
    }
  };
