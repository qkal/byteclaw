...
function parseInlineMarkdown(text: string): StoryInline[] {
  ...
  while (remaining.length > 0) {
    ...
    if (linkMatch) {
      result.push({ link: { content: linkMatch[1], href: linkMatch[2] } });
    ...
    if (imageMatch) {
      ...
      result.push({
        __image: { alt: imageMatch[1], src: imageMatch[2] },
    ...
    if (urlMatch) {
      result.push({ link: { content: urlMatch[1], href: urlMatch[1] } });
...
function processInlinesForImages(inlines: StoryInline[]): {
  ...
  return { imageBlocks, inlines: cleanInlines };
}
