...
export interface UrbitSseLogger {
  log?: (message: string) => void;
  error?: (message: string) => void;
}

interface UrbitSseOptions {
  ship?: string;
...
export class UrbitSSEClient {
  ...
  async subscribe(params: {
    ...
    this.eventHandlers.set(subId, { err: params.err, event: params.event, quit: params.quit });
  ...
  processEvent(eventData: string) {
    ...
    for (const line of lines) {
      if (line.startsWith("id: ")) {
        eventId = Number.parseInt(line.substring(4), 10);
      }
    ...
    if (eventId !== null && !isNaN(eventId)) {
      if (eventId > this.lastHeardEventId) {
        ...
        if (eventId - this.lastAcknowledgedEventId > this.ackThreshold) {
          ...
          this.ack(eventId).catch((error) => {
            this.logger.error?.(`Failed to ack event ${eventId}: ${String(error)}`);
          });
        }
      }
    }
  ...
  async scry(path: string) {
    return await scryUrbitPath(
      ...
      { auditContext: "tlon-urbit-scry", path },
    );
  }
  ...
  async attemptReconnect() {
    ...
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      ...
      const extendedBackoff = 10_000; // 10 seconds
