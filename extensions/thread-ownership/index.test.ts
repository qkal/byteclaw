...
describe("thread-ownership plugin", () => {
  ...
  const api = {
    config: {
      agents: {
        list: [{ default: true, id: "test-agent", identity: { name: "TestBot" } }],
      },
    },
    id: "thread-ownership",
  ...
  describe("message_sending", () => {
    ...
    async function sendSlackThreadMessage() {
      return await hooks.message_sending(
        { content: "hello", metadata: { channelId: "C123", threadTs: "1234.5678" }, to: "C123" },
  ...
  describe("message_received @-mention tracking", () => {
    ...
    it("tracks @-mentions and skips ownership check for mentioned threads", async () => {
      ...
      await hooks.message_received(
        { content: "Hey @TestBot help me", metadata: { channelId: "C456", threadTs: "9999.0001" } },
        { channelId: "slack", conversationId: "C456" },
      );

      // Now send in the same thread -- should skip the ownership HTTP call.
      const result = await hooks.message_sending(
        { content: "Sure!", metadata: { channelId: "C456", threadTs: "9999.0001" }, to: "C456" },
        { channelId: "slack", conversationId: "C456" },
      );
    ...
    it("ignores @-mentions on non-slack channels", async () => {
      ...
      await hooks.message_received(
        { content: "Hey @TestBot", metadata: { channelId: "C999", threadTs: "7777.0001" } },
      ...
      await hooks.message_sending(
        { content: "Sure!", metadata: { channelId: "C999", threadTs: "7777.0001" }, to: "C999" },
    ...
    it("tracks bot user ID mentions via <@U999> syntax", async () => {
      await hooks.message_received(
        { content: "Hey <@U999> help", metadata: { channelId: "C789", threadTs: "8888.0001" } },
        { channelId: "slack", conversationId: "C789" },
      );

      const result = await hooks.message_sending(
        { content: "On it!", metadata: { channelId: "C789", threadTs: "8888.0001" }, to: "C789" },
        { channelId: "slack", conversationId: "C789" },
      );
