...
import { type OpenClawPluginApi, definePluginEntry } from "openclaw/plugin-sdk/plugin-entry";
...
interface ActiveRecallPluginConfig {
  enabled?: boolean;
  agents?: string[];
  model?: string;
...
interface ResolvedActiveRecallPluginConfig {
  enabled: boolean;
  agents: string[];
  model?: string;
  modelFallbackPolicy: "default-remote" | "resolved-only";
...
interface ActiveRecallRecentTurn {
  role: "user" | "assistant";
  text: string;
}

interface PluginDebugEntry {
  pluginId: string;
  lines: string[];
}
...
interface CachedActiveRecallResult {
  expiresAt: number;
...
interface ActiveMemoryToggleStore {
...
async function persistPluginStatusLines(params: {
  ...
  try {
    ...
    if (!params.statusLine && !debugLine) {
      const store = params.api.runtime.agent.session.loadSessionStore(storePath);
      const existingEntry = resolveSessionStoreEntry({ sessionKey, store }).existing;
...
async function runRecallSubagent(params: {
  ...
  const persistedDir = params.config.persistTranscripts
    ...
    : undefined;
  if (persistedDir) {
    await fs.mkdir(persistedDir, { mode: 0o700, recursive: true });
...
async function maybeResolveActiveRecall(params: {
  ...
  if (cached) {
    await persistPluginStatusLines({
      ...
      sessionKey: params.sessionKey,
      statusLine: `${buildPluginStatusLine({ config: params.config, result: cached })} cached`,
    });
  ...
  try {
    ...
    await persistPluginStatusLines({
      ...
      sessionKey: params.sessionKey,
      statusLine: buildPluginStatusLine({ config: params.config, result }),
    });
