...
describe("active-memory plugin", () => {
  ...
  it("registers a session-scoped active-memory toggle command", async () => {
    ...
    const disabledResult = await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order? active memory toggle" },
      {
        agentId: "main",
  ...
  it("supports an explicit global active-memory config toggle", async () => {
    ...
    await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order while global active memory is off?" },
      {
        agentId: "main",
        messageProvider: "webchat",
    ...
    await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order after global active memory is back on?" },
      {
        agentId: "main",
        messageProvider: "webchat",
        sessionKey: "agent:main:global-toggle",
  ...
  it("does not run for agents that are not explicitly targeted", async () => {
    const result = await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order?" },
      {
        agentId: "support",
        messageProvider: "webchat",
  ...
  it("defaults to direct-style sessions only", async () => {
    const result = await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should we order?" },
      {
        agentId: "main",
        channelId: "telegram",
  ...
  it("injects system context on a successful recall hit", async () => {
    const result = await hooks.before_prompt_build(
      {
        messages: [
          { content: "i want something greasy tonight", role: "user" },
          { content: "let's narrow it down", role: "assistant" },
        ],
  ...
  it("preserves canonical parent session scope in the blocking memory subagent session key", async () => {
    await hooks.before_prompt_build(
      { messages: [], prompt: "what should i grab on the way?" },
      {
        agentId: "main",
        channelId: "telegram",
        messageProvider: "telegram",
  ...
  it("falls back to the current session model when no plugin model is configured", async () => {
    ...
    await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order? temp transcript" },
      {
        agentId: "main",
        messageProvider: "webchat",
        modelId: "glm-5",
  ...
  it("can disable default remote model fallback", async () => {
    ...
    const result = await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order? no fallback" },
      {
        agentId: "main",
        messageProvider: "webchat",
  ...
  it("persists a readable debug summary alongside the status line", async () => {
    ...
    await hooks.before_prompt_build(
      ...
      { agentId: "main", messageProvider: "webchat", sessionKey, trigger: "user" },
    );
  ...
  it("replaces stale structured active-memory lines on a later empty run", async () => {
    ...
    hoisted.sessionStore[sessionKey] = {
      pluginDebugEntries: [
        ...
        { lines: ["Other Plugin: keep me"], pluginId: "other-plugin" },
      ],
      sessionId: "s-main",
      updatedAt: 0,
    };
    ...
    await hooks.before_prompt_build(
      { messages: [], prompt: "what's up with you?" },
      { agentId: "main", messageProvider: "webchat", sessionKey, trigger: "user" },
    );
  ...
  it("returns nothing when the subagent says none", async () => {
    ...
    const result = await hooks.before_prompt_build(
      { messages: [], prompt: "fair, okay gonna do them by throwing them in the garbage" },
      {
        agentId: "main",
        messageProvider: "webchat",
        sessionKey: "agent:main:main",
  ...
  it("does not cache timeout results", async () => {
    ...
    await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order? timeout test" },
      {
        agentId: "main",
        messageProvider: "webchat",
  ...
  it("does not share cached recall results across session-id-only contexts", async () => {
    ...
    await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order? session id cache" },
      {
        agentId: "main",
        messageProvider: "webchat",
        sessionId: "session-a",
  ...
  it("uses a canonical agent session key when only sessionId is available", async () => {
    ...
    await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order? session id only" },
      {
        agentId: "main",
        messageProvider: "webchat",
        sessionId: "session-a",
  ...
  it("uses the resolved canonical session key for non-webchat chat-type checks", async () => {
    ...
    const result = await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order? session id only telegram" },
      {
        agentId: "main",
        channelId: "telegram",
        messageProvider: "telegram",
  ...
  it("clears stale status on skipped non-interactive turns even when agentId is missing", async () => {
    ...
    const result = await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order?" },
      { messageProvider: "webchat", sessionKey, trigger: "heartbeat" },
    );
  ...
  it("supports message mode by sending only the latest user message", async () => {
    ...
    await hooks.before_prompt_build(
      {
        messages: [
          { content: "i have a flight tomorrow", role: "user" },
    ...
    await hooks.before_prompt_build(
      {
        messages: [
          { content: "i have a flight tomorrow", role: "user" },
          { content: "got it", role: "assistant" },
          { content: "packing is annoying", role: "user" },
        ],
        prompt: "what should i grab on the way?",
      },
      {
  ...
  it("trusts the subagent's relevance decision for explicit preference recall prompts", async () => {
    ...
    const result = await hooks.before_prompt_build(
      { messages: [], prompt: "u remember my flight preferences" },
      {
        agentId: "main",
        messageProvider: "webchat",
        sessionKey: "agent:main:main",
  ...
  it("applies total summary truncation after normalizing the subagent reply", async () => {
    ...
    const result = await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order? word-boundary-truncation-40" },
      {
        agentId: "main",
        messageProvider: "webchat",
        sessionKey: "agent:main:main",
  ...
  it("uses the configured maxSummaryChars value in the subagent prompt", async () => {
    ...
    await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order? prompt-count-check" },
      {
        agentId: "main",
        messageProvider: "webchat",
  ...
  it("keeps subagent transcripts off disk by default by using a temp session file", async () => {
    ...
    await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order? temp transcript path" },
      {
        agentId: "main",
        messageProvider: "webchat",
        sessionKey: "agent:main:main",
  ...
  it("persists subagent transcripts in a separate directory when enabled", async () => {
    ...
    await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order? persist transcript" },
      { agentId: "main", messageProvider: "webchat", sessionKey, trigger: "user" },
    );
    ...
    expect(mkdirSpy).toHaveBeenCalledWith(expectedDir, { mode: 0o700, recursive: true });
  ...
  it("falls back to the default transcript directory when transcriptDir is unsafe", async () => {
    ...
    await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order? unsafe transcript dir" },
      {
        agentId: "main",
        messageProvider: "webchat",
  ...
  it("scopes persisted subagent transcripts by agent", async () => {
    ...
    await hooks.before_prompt_build(
      { messages: [], prompt: "what wings should i order? support agent transcript" },
      {
  ...
  it("sanitizes control characters out of debug lines", async () => {
    ...
    await hooks.before_prompt_build(
      { messages: [], prompt: "what should i order?" },
      { agentId: "main", messageProvider: "webchat", sessionKey, trigger: "user" },
    );
  ...
  it("caps the active-memory cache size and evicts the oldest entries", async () => {
    ...
    for (let index = 0; index <= 1000; index += 1) {
      await hooks.before_prompt_build(
        { messages: [], prompt: `cache pressure prompt ${index}` },
        {
          agentId: "main",
          messageProvider: "webchat",
    ...
    await hooks.before_prompt_build(
      { messages: [], prompt: "cache pressure prompt 0" },
      {
        agentId: "main",
        messageProvider: "webchat",
        sessionKey: "agent:main:cache-cap",
