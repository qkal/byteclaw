...
import { type ProfilePublishResult, publishProfile as publishProfileFn } from "./nostr-profile.js";
import {
...
import { type SeenTracker, createSeenTracker } from "./seen-tracker.js";
...
const CIRCUIT_BREAKER_RESET_MS = 30_000; // 30 seconds before half-open

// Health tracker configuration
const HEALTH_WINDOW_MS = 60_000; // 1 minute window for health stats
...
function createRelayHealthTracker(): RelayHealthTracker {
  ...
  return {
    getScore(relay: string): number {
      ...
      const latencyPenalty = Math.min(0.2, avgLatency / 10_000);
...
export async function startNostrBus(options: NostrBusOptions): Promise<NostrBusHandle> {
  ...
  function scheduleStatePersist(eventCreatedAt: number, eventId: string): void {
    ...
    pendingWrite = setTimeout(() => {
      writeNostrBusState({
        ...
        lastProcessedAt,
        recentEventIds,
      }).catch((error) => onError?.(error as Error, "persist state"));
  ...
  async function handleEvent(event: Event): Promise<void> {
    try {
      ...
      onError?.(error as Error, `event ${event.id}`);
    } finally {
  ...
  const sub = pool.subscribeMany(
    relays,
    [{ "#p": [pk], kinds: [4], since }] as unknown as Parameters<typeof pool.subscribeMany>[1],
    {
      onclose: (reason) => {
    ...
    close: () => {
      ...
      if (pendingWrite) {
        ...
        writeNostrBusState({
          ...
          lastProcessedAt,
          recentEventIds,
        }).catch((error) => onError?.(error as Error, "persist state on close"));
      }
    },
...
async function sendEncryptedDm(
  ...
  for (const relay of sortedRelays) {
    ...
    try {
      ...
      metrics.emit("relay.error", 1, { latency, relay });
