...
function enforceGatewayMutationScope(
  ...
  sendJson(res, 403, { error: `missing scope: ${PROFILE_MUTATION_SCOPE}`, ok: false });
  return false;
}
...
export function createNostrProfileHttpHandler(
  ...
  return async (req, res) => {
    ...
    try {
      ...
      // Method not allowed
      sendJson(res, 405, { error: "Method not allowed", ok: false });
      return true;
    } catch (error) {
      ctx.log?.error(`Profile HTTP error: ${String(error)}`);
      sendJson(res, 500, { error: "Internal server error", ok: false });
      return true;
    }
  };
}
...
async function handleUpdateProfile(
  ...
  if (!checkRateLimit(accountId)) {
    sendJson(res, 429, { error: "Rate limit exceeded (5 requests/minute)", ok: false });
    return true;
  }
  ...
  try {
    ...
    sendJson(res, 400, { error: String(error), ok: false });
    return true;
  }
  ...
  if (!parseResult.success) {
    ...
    sendJson(res, 400, { details: errors, error: "Validation failed", ok: false });
    return true;
  }
  ...
  if (profile.picture) {
    ...
    if (!pictureCheck.ok) {
      sendJson(res, 400, { error: `picture: ${pictureCheck.error}`, ok: false });
      return true;
    }
  }
  ...
  if (profile.banner) {
    ...
    if (!bannerCheck.ok) {
      sendJson(res, 400, { error: `banner: ${bannerCheck.error}`, ok: false });
      return true;
    }
  }
  ...
  if (profile.website) {
    ...
    if (!websiteCheck.ok) {
      sendJson(res, 400, { error: `website: ${websiteCheck.error}`, ok: false });
      return true;
    }
  }
  ...
  try {
    ...
    ctx.log?.error(`[${accountId}] Profile publish error: ${String(error)}`);
    sendJson(res, 500, { error: `Publish failed: ${String(error)}`, ok: false });
  }
...
async function handleImportProfile(
  ...
  if (!accountInfo) {
    sendJson(res, 404, { error: `Account not found: ${accountId}`, ok: false });
    return true;
  }
  ...
  if (!pubkey) {
    sendJson(res, 400, { error: "Account has no public key configured", ok: false });
    return true;
  }
