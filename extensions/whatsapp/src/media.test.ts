...
function buildDeterministicBytes(length: number): Buffer {
  ...
  for (let i = 0; i < length; i++) {
    seed = (1_103_515_245 * seed + 12_345) & 0x7f_ff_ff_ff;
    buffer[i] = seed & 0xff;
  }
  return buffer;
}
...
beforeAll(async () => {
  ...
  tinyPngBuffer = await sharp({
    create: { background: "#00ff00", channels: 3, height: 10, width: 10 },
  })
    .png()
    .toBuffer();
  ...
  alphaPngBuffer = await sharp({
    create: {
      background: { alpha: 0.5, b: 0, g: 0, r: 255 },
      channels: 4,
      height: 64,
      width: 64,
    },
  ...
  fallbackPngBuffer = await sharp(raw, { raw: { channels: 4, height: size, width: size } })
    .png()
    .toBuffer();
...
describe("local media root guard", () => {
  ...
  it("rejects Windows network paths before filesystem checks", async () => {
    ...
    try {
      await expect(
        loadWebMedia(String.raw`\\attacker\share\evil.png`, 1024 * 1024, {
          localRoots: [resolvePreferredOpenClawTmpDir()],
        }),
