...
import { type WhatsAppSelfIdentity, resolveComparableIdentity } from "./identity.js";
import { type WebChannel, resolveUserPath } from "./text-runtime.js";
...
export async function logoutWeb(params: {
  ...
  if (params.isLegacyAuthDir) {
    ...
    await fs.rm(resolvedAuthDir, { force: true, recursive: true });
  }
...
export async function readWebSelfIdentity(
  ...
  maybeRestoreCredsFromBackup(resolvedAuthDir);
  try {
    const raw = await fs.readFile(resolveWebCredsPath(resolvedAuthDir), "utf8");
    const parsed = JSON.parse(raw) as { me?: { id?: string; lid?: string } } | undefined;
