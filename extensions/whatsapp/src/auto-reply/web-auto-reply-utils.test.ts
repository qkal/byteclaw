...
describe("isBotMentionedFromTargets", () => {
  ...
  function expectMentioned(
    msg: WebInboundMsg,
    cfg: { mentionRegexes: RegExp[]; allowFrom?: (string | number)[] },
    expected: boolean,
  ) {
  ...
  it("ignores JID mentions in self-chat mode", () => {
    const cfg = { allowFrom: ["+999"], mentionRegexes: [/\bopenclaw\b/i] };
    const msg = makeMsg({
      body: "@owner ping",
  ...
  it("uses channel reset overrides when configured", async () => {
    ...
    try {
      await withTempDir("openclaw-snapshot-", async (root) => {
        ...
        const cfg = {
          session: {
            reset: { atHour: 4, idleMinutes: 240, mode: "daily" },
            resetByChannel: {
              whatsapp: { idleMinutes: 360, mode: "idle" },
            },
            store: storePath,
          },
...
describe("web auto-reply util", () => {
  ...
  describe("isLikelyWhatsAppCryptoError", () => {
    ...
    const cases: { name: string; value: unknown; expected: boolean }[] = [
      { expected: false, name: "returns false for non-matching Error", value: new Error("boom") },
      { expected: false, name: "returns false for non-matching string", value: "boom" },
      {
        expected: false,
      ...
      { expected: false, name: "handles null reason without throwing", value: null },
      { expected: false, name: "handles number reason without throwing", value: 123 },
      { expected: false, name: "handles boolean reason without throwing", value: true },
      { expected: false, name: "handles bigint reason without throwing", value: 123n },
      { expected: false, name: "handles symbol reason without throwing", value: Symbol("bad mac") },
      {
        expected: false,
