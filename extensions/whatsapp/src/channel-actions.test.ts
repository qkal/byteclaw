...
describe("whatsapp channel action helpers", () => {
  ...
  it("defaults to minimal reaction guidance when reactions are available", () => {
    ...
    expect(resolveWhatsAppAgentReactionGuidance({ accountId: "default", cfg })).toBe("minimal");
  });
  ...
  it("omits reaction guidance when WhatsApp reactions are disabled", () => {
    ...
    expect(resolveWhatsAppAgentReactionGuidance({ accountId: "default", cfg })).toBeUndefined();
  });
  ...
  it("advertises react when agent reactions are enabled", () => {
    ...
    expect(describeWhatsAppMessageActions({ accountId: "default", cfg })?.actions).toEqual([
      "react",
      "poll",
    ]);
  });
  ...
  it("returns null when WhatsApp is not configured", () => {
    expect(
      describeWhatsAppMessageActions({ accountId: "default", cfg: {} as OpenClawConfig }),
    ).toBeNull();
  });
  ...
  it("uses the active account reactionLevel for discovery", () => {
    ...
    expect(describeWhatsAppMessageActions({ accountId: "work", cfg })?.actions).toEqual([
      "react",
      "poll",
    ]);
  });
