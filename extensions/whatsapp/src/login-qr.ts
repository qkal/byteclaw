...
import { type RuntimeEnv, defaultRuntime } from "openclaw/plugin-sdk/runtime-env";
...
function attachLoginWaiter(accountId: string, login: ActiveLogin) {
  login.waitPromise = waitForWaConnection(login.sock)
    ...
    .catch((error) => {
      ...
      current.errorStatus = getStatusCode(error);
    });
}
...
async function restartLoginSocket(login: ActiveLogin, runtime: RuntimeEnv) {
  ...
  try {
    ...
    login.error = formatError(error);
    login.errorStatus = getStatusCode(error);
    return false;
  }
}
...
export async function startWebLoginWithQr(
  ...
  const account = resolveWhatsAppAccount({ accountId: opts.accountId, cfg });
  ...
  try {
    ...
    await resetActiveLogin(account.accountId);
    return {
      message: `Failed to start WhatsApp login: ${String(error)}`,
    };
  }
  ...
  try {
    ...
    await resetActiveLogin(account.accountId);
    return {
      message: `Failed to get QR: ${String(error)}`,
    };
  }
