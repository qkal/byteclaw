...
export async function detectWhatsAppLinked(
  ...
  const { authDir } = resolveWhatsAppAuthDir({ accountId, cfg });
...
async function promptWhatsAppDmAccess(params: {
  ...
  const account = resolveWhatsAppAccount({ accountId, cfg: params.cfg });
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      { label: "This is my personal phone number", value: "personal" },
      { label: "Separate phone just for OpenClaw", value: "separate" },
    ],
  });
  ...
  const policy = (await params.prompter.select({
    ...
    options: [
      ...
      { label: "Allowlist only (block unknown senders)", value: "allowlist" },
      { label: "Open (public inbound DMs)", value: "open" },
      { label: "Disabled (ignore WhatsApp DMs)", value: "disabled" },
    ],
  })) as DmPolicy;
  ...
  const allowOptions =
    existingAllowFrom.length > 0
      ? ([
          { label: "Keep current allowFrom", value: "keep" },
          {
            label: "Unset allowFrom (use pairing approvals only)",
            value: "unset",
          },
          { label: "Set allowFrom to specific numbers", value: "list" },
        ] as const)
      : ([
          { label: "Unset allowFrom (default)", value: "unset" },
          { label: "Set allowFrom to specific numbers", value: "list" },
        ] as const);
