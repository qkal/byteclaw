...
const PREAUTH_WEBHOOK_BODY_TIMEOUT_MS = 5000;
  ...
  const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      try {
        ...
        onError?.(error instanceof Error ? error : new Error(formatError(error)));
      }
    } catch (error) {
      ...
      const error = error instanceof Error ? error : new Error(formatError(error));
      onError?.(error);
...
export interface NextcloudTalkMonitorOptions {
  accountId?: string;
  config?: CoreConfig;
  runtime?: RuntimeEnv;
