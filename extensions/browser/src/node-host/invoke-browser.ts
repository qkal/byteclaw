...
async function readBrowserProxyFile(filePath: string): Promise<BrowserProxyFile | null> {
  ...
  return { base64: buffer.toString("base64"), mimeType, path: filePath };
}
...
export async function runBrowserProxyCommand(paramsJSON?: string | null): Promise<string> {
  ...
  try {
    ...
    if (!isBrowserProxyTimeoutError(error)) {
      throw error;
    }
    ...
    const loaded = await Promise.all(
      paths.map(async (p) => {
        try {
          ...
          throw new Error(`browser proxy file read failed for ${p}: ${String(error)}`, {
            cause: error,
          });
        }
      }),
  ...
  const payload: BrowserProxyResult = files ? { files, result } : { result };
