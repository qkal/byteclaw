...
function formatTabsToolResult(tabs: unknown[]): AgentToolResult<unknown> {
  ...
  const content: AgentToolResult<unknown>["content"] = [
    { text: wrapped.wrappedText, type: "text" },
  ];
  return {
    content,
...
function formatConsoleToolResult(result: {
  ...
  return {
    content: [{ text: wrapped.wrappedText, type: "text" as const }],
    details: {
      ...wrapped.safeDetails,
...
export async function executeSnapshotAction(params: {
  ...
  if (snapshot.format === "ai") {
    ...
    return {
      content: [{ text: wrappedSnapshot, type: "text" as const }],
      details: safeDetails,
    };
  }
  {
...
export async function executeActAction(params: {
  ...
  try {
    ...
    if (isChromeStaleTargetError(profile, error)) {
