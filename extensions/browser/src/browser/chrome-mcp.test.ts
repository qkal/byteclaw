...
describe("chrome MCP page parsing", () => {
  ...
  it("preserves session after tool-level errors (isError)", async () => {
    ...
    const factory: ChromeMcpSessionFactory = async () => {
      ...
      const callTool = vi.fn(async ({ name }: ToolCall) => {
        if (name === "evaluate_script") {
          return {
            content: [{ text: "element not found", type: "text" }],
            isError: true,
          };
        }
        if (name === "list_pages") {
          return {
            content: [{ text: "## Pages\n1: https://example.com [selected]", type: "text" }],
          };
        }
        throw new Error(`unexpected tool ${name}`);
      });
    ...
    await expect(
      evaluateChromeMcpScript({ fn: "() => null", profileName: "chrome-live", targetId: "1" }),
  ...
  it("creates a fresh session when userDataDir changes for the same profile", async () => {
    ...
    const factoryCalls: { profileName: string; userDataDir?: string }[] = [];
