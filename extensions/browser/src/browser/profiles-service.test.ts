...
describe("BrowserProfilesService", () => {
  ...
  it("allocates next local port for new profiles", async () => {
    ...
    expect(result.cdpPort).toBe(18_801);
    expect(result.isRemote).toBe(false);
    expect(state.resolved.profiles.work?.cdpPort).toBe(18_801);
  ...
  it("falls back to derived CDP range when resolved CDP range is missing", async () => {
    ...
    expect(result.cdpPort).toBe(30_009);
    expect(state.resolved.profiles.work?.cdpPort).toBe(30_009);
    expect(writeConfigFile).toHaveBeenCalled();
  });

  it("allocates from configured cdpPortRangeStart for new local profiles", async () => {
    const { result, state } = await createWorkProfileWithConfig({
      browserConfig: { cdpPortRangeStart: 19_000, profiles: {} },
      resolved: resolveBrowserConfig({ cdpPortRangeStart: 19_000 }),
    });

    expect(result.cdpPort).toBe(19_001);
    expect(result.isRemote).toBe(false);
    expect(state.resolved.profiles.work?.cdpPort).toBe(19_001);
    expect(writeConfigFile).toHaveBeenCalled();
  });
