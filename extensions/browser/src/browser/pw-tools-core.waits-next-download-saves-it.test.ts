...
describe("pw-tools-core", () => {
  ...
  async function waitForImplicitDownloadOutput(params: {
    ...
    const res = await p;
    const outPath = (vi.mocked(saveAs).mock.calls as unknown as [string][])[0]?.[0];
    return { outPath, res };
  }
  ...
  it("waits for the next download and atomically finalizes explicit output paths", async () => {
    await withTempDir(async (tempDir) => {
      ...
      const res = await p;
      await expectAtomicDownloadSave({ content: "file-content", saveAs, targetPath, tempDir });
  ...
  it("clicks a ref and atomically finalizes explicit download paths", async () => {
    await withTempDir(async (tempDir) => {
      ...
      const res = await p;
      await expectAtomicDownloadSave({ content: "report-content", saveAs, targetPath, tempDir });
      await expect(fs.realpath(res.path)).resolves.toBe(await fs.realpath(targetPath));
    });
  });
