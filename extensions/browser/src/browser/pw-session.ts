    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        ...
        const browser = await withNoProxyForCdpUrl(endpoint, () =>
          chromium.connectOverCDP(endpoint, { headers, timeout }),
        );
...
function matchPageByTargetList(
  pages: Page[],
  targets: { id: string; url: string; title?: string }[],
  targetId: string,
): Page | null {
...
export async function assertPageNavigationCompletedSafely(opts: {
  ...
  try {
    ...
    if (isPolicyDenyNavigationError(error)) {
...
export function refLocator(page: Page, ref: string) {
  ...
  if (/^e\d+$/.test(normalized)) {
    ...
    const locator = info.name
      ? locAny.getByRole(info.role as never, { exact: true, name: info.name })
...
export async function closePlaywrightBrowserConnection(opts?: { cdpUrl?: string }): Promise<void> {
  ...
  const connections = [...cachedByCdpUrl.values()];
      ...
      try {
        if (needsAttach) {
          const attached = (await runWithTimeout(
            send("Target.attachToTarget", { flatten: true, targetId: opts.targetId }),
            1500,
...
export async function createPageViaPlaywright(opts: {
  ...
  if (targetUrl !== "about:blank") {
    ...
    try {
      ...
      if (isPolicyDenyNavigationError(error) || error instanceof BlockedBrowserTargetError) {
        throw error;
      }
    }
