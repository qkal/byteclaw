...
describe("port allocation", () => {
  it("allocates within an explicit range", () => {
    ...
    expect(allocateCdpPort(usedPorts, { end: 20_002, start: 20_000 })).toBe(20_000);
    usedPorts.add(20_000);
    expect(allocateCdpPort(usedPorts, { end: 20_002, start: 20_000 })).toBe(20_001);
  });

  it("allocates next available port from default range", () => {
    const cases = [
      { expected: CDP_PORT_RANGE_START, name: "none used", used: new Set<number>() },
      {
...
describe("getUsedPorts", () => {
  ...
  it("extracts ports from profile configs", () => {
    const profiles = {
      openclaw: { cdpPort: 18_792 },
      personal: { cdpPort: 18_795 },
      work: { cdpPort: 18_793 },
    };
    const used = getUsedPorts(profiles);
    expect(used).toEqual(new Set([18_792, 18_793, 18_795]));
  });
...
describe("color allocation", () => {
  it("allocates next unused color from palette", () => {
    const cases = [
      { expected: PROFILE_COLORS[0], name: "none used", used: new Set<string>() },
      {
