...
export async function setHttpCredentialsViaPlaywright(opts: {
  ...
  await page.context().setHTTPCredentials({ password, username });
}
  ...
  await withPageScopedCdpClient({
    ...
    fn: async (send) => {
      try {
        ...
        if (String(error).includes("Another locale override is already in effect")) {
          return;
        }
        throw error;
      }
    ...
    fn: async (send) => {
      try {
        ...
        if (msg.includes("Invalid timezone")) {
          throw new Error(`Invalid timezone ID: ${timezoneId}`, { cause: error });
        }
        throw error;
      }
    },
