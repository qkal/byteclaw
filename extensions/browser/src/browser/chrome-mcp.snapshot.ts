...
export function buildAiSnapshotFromChromeMcpSnapshot(params: {
  ...
  const visit = (node: ChromeMcpSnapshotNode, depth: number) => {
    ...
    const includeNode = shouldIncludeNode({ name, options: params.options, role });
    if (includeNode) {
      ...
      if (ref && shouldCreateRef(role, name)) {
        ...
        refs[ref] = nth === undefined ? { name, role } : { name, nth, role };
        line += ` [ref=${ref}]`;
      }
      if (value) {
  ...
  return truncated ? { refs, snapshot, stats, truncated } : { refs, snapshot, stats };
}
