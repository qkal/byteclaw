...
async function getRestoredPageForTarget(opts: TargetOpts) {
  ...
  restoreRoleRefsForTarget({ cdpUrl: opts.cdpUrl, page, targetId: opts.targetId });
  return page;
}
...
export async function clickViaPlaywright(opts: {
  ...
  try {
    ...
    throw toAIFriendlyError(error, label);
  }
}
...
export async function dragViaPlaywright(opts: {
  ...
  try {
    ...
    throw toAIFriendlyError(error, `${startLabel} -> ${endLabel}`);
  }
}
...
export async function typeViaPlaywright(opts: {
  ...
  try {
    if (opts.slowly) {
      ...
      await locator.type(text, { delay: 75, timeout });
    } else {
...
export async function setInputFilesViaPlaywright(opts: {
  ...
  try {
    ...
    throw toAIFriendlyError(error, inputRef || element);
  }
  try {
...
export async function batchViaPlaywright(opts: {
  ...
  depth?: number;
  signal?: AbortSignal;
}): Promise<{ results: { ok: boolean; error?: string }[] }> {
  ...
  for (const action of opts.actions) {
    ...
    try {
      ...
      results.push({ error: message, ok: false });
