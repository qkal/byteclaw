...
describe("pw-role-snapshot", () => {
  it("adds refs for interactive elements", () => {
    ...
    expect(res.refs.e1).toMatchObject({ name: "Submit", role: "button" });
    expect(res.refs.e2).toMatchObject({ name: "Learn more", role: "link" });
  });
  ...
  it("preserves Playwright aria-ref ids in ai snapshots", () => {
    ...
    expect(res.refs.e5).toMatchObject({ name: "Home", role: "link" });
    expect(res.refs.e7).toMatchObject({ name: "Save", role: "button" });
  });
});
