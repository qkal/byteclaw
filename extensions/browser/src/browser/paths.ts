...
async function validateCanonicalPathWithinRoot(params: {
  ...
  try {
    ...
    return isNotFoundPathError(error) ? "not-found" : "invalid";
  }
}
...
export function resolvePathWithinRoot(params: {
  ...
  if (!raw) {
    if (!params.defaultFileName) {
      return { error: "path is required", ok: false };
    }
  ...
  if (!rel || rel.startsWith("..") || path.isAbsolute(rel)) {
    return { error: `Invalid path: must stay within ${params.scopeLabel}`, ok: false };
  }
...
export function resolvePathsWithinRoot(params: {
  ...
  for (const raw of params.requestedPaths) {
    ...
    if (!pathResult.ok) {
      return { error: pathResult.error, ok: false };
    }
