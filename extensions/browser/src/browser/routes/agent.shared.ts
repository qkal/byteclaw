...
interface RouteWithTabParams<T> {
  req: BrowserRequest;
  res: BrowserResponse;
  ctx: BrowserRouteContext;
  targetId?: string;
...
export async function withRouteTabContext<T>(
  ...
  try {
    ...
    handleRouteError(params.ctx, params.res, error);
    return undefined;
  }
}

interface RouteWithPwParams<T> {
  req: BrowserRequest;
  res: BrowserResponse;
  ctx: BrowserRouteContext;
  targetId?: string;
...
export async function withPlaywrightRouteContext<T>(
  ...
  return await withRouteTabContext({
    ...
    run: async ({ profileCtx, tab, cdpUrl }) => {
      ...
      return await params.run({ cdpUrl, profileCtx, pw, tab });
    },
