...
export async function evaluateJavaScript(opts: {
  ...
  return await withCdpSocket(opts.wsUrl, async (send) => {
    ...
    return { exceptionDetails: evaluated.exceptionDetails, result };
  });
}
...
export function formatAriaSnapshot(nodes: RawAXNode[], limit: number): AriaSnapshotNode[] {
  ...
  const stack: { id: string; depth: number }[] = [{ depth: 0, id: root.nodeId }];
  while (stack.length && out.length < limit) {
    ...
    for (let i = children.length - 1; i >= 0; i--) {
      ...
      if (child) {
        stack.push({ depth: depth + 1, id: child });
      }
    }
  }
...
export async function querySelector(opts: {
  ...
  const maxHtml = Math.max(0, Math.min(20_000, Math.floor(opts.maxHtmlChars ?? 1500)));
