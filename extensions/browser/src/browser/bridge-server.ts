...
interface ResolvedNoVncObserver {
  noVncPort: number;
  password?: string;
}
...
export async function startBrowserBridgeServer(params: {
  ...
  installBrowserAuthMiddleware(app, { password: authPassword, token: authToken });
  ...
  setBridgeAuthForPort(resolvedPort, { password: authPassword, token: authToken });

  const baseUrl = `http://${host}:${resolvedPort}`;
  return { baseUrl, port: resolvedPort, server, state };
}
