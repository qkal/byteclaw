...
function readTargetUrlParam(params: Record<string, unknown>) {
  return (
    ...
    readStringParam(params, "url", { label: "targetUrl", required: true })
  );
}
...
const BROWSER_PROXY_GATEWAY_TIMEOUT_SLACK_MS = 5000;

interface BrowserNodeTarget {
  nodeId: string;
  label?: string;
}
...
async function resolveBrowserNodeTarget(params: {
  ...
  if (requested) {
    ...
    return { label: node?.displayName ?? node?.remoteIp ?? nodeId, nodeId };
  }
      ...
      switch (action) {
        ...
        case "pdf": {
          ...
          const result = proxyRequest
            ...
            : await browserToolDeps.browserPdfSave(baseUrl, { profile, targetId });
          return {
            content: [{ text: `FILE:${result.path}`, type: "text" as const }],
            details: result,
          };
        }
        case "upload": {
