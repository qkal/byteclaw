...
import { type PluginRuntime, SILENT_REPLY_TOKEN } from "openclaw/plugin-sdk/msteams";
...
function requireSentMessage(sent: { text?: string; entities?: unknown[] }[]) {
...
describe("msteams messenger", () => {
  ...
  describe("renderReplyPayloadsToMessages", () => {
    ...
    it("does not filter non-exact silent reply prefixes", () => {
      const messages = renderReplyPayloadsToMessages(
        ...
        { tableMode: "code", textChunkLimit: 4000 },
      );
    ...
    it("splits media into separate messages by default", () => {
      const messages = renderReplyPayloadsToMessages(
        [{ mediaUrl: "https://example.com/a.png", text: "hi" }],
        { tableMode: "code", textChunkLimit: 4000 },
      );
    ...
    it("supports inline media mode", () => {
      const messages = renderReplyPayloadsToMessages(
        [{ mediaUrl: "https://example.com/a.png", text: "hi" }],
        { mediaMode: "inline", tableMode: "code", textChunkLimit: 4000 },
      );
      expect(messages).toEqual([{ mediaUrl: "https://example.com/a.png", text: "hi" }]);
    });
  ...
  describe("sendMSTeamsMessages", () => {
    ...
    it("preserves parsed mentions when appending OneDrive fallback file links", async () => {
      ...
      try {
        const sent: { text?: string; entities?: unknown[] }[] = [];
        const ctx = {
          deleteActivity: noopDeleteActivity,
          sendActivity: async (activity: unknown) => {
        ...
        const ids = await sendMSTeamsMessages({
          ...
          messages: [{ mediaUrl: localFile, text: "Hello @[John](29:08q2j2o3jc09au90eucae)" }],
          replyStyle: "thread",
          tokenProvider: {
    ...
    it("retries thread sends on throttling (429)", async () => {
      ...
      const retryEvents: { nextAttempt: number; delayMs: number }[] = [];
      ...
      const ids = await sendMSTeamsMessages({
        ...
        conversationRef: baseRef,
        messages: [{ text: "one" }],
        onRetry: (e) => retryEvents.push({ delayMs: e.delayMs, nextAttempt: e.nextAttempt }),
        replyStyle: "thread",
        retry: { baseDelayMs: 0, maxAttempts: 2, maxDelayMs: 0 },
      });

      expect(attempts).toEqual(["one", "one"]);
      expect(ids).toEqual(["id:one"]);
      expect(retryEvents).toEqual([{ delayMs: 0, nextAttempt: 2 }]);
    });
    ...
    it("does not retry thread sends on client errors (4xx)", async () => {
      ...
      await expect(
        sendMSTeamsMessages({
          ...
          messages: [{ text: "one" }],
          replyStyle: "thread",
          retry: { baseDelayMs: 0, maxAttempts: 3, maxDelayMs: 0 },
        }),
  ...
  describe("buildActivity AI metadata", () => {
    const baseRef: StoredConversationReference = {
      ...
      agent: { id: "bot123", name: "Bot" },
      channelId: "msteams",
      conversation: { conversationType: "personal", id: "conv123" },
      serviceUrl: "https://service.example.com",
      user: { id: "user123", name: "User" },
    };
    ...
    it("adds AI-generated entity to text messages", async () => {
      ...
      const entities = activity.entities as Record<string, unknown>[];
