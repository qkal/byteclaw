...
interface RemoteMediaFetchParams {
  url: string;
  maxBytes?: number;
  filePathHint?: string;
...
interface AttachmentAuthRetryScenario {
  ...
  describe("downloadMSTeamsAttachments", () => {
    ...
    it("preserves auth fallback when dispatcher-mode fetch returns a redirect", async () => {
      ...
      const media = await downloadAttachmentsWithFetch(
        ...
        fetchMock,
        { authAllowHosts: [TEST_HOST], tokenProvider },
      );
    ...
    describe("OneDrive/SharePoint shared links", () => {
      ...
      it.each([
        ...
        const calledUrls = (fetchMock.mock.calls as [RequestInfo | URL, RequestInit?][]).map(
