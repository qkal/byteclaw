 ...
 */
export interface BotFrameworkUserTokenResponse {
  channelId?: string;
...
export interface MSTeamsSsoDeps {
...
export interface MSTeamsSsoUser {
...
export interface SigninTokenExchangeValue {
  id?: string;
  connectionName?: string;
  token?: string;
}

export interface SigninVerifyStateValue {
  state?: string;
}
...
export function parseSigninTokenExchangeValue(value: unknown): SigninTokenExchangeValue | null {
  ...
  return { connectionName, id, token };
}
...
interface UserTokenServiceCallParams {
  baseUrl: string;
  path: string;
...
async function callUserTokenService(
  ...
  return { channelId, connectionName, expiration, token };
}
...
export async function handleSigninTokenExchangeInvoke(params: {
  ...
  if (!user.userId) {
    return { code: "missing_user", message: "no user id on invoke activity", ok: false };
  }
  const connectionName = value.connectionName?.trim() || deps.connectionName;
  if (!connectionName) {
    return { code: "missing_connection", message: "no OAuth connection name", ok: false };
  }
  if (!value.token) {
    return { code: "missing_token", message: "no exchangeable token on invoke", ok: false };
  }
  ...
  return { expiresAt: result.expiration, ok: true, token: result.token };
}
...
export async function handleSigninVerifyStateInvoke(params: {
  ...
  if (!state) {
    return { code: "missing_state", message: "no state code on invoke", ok: false };
  }
