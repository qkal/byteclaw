...
import { type MSTeamsPollStore, createMSTeamsPollStoreFs } from "./polls.js";
import {
...
export interface MonitorMSTeamsOpts {
  cfg: OpenClawConfig;
  runtime?: RuntimeEnv;
...
export interface MonitorMSTeamsResult {
  app: unknown;
...
export async function monitorMSTeamsProvider(
  ...
  try {
    ...
    if (Array.isArray(groupAllowFrom) && groupAllowFrom.length > 0) {
      ...
      if (groupEntries.length > 0) {
        ...
        groupAllowFrom = mergeAllowlist({ additions, existing: groupAllowFrom });
      }
    }

    if (teamsConfig && Object.keys(teamsConfig).length > 0) {
      const entries: { input: string; teamKey: string; channelKey?: string }[] = [];
    ...
    runtime.log?.(`msteams resolve failed; using config entries. ${formatUnknownError(error)}`);
  }
  ...
  expressApp.use((req: Request, res: Response, next: (err?: unknown) => void) => {
    ...
    jwtValidator
      ...
      .catch((error) => {
        log.debug?.(`JWT validation error: ${formatUnknownError(error)}`);
        res.status(401).json({ error: "Unauthorized" });
      });
  });
  ...
  const messageHandler = (req: Request, res: Response) => {
    void adapter
      ...
      .catch((error: unknown) => {
        log.error("msteams webhook failed", { error: formatUnknownError(error) });
      });
  };
