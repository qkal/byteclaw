...
const clientConstructorState = vi.hoisted(() => ({
  calls: [] as { serviceUrl: string; options: unknown }[],
}));
...
const jwtState = vi.hoisted(() => ({
  ...
  verifyCalls: [] as { token: string; options: unknown }[],
}));
...
const jwtMockImpl = {
  ...
  verify: (token: string, _key: string, options: unknown) => {
    jwtState.verifyCalls.push({ options, token });
...
function createSdkStub(): MSTeamsTeamsSdk {
  ...
  class ClientStub {
    constructor(serviceUrl: string, options: unknown) {
      clientConstructorState.calls.push({ options, serviceUrl });
    }
