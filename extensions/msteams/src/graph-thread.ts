import { type GraphResponse, fetchGraphJson } from "./graph.js";
...
export async function resolveTeamGroupId(
  ...
  try {
    ...
    const team = await fetchGraphJson<{ id?: string }>({ path, token });
...
export async function fetchChannelMessage(
  ...
  try {
    return await fetchGraphJson<GraphThreadMessage>({ path, token });
  } catch {
    return undefined;
  }
}
...
export async function fetchThreadReplies(
  ...
  const res = await fetchGraphJson<GraphResponse<GraphThreadMessage>>({ path, token });
  return res.value ?? [];
}
