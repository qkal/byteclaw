...
interface InlineImageLimitOptions {
...
export function readNestedString(value: unknown, keys: (string | number)[]): string | undefined {
...
function decodeDataImageWithLimits(
  ...
  try {
    ...
    return {
      candidate: { contentType, data, kind: "data", placeholder: "<media:image>" },
      estimatedBytes,
    };
  } catch {
    return { candidate: null, estimatedBytes: 0 };
...
export interface MSTeamsAttachmentFetchPolicy {
  allowHosts: string[];
