...
describe("TeamsHttpStream", () => {
  ...
  it("sends first chunk as typing activity with streaminfo", async () => {
    ...
    const entities = firstActivity.entities as Record<string, unknown>[];
    expect(entities).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ streamType: "streaming", type: "streaminfo" }),
      ]),
    );
  });
  ...
  it("sends final message activity on finalize", async () => {
    ...
    const entities = finalActivity!.entities as Record<string, unknown>[];
    expect(entities).toEqual(
    ...
    expect(entities).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ streamType: "final", type: "streaminfo" }),
      ]),
    );
  });
  ...
  it("informative update establishes streamId for subsequent chunks", async () => {
    ...
    const entities = chunk.entities as Record<string, unknown>[];
    expect(entities).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ streamId: "stream-1", type: "streaminfo" }),
      ]),
    );
  });
