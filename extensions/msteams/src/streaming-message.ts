...
import { type DraftStreamLoop, createDraftStreamLoop } from "openclaw/plugin-sdk/channel-lifecycle";
...
export class TeamsHttpStream {
  ...
  async finalize(): Promise<void> {
    ...
    try {
      const entities: Record<string, unknown>[] = [AI_GENERATED_ENTITY];
      if (this.streamId) {
  ...
  private async pushStreamChunk(text: string): Promise<boolean> {
    ...
    try {
      ...
      const axiosData = (error as { response?: { data?: unknown; status?: number } })?.response;
      const statusCode = axiosData?.status ?? (error as { statusCode?: number })?.statusCode;
