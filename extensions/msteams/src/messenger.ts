...
export interface MSTeamsAdapter {
...
export interface MSTeamsReplyRenderOptions {
 ...
 */
export interface MSTeamsRenderedMessage {
  text?: string;
  mediaUrl?: string;
}

export interface MSTeamsSendRetryOptions {
  maxAttempts?: number;
  baseDelayMs?: number;
  maxDelayMs?: number;
}

export interface MSTeamsSendRetryEvent {
  messageIndex: number;
  messageCount: number;
  nextAttempt: number;
  maxAttempts: number;
...
function resolveRetryOptions(
  ...
  if (!retry) {
    return { baseDelayMs: 0, enabled: false, maxAttempts: 1, maxDelayMs: 0 };
  }
  return {
...
export function renderReplyPayloadsToMessages(
  ...
  for (const payload of replies) {
    ...
    if (!reply.hasMedia) {
      pushTextMessages(out, reply.text, { chunkLimit, chunkMode, chunkText });
      continue;
    }
    ...
    if (mediaMode === "inline") {
      ...
      if (firstMedia) {
        out.push({ mediaUrl: firstMedia, text: reply.text || undefined });
    ...
    if (isLocalPath(msg.mediaUrl)) {
      ...
      if (
        ...
        const { activity: consentActivity } = prepareFileConsentActivity({
          ...
          media: { buffer: media.buffer, contentType, filename: fileName },
        });
...
export async function sendMSTeamsMessages(params: {
  ...
  const sendMessageInContext = async (
    ...
    const response = await sendWithRetry(
      ...
      { messageCount: messages.length, messageIndex },
    );
