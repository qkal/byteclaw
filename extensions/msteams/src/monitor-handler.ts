...
export interface MSTeamsActivityHandler {
  onMessage: (
  ...
  if (consentResponse.action === "accept" && consentResponse.uploadInfo) {
    if (pendingFile) {
      ...
      try {
        ...
        log.error("file upload failed", { error: formatUnknownError(error), uploadId });
...
async function handleFeedbackInvoke(
  ...
  if (isNegative && msteamsCfg?.feedbackReflection !== false) {
    ...
    runFeedbackReflection({
      ...
      deps.log.error("feedback reflection failed", { error: formatUnknownError(error) });
    });
  }
...
export function registerMSTeamsHandlers<T extends MSTeamsActivityHandler>(
    ...
    handler.run = async (context: unknown) => {
      ...
      if (ctx.activity?.type === "invoke" && ctx.activity?.name === "fileConsent/invoke") {
        ...
        try {
          ...
          deps.log.debug?.("file consent handler error", { error: formatUnknownError(error) });
        }
        return;
      }
      ...
      if (
        ...
        await ctx.sendActivity({ type: "invokeResponse", value: { body: {}, status: 200 } });
  ...
  handler.onMessage(async (context, next) => {
    try {
      ...
      deps.runtime.error?.(`msteams handler failed: ${formatUnknownError(error)}`);
    }
    await next();
  });
      ...
      if (member.id === botId) {
        ...
        if (isPersonal && msteamsCfg?.welcomeCard !== false) {
          ...
          try {
            ...
            deps.log.debug?.("failed to send welcome card", { error: formatUnknownError(error) });
          }
          ...
          try {
            ...
            deps.log.debug?.("failed to send group welcome", { error: formatUnknownError(error) });
          }
        } else {
