...
export interface ListChannelsMSTeamsParams {
  cfg: OpenClawConfig;
  teamId: string;
}

export interface ListChannelsMSTeamsResult {
  channels: {
    id: string | undefined;
...
export interface GetChannelInfoMSTeamsParams {
  cfg: OpenClawConfig;
  teamId: string;
  channelId: string;
}

export interface GetChannelInfoMSTeamsResult {
  channel: {
    id: string | undefined;
    displayName: string | undefined;
    description: string | undefined;
...
export async function getChannelInfoMSTeams(
  ...
  const ch = await fetchGraphJson<GraphTeamsChannel>({ path, token });
  return {
    channel: {
