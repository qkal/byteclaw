 ...
 */
export interface MSTeamsTeamsSdk {
 ...
 */
export interface MSTeamsTokenProvider {
  getAccessToken: (scope: string) => Promise<string>;
}

interface MSTeamsBotIdentity {
  id?: string;
  name?: string;
}

interface MSTeamsSendContext {
...
function normalizeOutboundActivity(textOrActivity: string | object): Record<string, unknown> {
  return typeof textOrActivity === "string"
    ? ({ text: textOrActivity, type: "message" } as Record<string, unknown>)
...
function createProcessContext(params: {
  ...
  return {
    ...
    ...sendContext,
    async sendActivities(activities: ({ type: string } & Record<string, unknown>)[]) {
      const results = [];
  ...
  return {
    ...
    async process(req, res, logic) {
      ...
      try {
        ...
        if (!isInvoke) {
          response.status(500).send({ error: formatUnknownError(error) });
        }
      }
    },
 ...
 */
const BOT_FRAMEWORK_ISSUERS: readonly {
