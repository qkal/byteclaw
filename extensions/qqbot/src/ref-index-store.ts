...
function loadFromFile(): Map<string, RefIndexEntry & { _createdAt: number }> {
  ...
  try {
    ...
    const raw = fs.readFileSync(REF_INDEX_FILE, "utf8");
    ...
    debugError(`[ref-index-store] Failed to load: ${String(error)}`);
    cache = new Map();
  }
...
function appendLine(line: RefIndexLine): void {
  try {
    ...
    debugError(`[ref-index-store] Failed to append: ${String(error)}`);
  }
}
...
function compactFile(): void {
  ...
  try {
    ...
    debugError(
      `[ref-index-store] Compact failed: ${error instanceof Error ? error.message : JSON.stringify(error)}`,
    );
  }
}
