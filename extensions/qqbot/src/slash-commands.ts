...
const frameworkCommands = new Map<string, SlashCommand>();
...
export function getFrameworkCommands(): QQBotFrameworkCommand[] {
  return [...frameworkCommands.values()].map((cmd) => ({
...
function collectRecentLogFiles(logDirs: string[]): LogCandidate[] {
  ...
  const pushFile = (filePath: string, sourceDir: string) => {
    ...
    try {
      ...
      candidates.push({ filePath: normalized, mtimeMs: stat.mtimeMs, sourceDir });
    } catch {
...
function buildBotLogsResult(): SlashCommandResult {
  ...
  const topSources = [...new Set(recentFiles.map((item) => item.sourceDir))].slice(0, 3);
