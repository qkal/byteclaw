...
import { debugError, debugLog, debugWarn } from "./debug-log.js";
...
function loadSilkWasm(): Promise<SilkWasm | null> {
  ...
  _silkWasmPromise = import("silk-wasm").catch((error) => {
    debugWarn(
      `[audio-convert] silk-wasm not available; SILK encode/decode disabled (${formatErrorMessage(error)})`,
    );
    return null;
  });
  return _silkWasmPromise;
...
export async function convertSilkToWav(
  ...
  return { duration: result.duration, wavPath };
}
...
interface QQBotTtsProviderConfig {
  baseUrl?: string;
  apiKey?: string;
  authStyle?: string;
  queryParams?: Record<string, string>;
...
export async function textToSpeechPCM(
  ...
  const formats: { format: string; needsDecode: boolean }[] = [
  ...
  for (const { format, needsDecode } of formats) {
    ...
    const ttsTimeout = setTimeout(() => controller.abort(), 120_000);
...
export async function textToSilk(
  ...
  return { duration, silkBase64: silkBuffer.toString("base64"), silkPath };
}
...
export async function audioFileToSilkBase64(
  ...
  if (ffmpegCmd) {
    try {
      ...
      debugError(`[audio-convert] ffmpeg conversion failed: ${formatErrorMessage(error)}`);
    }
  }
...
export async function waitForFile(
  ...
  const emptyGiveUpMs = 10_000;
  const noFileGiveUpMs = 15_000;
...
async function wasmDecodeMp3ToPCM(buf: Buffer, targetRate: number): Promise<Buffer | null> {
  try {
    ...
    for (let i = 0; i < floatMono.length; i++) {
      ...
      const val = clamped < 0 ? clamped * 32_768 : clamped * 32_767;
    ...
    if (decoded.sampleRate !== targetRate) {
      ...
      for (let i = 0; i < outputSamples; i++) {
        ...
        outView.setInt16(i * 2, Math.max(-32_768, Math.min(32_767, sample)), true);
      }
      pcm = output;
    }
    ...
    debugError(`[audio-convert] WASM MP3 decode failed: ${formatErrorMessage(error)}`);
    if (error instanceof Error && error.stack) {
      debugError(`[audio-convert] stack: ${error.stack}`);
    }
    return null;
  }
}
    ...
    if (chunkId === "data") {
      ...
      if (sampleRate !== targetRate) {
        ...
        for (let i = 0; i < outSamples; i++) {
          ...
          outV.setInt16(
            i * 2,
            Math.max(-32_768, Math.min(32_767, Math.round(s0 + (s1 - s0) * f))),
            true,
          );
        }
        pcm = out;
