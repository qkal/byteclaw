...
export const DEFAULT_IMAGE_SIZE: ImageSize = { height: 512, width: 512 };
...
export async function getImageSizeFromUrl(
  ...
  try {
    ...
    debugLog(`[image-size] Error fetching ${url.slice(0, 60)}...: ${String(error)}`);
    return null;
  }
}
...
export function getImageSizeFromDataUrl(dataUrl: string): ImageSize | null {
  try {
    ...
    debugLog(`[image-size] Error parsing Base64: ${String(error)}`);
    return null;
  }
}
...
export function extractQQBotImageSize(markdownImage: string): ImageSize | null {
  ...
  if (match) {
    return { height: Number.parseInt(match[2], 10), width: Number.parseInt(match[1], 10) };
  }
  return null;
}
