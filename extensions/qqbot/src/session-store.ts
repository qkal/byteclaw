...
export function loadSession(accountId: string, expectedAppId?: string): SessionState | null {
  ...
  try {
    ...
    debugError(`[session-store] Failed to load session for ${accountId}: ${String(error)}`);
    return null;
  }
}
...
function doSaveSession(state: SessionState): void {
  ...
  try {
    ...
    debugError(`[session-store] Failed to save session for ${state.accountId}: ${String(error)}`);
  }
}
...
export function clearSession(accountId: string): void {
  ...
  try {
    ...
    debugError(`[session-store] Failed to clear session for ${accountId}: ${String(error)}`);
  }
}
