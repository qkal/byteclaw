...
async function doFetchToken(appId: string, clientSecret: string): Promise<string> {
  ...
  try {
    ...
    debugError(`[qqbot-api:${appId}] <<< Network error:`, error);
    throw new Error(`Network error getting access_token: ${formatErrorMessage(error)}`, {
      cause: error,
    });
  }
  ...
  try {
    ...
    debugError(`[qqbot-api:${appId}] <<< Parse error:`, error);
    throw new Error(`Failed to parse access_token response: ${formatErrorMessage(error)}`, {
      cause: error,
    });
  }
...
export function getTokenStatus(appId: string): {
  ...
  if (tokenFetchPromises.has(appId)) {
    return { expiresAt: tokenCacheMap.get(appId)?.expiresAt ?? null, status: "refreshing" };
  }
  const cached = tokenCacheMap.get(appId);
  if (!cached) {
    return { expiresAt: null, status: "none" };
  }
  const remaining = cached.expiresAt - Date.now();
  const isValid = remaining > Math.min(5 * 60 * 1000, remaining / 3);
  return { expiresAt: cached.expiresAt, status: isValid ? "valid" : "expired" };
}
...
export async function apiRequest<T = unknown>(
  ...
  try {
    ...
    if (error instanceof Error && error.name === "AbortError") {
      ...
      throw new Error(`Request timeout[${path}]: exceeded ${timeout}ms`, { cause: error });
    }
    debugError(`[qqbot-api] <<< Network error:`, error);
    throw new Error(`Network error [${path}]: ${formatErrorMessage(error)}`, { cause: error });
  } finally {
    clearTimeout(timeoutId);
  }
  ...
  try {
    ...
    throw new Error(`Failed to parse response[${path}]: ${formatErrorMessage(error)}`, {
      cause: error,
    });
  }
...
async function sendAndNotify(
  ...
  if (result.ext_info?.ref_idx && hook) {
    try {
      ...
      debugError(`[qqbot-api:${appId}] onMessageSent hook error: ${String(error)}`);
    }
  }
  return result;
}
...
export async function uploadC2CMedia(
  ...
  if (fileData) {
    ...
    if (cachedInfo) {
      return { file_info: cachedInfo, file_uuid: "", ttl: 0 };
    }
  }
...
export function startBackgroundTokenRefresh(
  ...
  const refreshLoop = async () => {
    ...
    while (!signal.aborted) {
      try {
        ...
        log?.error?.(`[qqbot-api:${appId}] Background token refresh failed: ${String(error)}`);
  ...
  refreshLoop().catch((error) => {
    backgroundRefreshControllers.delete(appId);
    log?.error?.(`[qqbot-api:${appId}] Background token refresh crashed: ${error}`);
  });
}
