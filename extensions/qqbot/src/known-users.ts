...
function loadUsersFromFile(): Map<string, KnownUser> {
  ...
  try {
    if (fs.existsSync(KNOWN_USERS_FILE)) {
      const data = fs.readFileSync(KNOWN_USERS_FILE, "utf8");
    ...
    debugError(`[known-users] Failed to load users: ${String(error)}`);
    usersCache = new Map();
  }
...
function doSaveUsersToFile(): void {
  ...
  try {
    ...
    fs.writeFileSync(KNOWN_USERS_FILE, JSON.stringify(users, null, 2), "utf8");
    isDirty = false;
  } catch (error) {
    debugError(`[known-users] Failed to save users: ${String(error)}`);
  }
}
...
export function getKnownUser(
  ...
  const cache = loadUsersFromFile();
  const key = makeUserKey({ accountId, groupOpenid, openid, type });
