...
interface NormalizedFunctionCallOutput {
...
function normalizeXaiResponsesFunctionCallOutput(
  ...
  if (!item || typeof item !== "object") {
    return { imageParts: [], normalizedItem: item };
  }

  const itemObj = item as Record<string, unknown>;
  if (itemObj.type !== "function_call_output" || !Array.isArray(itemObj.output)) {
    return { imageParts: [], normalizedItem: itemObj };
  }

  const outputParts = itemObj.output as Record<string, unknown>[];
...
function normalizeXaiResponsesToolResultPayload(
  ...
  const collectedImageParts: Record<string, unknown>[] = [];
  ...
  if (collectedImageParts.length > 0) {
    normalizedInput.push({
      content: [
        { text: TOOL_RESULT_IMAGE_REPLAY_TEXT, type: "input_text" },
        ...collectedImageParts,
      ],
      role: "user",
      type: "message",
