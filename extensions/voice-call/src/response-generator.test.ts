...
function createAgentRuntime(payloads: Record<string, unknown>[]) {
  const runEmbeddedPiAgent = vi.fn(async () => ({
    meta: { aborted: false, durationMs: 12 },
    payloads,
  }));
  ...
  return { runEmbeddedPiAgent, runtime };
}
...
describe("generateVoiceResponse", () => {
  it("suppresses reasoning payloads and reads structured spoken output", async () => {
    const { runtime, runEmbeddedPiAgent } = createAgentRuntime([
      { isReasoning: true, text: "Reasoning: hidden" },
