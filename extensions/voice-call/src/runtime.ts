...
import { type TunnelResult, startTunnel } from "./tunnel.js";
...
interface ResolvedRealtimeProvider {
  ...
  try {
    ...
    if (!publicUrl && config.tunnel?.provider && config.tunnel.provider !== "none") {
      try {
        ...
        log.error(`[voice-call] Tunnel setup failed: ${formatErrorMessage(error)}`);
      }
    }
    ...
    if (provider.name === "twilio" && config.streaming?.enabled) {
      ...
      if (ttsRuntime?.textToSpeechTelephony) {
        try {
          ...
          log.warn(`[voice-call] Failed to initialize telephony TTS: ${formatErrorMessage(error)}`);
        }
      } else {
