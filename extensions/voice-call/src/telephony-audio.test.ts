...
describe("telephony-audio resamplePcmTo8k", () => {
  it("returns identical buffer for 8k input", () => {
    const pcm8k = makeSinePcm(8000, 1000, 0.2);
    const resampled = resamplePcmTo8k(pcm8k, 8000);
    expect(resampled).toBe(pcm8k);
  });

  it("preserves low-frequency speech-band energy when downsampling", () => {
    const input = makeSinePcm(48_000, 1000, 0.6);
    const output = resamplePcmTo8k(input, 48_000);
    expect(output.length).toBe(9600);
    expect(rmsPcm(output)).toBeGreaterThan(7500);
  });

  it("attenuates out-of-band high frequencies before 8k telephony conversion", () => {
    const lowTone = resamplePcmTo8k(makeSinePcm(48_000, 1000, 0.6), 48_000);
    const highTone = resamplePcmTo8k(makeSinePcm(48_000, 6000, 0.6), 48_000);
