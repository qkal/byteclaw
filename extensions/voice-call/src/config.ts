...
export const VoiceCallServeConfigSchema = z
  ...
  .strict()
  .default({ bind: "127.0.0.1", path: "/voice/webhook", port: 3334 });
...
export const VoiceCallTunnelConfigSchema = z
  ...
  .strict()
  .default({ allowNgrokFreeTierLoopbackBypass: false, provider: "none" });
...
export const VoiceCallRealtimeConfigSchema = z
  ...
  .strict()
  .default({ enabled: false, providers: {}, tools: [] });
...
export const VoiceCallConfigSchema = z
  .object({
    ...
    transcriptTimeoutMs: z.number().int().positive().default(180_000),

    /** Ring timeout for outbound calls (ms) */
    ringTimeoutMs: z.number().int().positive().default(30_000),
    ...
    responseTimeoutMs: z.number().int().positive().default(30_000),
  })
  .strict();
