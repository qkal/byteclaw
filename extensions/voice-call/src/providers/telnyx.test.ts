...
function expectReplayVerification(
  results: { ok: boolean; isReplay?: boolean; verifiedRequestKey?: string }[],
) {
...
describe("TelnyxProvider.verifyWebhook", () => {
  ...
  it("verifies a valid signature with a raw Ed25519 public key (Base64)", () => {
    ...
    expectWebhookVerificationSucceeds({ privateKey, publicKey: rawPublicKeyBase64 });
  });
  ...
  it("verifies a valid signature with a DER SPKI public key (Base64)", () => {
    ...
    expectWebhookVerificationSucceeds({ privateKey, publicKey: spkiDerBase64 });
  });
