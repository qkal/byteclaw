...
import { type CallId, type CallRecord, CallRecordSchema, TerminalStates } from "../types.js";

export function persistCallRecord(storePath: string, call: CallRecord): void {
  const logPath = path.join(storePath, "calls.jsonl");
  const line = `${JSON.stringify(call)}\n`;
  // Fire-and-forget async write to avoid blocking event loop.
  fsp.appendFile(logPath, line).catch((error) => {
    console.error("[voice-call] Failed to persist call record:", error);
  });
}
...
export function loadActiveCallsFromStore(storePath: string): {
  ...
  return { activeCalls, processedEventIds, providerCallIdMap, rejectedProviderCallIds };
}
