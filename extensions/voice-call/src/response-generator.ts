...
export interface VoiceResponseParams {
  /** Voice call config */
  ...
  transcript: { speaker: "user" | "bot"; text: string }[];
  /** Latest user message */
  userMessage: string;
}
...
export async function generateVoiceResponse(
  ...
  if (!coreConfig) {
    return { error: "Core config unavailable for voice response", text: null };
  }
  ...
  const { provider, model } = resolveVoiceResponseModel({ agentRuntime, voiceConfig });

  // Resolve thinking level
  const thinkLevel = agentRuntime.resolveThinkingDefault({ cfg, model, provider });
  ...
  try {
    ...
    if (!text && result.meta?.aborted) {
      return { error: "Response generation was aborted", text: null };
    }

    return { text };
  } catch (error) {
    console.error(`[voice-call] Response generation failed:`, error);
    return { error: String(error), text: null };
  }
}
