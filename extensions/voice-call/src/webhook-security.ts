...
function importEd25519PublicKey(publicKey: string): crypto.KeyObject | string {
  ...
  if (decoded.length === 32) {
    ...
    return crypto.createPublicKey({
      format: "jwk",
      key: { crv: "Ed25519", kty: "OKP", x: base64UrlEncode(decoded) },
    });
  }
...
export function verifyTelnyxWebhook(
  ...
  try {
    ...
    const isReplay = markReplay(telnyxReplayCache, replayKey);
    return { isReplay, ok: true, verifiedRequestKey: replayKey };
  } catch (error) {
    return {
      ok: false,
      reason: `Verification error: ${formatErrorMessage(error)}`,
    };
  }
}
...
export function verifyTwilioWebhook(
  ...
  if (isValid) {
    ...
    const isReplay = markReplay(twilioReplayCache, replayKey);
    return { isReplay, ok: true, verificationUrl, verifiedRequestKey: replayKey };
  }
  ...
  for (const candidateUrl of variants) {
    ...
    const isReplay = markReplay(twilioReplayCache, replayKey);
    return { isReplay, ok: true, verificationUrl: candidateUrl, verifiedRequestKey: replayKey };
  }
...
export function verifyPlivoWebhook(
  ...
  if (signatureV3 && nonceV3) {
    ...
    const isReplay = markReplay(plivoReplayCache, replayKey);
    return { isReplay, ok: true, verificationUrl, verifiedRequestKey: replayKey, version: "v3" };
  }
  ...
  if (signatureV2 && nonceV2) {
    ...
    const isReplay = markReplay(plivoReplayCache, replayKey);
    return { isReplay, ok: true, verificationUrl, verifiedRequestKey: replayKey, version: "v2" };
  }
