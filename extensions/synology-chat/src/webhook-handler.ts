...
import { RateLimiter, authorizeUserForDm, sanitizeInput, validateToken } from "./security.js";
import type { ResolvedSynologyChatAccount, SynologyWebhookPayload } from "./types.js";
...
const PREAUTH_BODY_TIMEOUT_MS = 5000;
const PREAUTH_MAX_REQUESTS_PER_MINUTE = 10;
const INVALID_TOKEN_WINDOW_MS = 60_000;
const INVALID_TOKEN_MAX_TRACKED_KEYS = 5000;

interface InvalidTokenRateLimitState {
  count: number;
...
interface AuthorizedSynologyWebhook {
...
async function parseWebhookPayloadRequest(params: {
  ...
  try {
    ...
    params.log?.warn("Failed to parse webhook payload", error);
...
function authorizeSynologyWebhook(params: {
  ...
  if (params.invalidTokenRateLimiter.isLocked(invalidTokenRateLimitKey)) {
    ...
    return { error: "Rate limit exceeded", ok: false, statusCode: 429 };
  }
  ...
  if (!validateToken(params.payload.token, params.account.token)) {
    ...
    return { error: "Invalid token", ok: false, statusCode: 401 };
  }
  ...
  if (!auth.allowed) {
    if (auth.reason === "disabled") {
      return { error: "DMs are disabled", ok: false, statusCode: 403 };
    }
    ...
    return { error: "User not authorized", ok: false, statusCode: 403 };
  }
  ...
  return { commandAuthorized: auth.allowed, ok: true };
}
