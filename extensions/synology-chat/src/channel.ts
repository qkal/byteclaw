...
interface SynologyChannelGatewayContext {
  cfg: OpenClawConfig;
  accountId: string;
...
interface SynologyChannelOutboundContext {
  cfg: OpenClawConfig;
  to: string;
  text?: string;
  mediaUrl?: string;
...
interface SynologySecurityWarningContext {
  cfg: OpenClawConfig;
...
interface SynologyChatOutboundResult {
  ...
  return createChatChannelPlugin({
    base: {
      ...
      gateway: {
        startAccount: async (ctx: SynologyChannelGatewayContext) => {
          ...
          if (!validateSynologyGatewayAccountStartup({ account, accountId, cfg, log }).ok) {
    ...
    outbound: {
      ...
      sendMedia: async ({ to, mediaUrl, accountId, cfg }: SynologyChannelOutboundContext) => {
        ...
        return attachChannelToResult(CHANNEL_ID, { chatId: to, messageId: `sc-${Date.now()}` });
      },
