...
vi.mock("node:https", () => {
  ...
  const mockGet = vi.fn();
  return { default: { get: mockGet, request: mockRequest }, get: mockGet, request: mockRequest };
});
...
function mockUserListResponseImpl(
  ...
  const impl: MockRequestHandler = (_url, _opts, callback) => {
    ...
    process.nextTick(() => {
      callback?.(res);
      res.emit("data", Buffer.from(JSON.stringify({ data: { users }, success: true })));
      res.emit("end");
    });
    return createMockRequestEmitter();
  };
...
describe("resolveLegacyWebhookNameToChatUserId", () => {
  ...
  it("resolves user by nickname (webhook username = Chat nickname)", async () => {
    mockUserListResponse([
      { nickname: "jmn", user_id: 4, username: "jmn67" },
      { nickname: "sarah", user_id: 7, username: "she67" },
    ]);
  ...
  it("resolves user by username when nickname does not match", async () => {
    mockUserListResponse([
      { nickname: "", user_id: 4, username: "jmn67" },
      { nickname: "sarah", user_id: 7, username: "she67" },
    ]);
  ...
  it("is case-insensitive", async () => {
    mockUserListResponse([{ nickname: "JMN", user_id: 4, username: "JMN67" }]);
    fakeNowMs += 10 * 60 * 1000;
    vi.setSystemTime(fakeNowMs);
    const result = await resolveLegacyWebhookNameToChatUserId({
      incomingUrl: baseUrl,
  ...
  it("keeps user cache scoped per incoming URL", async () => {
    mockUserListResponseOnce([{ nickname: "jmn", user_id: 4, username: "jmn67" }]);
    mockUserListResponseOnce([{ nickname: "jmn", user_id: 9, username: "jmn67" }]);
...
describe("fetchChatUsers", () => {
  ...
  it("filters malformed user entries while keeping valid ones", async () => {
    ...
    expect(users).toEqual([{ nickname: "jmn", user_id: 4, username: "jmn67" }]);
  });
