...
export async function handleMatrixAction(
  ...
  const isActionEnabled = createActionGate(resolveMatrixAccountConfig({ accountId, cfg }).actions);
  const clientOpts = {
    cfg,
    ...(accountId ? { accountId } : {}),
  };
  ...
  if (pinActions.has(action)) {
    ...
    return jsonResult({ events: result.events, ok: true, pinned: result.pinned });
  }
  ...
  if (action === "memberInfo") {
    ...
    return jsonResult({ member: result, ok: true });
  }
  ...
  if (verificationActions.has(action)) {
    ...
    if (action === "verificationCancel") {
      ...
      const verification = await cancelMatrixVerification(
        readStringParam({ requestId }, "requestId", { required: true }),
        { code: code ?? undefined, reason: reason ?? undefined, ...clientOpts },
      );
      return jsonResult({ ok: true, verification });
    }
