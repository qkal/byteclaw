...
async function resolveMatrixMonitorUserEntries(params: {
  ...
  const directMatches: { input: string; resolved: boolean; id?: string }[] = [];
  const pending: { input: string; query: string }[] = [];
...
async function resolveMatrixMonitorRoomsConfig(params: {
  ...
  const pending: { input: string; query: string; config: MatrixRoomConfig }[] = [];
  for (const [entry, roomConfig] of Object.entries(roomsConfig)) {
    ...
    pending.push({ config: roomConfig, input, query: cleaned });
  }
