...
function normalizeAllowList(list?: (string | number)[]) {
...
export function normalizeMatrixAllowList(list?: (string | number)[]) {
...
export function resolveMatrixAllowListMatch(params: {
  ...
  const candidates: { value?: string; source: MatrixAllowListMatchSource }[] = [
    { source: "id", value: userId },
    { source: "prefixed-id", value: userId ? `matrix:${userId}` : "" },
    { source: "prefixed-user", value: userId ? `user:${userId}` : "" },
  ];
