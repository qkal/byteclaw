...
interface MatrixVerificationSummaryLike {
  id: string;
...
interface MatrixDirectRoomDeps {
...
function readVerificationSignal(event: MatrixRawEvent): {
  ...
  if (type === `${matrixVerificationConstants.eventPrefix}request`) {
    return { flowId, stage: "request" };
  }
  if (type === `${matrixVerificationConstants.eventPrefix}ready`) {
    return { flowId, stage: "ready" };
  }
  if (type === "m.key.verification.start") {
    return { flowId, stage: "start" };
  }
  if (type === "m.key.verification.cancel") {
    return { flowId, stage: "cancel" };
  }
  if (type === "m.key.verification.done") {
    return { flowId, stage: "done" };
  }
  return { flowId, stage: "other" };
}
...
async function sendVerificationNotice(params: {
  ...
  try {
    ...
    params.logVerboseMessage(
      `matrix: failed sending verification notice room=${roomId}: ${String(error)}`,
    );
  }
}
...
export function createMatrixVerificationEventRouter(params: {
  ...
  function routeVerificationEvent(roomId: string, event: MatrixRawEvent): boolean {
    ...
    void (async () => {
      ...
      const stageNotice = formatVerificationStageNotice({ event, senderId, stage: signal.stage });
      const { summary, sasNotice } = await resolveVerificationSasNoticeForSignal(params.client, {
        ...
        senderId,
        stage: signal.stage,
      }).catch(() => ({ sasNotice: null, summary: null }));
      ...
      params.logVerboseMessage(`matrix: failed routing verification event: ${String(error)}`);
    });
