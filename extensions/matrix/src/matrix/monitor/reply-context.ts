...
export function createMatrixReplyContextResolver(params: {
  ...
  return async (input: { roomId: string; eventId: string }): Promise<MatrixReplyContext> => {
    ...
    const event = await params.client.getEvent(input.roomId, input.eventId).catch((error) => {
      params.logVerboseMessage(
        `matrix: failed resolving reply context room=${input.roomId} id=${input.eventId}: ${String(error)}`,
      );
      return null;
    });
    if (!event) {
