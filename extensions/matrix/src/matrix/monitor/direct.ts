...
interface DirectRoomTrackerOptions {
...
export function createDirectRoomTracker(client: MatrixClient, opts: DirectRoomTrackerOptions = {}) {
  ...
  const resolveJoinedMembers = async (roomId: string): Promise<string[] | null> => {
    ...
    try {
      ...
      log(`matrix: dm member lookup failed room=${roomId} (${String(error)})`);
      return null;
    }
  };
  ...
  const canPromoteRecentInvite = async (roomId: string): Promise<boolean> => {
    try {
      ...
      log(`matrix: recent invite promotion veto failed room=${roomId} (${String(error)})`);
      return false;
    }
  };
  ...
  const shouldKeepLocallyPromotedDirectRoom = async (
    ...
    try {
      ...
      log(`matrix: local promotion keep-check failed room=${roomId} (${String(error)})`);
      return undefined;
    }
  };
  ...
  return {
    ...
    isDirectMessage: async (params: DirectMessageCheck): Promise<boolean> => {
      ...
      try {
        ...
        log(`matrix: dm cache refresh failed (${String(error)})`);
      }
