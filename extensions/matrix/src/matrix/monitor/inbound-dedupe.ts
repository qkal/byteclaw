...
interface StoredMatrixInboundDedupeEntry {
  key: string;
  ts: number;
}

interface StoredMatrixInboundDedupeState {
  version: number;
  entries: StoredMatrixInboundDedupeEntry[];
}

export interface MatrixInboundEventDeduper {
...
function toStoredState(params: {
  ...
  return {
    entries: [...params.seen.entries()].map(([key, ts]) => ({ key, ts })),
...
export async function createMatrixInboundEventDeduper(params: {
  ...
  try {
    ...
    pruneSeenEvents({ maxEntries, nowMs: nowMs(), seen, ttlMs });
  } catch (error) {
    LogService.warn("MatrixInboundDedupe", "Failed loading Matrix inbound dedupe store:", error);
  }
  ...
  return {
    claimEvent: ({ roomId, eventId }) => {
      const key = buildEventKey({ eventId, roomId });
      if (!key) {
        return true;
      }
      pruneSeenEvents({ maxEntries, nowMs: nowMs(), seen, ttlMs });
