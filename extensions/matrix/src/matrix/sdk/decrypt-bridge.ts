...
import { type MatrixEvent, MatrixEventEvent } from "matrix-js-sdk/lib/matrix.js";
import { LogService, noop } from "./logger.js";

interface MatrixDecryptIfNeededClient {
...
interface MatrixDecryptRetryState {
  event: MatrixEvent;
  roomId: string;
  eventId: string;
  attempts: number;
...
interface DecryptBridgeRawEvent {
  event_id: string;
}

interface MatrixCryptoRetrySignalSource {
...
export class MatrixDecryptBridge<TRawEvent extends DecryptBridgeRawEvent> {
  ...
  retryPendingNow(reason: string): void {
    const pending = [...this.decryptRetries.entries()];
    if (pending.length === 0) {
      return;
    }
  ...
  async drainPendingDecryptions(reason: string): Promise<void> {
    for (let attempts = 0; attempts < MATRIX_DECRYPT_RETRY_MAX_ATTEMPTS; attempts += 1) {
      ...
      const hasPendingRetryTimers = [...this.decryptRetries.values()].some(
