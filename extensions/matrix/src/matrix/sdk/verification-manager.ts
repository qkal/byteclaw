...
export interface MatrixShowSasCallbacks {
  sas: {
    decimal?: [number, number, number];
    emoji?: [string, string][];
  };
...
export interface MatrixShowQrCodeCallbacks {
  confirm: () => void;
  cancel: () => void;
}

export interface MatrixVerifierLike {
  verify: () => Promise<void>;
  cancel: (e: Error) => void;
...
export interface MatrixVerificationRequestLike {
  transactionId?: string;
  roomId?: string;
  initiatedByMe: boolean;
  otherUserId: string;
...
export interface MatrixVerificationCryptoApi {
...
interface MatrixVerificationSession {
  id: string;
...
export class MatrixVerificationManager {
  ...
  private pruneVerificationSessions(nowMs: number): void {
    ...
    const sortedByAge = [...this.verificationSessions.entries()].toSorted(
  ...
  private maybeAutoAcceptInboundRequest(session: MatrixVerificationSession): void {
    ...
    void request
      ...
      .catch((error) => {
        ...
        session.error = formatMatrixErrorMessage(error);
        this.touchVerificationSession(session);
      });
  }
  ...
  listVerifications(): MatrixVerificationSummary[] {
    this.pruneVerificationSessions(Date.now());
    const summaries = [...this.verificationSessions.values()].map((session) =>
