...
import { type HttpMethod, type QueryParams, createMatrixGuardedFetch } from "./sdk/transport.js";
import type {
...
export interface MatrixOwnDeviceVerificationStatus {
...
export interface MatrixRoomKeyBackupStatus {
...
export interface MatrixRoomKeyBackupRestoreResult {
  success: boolean;
  error?: string;
  backupVersion: string | null;
  imported: number;
...
export interface MatrixRoomKeyBackupResetResult {
  success: boolean;
  error?: string;
...
export interface MatrixOwnCrossSigningPublicationStatus {
  userId: string | null;
...
export interface MatrixVerificationBootstrapResult {
  success: boolean;
  error?: string;
...
export interface MatrixOwnDeviceInfo {
  deviceId: string;
...
export interface MatrixOwnDeviceDeleteResult {
...
export class MatrixClient {
  ...
  readonly dms = {
    ...
    update: async (): Promise<boolean> => await this.refreshDmCache(),
  };
  ...
  private async initializeCryptoIfNeeded(abortSignal?: AbortSignal): Promise<void> {
    ...
    try {
      ...
      LogService.warn("MatrixClientLite", "Failed to initialize rust crypto:", error);
    }
  }
  ...
  async downloadContent(
    ...
    try {
      ...
      if (!isUnsupportedAuthenticatedMediaEndpointError(error)) {
        throw error;
      }
    }
      ...
      if (shouldLoadBackupKey) {
        if (
          ...
          try {
            ...
            keyLoadError = formatMatrixErrorMessage(error);
          }
        } else {
          keyLoadError =
  ...
  async verifyWithRecoveryKey(
    ...
    try {
      ...
      return await fail(formatMatrixErrorMessage(error));
    }
  ...
  async bootstrapOwnDeviceVerification(params?: {
    ...
    try {
      ...
      this.recoveryKeyStore.discardStagedRecoveryKey();
      bootstrapError = formatMatrixErrorMessage(error);
    }
    ...
    if (uniqueDeviceIds.length > 0) {
      try {
        ...
        const session =
          ...
          typeof error.data.session === "string"
            ? error.data.session
            : null;
  ...
  private async shouldForceSecretStorageRecreationForBackupReset(
    ...
    try {
      ...
      return isRepairableSecretStorageAccessError(error);
    }
  }
  ...
  private async resolveRoomKeyBackupTrustState(
    ...
    return { matchesDecryptionKey, serverVersion, trusted };
  }
