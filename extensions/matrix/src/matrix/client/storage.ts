...
interface LegacyMoveRecord {
  sourcePath: string;
  targetPath: string;
  label: string;
}

interface StoredRootMetadata {
  homeserver?: string;
  userId?: string;
  accountId?: string;
...
function resolveLegacyStoragePaths(env: NodeJS.ProcessEnv = process.env): {
  ...
  return { cryptoPath: legacy.cryptoPath, storagePath: legacy.storagePath };
}
...
export async function maybeMigrateLegacyStorage(params: {
  ...
  try {
    ...
    throw new Error(
      rollbackError
        ? `Failed migrating legacy Matrix client storage: ${String(error)}. Rollback also failed: ${rollbackError}`
        : `Failed migrating legacy Matrix client storage: ${String(error)}`,
      { cause: error },
    );
  }
  if (moved.length > 0) {
...
function rollbackLegacyMoves(moved: LegacyMoveRecord[]): string | null {
  for (const entry of moved.toReversed()) {
    try {
      ...
      return `${entry.label} (${entry.targetPath} -> ${entry.sourcePath}): ${String(error)}`;
    }
  }
  return null;
}
