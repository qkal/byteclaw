...
describe("Matrix auth/config live surfaces", () => {
    ...
    const cfg = {
      channels: {
        matrix: {
          accounts: {
            ops: {
              accessToken: { id: "MATRIX_OPS_ACCESS_TOKEN", provider: "default", source: "env" },
      ...
      channels: {
        matrix: {
          accounts: {
            ops: {
              homeserver: "https://ops.example.org",
              password: { id: "MATRIX_OPS_PASSWORD", provider: "default", source: "env" },
            },
          },
        },
      },
  ...
  it("does not bypass env provider allowlists during startup fallback", () => {
    const cfg = {
      channels: {
        matrix: {
          accessToken: { id: "MATRIX_ACCESS_TOKEN", provider: "matrix-env", source: "env" },
          homeserver: "https://cfg.example.org",
        },
      },
      secrets: {
  ...
  it("rejects explicit non-default account ids that are neither configured nor scoped in env", () => {
    ...
    expect(() =>
      resolveMatrixAuthContext({ accountId: "typo", cfg, env: {} as NodeJS.ProcessEnv }),
  ...
  it("allows explicit non-default account ids backed only by scoped env vars", () => {
    ...
    expect(resolveMatrixAuthContext({ accountId: "ops", cfg, env }).accountId).toBe("ops");
  });
