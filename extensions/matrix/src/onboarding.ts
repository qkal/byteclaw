...
const matrixInviteAutoJoinOptions: {
  ...
  { label: "Always (join every invite)", value: "always" },
  { label: "Off (do not auto-join invites)", value: "off" },
];
...
async function promptMatrixAllowFrom(params: {
  ...
  const existingConfig = resolveMatrixAccountConfig({ accountId, cfg });
  const existingAllowFrom = existingConfig.dm?.allowFrom ?? [];
  const account = resolveMatrixAccount({ accountId, cfg });
...
async function configureMatrixInviteAutoJoin(params: {
  ...
  const existingConfig = resolveMatrixAccountConfig({ accountId, cfg: params.cfg });
    ...
    if (accessConfig.policy !== "allowlist") {
      ...
      if (accessConfig.entries.length > 0) {
        try {
          ...
          await params.prompter.note(
            `Room lookup failed; keeping entries as typed. ${String(error)}`,
            "Matrix rooms",
          );
        }
      }
...
async function runMatrixConfigure(params: {
  ...
  if (params.intent === "add-account") {
    ...
    next = updateMatrixAccountConfig(next, accountId, { enabled: true, name: enteredName });
  } else {
  ...
  const existing = resolveMatrixAccountConfig({ accountId, cfg: next });
  const account = resolveMatrixAccount({ accountId, cfg: next });
  ...
  if (!hasConfiguredSecretInput(accessToken) && !hasConfiguredSecretInput(password)) {
    const authMode = await params.prompter.select({
      ...
      options: [
        { label: "Access token (user ID fetched automatically)", value: "token" },
        { label: "Password (requires user ID)", value: "password" },
      ],
    });
