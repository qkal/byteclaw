...
import { type MatrixProfileUpdateResult, applyMatrixProfileUpdate } from "./profile-update.js";
import { type ChannelSetupInput, formatZonedTimestamp, normalizeAccountId } from "./runtime-api.js";
...
function resolveMatrixCliAccountId(accountId?: string): string {
  ...
  return resolveMatrixAuthContext({ accountId, cfg }).accountId;
}
...
interface MatrixCliAccountAddResult {
  accountId: string;
  configPath: string;
  useEnv: boolean;
  deviceHealth: {
...
async function addMatrixAccount(params: {
  ...
  const accountConfig = resolveMatrixAccountConfig({ accountId, cfg: updated });
...
async function repairMatrixDirectRoom(params: {
  ...
  const cfg = getMatrixRuntime().config.loadConfig() as CoreConfig;
  const account = resolveMatrixAccount({ accountId: params.accountId, cfg });
...
interface MatrixCliCommandConfig<TResult> {
  verbose: boolean;
  json: boolean;
  run: () => Promise<TResult>;
...
interface MatrixCliBackupStatus {
...
interface MatrixCliVerificationStatus {
...
interface MatrixCliDirectRoomCandidate {
  roomId: string;
...
interface MatrixCliDirectRoomInspection {
  accountId: string;
  remoteUserId: string;
    ...
    .action(
      async (options: { account?: string; yes?: boolean; verbose?: boolean; json?: boolean }) => {
        ...
        await runMatrixCliCommand({
          ...
          json: options.json === true,
          onJsonError: (message) => ({ error: message, success: false }),
          onText: (result, verbose) => {
            printAccountLabel(accountId);
