...
describe("convertTableToFlexBubble", () => {
  it("replaces empty cells with placeholders", () => {
    ...
    const body = bubble.body as {
      contents: { contents?: { contents?: Array<{ text: string }> }[] }[];
    };
    const rowsBox = body.contents[2] as { contents: { contents: { text: string }[] }[] };
  ...
  it("strips bold markers and applies weight for fully bold cells", () => {
    ...
    const bubble = convertTableToFlexBubble(table);
    const body = bubble.body as {
      contents: { contents?: { text: string; weight?: string }[] }[];
    };
    const headerRow = body.contents[0] as { contents: { text: string; weight?: string }[] };
    const dataRow = body.contents[2] as { contents: { text: string; weight?: string }[] };
...
describe("convertCodeBlockToFlexBubble", () => {
  it("creates a code card with language label", () => {
    const block = { code: "const x = 1;", language: "typescript" };
  ...
  it("truncates very long code", () => {
    ...
    const body = bubble.body as { contents: { contents: { text: string }[] }[] };
