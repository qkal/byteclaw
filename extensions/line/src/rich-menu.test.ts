...
describe("messageAction", () => {
  it("creates message actions with explicit or default text", () => {
    const cases = [
      { expectedText: "/help", label: "Help", name: "explicit text", text: "/help" },
      { expectedText: "Click", label: "Click", name: "defaults to label", text: undefined },
    ] as const;
...
describe("datetimePickerAction", () => {
  it("creates picker actions for all supported modes", () => {
    const cases = [
      { data: "date_picked", label: "Pick date", mode: "date" as const },
      { data: "time_picked", label: "Pick time", mode: "time" as const },
      { data: "datetime_picked", label: "Pick datetime", mode: "datetime" as const },
    ];
    for (const testCase of cases) {
...
describe("createGridLayout", () => {
  ...
  it("computes expected 2x3 layout for supported menu heights", () => {
    ...
    const cases = [
      { firstRowY: 0, height: 1686, rowHeight: 843, secondRowY: 843 },
      { firstRowY: 0, height: 843, rowHeight: 421, secondRowY: 421 },
    ] as const;
    for (const testCase of cases) {
