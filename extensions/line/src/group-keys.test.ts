...
describe("account-scoped LINE groups", () => {
  it("resolves the effective account-scoped groups map", () => {
    ...
    expect(resolveExactLineGroupConfigKey({ accountId: "work", cfg, groupId: "g1" })).toBe(
      "group:g1",
    );
    expect(resolveExactLineGroupConfigKey({ accountId: "default", cfg, groupId: "g1" })).toBe(
      undefined,
    );
  });
});
...
describe("line group policy", () => {
  ...
  it("uses account-scoped prefixed LINE group config for requireMention", () => {
    ...
    expect(resolveLineGroupRequireMention({ accountId: "work", cfg, groupId: "g123" })).toBe(false);
  });
});
