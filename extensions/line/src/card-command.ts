...
function parseActions(actionsStr: string | undefined): CardAction[] {
  ...
  for (const part of actionsStr.split(",")) {
    ...
    if (actionData.startsWith("http://") || actionData.startsWith("https://")) {
      results.push({
        action: { label: label.slice(0, 20), type: "uri", uri: actionData },
        label,
      });
      ...
      results.push({
        action: { label: label.slice(0, 20), text: actionData, type: "message" },
        label,
      });
    }
  }
 ...
 */
function parseReceiptItems(itemsStr: string): { name: string; value: string }[] {
  return itemsStr
    .split(",")
    .map((part) => {
      ...
      try {
        switch (type) {
          ...
          case "receipt": {
            ...
            const bubble = createReceiptCard({ footer, items, title, total });
            return buildLineReply({
              flexMessage: {
        ...
        return { text: `Error creating card: ${String(error)}` };
      }
    },
    name: "card",
    requireAuth: false,
