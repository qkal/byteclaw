...
import { type RuntimeEnv, danger, logVerbose } from "openclaw/plugin-sdk/runtime-env";
...
export function createLineWebhookMiddleware(
  ...
  return async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
    try {
      ...
      if (Buffer.byteLength(rawBody, "utf8") > LINE_WEBHOOK_MAX_RAW_BODY_BYTES) {
      ...
      runtime?.error?.(danger(`line webhook error: ${String(error)}`));
      if (!res.headersSent) {
...
export function startLineWebhook(options: StartLineWebhookOptions): {
  ...
  return { handler: middleware, path };
}
