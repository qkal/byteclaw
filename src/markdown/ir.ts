...
function createTextToken(base: MarkdownToken, content: string): MarkdownToken {
  return { ...base, children: undefined, content, type: "text" };
}
...
function openStyle(state: RenderState, style: MarkdownStyle) {
  const target = resolveRenderTarget(state);
  target.openStyles.push({ start: target.text.length, style });
}
...
function closeStyle(state: RenderState, style: MarkdownStyle) {
  ...
  for (let i = target.openStyles.length - 1; i >= 0; i -= 1) {
    if (target.openStyles[i]?.style === style) {
      ...
      if (end > start) {
        target.styles.push({ end, start, style });
      }
      return;
    }
  }
...
function renderInlineCode(state: RenderState, content: string) {
  ...
  target.text += content;
  target.styles.push({ end: start + content.length, start, style: "code" });
}
...
function renderCodeBlock(state: RenderState, content: string) {
  ...
  const start = target.text.length;
  target.text += code;
  target.styles.push({ end: start + code.length, start, style: "code_block" });
  if (state.env.listStack.length === 0) {
    target.text += "\n";
  }
}
...
function handleLinkClose(state: RenderState) {
  ...
  const end = target.text.length;
  if (end <= start) {
    target.links.push({ end, href, start });
    return;
  }
  target.links.push({ end, href, start });
}
...
function trimCell(cell: TableCell): TableCell {
  ...
  for (const span of cell.styles) {
    ...
    if (sliceEnd > sliceStart) {
      trimmedStyles.push({ end: sliceEnd, start: sliceStart, style: span.style });
    }
  }
  ...
  for (const span of cell.links) {
    ...
    const sliceEnd = Math.min(trimmedLength, span.end - start);
    if (sliceEnd > sliceStart) {
      trimmedLinks.push({ end: sliceEnd, href: span.href, start: sliceStart });
    }
  }
  return { links: trimmedLinks, styles: trimmedStyles, text: trimmedText };
}
    ...
    for (const row of rows) {
      ...
      if (rowLabel?.text) {
        ...
        if (labelEnd > labelStart) {
          state.styles.push({ end: labelEnd, start: labelStart, style: "bold" });
        }
        state.text += "\n";
      }
...
function renderTableAsCode(state: RenderState) {
  ...
  if (codeEnd > codeStart) {
    state.styles.push({ end: codeEnd, start: codeStart, style: "code_block" });
  }
  if (state.env.listStack.length === 0) {
    state.text += "\n";
  }
  ...
  for (const token of tokens) {
    switch (token.type) {
      ...
      case "bullet_list_open": {
        ...
        state.env.listStack.push({ index: 0, type: "bullet" });
        break;
      }
      ...
      case "ordered_list_open": {
        ...
        state.env.listStack.push({ index: start - 1, type: "ordered" });
        break;
      }
...
function clampStyleSpans(spans: MarkdownStyleSpan[], maxLength: number): MarkdownStyleSpan[] {
  ...
  for (const span of spans) {
    ...
    if (end > start) {
      clamped.push({ end, start, style: span.style });
    }
  }
  return clamped;
}
...
function clampLinkSpans(spans: MarkdownLinkSpan[], maxLength: number): MarkdownLinkSpan[] {
  ...
  for (const span of spans) {
    ...
    const end = Math.max(start, Math.min(span.end, maxLength));
    if (end > start) {
      clamped.push({ end, href: span.href, start });
    }
  }
  return clamped;
}
...
function resolveSliceBounds(
  ...
  return { end: sliceEnd, start: sliceStart };
}
