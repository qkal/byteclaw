...
export interface InlineCodeState {
  open: boolean;
  ticks: number;
}
...
interface InlineCodeSpansResult {
  spans: [number, number][];
  state: InlineCodeState;
}

export interface CodeSpanIndex {
...
function isInsideInlineSpan(index: number, spans: [number, number][]): boolean {
