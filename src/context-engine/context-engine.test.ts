...
function makeMockMessage(role: "user" | "assistant" = "user", text = "hello"): AgentMessage {
  return { content: text, role, timestamp: Date.now() } as AgentMessage;
}
...
function registerPromptTrackingEngine(engineId: string) {
  ...
  registerContextEngine(engineId, () => ({
    async assemble(params) {
      calls.push({ ...params });
      return { estimatedTokens: 0, messages: params.messages };
    },
    async compact() {
      return { compacted: false, ok: true };
    },
...
class LegacySessionKeyStrictEngine implements ContextEngine {
  ...
  readonly ingestCalls: Record<string, unknown>[] = [];
  readonly assembleCalls: Record<string, unknown>[] = [];
  readonly compactCalls: Record<string, unknown>[] = [];
  readonly maintainCalls: Record<string, unknown>[] = [];
    ...
    registerContextEngine("test-engine", () => {
      const engine: ContextEngine = {
        async assemble({ messages }) {
          return { estimatedTokens: 0, messages };
        },
        async compact() {
          return { compacted: false, ok: true };
        },
