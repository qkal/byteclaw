...
interface RegisterContextEngineForOwnerOptions {
...
interface SessionKeyCompatParams {
  sessionKey?: string;
  prompt?: string;
}
...
interface ContextEngineRegistryState {
  engines: Map<
    string,
    {
...
export function registerContextEngineForOwner(
  ...
  if (
    ...
    return { existingOwner: CORE_CONTEXT_ENGINE_OWNER, ok: false };
  }
  if (existing && existing.owner !== normalizedOwner) {
    return { existingOwner: existing.owner, ok: false };
  }
