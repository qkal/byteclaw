...
export type NodeListConnectedFn = () => { nodeId: string }[];

export interface NodeSubscriptionManager {
...
export function createNodeSubscriptionManager(): NodeSubscriptionManager {
  ...
  const sendToSession = (
    ...
    for (const nodeId of subs) {
      sendEvent({ event, nodeId, payloadJSON });
    }
  };
  ...
  const sendToAllConnected = (
    ...
    for (const node of listConnected()) {
      sendEvent({ event, nodeId: node.nodeId, payloadJSON });
    }
  };
