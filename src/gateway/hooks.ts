...
export interface HookAgentPolicyResolved {
  defaultAgentId: string;
  knownAgentIds: Set<string>;
  allowedAgentIds?: Set<string>;
}

export interface HookSessionPolicyResolved {
...
export async function readJsonBody(
  ...
  const result = await readJsonBodyWithLimit(req, { emptyObjectOnEmpty: true, maxBytes });
  if (result.ok) {
    return result;
  }
  if (result.code === "PAYLOAD_TOO_LARGE") {
    return { error: "payload too large", ok: false };
  }
  if (result.code === "REQUEST_BODY_TIMEOUT") {
    return { error: "request body timeout", ok: false };
  }
  if (result.code === "CONNECTION_CLOSED") {
    return { error: requestBodyErrorToText("CONNECTION_CLOSED"), ok: false };
  }
...
export function normalizeWakePayload(
  ...
  return { ok: true, value: { mode, text: normalizedText } };
}
...
export function resolveHookSessionKey(params: {
  ...
  if (requested) {
    if (params.source === "request" && !params.hooksConfig.sessionPolicy.allowRequestSessionKey) {
      return { error: getHookSessionKeyRequestPolicyError(), ok: false };
    }
    const allowedPrefixes = params.hooksConfig.sessionPolicy.allowedSessionKeyPrefixes;
    if (allowedPrefixes && !isSessionKeyAllowedByPrefix(requested, allowedPrefixes)) {
      return { error: getHookSessionKeyPrefixError(allowedPrefixes), ok: false };
    }
...
export function normalizeAgentPayload(payload: Record<string, unknown>):
  ...
  if (!channel) {
    return { error: getHookChannelError(), ok: false };
  }
  ...
  if (modelRaw !== undefined && !model) {
    return { error: "model required", ok: false };
  }
