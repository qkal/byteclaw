...
describe("startGatewayMemoryBackend", () => {
  ...
  it("initializes qmd backend for each configured agent", async () => {
    const cfg = createQmdConfig({ list: [{ default: true, id: "ops" }, { id: "main" }] });
    ...
    expect(getMemorySearchManagerMock).toHaveBeenNthCalledWith(1, { agentId: "ops", cfg });
    expect(getMemorySearchManagerMock).toHaveBeenNthCalledWith(2, { agentId: "main", cfg });
  ...
  it("logs a warning when qmd manager init fails and continues with other agents", async () => {
    const cfg = createQmdConfig({ list: [{ default: true, id: "main" }, { id: "ops" }] });
    const log = createGatewayLogMock();
    getMemorySearchManagerMock
      .mockResolvedValueOnce({ error: "qmd missing", manager: null })
  ...
  it("skips agents with memory search disabled", async () => {
    ...
    expect(getMemorySearchManagerMock).toHaveBeenCalledWith({ agentId: "main", cfg });
    expect(log.info).toHaveBeenCalledWith(
      'qmd memory startup initialization armed for agent "main"',
    );
    expect(log.warn).not.toHaveBeenCalled();
