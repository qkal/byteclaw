...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    ...
    params.log.warn(`startup model warmup failed for ${provider}/${model}: ${String(error)}`);
  }
}
...
export async function startGatewaySidecars(params: {
  ...
  try {
    ...
    params.log.warn(`session lock cleanup failed on startup: ${String(error)}`);
  }
  ...
  try {
    ...
    params.logHooks.error(`failed to load hooks: ${String(error)}`);
  }
  ...
  if (!skipChannels) {
    try {
      ...
      params.logChannels.error(`channel startup failed: ${String(error)}`);
    }
  } else {
  ...
  try {
    ...
    params.log.warn(`plugin services failed to start: ${String(error)}`);
  }
  ...
  if (params.cfg.acp?.enabled) {
    void getAcpSessionManager()
      ...
      .catch((error) => {
        params.log.warn(`acp startup identity reconcile failed: ${String(error)}`);
      });
  }

  void startGatewayMemoryBackend({ cfg: params.cfg, log: params.log }).catch((error) => {
    params.log.warn(`qmd memory startup initialization failed: ${String(error)}`);
  });
...
export async function startGatewayPostAttachRuntime(params: {
  ...
  return { pluginServices, stopGatewayUpdateCheck, tailscaleCleanup };
}
