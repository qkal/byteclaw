...
const CHAT_RESPONSE_TIMEOUT_MS = 4000;
...
describe("gateway server chat", () => {
  ...
  const buildNoReplyHistoryFixture = (includeMixedAssistant = false) => [
    ...
    {
      content: [{ text: "real reply", type: "text" }],
      role: "assistant",
      timestamp: 3,
    },
    {
  ...
  const waitForAgentRunOk = async (runId: string, timeoutMs = 1000) => {
  ...
  test("chat.history hides commentary-only assistant entries", async () => {
    const historyMessages = await loadChatHistoryWithMessages([
      ...
      {
        content: [{ text: "thinking like caveman", type: "text" }],
        phase: "commentary",
        role: "assistant",
        timestamp: 2,
      },
  ...
  test("chat.history hides assistant announce/reply skip-only entries", async () => {
    const historyMessages = await loadChatHistoryWithMessages([
      {
        content: [{ text: "ANNOUNCE_SKIP", type: "text" }],
        role: "assistant",
        timestamp: 1,
      },
      {
        content: [{ text: "REPLY_SKIP", type: "text" }],
        role: "assistant",
        timestamp: 2,
      },
      {
  ...
  test("routes /btw replies through side-result events without transcript injection", async () => {
    await withMainSessionStore(async (dir) => {
      await fs.writeFile(
        ...
        `${JSON.stringify({
          message: {
            content: [{ text: "main thread context", type: "text" }],
            role: "user",
            timestamp: Date.now(),
          },
        })}\n`,
  ...
  test("routes block-streamed /btw replies through side-result events", async () => {
    await withMainSessionStore(async (dir) => {
      await fs.writeFile(
        ...
        `${JSON.stringify({
          message: {
            content: [{ text: "existing context", type: "text" }],
            role: "assistant",
            timestamp: Date.now(),
          },
        })}\n`,
    ...
    await withGatewayServer(async ({ port }) => {
      await withMainSessionStore(async () => {
        ...
        try {
          ...
          const raw = await fs.readFile(testState.sessionStorePath!, "utf8");
  ...
  test("agent.wait keeps lifecycle wait active while same-runId chat.send is active", async () => {
    await withMainSessionStore(async () => {
      ...
      try {
        ...
        emitAgentEvent({
          data: { endedAt: 2, phase: "end", startedAt: 1 },
          runId,
          stream: "lifecycle",
        });
  ...
  test("agent events include sessionKey and agent.wait covers lifecycle flows", async () => {
    ...
    try {
      ...
      {
        emitAgentEvent({
          data: { endedAt: 55, phase: "end", startedAt: 50 },
