...
describe("createHooksRequestHandler timeout status mapping", () => {
  ...
  test("returns 408 for request body timeout", async () => {
    readJsonBodyMock.mockResolvedValue({ error: "request body timeout", ok: false });
    const dispatchWakeHook = vi.fn();
    const dispatchAgentHook = vi.fn(() => "run-1");
    const handler = createHooksHandler({ dispatchAgentHook, dispatchWakeHook });
    ...
    expect(end).toHaveBeenCalledWith(JSON.stringify({ error: "request body timeout", ok: false }));
