...
import { type TailscaleWhoisIdentity, readTailscaleWhoisIdentity } from "../infra/tailscale.js";
...
export function resolveGatewayAuth(params: {
  ...
  const { password } = resolvedCredentials;
  const { trustedProxy } = authConfig;
...
async function authorizeGatewayConnectCore(
  ...
  if (auth.mode === "trusted-proxy") {
    ...
    if ("user" in result) {
      ...
      return { method: "trusted-proxy", ok: true, user: result.user };
    }
    return { ok: false, reason: result.reason };
  }

  if (auth.mode === "none") {
    return { method: "none", ok: true };
  }
  ...
  if (auth.mode === "password") {
    ...
    limiter?.reset(ip, rateLimitScope);
    return { method: "password", ok: true };
  }
