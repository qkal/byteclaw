...
function parseSseEvents(text: string): { event?: string; data: string }[] {
  ...
  for (const line of lines) {
    if (line.startsWith("event: ")) {
      ...
      events.push({ data: currentData.join("\n"), event: currentEvent });
...
function buildUrlInputMessage(params: {
  ...
  return [
    {
      content: [
        { text: params.text ?? "read this", type: "input_text" },
        {
...
describe("OpenResponses HTTP API (e2e)", () => {
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    const mockAgentOnce = (payloads: { text: string }[], meta?: unknown) => {
      agentCommand.mockClear();
      agentCommand.mockResolvedValueOnce({ meta, payloads } as never);
    };
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        body: JSON.stringify({ input: "hi", model: "openclaw" }),
      ...
      agentCommand.mockClear();
      const resInvalidModel = await postResponses(port, { input: "hi", model: "openai/" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { input: "hi", model: "openclaw/beta" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { input: "hi", model: "openclaw/default" });
      ...
      const resArray = await postResponses(port, {
        input: [{ content: "hello there", role: "user", type: "message" }],
        model: "openclaw",
      });
      ...
      const resSystemDeveloper = await postResponses(port, {
        input: [
          { content: "You are a helpful assistant.", role: "system", type: "message" },
          { content: "Be concise.", role: "developer", type: "message" },
      ...
      const resHistory = await postResponses(port, {
        input: [
          { content: "You are a helpful assistant.", role: "system", type: "message" },
          { content: "Hello, who are you?", role: "user", type: "message" },
          { content: "I am Claude.", role: "assistant", type: "message" },
          { content: "What did I just ask you?", role: "user", type: "message" },
        ],
        model: "openclaw",
      });
      ...
      const resFunctionOutput = await postResponses(port, {
        input: [
          { content: "What's the weather?", role: "user", type: "message" },
          { call_id: "call_1", output: "Sunny, 70F.", type: "function_call_output" },
        ],
        model: "openclaw",
      });
      ...
      const resToolChoice = await postResponses(port, {
        input: "hi",
        model: "openclaw",
        tool_choice: { function: { name: "get_time" }, type: "function" },
        tools: [
          {
            description: "Get weather",
            name: "get_weather",
      ...
      const clientTools =
        (
          optsToolChoice as
            | {
                clientTools?: { function?: { name?: string; strict?: boolean } }[];
              }
            | undefined
        )?.clientTools ?? [];
      ...
      const resUnknownTool = await postResponses(port, {
        input: "hi",
        model: "openclaw",
        tool_choice: { function: { name: "unknown_tool" }, type: "function" },
        tools: WEATHER_TOOL,
      });
      ...
      mockAgentOnce([{ text: "ok" }], {
        agentMeta: {
          usage: { cacheRead: 1, cacheWrite: 1, input: 3, output: 5 },
        },
      });
      ...
      const output = shapeJson.output as Record<string, unknown>[];
      ...
      const resNoUser = await postResponses(port, {
        input: [{ content: "yo", role: "system", type: "message" }],
        model: "openclaw",
      });
  ...
  it("treats write-scoped HTTP callers as non-owner and admin-scoped callers as owner", async () => {
    ...
    const streamingResponse = await postResponses(
      port,
      { input: "hi", model: "openclaw", stream: true },
      { "x-openclaw-scopes": "operator.admin, operator.write" },
    );
  ...
  it("preserves assistant text alongside non-stream function_call output", async () => {
    ...
    expect(
      ((json.output?.[0]?.content as Record<string, unknown>[] | undefined)?.[0]?.text as
        | string
        | undefined) ?? "",
  ...
  it("reuses the prior session when previous_response_id is provided", async () => {
    ...
    const secondResponse = await postResponses(port, {
      input: [{ call_id: "call_1", output: "Sunny, 70F.", type: "function_call_output" }],
      model: "openclaw",
  ...
  it("stores response session mappings when the response is emitted", async () => {
    ...
    let release: ((value: { payloads: { text: string }[] }) => void) | undefined;
    agentCommand.mockImplementationOnce(
      () =>
        new Promise<{ payloads: { text: string }[] }>((resolve) => {
          release = resolve;
        }) as never,
    );
