...
interface NodePendingWorkState {
  revision: number;
...
export function enqueueNodePendingWork(params: {
  ...
  if (existing) {
    return { deduped: true, item: existing, revision: state.revision };
  }
  const item: NodePendingWorkItem = {
    createdAtMs: nowMs,
    expiresAtMs:
      typeof params.expiresInMs === "number" && Number.isFinite(params.expiresInMs)
        ? nowMs + Math.max(1000, Math.trunc(params.expiresInMs))
        : null,
    id: randomUUID(),
  ...
  state.revision += 1;
  return { deduped: false, item, revision: state.revision };
}

export function drainNodePendingWork(nodeId: string, opts: DrainOptions = {}): DrainResult {
  const normalizedNodeId = nodeId.trim();
  if (!normalizedNodeId) {
    return { hasMore: false, items: [], revision: 0 };
  }
...
export function acknowledgeNodePendingWork(params: { nodeId: string; itemIds: string[] }): {
  ...
  const nodeId = params.nodeId.trim();
  if (!nodeId) {
    return { removedItemIds: [], revision: 0 };
  }
  ...
  return { removedItemIds, revision: state.revision };
}
