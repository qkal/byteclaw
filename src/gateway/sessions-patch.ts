...
function invalid(message: string): { ok: false; error: ErrorShape } {
  return { error: errorShape(ErrorCodes.INVALID_REQUEST, message), ok: false };
}
...
export async function applySessionsPatchToStore(params: {
  ...
  const resolvedDefault = resolveDefaultModelForAgent({ agentId: sessionAgentId, cfg });
  const subagentModelHint = isSubagentSessionKey(storeKey)
    ? resolveSubagentConfiguredModelSelection({ agentId: sessionAgentId, cfg })
    : undefined;
