...
describe("diffConfigPaths", () => {
  ...
  it("does not report unchanged arrays of objects as changed", () => {
    const prev = {
      memory: {
        qmd: {
          paths: [{ name: "docs", path: "~/docs", pattern: "**/*.md" }],
          scope: {
            rules: [{ include: ["docs"], when: { channel: "slack" } }],
          },
        },
      },
    };
  ...
  it("reports changed arrays of objects", () => {
    ...
    const next = {
      memory: {
        qmd: {
          paths: [{ name: "docs", path: "~/docs", pattern: "**/*.txt" }],
        },
      },
    };
  ...
  it("collapses changed agents.list heartbeat entries to agents.list", () => {
    const prev = {
      agents: {
        list: [{ heartbeat: { every: "5m", lightContext: false }, id: "ops" }],
      },
    };
    const next = {
      agents: {
        list: [{ heartbeat: { every: "5m", lightContext: true }, id: "ops" }],
      },
    };
...
describe("startGatewayConfigReloader", () => {
  ...
  it("retries missing snapshots and reloads once config file reappears", async () => {
    const readSnapshot = vi
      ...
      .mockResolvedValueOnce(makeSnapshot({ exists: false, hash: "missing-1", raw: null }))
  ...
  it("caps missing-file retries and skips reload after retry budget is exhausted", async () => {
    const readSnapshot = vi
      .fn<() => Promise<ConfigFileSnapshot>>()
      .mockResolvedValue(makeSnapshot({ exists: false, hash: "missing", raw: null }));
    const { watcher, onHotReload, onRestart, log, reloader } = createReloaderHarness(readSnapshot);
  ...
  it("contains restart callback failures and retries on subsequent changes", async () => {
    const readSnapshot = vi
      ...
      .mockResolvedValueOnce(
        makeSnapshot({
          config: {
            gateway: { port: 18_790, reload: { debounceMs: 0 } },
          },
          hash: "restart-1",
        }),
      )
      .mockResolvedValueOnce(
        makeSnapshot({
          config: {
            gateway: { port: 18_791, reload: { debounceMs: 0 } },
          },
          hash: "restart-2",
        }),
      );
  ...
  it("reuses in-process write notifications and dedupes watcher rereads by persisted hash", async () => {
    const readSnapshot = vi
      ...
      .mockResolvedValueOnce(
        makeSnapshot({
          config: {
            gateway: { port: 19_001, reload: { debounceMs: 0 } },
          },
          hash: "external-1",
          runtimeConfig: {
            gateway: { port: 19_001, reload: { debounceMs: 0 } },
  ...
  it("dedupes the first watcher reread for startup internal writes", async () => {
    const readSnapshot = vi
      ...
      .mockResolvedValueOnce(
        makeSnapshot({
          config: {
            gateway: { auth: { mode: "token", token: "startup" }, reload: { debounceMs: 0 } },
          },
