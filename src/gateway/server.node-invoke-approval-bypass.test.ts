...
const NODE_CONNECT_TIMEOUT_MS = 3000;
const CONNECT_REQ_TIMEOUT_MS = 2000;
...
async function getConnectedNodeId(ws: WebSocket): Promise<string> {
  const nodes = await rpcReq<{ nodes?: { nodeId: string; connected?: boolean }[] }>(
    ws,
    "node.list",
    {},
  );
...
async function requestAllowOnceApproval(
  ...
  await rpcReq(ws, "exec.approval.resolve", { decision: "allow-once", id: approvalId });
...
describe("node.invoke approval bypass", () => {
  ...
  const connectOperator = async (scopes: string[]) => await connectOperatorWithRetry(scopes);
  ...
  const connectLinuxNode = async (
    ...
    const client = new GatewayClient({
      ...
      connectChallengeTimeoutMs: 2000,
      token: "secret",
      role: "node",
      ...
      const cases = [
        ...
        },
        {
          ...
          payload: {
            ...
            idempotencyKey: crypto.randomUUID(),
            nodeId,
            params: { baseHash: "nope", file: { agents: {}, version: 1 } },
          },
        },
      ] as const;
