...
interface DoctorMemoryDreamingPhasePayload {
  enabled: boolean;
  cron: string;
...
interface DoctorMemoryDreamingEntryPayload {
  key: string;
  path: string;
  startLine: number;
  endLine: number;
...
interface DoctorMemoryDreamingPayload {
  enabled: boolean;
  timezone?: string;
...
export interface DoctorMemoryStatusPayload {
  agentId: string;
  provider?: string;
  embedding: {
    ok: boolean;
...
export interface DoctorMemoryDreamDiaryPayload {
  agentId: string;
  found: boolean;
  path: string;
  content?: string;
...
export interface DoctorMemoryDreamActionPayload {
  agentId: string;
...
async function loadDreamingStoreStats(
  ...
  try {
    ...
    try {
      const phaseRaw = await fs.readFile(phaseSignalPath, "utf8");
      ...
      if (code !== "ENOENT") {
        phaseSignalError = formatError(error);
      }
    }
...
interface ManagedDreamingCronStatus {
  managedCronPresent: boolean;
  nextRunAtMs?: number;
}
  ...
  "doctor.memory.status": async ({ respond, context }) => {
    ...
    try {
      ...
      if (!embedding.ok && !embedding.error) {
        embedding = { error: "memory embeddings unavailable", ok: false };
      }
      ...
      const payload: DoctorMemoryStatusPayload = {
        ...
        embedding: {
          ok: false,
          error: `gateway memory probe failed: ${formatError(error)}`,
        },
      };
      respond(true, payload, undefined);
    } finally {
