...
function normalizeChoice(choice: CommandArgChoice): { value: string; label: string } {
  return typeof choice === "string" ? { label: choice, value: choice } : choice;
}
...
export function buildCommandsListResult(params: {
  ...
  const skillCommands = listSkillCommandsForAgents({ agentIds: [params.agentId], cfg: params.cfg });
  ...
  commands.push(...buildPluginCommandEntries({ nameSurface, provider }));
