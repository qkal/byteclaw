...
function resolveChatSendOriginatingRoute(params: {
  ...
  if (
    params.explicitOrigin?.originatingChannel &&
    params.explicitOrigin.originatingTo
  ) {
    return {
      originatingChannel: params.explicitOrigin.originatingChannel,
      originatingTo: params.explicitOrigin.originatingTo,
      ...(params.explicitOrigin.accountId
        ? { accountId: params.explicitOrigin.accountId }
        : {}),
  ...
  const normalizedSessionScopeHead =
    normalizeLowercaseStringOrEmpty(sessionScopeHead);
  ...
  const hasClientMetadata =
    (typeof params.client?.mode === 'string' &&
      params.client.mode.trim().length > 0) ||
    (typeof params.client?.id === 'string' &&
      params.client.id.trim().length > 0);
...
function canInjectSystemProvenance(
  client: GatewayRequestHandlerOptions['client'],
): boolean {
...
function resolveChatSendTranscriptMediaFields(savedImages: SavedMedia[]) {
  ...
  const mediaTypes = savedImages.map(
...
async function rewriteChatSendUserTurnMediaPaths(params: {
  ...
  const target = [...branch].toReversed().find((entry) => {
    ...
    const existingPaths = Array.isArray(
      (entry.message as { MediaPaths?: unknown }).MediaPaths,
    )
      ? (entry.message as { MediaPaths?: unknown[] }).MediaPaths
      : undefined;
    if (
      (typeof (entry.message as { MediaPath?: unknown }).MediaPath ===
        'string' &&
...
function sanitizeChatHistoryMessage(
  ...
  if (typeof entry.content === 'string') {
    ...
    const updated = entry.content.map((block) =>
      sanitizeChatHistoryContentBlock(block, maxChars),
    );
    ...
    if (hasPhaseMetadata) {
      const stripped = stripInlineDirectiveTagsForDisplay(
        extractAssistantHistoryText(entry) ?? '',
      );
      const res = truncateChatHistoryText(stripped.text, maxChars);
...
function normalizeExplicitChatSendOrigin(
  ...
  if (!normalizedChannel) {
    return {
      error:
        'originatingChannel is required when using originating route fields',
      ok: false,
    };
  }
  if (!originatingTo) {
...
function abortChatRunsForSessionKeyWithPartials(params: {
  ...
  const { matchedSessionRuns, authorizedRunIds } =
...
function broadcastSideResult(params: {
  ...
  params.context.nodeSendToSession(
    params.payload.sessionKey,
    'chat.side_result',
    {
      ...params.payload,
      seq,
    ...
    try {
      ...
      void dispatchInboundMessage({
        ...
        replyOptions: {
          ...
          imageOrder:
            parsedImageOrder.length > 0 ? parsedImageOrder : undefined,
          ...
          if (!agentRunStarted) {
            ...
            if (btwReplies.length > 0 && btwText) {
              ...
              const combinedReply = finalPayloads
                ...
                .trim();
              const audioBlocks =
                buildWebchatAudioContentBlocksFromReplyPayloads(finalPayloads);
              ...
              if (assistantContent.length > 0) {
                ...
                const transcriptFallbackText =
                  combinedReply ||
                  (audioBlocks.length > 0 ? 'Audio reply' : '');
                const appended = appendAssistantTranscriptMessage({
                  agentId,
      ...
      const errorResponse = errorShape(ErrorCodes.UNAVAILABLE, String(error));
      const payload = {
        runId: clientRunId,
        status: 'error' as const,
        summary: String(error),
      ...
      respond(false, payload, errorResponse, {
        runId: clientRunId,
