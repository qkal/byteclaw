...
function resolveAgentWorkspaceFileOrRespondError(
  ...
  return { agentId, cfg, name, workspaceDir };
}
...
function resolveNotFoundWorkspaceFilePathResult(params: {
  ...
  return { kind: "invalid", reason: "file not found", requestPath: params.requestPath };
}
...
async function resolveAgentWorkspaceFilePath(params: {
  ...
  if (candidateLstat.isSymbolicLink()) {
    ...
    if (!targetStat.isFile()) {
      return { kind: "invalid", reason: "path is not a regular file", requestPath };
    }
    if (targetStat.nlink > 1) {
      return { kind: "invalid", reason: "hardlinked file path not allowed", requestPath };
    }
    return { ioPath: targetReal, kind: "ready", requestPath, workspaceReal };
  }
...
async function listAgentFiles(workspaceDir: string, options?: { hideBootstrap?: boolean }) {
  ...
  for (const name of bootstrapFileNames) {
    ...
    if (meta) {
      ...
      files.push({ missing: true, name, path: filePath });
    }
  }
...
function respondWorkspaceFileMissing(params: {
  ...
  params.respond(
    true,
    {
      ...
      file: { missing: true, name: params.name, path: params.filePath },
...
export const agentsHandlers: GatewayRequestHandlers = {
  "agents.create": async ({ params, respond }) => {
    ...
    nextConfig = applyAgentConfig(nextConfig, { agentDir, agentId });
    ...
    respond(true, { agentId, model, name: safeName, ok: true, workspace: workspaceDir }, undefined);
  },
  "agents.delete": async ({ params, respond }) => {
    ...
    respond(true, { agentId, ok: true, removedBindings: result.removedBindings }, undefined);
  },
  "agents.files.get": async ({ params, respond }) => {
    ...
    if (resolvedPath.kind === "missing") {
      respondWorkspaceFileMissing({ agentId, filePath, name, respond, workspaceDir });
      return;
    }
  ...
  "agents.files.list": async ({ params, respond }) => {
    ...
    respond(true, { agentId, files, workspace: workspaceDir }, undefined);
  },
  ...
  "agents.update": async ({ params, respond }) => {
    ...
    respond(true, { agentId, ok: true }, undefined);
  },
};
