...
async function invokeSecureGatewayRoute(params: {
  ...
  return { exactPluginHandler, handled, prefixGatewayHandler };
}
...
describe("createGatewayPluginRequestHandler", () => {
  ...
  it("handles exact route matches", async () => {
    ...
    const handler = createGatewayPluginRequestHandler({
      log: createPluginLog(),
      registry: createTestRegistry({
        httpRoutes: [createRoute({ handler: routeHandler, path: "/demo" })],
      }),
    });
  ...
  it("prefers exact matches before prefix matches", async () => {
    ...
    const handler = createGatewayPluginRequestHandler({
      ...
      registry: createTestRegistry({
        httpRoutes: [
          createRoute({ handler: prefixHandler, match: "prefix", path: "/api" }),
          createRoute({ handler: exactHandler, match: "exact", path: "/api/demo" }),
        ],
      }),
    });
  ...
  it("supports route fallthrough when handler returns false", async () => {
    ...
    const handler = createGatewayPluginRequestHandler({
      ...
      registry: createTestRegistry({
        httpRoutes: [
          createRoute({ handler: first, match: "exact", path: "/hook" }),
          createRoute({ handler: second, match: "prefix", path: "/hook" }),
        ],
      }),
    });
  ...
  it("matches canonicalized route variants", async () => {
    ...
    const handler = createGatewayPluginRequestHandler({
      log: createPluginLog(),
      registry: createTestRegistry({
        httpRoutes: [createRoute({ handler: routeHandler, path: "/api/demo" })],
      }),
    });
  ...
  it("falls back to the provided registry when the pinned route registry is empty", async () => {
    ...
    const explicitRegistry = createTestRegistry({
      httpRoutes: [createRoute({ auth: "plugin", handler: explicitRouteHandler, path: "/demo" })],
    });
  ...
  it("prefers the pinned route registry over a stale explicit registry", async () => {
    ...
    const staleExplicitRegistry = createTestRegistry({
      httpRoutes: [createRoute({ auth: "plugin", path: "/plugins/diffs" })],
    });
    const routeHandler = vi.fn(async (_req, res: ServerResponse) => {
      res.statusCode = 204;
      return true;
...
describe("plugin HTTP route auth checks", () => {
  ...
  it("enforces auth for protected and gateway-auth routes", () => {
    const registry = createTestRegistry({
      httpRoutes: [
        createRoute({ auth: "plugin", match: "prefix", path: "/googlechat" }),
        createRoute({ auth: "gateway", path: "/api/demo" }),
      ],
    });
  ...
  it("enforces auth when any overlapping matched route requires gateway auth", () => {
    const registry = createTestRegistry({
      httpRoutes: [
        createRoute({ auth: "plugin", match: "exact", path: "/plugin/secure/report" }),
        createRoute({ auth: "gateway", match: "prefix", path: "/plugin/secure" }),
      ],
    });
