...
import { type InternalHookEvent, isSessionPatchEvent } from "../hooks/internal-hooks.js";
...
describe("gateway server sessions", () => {
  ...
  test("sessions.create stores dashboard session model and parent linkage, and creates a transcript", async () => {
    ...
    const rawStore = JSON.parse(await fs.readFile(storePath, "utf8")) as Record<
      string,
      {
        sessionId?: string;
        label?: string;
  ...
  test("sessions.list surfaces transcript usage and model fallbacks from the transcript", async () => {
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({ id: "sess-parent", type: "session", version: 1 })}\n`,
      "utf8",
    );
    await fs.writeFile(
  ...
  test("sessions.changed mutation events include live usage metadata", async () => {
    ...
    expect(respond).toHaveBeenCalledWith(
      true,
      expect.objectContaining({ key: "agent:main:main", ok: true }),
      undefined,
    );
  ...
  test("sessions.changed mutation events include subagent ownership metadata", async () => {
    ...
    expect(respond).toHaveBeenCalledWith(
      true,
      expect.objectContaining({ key: "agent:main:subagent:child", ok: true }),
      undefined,
    );
    expect(broadcastToConnIds).toHaveBeenCalledWith(
      "sessions.changed",
  ...
  test("lists and patches session store via sessions.* RPC", async () => {
    ...
    await fs.writeFile(
      ...
      `${Array.from({ length: 10 })
        .map((_, idx) => JSON.stringify({ content: `line ${idx}`, role: "user" }))
        .join("\n")}\n`,
      "utf8",
    );
    await fs.writeFile(
      path.join(dir, "sess-group.jsonl"),
      `${JSON.stringify({ content: "group line 0", role: "user" })}\n`,
      "utf8",
    );
    ...
    const listAfterModelPatch = await rpcReq<{
      sessions: { key: string; modelProvider?: string; model?: string }[];
    }>(ws, "sessions.list", {});
    ...
    const compactedLines = (await fs.readFile(path.join(dir, "sess-main.jsonl"), "utf8"))
      .split(/\r?\n/)
    ...
    const storeAfterReset = JSON.parse(await fs.readFile(storePath, "utf8")) as Record<
      string,
  ...
  test("sessions.compaction.* lists checkpoints and branches or restores from pre-compaction snapshots", async () => {
    ...
    const listedCheckpoints = await rpcReq<{
      ok: true;
      key: string;
      checkpoints: { checkpointId: string; summary?: string; tokensBefore?: number }[];
    ...
    const storeAfterBranch = JSON.parse(await fs.readFile(storePath, "utf8")) as Record<
      string,
      {
        parentSessionKey?: string;
    ...
    const storeAfterRestore = JSON.parse(await fs.readFile(storePath, "utf8")) as Record<
      string,
    ...
    await withEnvAsync({ OPENCLAW_CONFIG_PATH: undefined }, async () => {
      ...
      const cfg = {
        agents: {
          ...
          list: [{ default: true, id: "main", workspace: dir }],
        },
  ...
  test("sessions.preview resolves legacy mixed-case main alias with custom mainKey", async () => {
    const { dir, storePath } = await createSessionStoreDir();
    testState.agentsConfig = { list: [{ default: true, id: "ops" }] };
    testState.sessionConfig = { mainKey: "work" };
    const sessionId = "sess-legacy-main";
    const transcriptPath = path.join(dir, `${sessionId}.jsonl`);
    const lines = [
      JSON.stringify({ id: sessionId, type: "session", version: 1 }),
      JSON.stringify({ message: { content: "Legacy alias transcript", role: "assistant" } }),
    ];
    await fs.writeFile(transcriptPath, lines.join("\n"), "utf8");
    await fs.writeFile(
      storePath,
  ...
  test("sessions.preview prefers the freshest duplicate row for a legacy mixed-case main alias", async () => {
    ...
    await fs.writeFile(
      staleTranscriptPath,
      [
        JSON.stringify({ id: "sess-stale-main", type: "session", version: 1 }),
        JSON.stringify({ message: { content: "stale preview", role: "assistant" } }),
      ].join("\n"),
      "utf8",
    );
    await fs.writeFile(
      freshTranscriptPath,
      [
        JSON.stringify({ id: "sess-fresh-main", type: "session", version: 1 }),
        JSON.stringify({ message: { content: "fresh preview", role: "assistant" } }),
      ].join("\n"),
      "utf8",
    );
    await fs.writeFile(
  ...
  test("sessions.resolve and mutators clean legacy main-alias ghost keys", async () => {
    ...
    await fs.writeFile(
      transcriptPath,
      `${Array.from({ length: 8 })
        .map((_, idx) => JSON.stringify({ content: `line ${idx}`, role: "assistant" }))
        .join("\n")}\n`,
      "utf8",
    );
  ...
  test("sessions.delete emits session_end with deleted reason and no replacement", async () => {
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({
        id: "m-delete",
        message: { content: "delete me", role: "user" },
        type: "message",
      })}\n`,
      "utf8",
    );
    ...
    const [event, context] = (
      sessionLifecycleHookMocks.runSessionEnd.mock.calls as unknown as [unknown, unknown][]
  ...
  test("sessions.reset emits internal command hook with reason", async () => {
    ...
    const event = (
      sessionHookMocks.triggerInternalHook.mock.calls as unknown as [unknown][]
  ...
  test("sessions.reset emits before_reset hook with transcript context", async () => {
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({
        id: "m1",
        message: { content: "hello from transcript", role: "user" },
        type: "message",
      })}\n`,
      "utf8",
    );
    ...
    const [event, context] = (
      beforeResetHookMocks.runBeforeReset.mock.calls as unknown as [unknown, unknown][]
    )[0] ?? [undefined, undefined];
    expect(event).toMatchObject({
      messages: [
        {
  ...
  test("sessions.reset emits enriched session_end and session_start hooks", async () => {
    ...
    const [startEvent, startContext] = (
      sessionLifecycleHookMocks.runSessionStart.mock.calls as unknown as [unknown, unknown][]
    )[0] ?? [undefined, undefined];
  ...
  test("sessions.reset emits before_reset for the entry actually reset under the store lock", async () => {
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({
        id: "m-old",
        message: { content: "old transcript", role: "user" },
        type: "message",
      })}\n`,
      "utf8",
    );
    await fs.writeFile(
      ...
      `${JSON.stringify({
        id: "m-new",
        message: { content: "new transcript", role: "user" },
        type: "message",
      })}\n`,
      "utf8",
    );
