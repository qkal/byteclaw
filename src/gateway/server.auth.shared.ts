...
async function resolvePairedTokenForDeviceIdentityPath(deviceIdentityPath: string): Promise<{
  ...
  return { deviceToken: String(deviceToken ?? ""), identity: { deviceId: identity.deviceId } };
}
...
async function startRateLimitedTokenServerWithPairedDeviceToken() {
  ...
  try {
    const initial = await connectReq(ws, { deviceIdentityPath, token: "secret" });
    if (!initial.ok) {
    ...
    ws.close();
    return { deviceIdentityPath, deviceToken: String(deviceToken ?? ""), port, prevToken, server };
  } catch (error) {
    ws.close();
    await server.close();
...
async function ensurePairedDeviceTokenForCurrentIdentity(ws: WebSocket): Promise<{
  ...
  const res = await connectReq(ws, { deviceIdentityPath, token: "secret" });
  if (!res.ok) {
    await approvePendingPairingIfNeeded();
  }
