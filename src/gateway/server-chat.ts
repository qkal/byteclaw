...
import { type ErrorKind, detectErrorKind } from "../infra/errors.js";
...
export interface ChatRunRegistry {
...
export function createChatRunRegistry(): ChatRunRegistry {
  ...
  return { add, clear, peek, remove, shift };
}
...
export interface ToolEventRecipientRegistry {
  add: (runId: string, connId: string) => void;
  get: (runId: string) => ReadonlySet<string> | undefined;
  markFinal: (runId: string) => void;
}

export interface SessionEventSubscriberRegistry {
...
export interface SessionMessageSubscriberRegistry {
...
export function createAgentEventHandler({
  ...
  const finalizeLifecycleEvent = (
    ...
    if (sessionKey) {
      void persistGatewaySessionLifecycleEvent({ event: evt, sessionKey }).catch(() => undefined);
  ...
  const emitChatDelta = (
    ...
    const payload = {
      message: {
        content: [{ text: mergedText, type: "text" }],
        role: "assistant",
        timestamp: now,
      },
      runId: clientRunId,
  ...
  const resolveBufferedChatTextState = (clientRunId: string, sourceRunId: string) => {
    ...
    return { shouldSuppressSilent, text };
  };
    ...
    const toolPayload =
      isToolEvent && toolVerbose !== "full"
        ? (() => {
            ...
            return sessionKey
              ? { ...eventForClients, data, sessionKey }
