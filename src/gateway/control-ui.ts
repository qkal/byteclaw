...
export interface ControlUiRequestOptions {
  basePath?: string;
...
function resolveSafeAvatarFile(filePath: string): { path: string; fd: number } | null {
  ...
  return { fd: opened.fd, path: opened.path };
}
...
export function handleControlUiHttpRequest(
  ...
  if (pathname === bootstrapConfigPath) {
    ...
    const identity = config
      ? resolveAssistantIdentity({ agentId: opts?.agentId, cfg: config })
