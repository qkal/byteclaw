    ...
    try {
      ...
      if (parsed?.type === "compaction") {
        ...
        messages.push({
          ...
          content: [{ text: "Compaction", type: "text" }],
          role: "system",
          timestamp,
        });
      }
...
const LAST_MSG_MAX_BYTES = 16_384;
...
export function readLatestSessionUsageFromTranscript(
  ...
  return withOpenTranscriptFd(filePath, (fd) => {
    ...
    const chunk = fs.readFileSync(fd, "utf8");
...
interface TranscriptContentEntry {
  type?: string;
  text?: string;
  name?: string;
}

interface TranscriptPreviewMessage {
  role?: string;
