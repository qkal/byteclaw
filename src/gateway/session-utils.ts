...
function resolveChildSessionKeys(
  ...
  const childSessions = [...childSessionKeys];
...
export function loadSessionEntry(sessionKey: string) {
  ...
  return { canonicalKey, cfg, entry: freshestMatch?.entry, legacyKey, store, storePath };
}
...
export function migrateAndPruneGatewaySessionStoreKey(params: {
  ...
  return { entry: params.store[primaryKey], primaryKey, target };
}
...
function listConfiguredAgentIds(cfg: OpenClawConfig): string[] {
  ...
  const sorted = [...ids].filter(Boolean);
...
export function canonicalizeSpawnedByForAgent(
  ...
  return canonicalizeMainSessionAlias({ agentId: resolvedAgent, cfg, sessionKey: result });
}
...
function buildGatewaySessionStoreScanTargets(params: {
  ...
  const agentMainKey = resolveAgentMainSessionKey({ agentId: params.agentId, cfg: params.cfg });
...
export function resolveGatewaySessionStoreTarget(params: {
  ...
  if (canonicalKey === "global" || canonicalKey === "unknown") {
    ...
    return { agentId, canonicalKey, storeKeys, storePath };
  }
...
export function loadCombinedSessionStoreForGateway(cfg: OpenClawConfig): {
  ...
  if (storeConfig && !isStorePathTemplate(storeConfig)) {
    ...
    return { store: combined, storePath };
  }
...
export function resolveSessionModelRef(
  ...
  const resolved = agentId
    ? resolveDefaultModelForAgent({ agentId, cfg })
    : resolveConfiguredModelRef({
        cfg,
        defaultModel: DEFAULT_MODEL,
        defaultProvider: DEFAULT_PROVIDER,
...
export function resolveSessionModelIdentityRef(
  ...
  if (runtimeModel) {
    ...
    if (inferredProvider) {
      return { model: runtimeModel, provider: inferredProvider };
    }
    if (runtimeModel.includes("/")) {
      const parsedRuntime = parseModelRef(runtimeModel, DEFAULT_PROVIDER);
      if (parsedRuntime) {
        return { model: parsedRuntime.model, provider: parsedRuntime.provider };
      }
  ...
  if (fallbackRef) {
    ...
    if (parsedFallback) {
      return { model: parsedFallback.model, provider: parsedFallback.provider };
    }
    const inferredProvider = inferUniqueProviderFromConfiguredModels({
      cfg,
      model: fallbackRef,
    ...
    if (inferredProvider) {
      return { model: fallbackRef, provider: inferredProvider };
    }
