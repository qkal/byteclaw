...
import type { ModelProviderConfig, ModelsConfig, OpenClawConfig } from "../config/types.js";
...
async function withGatewayLiveTimeout<T>(params: {
  ...
  const heartbeat = setInterval(() => {
    heartbeatCount += 1;
    logProgress(
      `${params.context}: still running (${Math.max(1, Math.round((Date.now() - startedAt) / 1000))}s)`,
    );
  ...
  try {
    ...
    if (heartbeatCount > 0) {
      logProgress(
        `${params.context}: completed after ${Math.max(1, Math.round((Date.now() - startedAt) / 1000))}s`,
      );
    }
  }
}
...
describe("shouldSkipToolNonceProbeMissForLiveModel", () => {
  it.each([
    { expected: true, modelKey: "anthropic/claude-opus-4-6" },
    { expected: true, modelKey: "minimax/minimax-m1" },
    { expected: true, modelKey: "opencode/big-pickle" },
    { expected: true, modelKey: "opencode-go/glm-5" },
    { expected: true, modelKey: "xai/grok-4.1-fast" },
    { expected: true, modelKey: "zai/glm-4.7" },
    { expected: true, modelKey: "google/gemini-3-flash-preview" },
    { expected: false, modelKey: "openai/gpt-5.4" },
...
describe("getHighSignalLiveModelPriorityIndex", () => {
  it("prefers curated Google replacements over big-pickle", () => {
    expect(
      getHighSignalLiveModelPriorityIndex({ id: "gemini-3.1-pro-preview", provider: "google" }),
    ).toBe(2);
    expect(
      getHighSignalLiveModelPriorityIndex({ id: "gemini-3-flash-preview", provider: "google" }),
    ).toBe(3);
    expect(getHighSignalLiveModelPriorityIndex({ id: "big-pickle", provider: "opencode" })).toBe(
      null,
    );
  });
});
...
describe("shouldSkipEmptyResponseForLiveModel", () => {
  it.each([
    { allowNotFoundSkip: false, expected: true, provider: "google" },
    { allowNotFoundSkip: false, expected: true, provider: "google-antigravity" },
    { allowNotFoundSkip: false, expected: true, provider: "openrouter" },
    { allowNotFoundSkip: false, expected: true, provider: "opencode" },
    { allowNotFoundSkip: false, expected: true, provider: "opencode-go" },
    { allowNotFoundSkip: false, expected: false, provider: "minimax" },
    { allowNotFoundSkip: true, expected: true, provider: "minimax" },
    { allowNotFoundSkip: true, expected: true, provider: "zai" },
    { allowNotFoundSkip: true, expected: true, provider: "openai-codex" },
    { allowNotFoundSkip: true, expected: false, provider: "xai" },
  ])(
    "returns $expected for $provider (allowNotFoundSkip=$allowNotFoundSkip)",
    ({ provider, allowNotFoundSkip, expected }) => {
      expect(shouldSkipEmptyResponseForLiveModel({ allowNotFoundSkip, provider })).toBe(expected);
    },
  );
});
...
async function connectClient(params: { url: string; token: string; timeoutMs?: number }) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    try {
      ...
      if (!isRetryableGatewayConnectError(lastError) || remainingMs <= 5000) {
        throw lastError;
      }
...
async function waitForSessionAssistantText(params: {
  ...
  while (Date.now() - startedAt < GATEWAY_LIVE_PROBE_TIMEOUT_MS) {
    ...
    if (Date.now() - lastHeartbeatAt >= GATEWAY_LIVE_HEARTBEAT_MS) {
      ...
      logProgress(
        `${params.context}: waiting for transcript (${Math.max(1, Math.round((Date.now() - startedAt) / 1000))}s)`,
      );
    }
...
interface GatewayModelSuiteParams {
  label: string;
  cfg: OpenClawConfig;
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const sanitizedCfg: OpenClawConfig = {
    ...params.cfg,
    auth: sanitizeAuthConfig({ agentDir, cfg: params.cfg }),
  };
  ...
  try {
    ...
    logProgress(
      `[${params.label}] heartbeat=${Math.max(1, Math.round(GATEWAY_LIVE_HEARTBEAT_MS / 1000))}s probe-timeout=${Math.max(1, Math.round(GATEWAY_LIVE_PROBE_TIMEOUT_MS / 1000))}s model-timeout=${Math.max(1, Math.round(GATEWAY_LIVE_MODEL_TIMEOUT_MS / 1000))}s`,
    );
    ...
    for (const [index, model] of params.candidates.entries()) {
      ...
      for (let attempt = 0; attempt < attemptMax; attempt += 1) {
        ...
        try {
          ...
          failures.push({ error: message, model: modelKey });
          break;
        }
      }
    }
    ...
    await fs.rm(tempDir, { force: true, maxRetries: 5, recursive: true, retryDelay: 50 });
    if (tempAgentDir) {
      await fs.rm(tempAgentDir, { force: true, maxRetries: 5, recursive: true, retryDelay: 50 });
    }
    if (tempStateDir) {
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        const wanted = filter
          ...
          : all.filter((m) => isHighSignalLiveModelRef({ id: m.id, provider: m.provider }));

        const candidates: Model<Api>[] = [];
        const skipped: { model: string; error: string }[] = [];
        for (const model of wanted) {
          if (shouldSuppressBuiltInModel({ id: model.id, provider: model.provider })) {
            continue;
          }
          ...
          try {
            ...
            skipped.push({ error: String(error), model: modelRef });
          }
        }
