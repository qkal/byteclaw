...
export function collectConfiguredModelPricingRefs(config: OpenClawConfig): ModelRef[] {
  ...
  addModelListLike({ aliasIndex, refs, value: config.agents?.defaults?.model });
  addModelListLike({ aliasIndex, refs, value: config.agents?.defaults?.imageModel });
  addModelListLike({ aliasIndex, refs, value: config.agents?.defaults?.pdfModel });
  addResolvedModelRef({ aliasIndex, raw: config.agents?.defaults?.compaction?.model, refs });
  addResolvedModelRef({ aliasIndex, raw: config.agents?.defaults?.heartbeat?.model, refs });
  addModelListLike({ aliasIndex, refs, value: config.tools?.subagents?.model });
  addResolvedModelRef({ aliasIndex, raw: config.messages?.tts?.summaryModel, refs });
  addResolvedModelRef({ aliasIndex, raw: config.hooks?.gmail?.model, refs });

  for (const agent of config.agents?.list ?? []) {
    addModelListLike({ aliasIndex, refs, value: agent.model });
    addModelListLike({ aliasIndex, refs, value: agent.subagents?.model });
    addResolvedModelRef({ aliasIndex, raw: agent.heartbeat?.model, refs });
  }

  for (const mapping of config.hooks?.mappings ?? []) {
    addResolvedModelRef({ aliasIndex, raw: mapping.model, refs });
  }
  ...
  addConfiguredWebSearchPluginModels({ aliasIndex, config, refs });

  for (const entry of config.tools?.media?.models ?? []) {
    addProviderModelPair({ model: entry.model, provider: entry.provider, refs });
  }
