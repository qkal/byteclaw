...
describe("gateway pre-auth hardening", () => {
  it("rejects upgrades before websocket handlers attach (pre-auth budget enforced, then released)", async () => {
    ...
    const resolvedAuth: ResolvedGatewayAuth = { allowTailscale: false, mode: "none" };
  ...
  it("closes idle unauthenticated sockets after the handshake timeout", async () => {
    ...
    try {
      ...
      expect(close.elapsedMs).toBeLessThan(2500);
    } finally {
      await harness.close();
    }
  });
    ...
    try {
      ...
      ws.send(
        JSON.stringify({
          ...
          method: "connect",
          params: {
            client: { id: "test", mode: "test", platform: "test", version: "1.0.0" },
            maxProtocol: 3,
            minProtocol: 3,
            pathEnv: large,
            role: "operator",
