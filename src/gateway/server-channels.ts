...
export function createChannelManager(opts: ChannelManagerOptions): ChannelManager {
  ...
  const isHealthMonitorEnabled = (channelId: ChannelId, accountId: string): boolean => {
    ...
    try {
      ...
      channelLogs[channelId].warn?.(
        `[${channelId}:${accountId}] health-monitor: failed to resolve account; skipping monitor (${formatErrorMessage(error)})`,
      );
      return false;
    }
  ...
  const startChannelInternal = async (
    ...
    const cfg = loadConfig();
    resetDirectoryCache({ accountId, channel: channelId });
    const store = getStore(channelId);
    ...
    for (const plugin of listChannelPlugins()) {
      try {
        ...
        channelLogs[plugin.id]?.error?.(
          `[${plugin.id}] channel startup failed: ${formatErrorMessage(error)}`,
        );
      }
    }
  };
