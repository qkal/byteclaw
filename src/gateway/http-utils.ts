...
export interface AuthorizedGatewayHttpRequest {
...
export async function authorizeGatewayHttpRequestOrReply(params: {
  ...
  const authResult = await authorizeHttpGatewayConnect({
    ...
    auth: params.auth,
    browserOriginPolicy,
    connectAuth: token ? { password: token, token } : null,
...
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  const defaultModelRef = resolveDefaultModelForAgent({ agentId: params.agentId, cfg });
...
export function resolveGatewayRequestContext(params: {
  ...
  const agentId = resolveAgentIdForRequest({ model: params.model, req: params.req });
  ...
  return { agentId, messageChannel, sessionKey };
}
