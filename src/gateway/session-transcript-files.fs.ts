...
export interface ArchivedSessionTranscript {
  sourcePath: string;
  archivedPath: string;
}
...
export function resolveSessionTranscriptCandidates(
  ...
  if (storePath) {
    ...
    if (sessionFile && sessionFileState !== "stale") {
      pushCandidate(() =>
        resolveSessionFilePath(sessionId, { sessionFile }, { agentId, sessionsDir }),
      );
    }
...
export async function cleanupArchivedSessionTranscripts(opts: {
  ...
  const directories = [...new Set(opts.directories.map((dir) => path.resolve(dir)))];
  let removed = 0;
  let scanned = 0;
