...
export interface ParsedMessageWithImages {
  message: string;
...
export async function parseMessageWithAttachments(
  ...
  if (!attachments || attachments.length === 0) {
    return { imageOrder: [], images: [], message, offloadedRefs: [] };
  }
  ...
  try {
    for (const [idx, att] of attachments.entries()) {
      ...
      images.push({ data: b64, mimeType: finalMime, type: "image" });
