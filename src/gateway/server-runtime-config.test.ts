...
describe("resolveGatewayRuntimeConfig", () => {
  describe("trusted-proxy auth mode", () => {
    ...
    it.each([
      ...
      {
        cfg: {
          gateway: { auth: TRUSTED_PROXY_AUTH, bind: "loopback" as const, trustedProxies: ["::1"] },
        },
        expectedBindHost: "127.0.0.1",
      ...
      const result = await resolveGatewayRuntimeConfig({ cfg, port: 18_789 });
    ...
    it.each([
      {
        cfg: {
          gateway: { auth: TRUSTED_PROXY_AUTH, bind: "loopback" as const, trustedProxies: [] },
        },
        expectedMessage:
      ...
      await expect(resolveGatewayRuntimeConfig({ cfg, port: 18_789 })).rejects.toThrow(
        expectedMessage,
      );
    });
  ...
  describe("token/password auth modes", () => {
    ...
    it.each([
      ...
      {
        cfg: { gateway: { auth: { mode: "none" as const }, bind: "loopback" as const } },
    ...
    it.each([
      {
        cfg: { gateway: { auth: { mode: "token" as const }, bind: "lan" as const } },
        expectedMessage:
          "gateway auth mode is token, but no token was configured (set gateway.auth.token or OPENCLAW_GATEWAY_TOKEN)",
        name: "token mode without token",
      },
      {
        cfg: { gateway: { auth: { mode: "none" as const }, bind: "lan" as const } },
      ...
      {
        cfg: { gateway: { auth: TOKEN_AUTH, bind: "custom" as const } },
      ...
      await expect(resolveGatewayRuntimeConfig({ cfg, host, port: 18_789 })).rejects.toThrow(
        expectedMessage,
      );
    });
  ...
  describe("container-aware bind default", () => {
    ...
    it("respects explicit loopback config even inside a container", async () => {
      ...
      vi.spyOn(fs, "accessSync").mockImplementation(() => undefined); // /.dockerenv exists
      const result = await resolveGatewayRuntimeConfig({
        cfg: { gateway: { auth: { mode: "none" }, bind: "loopback" } },
        port: 18_789,
      });
      expect(result.bindHost).toBe("127.0.0.1");
    });
