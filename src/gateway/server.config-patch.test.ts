...
afterAll(async () => {
  ...
  startedServer = null;
  await fs.rm(sharedTempRoot, { force: true, recursive: true });
});
...
describe("gateway config methods", () => {
  it("rejects config.set when SecretRef resolution fails", async () => {
    ...
    telegram.botToken = { id: missingEnvVar, provider: "default", source: "env" };
  ...
  it("returns a path-scoped config schema lookup", async () => {
    const res = await rpcReq<{
      path: string;
      hintPath?: string;
      children?: { key: string; path: string; required: boolean; hintPath?: string }[];
...
describe("gateway config.apply", () => {
  ...
  it("rejects invalid raw config", async () => {
    ...
    const res = await sendConfigApply({ baseHash: currentHash, raw: "{" });
    expect(res.ok).toBe(false);
...
describe("gateway server sessions", () => {
  it("filters sessions by agentId", async () => {
    ...
    testState.agentsConfig = {
      list: [{ default: true, id: "home" }, { id: "work" }],
    };
