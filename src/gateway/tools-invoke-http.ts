...
export async function handleToolsInvokeHttpRequest(
  ...
  if (!tool) {
    sendJson(res, 404, {
      error: { message: `Tool not available: ${toolName}`, type: "not_found" },
      ok: false,
    });
    return true;
  }
  ...
  try {
    ...
    const hookResult = await runBeforeToolCallHook({
      ctx: {
        agentId,
        loopDetection: resolveToolLoopDetectionConfig({ agentId, cfg }),
        sessionKey,
      },
      params: toolArgs,
      toolCallId,
    ...
    if (hookResult.blocked) {
      sendJson(res, 403, {
        error: { message: hookResult.reason, type: "tool_call_blocked" },
        ok: false,
      });
      return true;
    }
    ...
    const inputStatus = resolveToolInputErrorStatus(error);
    if (inputStatus !== null) {
      sendJson(res, inputStatus, {
        error: { message: getErrorMessage(error) || "invalid tool arguments", type: "tool_error" },
        ok: false,
      });
      return true;
    }
    logWarn(`tools-invoke: tool execution failed: ${String(error)}`);
    sendJson(res, 500, {
      error: { message: "tool execution failed", type: "tool_error" },
      ok: false,
    });
  }
