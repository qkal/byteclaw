...
import { getTotalQueueSize, setCommandLaneConcurrency } from "../process/command-queue.js";
...
import { type GatewayCronState, buildGatewayCronService } from "./server-cron.js";
...
interface GatewayHotReloadState {
  ...
  const applyHotReload = async (
    ...
    if (plan.reloadHooks) {
      try {
        ...
        params.logHooks.warn(`hooks config reload failed: ${String(error)}`);
      }
    }
