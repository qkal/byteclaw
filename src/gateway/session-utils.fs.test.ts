...
function buildBasicSessionTranscript(
  ...
  return [
    { id: sessionId, type: "session", version: 1 },
...
describe("readFirstUserMessageFromTranscript", () => {
  ...
  test.each([
    {
      ...
      lines: [
        JSON.stringify({ id: "test-session-1", type: "session", version: 1 }),
        JSON.stringify({ message: { content: "Hello world", role: "user" } }),
        JSON.stringify({ message: { content: "Hi there", role: "assistant" } }),
      ],
    ...
    },
    {
      ...
      lines: [
        JSON.stringify({ id: "test-session-2", type: "session", version: 1 }),
        JSON.stringify({
          message: {
    ...
    },
    {
      ...
      lines: [
        JSON.stringify({ id: "test-session-2b", type: "session", version: 1 }),
        JSON.stringify({
          message: {
  ...
  test("skips non-user messages to find first user message", () => {
    ...
    const lines = [
      ...
      JSON.stringify({ message: { content: "System prompt", role: "system" } }),
      JSON.stringify({ message: { content: "Greeting", role: "assistant" } }),
      JSON.stringify({ message: { content: "First user question", role: "user" } }),
    ];
    fs.writeFileSync(transcriptPath, lines.join("\n"), "utf8");
  ...
  test("skips inter-session user messages by default", () => {
    ...
    const lines = [
      ...
      JSON.stringify({
        message: { content: "Real user message", role: "user" },
      }),
    ];
    fs.writeFileSync(transcriptPath, lines.join("\n"), "utf8");
  ...
  test("handles malformed JSON lines gracefully", () => {
    ...
    const lines = [
      "not valid json",
      JSON.stringify({ message: { content: "Valid message", role: "user" } }),
    ];
    fs.writeFileSync(transcriptPath, lines.join("\n"), "utf8");
  ...
  test("returns null for empty content", () => {
    ...
    const lines = [
      ...
      JSON.stringify({ message: { content: "Second message", role: "user" } }),
    ];
    fs.writeFileSync(transcriptPath, lines.join("\n"), "utf8");
...
describe("readLastMessagePreviewFromTranscript", () => {
  ...
  test.each([
    {
      ...
      lines: [
        JSON.stringify({ message: { content: "First user", role: "user" } }),
        JSON.stringify({ message: { content: "First assistant", role: "assistant" } }),
        JSON.stringify({ message: { content: "Last user message", role: "user" } }),
      ],
    ...
    },
    {
      ...
      lines: [
        JSON.stringify({ message: { content: "User question", role: "user" } }),
        JSON.stringify({ message: { content: "Final assistant reply", role: "assistant" } }),
      ],
  ...
  test("skips system messages to find last user/assistant", () => {
    ...
    const transcriptPath = path.join(tmpDir, `${sessionId}.jsonl`);
    const lines = [
      JSON.stringify({ message: { content: "Real last", role: "user" } }),
      JSON.stringify({ message: { content: "System at end", role: "system" } }),
    ];
    fs.writeFileSync(transcriptPath, lines.join("\n"), "utf8");
  ...
  test("returns null when no user/assistant messages exist", () => {
    ...
    const lines = [
      JSON.stringify({ id: sessionId, type: "session", version: 1 }),
      JSON.stringify({ message: { content: "Only system", role: "system" } }),
    ];
    fs.writeFileSync(transcriptPath, lines.join("\n"), "utf8");
  ...
  test("handles malformed JSON lines gracefully (last preview)", () => {
    ...
    const transcriptPath = path.join(tmpDir, `${sessionId}.jsonl`);
    const lines = [
      JSON.stringify({ message: { content: "Valid first", role: "user" } }),
  ...
  test.each([
    {
      ...
      message: {
        content: [{ text: "Array content response", type: "text" }],
        role: "assistant",
      },
    ...
    },
    {
      ...
      message: {
        content: [{ text: "Output text response", type: "output_text" }],
        role: "assistant",
      },
    ...
    ({ sessionId, message, expected }) => {
      const transcriptPath = path.join(tmpDir, `${sessionId}.jsonl`);
      fs.writeFileSync(transcriptPath, JSON.stringify({ message }), "utf8");
      const result = readLastMessagePreviewFromTranscript(sessionId, storePath);
      expect(result, sessionId).toBe(expected);
    },
  );
  ...
  test("skips empty content to find previous message", () => {
    ...
    const transcriptPath = path.join(tmpDir, `${sessionId}.jsonl`);
    const lines = [
      JSON.stringify({ message: { content: "Has content", role: "assistant" } }),
      JSON.stringify({ message: { content: "", role: "user" } }),
    ];
    fs.writeFileSync(transcriptPath, lines.join("\n"), "utf8");
  ...
  test("reads from end of large file (16KB window)", () => {
    ...
    const transcriptPath = path.join(tmpDir, `${sessionId}.jsonl`);
    const padding = JSON.stringify({ message: { content: "x".repeat(500), role: "user" } });
    const lines: string[] = [];
    for (let i = 0; i < 30; i++) {
      lines.push(padding);
    }
    lines.push(JSON.stringify({ message: { content: "Last in large file", role: "assistant" } }));
    fs.writeFileSync(transcriptPath, lines.join("\n"), "utf8");
  ...
  test("handles valid UTF-8 content", () => {
    ...
    const validLine = JSON.stringify({
      message: { content: "Valid UTF-8: 你好世界 🌍", role: "user" },
    });
    fs.writeFileSync(transcriptPath, validLine, "utf8");
...
describe("shared transcript read behaviors", () => {
  ...
  test("uses sessionFile overrides when provided", () => {
    ...
    fs.writeFileSync(
      ...
      [
        JSON.stringify({ id: sessionId, type: "session", version: 1 }),
        JSON.stringify({ message: { content: "Custom file message", role: "user" } }),
      ].join("\n"),
      "utf8",
    );
    fs.writeFileSync(
      lastPath,
      JSON.stringify({ message: { content: "Custom file last", role: "assistant" } }),
      "utf8",
    );
  ...
  test("trims whitespace in extracted previews", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ message: { content: "  Padded message  ", role: "user" } }),
      "utf8",
    );
    fs.writeFileSync(
      path.join(tmpDir, `${lastSessionId}.jsonl`),
      JSON.stringify({ message: { content: "  Padded response  ", role: "assistant" } }),
      "utf8",
    );
...
describe("readSessionTitleFieldsFromTranscript cache", () => {
  ...
  test("invalidates cache when transcript changes", () => {
    ...
    fs.appendFileSync(
      transcriptPath,
      `\n${JSON.stringify({ message: { content: "New", role: "assistant" } })}`,
      "utf8",
    );
...
describe("readSessionMessages", () => {
  ...
  test("includes synthetic compaction markers for compaction entries", () => {
    ...
    const lines = [
      ...
      JSON.stringify({ message: { content: "World", role: "assistant" } }),
    ];
    fs.writeFileSync(transcriptPath, lines.join("\n"), "utf8");
  ...
  test.each([
    {
      message: { content: "from-ops", role: "user" },
    ...
    {
      message: { content: "from-custom-ops", role: "assistant" },
...
describe("readSessionPreviewItemsFromTranscript", () => {
  ...
  test("detects tool calls from tool_use/tool_call blocks and toolName field", () => {
    ...
    const lines = [
      ...
      JSON.stringify({ message: { content: "Done", role: "assistant" } }),
    ];
    writeTranscriptLines(sessionId, lines);
    const result = readPreview(sessionId);
  ...
  test("truncates preview text to max chars", () => {
    ...
    const lines = [JSON.stringify({ message: { content: longText, role: "assistant" } })];
    writeTranscriptLines(sessionId, lines);
...
describe("readLatestSessionUsageFromTranscript", () => {
  ...
  test("reads earlier assistant usage outside the old tail window", () => {
    ...
    writeTranscript(tmpDir, sessionId, [
      ...
      ...Array.from({ length: 80 }, () => ({ message: { content: filler, role: "user" } })),
      {
        message: {
          model: "gpt-5.4",
          provider: "openai",
...
describe("archiveSessionTranscripts", () => {
  ...
  test.each([
    {
      buildArgs: () => ({ reason: "reset" as const, sessionId: "sess-archive-1", storePath }),
