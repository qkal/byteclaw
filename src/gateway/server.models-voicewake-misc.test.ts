...
const whatsappOutbound: ChannelOutboundAdapter = {
  ...
  sendMedia: async ({ deps, to, text, mediaUrl }) => {
    ...
    return {
      channel: "whatsapp",
      ...(await (deps["whatsapp"] as Function)(to, text, { mediaUrl, verbose: false })),
    };
  },
...
interface ModelCatalogRpcEntry {
  id: string;
  name: string;
  provider: string;
  alias?: string;
...
interface PiCatalogFixtureEntry {
  id: string;
  provider: string;
  name?: string;
  contextWindow?: number;
...
const buildPiCatalogFixture = (): PiCatalogFixtureEntry[] => [
  { contextWindow: 0, id: "gpt-test-z", provider: "openai" },
  {
    contextWindow: 8000,
    id: "gpt-test-a",
    name: "A-Model",
