...
interface GatewayClientErrorShape {
  code?: string;
  message?: string;
  details?: unknown;
}
...
const STOP_AND_WAIT_TIMEOUT_MS = 1000;
...
export class GatewayClient {
  ...
  private createPendingStop(ws: WebSocket): Promise<void> {
    ...
    this.pendingStop = { promise, resolve, ws };
    return promise;
  }
  ...
  private handleMessage(raw: string) {
    try {
      ...
      logDebug(`gateway client parse error: ${String(error)}`);
    }
  }
  ...
  async request<T = Record<string, unknown>>(
    ...
    const id = randomUUID();
    const frame: RequestFrame = { id, method, params, type: "req" };
