  ...
  test("prefers connectChallengeTimeoutMs and still honors the legacy alias", () => {
    ...
    expect(
      resolveGatewayClientConnectChallengeTimeoutMs({
        connectChallengeTimeoutMs: 5000,
        connectDelayMs: 2000,
      }),
    ).toBe(5000);
  });
  ...
  test("closes on missing ticks", async () => {
    ...
    wss.on("connection", (socket) => {
      socket.once("message", (data) => {
        ...
        socket.send(JSON.stringify({ id, ok: true, payload: helloOk, type: "res" }));
      });
    });
...
-----END CERTIFICATE-----`;
    ...
    wss = new WebSocketServer({ maxPayload: 1024 * 1024, server: httpsServer });
