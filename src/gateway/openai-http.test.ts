...
describe("OpenAI-compatible HTTP API (e2e)", () => {
  ...
  it("handles request validation and routing", async () => {
    ...
    const mockAgentOnce = (payloads: { text: string }[]) => {
    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { messages: [{ content: "hi", role: "user" }], model: "openclaw" },
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { messages: [{ content: "hi", role: "user" }], model: "openclaw" },
          {
...

      {
        ...
        expect(firstCall?.images).toEqual([
          { data: "QUJDRA==", mimeType: "image/jpeg", type: "image" },
        ]);
        await res.text();
      }

      {
        ...
        const res = await postChatCompletions(port, {
          messages: [
            ...
            { content: "I can see it.", role: "assistant" },
            { content: "What color was it?", role: "user" },
          ],
          model: "openclaw",
        });
        expect(res.status).toBe(200);
...

      {
        ...
        const firstCall = getFirstAgentCall();
        expect(firstCall?.images).toEqual([
          { data: "QkJCQg==", mimeType: "image/png", type: "image" },
        ]);
        await res.text();
      }
...

      {
        ...
        const res = await postChatCompletions(port, {
          messages: [
            ...
            { content: "I am Claude.", role: "assistant" },
            { content: "What did I just ask you?", role: "user" },
          ],
          model: "openclaw",
        });
        expect(res.status).toBe(200);
...

      {
        ...
        const res = await postChatCompletions(port, {
          messages: [
            ...
            { content: "Checking the weather.", role: "assistant" },
            { content: "Sunny, 70F.", role: "tool" },
          ],
          model: "openclaw",
        });
        expect(res.status).toBe(200);
...

      {
        ...
        const res = await postChatCompletions(port, {
          messages: [
            ...
            { content: "Checking the image.", role: "assistant" },
            { content: "Vision tool says it is blue.", role: "tool" },
          ],
          model: "openclaw",
        });
        expect(res.status).toBe(200);
...

      {
        ...
        const choice0 = (json.choices as Record<string, unknown>[])[0] ?? {};
      ...
      {
        const res = await postChatCompletions(port, {
          messages: [{ content: "yo", role: "system" }],
          model: "openclaw",
        });
        expect(res.status).toBe(400);
  ...
  it("returns 429 for repeated failed auth when gateway.auth.rateLimit is configured", async () => {
    testState.gatewayAuth = {
      mode: "token",
      rateLimit: { exemptLoopback: false, lockoutMs: 60_000, maxAttempts: 1, windowMs: 60_000 },
      token: "secret",
    } as any;
    ...
    try {
      {
        ...
        const allContent = jsonChunks
          .flatMap((c) => (c.choices as Record<string, unknown>[] | undefined) ?? [])
