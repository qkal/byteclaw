...
describe("gateway plugin HTTP auth boundary", () => {
  ...
  test("does not shadow plugin routes mounted on probe paths", async () => {
    ...
    await withGatewayServer({
      ...
      resolvedAuth: AUTH_NONE,
      run: async (server) => {
        await expectHealthzPluginShadow({ handlePluginRequest, server });
      },
    });
  });
  ...
  test("rejects non-GET/HEAD methods on probe routes", async () => {
    await withGatewayServer({
      ...
      resolvedAuth: AUTH_NONE,
      run: async (server) => {
        const postResponse = await sendRequest(server, { method: "POST", path: "/healthz" });
        ...
        const headResponse = await sendRequest(server, { method: "HEAD", path: "/readyz" });
