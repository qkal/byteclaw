...
describe("auth rate limiter", () => {
  ...
  const baseConfig = { lockoutMs: 60_000, maxAttempts: 2, windowMs: 60_000 };
  ...
  it("allows requests when no failures have been recorded", () => {
    limiter = createAuthRateLimiter({ lockoutMs: 300_000, maxAttempts: 5, windowMs: 60_000 });
  ...
  it("decrements remaining count after each failure", () => {
    limiter = createAuthRateLimiter({ lockoutMs: 300_000, maxAttempts: 3, windowMs: 60_000 });
  ...
  it("does not extend lockout when failures are recorded while already locked", () => {
    ...
    try {
      ...
      expect(afterExtraFailure.retryAfterMs).toBeLessThanOrEqual(initialRetryAfter - 1000);
    } finally {
      vi.useRealTimers();
    }
  });
  ...
  it("expires old failures outside the window", () => {
    vi.useFakeTimers();
    try {
      limiter = createAuthRateLimiter({ lockoutMs: 60_000, maxAttempts: 3, windowMs: 10_000 });
  ...
  it("treats ipv4 and ipv4-mapped ipv6 forms as the same client", () => {
    limiter = createAuthRateLimiter({ lockoutMs: 60_000, maxAttempts: 1, windowMs: 60_000 });
  ...
  it("prune removes stale entries", () => {
    vi.useFakeTimers();
    try {
      limiter = createAuthRateLimiter({ lockoutMs: 5000, maxAttempts: 5, windowMs: 5000 });
  ...
  it("prune keeps entries that are still locked out", () => {
    vi.useFakeTimers();
    try {
      limiter = createAuthRateLimiter({ lockoutMs: 30_000, maxAttempts: 1, windowMs: 5000 });
