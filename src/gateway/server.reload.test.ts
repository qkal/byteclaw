  ...
  async function writeTalkProviderApiKeyEnvRefConfig(refId = "TALK_API_KEY_REF") {
    await writeConfigFile({
      talk: {
        providers: {
          [TALK_TEST_PROVIDER_ID]: {
            apiKey: { id: refId, provider: "default", source: "env" },
          },
        },
      },
    });
  ...
  async function writeGatewayTraversalExecRefConfig() {
    await writeConfigFile({
      gateway: {
        auth: {
          mode: "token",
          token: { id: "a/../b", provider: "vault", source: "exec" },
        },
      },
      secrets: {
        providers: {
  ...
  async function writeGatewayTokenExecRefConfig(params: {
    ...
    await writeConfigFile({
      gateway: {
        auth: {
          mode: "token",
          token: { id: "gateway/token", provider: "vault", source: "exec" },
        },
      },
      secrets: {
        providers: {
      ...
      `${JSON.stringify(
        {
          channels: {
            telegram: {
              botToken: { id: "DISABLED_TELEGRAM_STARTUP_REF", provider: "default", source: "env" },
              enabled: false,
            },
          },
          tools: {
      ...
      `${JSON.stringify(
        {
          gateway: {
            auth: {
              mode: "token",
              token: { id: "MISSING_STARTUP_GW_TOKEN", provider: "default", source: "env" },
            },
          },
          secrets: {
            providers: {
      ...
      `${JSON.stringify(
        {
          ...
          profiles: {
            missing: {
              keyRef: { id: "MISSING_OPENCLAW_AUTH_REF", provider: "default", source: "env" },
              provider: "openai",
              type: "api_key",
            },
          },
...
describe("gateway agents", () => {
  it("lists configured agents via agents.list RPC", async () => {
    const { server, ws } = await startServerWithClient();
    await connectOk(ws);
    const res = await rpcReq<{ agents: { id: string }[] }>(ws, "agents.list", {});
