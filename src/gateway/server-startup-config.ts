...
interface GatewayStartupConfigOverrides {
...
export async function loadGatewayStartupConfigSnapshot(params: {
  ...
  const autoEnable = params.minimalTestGateway
    ? { changes: [] as string[], config: configSnapshot.config }
  ...
  try {
    ...
    params.log.warn(`gateway: failed to persist plugin auto-enable changes: ${String(error)}`);
  }
