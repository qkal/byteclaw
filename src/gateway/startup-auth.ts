...
export async function ensureGatewayStartupAuth(params: {
  ...
  const [resolvedTokenRefValue, resolvedPasswordRefValue] = await Promise.all([
    resolveGatewayTokenSecretRefValue({
      ...
      hasPasswordCandidate:
        hasGatewayPasswordOverrideCandidate({ authOverride: params.authOverride, env }) ||
  ...
  if (resolved.mode !== "token" || (resolved.token?.trim().length ?? 0) > 0) {
    assertHooksTokenSeparateFromGatewayAuth({ auth: resolved, cfg: params.cfg });
    return { auth: resolved, cfg: params.cfg, persistedGeneratedToken: false };
  }
  ...
  assertHooksTokenSeparateFromGatewayAuth({ auth: nextAuth, cfg: nextCfg });
  return {
    auth: nextAuth,
    cfg: nextCfg,
    generatedToken,
