...
function makeTranscriptAssistantMessage(params: {
  ...
  return {
    ...
    content: params.content ?? [{ text: params.text, type: "text" }],
...
async function readSseEvent(
  ...
  while (true) {
    ...
    if (boundary !== -1) {
      ...
      return { data: JSON.parse(data), event };
    }
  ...
  test("prefers the freshest duplicate row for direct history reads", async () => {
    ...
    await fs.writeFile(
      ...
      [
        ...
        JSON.stringify({
          message: { content: [{ text: "stale history", type: "text" }], role: "assistant" },
        }),
      ].join("\n"),
      "utf8",
    );
    await fs.writeFile(
      ...
      [
        ...
        JSON.stringify({
          message: { content: [{ text: "fresh history", type: "text" }], role: "assistant" },
        }),
      ].join("\n"),
      "utf8",
    );
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      const firstBody = (await firstPage.json()) as {
        sessionKey?: string;
        items?: { content?: { text?: string }[]; __openclaw?: { seq?: number } }[];
        messages?: { content?: { text?: string }[]; __openclaw?: { seq?: number } }[];
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      const secondBody = (await secondPage.json()) as {
        items?: { content?: { text?: string }[]; __openclaw?: { seq?: number } }[];
        messages?: { __openclaw?: { seq?: number } }[];
        nextCursor?: string;
        hasMore?: boolean;
      };
