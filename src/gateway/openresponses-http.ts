...
interface OpenResponsesHttpOptions {
...
interface ResponseSessionScope {
  authSubject: string;
  agentId: string;
...
function createResponseSessionScope(params: {
  ...
  return normalizeResponseSessionScope({
    ...
    authSubject: resolveResponseSessionAuthSubject({ auth: params.auth, req: params.req }),
...
export const __testing = {
  ...
  lookupResponseSessionAt(
    responseId: string | undefined,
    now: number,
    scope: ResponseSessionScope = { agentId: "main", authSubject: "test" },
  ) {
...
interface ResolvedResponsesLimits {
  maxBodyBytes: number;
  maxUrlParts: number;
...
function resolveStopReasonAndPendingToolCalls(meta: unknown): {
  ...
  if (!meta || typeof meta !== "object") {
    return { pendingToolCalls: undefined, stopReason: undefined };
  }
  const record = meta as { stopReason?: string; pendingToolCalls?: PendingToolCall[] };
  return { pendingToolCalls: record.pendingToolCalls, stopReason: record.stopReason };
}
...
export async function handleOpenResponsesHttpRequest(
  ...
  try {
    ...
    logWarn(`openresponses: request parsing failed: ${String(error)}`);
    sendJson(res, 400, {
  ...
  try {
    ...
    logWarn(`openresponses: tool configuration failed: ${String(error)}`);
    sendJson(res, 400, {
  ...
  if (!stream) {
    ...
    try {
      ...
      logWarn(`openresponses: non-stream response failed: ${String(error)}`);
      const response = createResponseResource({
  ...
  const maybeFinalize = () => {
    ...
    writeSseEvent(res, {
      ...
      item_id: outputItemId,
      output_index: 0,
      part: { text: finalizeRequested.text, type: "output_text" },
    ...
    rememberResponseSession();
    writeSseEvent(res, { response: finalResponse, type: "response.completed" });
    writeDone(res);
    res.end();
  };
  ...
  writeSseEvent(res, { response: initialResponse, type: "response.created" });
  writeSseEvent(res, { response: initialResponse, type: "response.in_progress" });
    ...
    try {
      ...
      if (
        ...
        writeSseEvent(res, {
          ...
          item_id: outputItemId,
          output_index: 0,
          part: { text: finalText, type: "output_text" },
          type: "response.content_part.done",
        });
        ...
        unsubscribe();
        rememberResponseSession();
        writeSseEvent(res, { response: incompleteResponse, type: "response.completed" });
        writeDone(res);
        res.end();
        return;
      }
      ...
      logWarn(`openresponses: streaming response failed: ${String(error)}`);
      ...
      rememberResponseSession();
      writeSseEvent(res, { response: errorResponse, type: "response.failed" });
      emitAgentEvent({
        data: { phase: "error" },
        runId: responseId,
        stream: "lifecycle",
