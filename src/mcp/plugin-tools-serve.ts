    ...
    try {
      ...
      return {
        content: Array.isArray(result.content)
          ? result.content
          : [{ text: String(result.content), type: "text" }],
      };
    } catch (error) {
      return {
        content: [{ text: `Tool error: ${formatErrorMessage(error)}`, type: "text" }],
        isError: true,
      };
    }
  });
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  servePluginToolsMcp().catch((error) => {
    process.stderr.write(`plugin-tools-serve: ${formatErrorMessage(error)}\n`);
    process.exit(1);
  });
}
