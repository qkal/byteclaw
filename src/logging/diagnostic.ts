...
export function logActiveRuns() {
  const activeSessions = [...diagnosticSessionStates.entries()]
...
export function startDiagnosticHeartbeat(
  ...
  heartbeatInterval = setInterval(() => {
    ...
    const activeCount = [...diagnosticSessionStates.values()].filter(
      (s) => s.state === "processing",
    ).length;
    const waitingCount = [...diagnosticSessionStates.values()].filter(
      (s) => s.state === "waiting",
    ).length;
    const totalQueued = [...diagnosticSessionStates.values()].reduce(
    ...
    void loadCommandPollBackoffRuntime()
      ...
      .catch((error) => {
        diag.debug(`command-poll-backoff prune failed: ${String(error)}`);
      });
