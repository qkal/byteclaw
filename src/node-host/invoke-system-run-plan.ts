...
export interface ApprovedCwdSnapshot {
  cwd: string;
  stat: fs.Stats;
}
...
interface FileOperandCollection {
  hits: number[];
...
export function buildSystemRunApprovalPlan(params: {
  ...
  if (!command.ok) {
    return { message: command.message, ok: false };
  }
  if (command.argv.length === 0) {
    return { message: "command required", ok: false };
  }
  ...
  if (!hardening.ok) {
    return { message: hardening.message, ok: false };
  }
  ...
  if (!mutableFileOperand.ok) {
    return { message: mutableFileOperand.message, ok: false };
  }
  return {
    ok: true,
    plan: {
