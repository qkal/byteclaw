...
import { type PdfExtractedImage, extractPdfContent } from "./pdf-extract.js";
import { readResponseWithLimit } from "./read-response-with-limit.js";

export type InputImageContent = PdfExtractedImage;

export interface InputFileExtractResult {
  filename: string;
  text?: string;
  images?: InputImageContent[];
}

export interface InputPdfLimits {
  maxPages: number;
  maxPixels: number;
  minTextChars: number;
}

export interface InputFileLimits {
  allowUrl: boolean;
  urlAllowlist?: string[];
...
export interface InputFileLimitsConfig {
  allowUrl?: boolean;
  allowedMimes?: string[];
  maxBytes?: number;
  maxChars?: number;
...
export interface InputFetchResult {
  buffer: Buffer;
  mimeType: string;
  contentType?: string;
}
...
function decodeTextContent(buffer: Buffer, charset: string | undefined): string {
  const encoding = normalizeOptionalLowercaseString(charset) || "utf8";
  try {
...
export async function extractImageContentFromSource(
  ...
  if (source.type === "base64") {
    rejectOversizedBase64Payload({ data: source.data, label: "Image", maxBytes: limits.maxBytes });
...
export async function extractFileContentFromSource(params: {
  ...
  if (source.type === "base64") {
    rejectOversizedBase64Payload({ data: source.data, label: "File", maxBytes: limits.maxBytes });
    const canonicalData = canonicalizeBase64(source.data);
    if (!canonicalData) {
