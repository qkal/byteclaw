...
async function sipsResizeToJpeg(params: {
  ...
  return await withTempDir(async (dir) => {
    ...
    await runExec(
      ...
      { maxBuffer: 1024 * 1024, timeoutMs: 20_000 },
    );
...
export async function getImageMetadata(buffer: Buffer): Promise<ImageMetadata | null> {
  ...
  try {
    ...
    return validateImagePixelLimit({ height, width });
  } catch {
    return null;
  }
}
...
export async function resizeToJpeg(params: {
  ...
  return await sharp(params.buffer)
    ...
    .jpeg({ mozjpeg: true, quality: params.quality })
    .toBuffer();
}
...
export async function convertHeicToJpeg(buffer: Buffer): Promise<Buffer> {
  ...
  const sharp = await loadSharp();
  return await sharp(buffer).jpeg({ mozjpeg: true, quality: 90 }).toBuffer();
}
    ...
    for (const compressionLevel of compressionLevels) {
      try {
        ...
        if (!smallest || size < smallest.size) {
          smallest = { buffer: out, compressionLevel, resizeSide: side, size };
        }
        if (size <= maxBytes) {
          return {
            buffer: out,
