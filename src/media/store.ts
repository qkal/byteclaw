...
import { type SafeOpenLikeError, isSafeOpenError, readLocalFileSafely } from "./store.runtime.js";
...
interface CleanOldMediaOptions {
  recursive?: boolean;
...
export async function ensureMediaDir() {
  ...
  await fs.mkdir(mediaDir, { mode: 0o700, recursive: true });
  return mediaDir;
}
    ...
    resolvePinnedHostnameImpl(parsedUrl.hostname)
      .then((pinned) => {
        const req = requestImpl(parsedUrl, { headers, lookup: pinned.lookup }, (res) => {
          ...
          pipeline(res, out)
            .then(() => {
              const sniffBuffer = Buffer.concat(sniffChunks, Math.min(sniffLen, 16_384));
...
export async function saveMediaSource(
  ...
  if (looksLikeUrl(source)) {
    ...
    return buildSavedMediaResult({ contentType: mime, dir, id, size });
  }
  // Local path
  try {
    ...
    const id = buildSavedMediaId({ baseId, ext });
    await writeSavedMediaBuffer({ buffer, dir, id });
    return buildSavedMediaResult({ contentType: mime, dir, id, size: stat.size });
  } catch (error) {
    if (isSafeOpenError(error)) {
      throw toSaveMediaSourceError(error);
    }
    throw error;
  }
}
...
export async function saveMediaBuffer(
  ...
  await writeSavedMediaBuffer({ buffer, dir, id });
  return buildSavedMediaResult({ contentType: mime, dir, id, size: buffer.byteLength });
}
