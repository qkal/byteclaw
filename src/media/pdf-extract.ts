...
async function loadCanvasModule(): Promise<CanvasModule> {
  if (!canvasModulePromise) {
    canvasModulePromise = import("@napi-rs/canvas").catch((error) => {
      ...
      throw new Error(
        `Optional dependency @napi-rs/canvas is required for PDF image extraction: ${String(error)}`,
      );
    });
  }
...
async function loadPdfJsModule(): Promise<PdfJsModule> {
  if (!pdfJsModulePromise) {
    pdfJsModulePromise = import("pdfjs-dist/legacy/build/pdf.mjs").catch((error) => {
      ...
      throw new Error(
        `Optional dependency pdfjs-dist is required for PDF extraction: ${String(error)}`,
      );
    });
  }
...
export async function extractPdfContent(params: {
  ...
  for (const pageNum of effectivePages) {
    ...
    images.push({ data: png.toString("base64"), mimeType: "image/png", type: "image" });
  }
