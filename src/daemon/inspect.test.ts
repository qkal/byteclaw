  ...
  it("collects only non-openclaw marker tasks from schtasks output", async () => {
    execSchtasksMock.mockResolvedValueOnce({
      ...
      stdout: [
        ...
        String.raw`Task To Run: C:\Program Files\OpenClaw\openclaw.exe gateway run`,
        "",
        "TaskName: Clawdbot Legacy",
        String.raw`Task To Run: C:\clawdbot\clawdbot.exe run`,
        "",
        "TaskName: Other Task",
        String.raw`Task To Run: C:\tools\helper.exe`,
        "",
      ].join("\n"),
    });
