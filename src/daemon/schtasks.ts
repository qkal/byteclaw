...
export function deriveScheduledTaskRuntimeStatus(parsed: ScheduledTaskInfo): {
  ...
  if (parsed.status?.trim()) {
    return { detail: UNKNOWN_STATUS_DETAIL, status: "unknown" };
  }
...
async function resolveScheduledTaskGatewayListenerPids(port: number): Promise<number[]> {
  ...
  const matchedGatewayPids = [...new Set(diagnostics.listeners.filter((listener) => typeof listener.pid === 'number' && listener.commandLine && isGatewayArgv(parseCmdScriptCommandLine(listener.commandLine), { allowGatewayBinary: true })).map((listener) => listener.pid as number))];
  if (matchedGatewayPids.length > 0) {
    return matchedGatewayPids;
  }

  return [...new Set(diagnostics.listeners.map((listener) => listener.pid).filter((pid): pid is number => typeof pid === 'number' && Number.isFinite(pid) && pid > 0))];
}
...
async function waitForGatewayPortRelease(port: number, timeoutMs = 5000): Promise<boolean> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const diagnostics = await inspectPortUsage(port).catch(() => null);
...
async function terminateBusyPortListeners(port: number): Promise<number[]> {
  ...
  const pids = [...new Set(diagnostics.listeners.map((listener) => listener.pid).filter((pid): pid is number => typeof pid === 'number' && Number.isFinite(pid) && pid > 0))];
  for (const pid of pids) {
    await terminateGatewayProcessTree(pid, 300);
  }
  return pids;
...
async function writeScheduledTaskScript({
  ...
  const taskDescription = resolveGatewayServiceDescription({ description, env, environment });
...
async function activateScheduledTask(params: {
  ...
  if (await updateExistingScheduledTask({ ...params, quotedScript, taskName })) {
    return;
  }
...
export async function stopScheduledTask({ stdout, env }: GatewayServiceControlArgs): Promise<void> {
  ...
  if (stopPort) {
    ...
    if (!released) {
      ...
      const releasedAfterForce = await waitForGatewayPortRelease(stopPort, 2000);
...
export async function restartScheduledTask({
  ...
  if (restartPort) {
    ...
    if (!released) {
      ...
      const releasedAfterForce = await waitForGatewayPortRelease(restartPort, 2000);
      if (!releasedAfterForce) {
