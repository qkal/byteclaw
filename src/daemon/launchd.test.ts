...
function expectLaunchctlEnableBootstrapOrder(env: Record<string, string | undefined>) {
  ...
  return { bootstrapIndex, domain, label, serviceId };
}
...
vi.mock("./exec-file.js", () => ({
  execFileUtf8: vi.fn(async (file: string, args: string[]) => {
    ...
    if (call[0] === "list") {
      return { code: 0, stderr: "", stdout: state.listOutput };
    }
    if (call[0] === "print") {
      if (state.printNotLoadedRemaining > 0) {
        state.printNotLoadedRemaining -= 1;
        return { code: 113, stderr: "Could not find service", stdout: "" };
      }
      return { code: 0, stderr: "", stdout: state.printOutput };
    }
    if (call[0] === "bootstrap" && state.bootstrapError) {
      return { code: state.bootstrapCode, stderr: state.bootstrapError, stdout: "" };
    }
    if (call[0] === "kickstart" && state.kickstartError && state.kickstartFailuresRemaining > 0) {
      state.kickstartFailuresRemaining -= 1;
      return { code: 1, stderr: state.kickstartError, stdout: "" };
    }
...
describe("launchd install", () => {
  ...
  it("restarts LaunchAgent with kickstart and no bootout", async () => {
    ...
    expect(cleanStaleGatewayProcessesSync).toHaveBeenCalledWith(18_789);
  ...
  it("uses the configured gateway port for stale cleanup", async () => {
    ...
    expect(cleanStaleGatewayProcessesSync).toHaveBeenCalledWith(19_001);
  });
