...
async function ensureSecureDirectory(targetPath: string): Promise<void> {
  await fs.mkdir(targetPath, { mode: LAUNCH_AGENT_DIR_MODE, recursive: true });
  try {
...
export async function repairLaunchAgentBootstrap(args: {
  ...
  if (boot.code !== 0) {
    ...
    if (!alreadyLoaded) {
      return { detail: detail || undefined, ok: false, status: "bootstrap-failed" };
    }
...
export async function findLegacyLaunchAgents(env: GatewayServiceEnv): Promise<LegacyLaunchAgent[]> {
  ...
  for (const label of resolveLegacyGatewayLaunchAgentLabels(env.OPENCLAW_PROFILE)) {
    ...
    if (loaded || exists) {
      results.push({ exists, label, loaded, plistPath });
    }
  }
  return results;
}
...
async function writeLaunchAgentPlist({
  ...
  const serviceDescription = resolveGatewayServiceDescription({ description, env, environment });
...
export async function restartLaunchAgent({
  ...
  if (!isLaunchctlNotLoaded(start)) {
    await ensureLaunchAgentLoadedAfterFailure({ domain, plistPath, serviceTarget });
