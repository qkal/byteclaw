...
function readQuantifier(source: string, index: number): QuantifierRead | null {
  ...
  if (ch === "*") {
    return { consumed, maxRepeat: null, minRepeat: 0 };
  }
  if (ch === "+") {
    return { consumed, maxRepeat: null, minRepeat: 1 };
  }
  if (ch === "?") {
    return { consumed, maxRepeat: 1, minRepeat: 0 };
  }
  if (ch !== "{") {
    return null;
  }
  ...
  return { consumed: i - index, maxRepeat, minRepeat };
}
...
export function compileSafeRegexDetailed(source: string, flags = ""): SafeRegexCompileResult {
  ...
  if (!trimmed) {
    return { flags, reason: "empty", regex: null, source: trimmed };
  }
  ...
  if (hasNestedRepetition(trimmed)) {
    result = { flags, reason: "unsafe-nested-repetition", regex: null, source: trimmed };
  } else {
    try {
      result = { flags, reason: null, regex: new RegExp(trimmed, flags), source: trimmed };
    } catch {
      result = { flags, reason: "invalid-regex", regex: null, source: trimmed };
    }
  }
