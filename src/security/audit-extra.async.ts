...
import { type ExecDockerRawResult, execDockerRaw } from "../agents/sandbox/docker.js";
...
async function listInstalledPluginDirs(params: {
  ...
  const entries = await fs.readdir(extensionsDir, { withFileTypes: true }).catch((error) => {
...
function resolveToolPolicies(params: {
  ...
  const policies: (SandboxToolPolicy | undefined)[] = [
    profilePolicy,
...
export async function collectStateDeepFilesystemFindings(params: {
  ...
  const ids = [...new Set([defaultAgentId, ...agentIds])].map((id) => normalizeAgentId(id));
...
export async function collectPluginsCodeSafetyFindings(params: {
  ...
  for (const pluginName of pluginDirs) {
    ...
    const summary = await getCodeSafetySummary({
      ...
      findings.push({
        checkId: "plugins.code_safety.scan_failed",
        detail: `Static code scan could not complete: ${String(error)}`,
        remediation:
          "Check file permissions and plugin layout, then rerun `openclaw security audit --deep`.",
        severity: "warn",
    ...
    for (const entry of entries) {
      ...
      const summary = await getCodeSafetySummary({
        ...
        findings.push({
          ...
          detail: `Static code scan could not complete for ${skillDir}: ${String(error)}`,
          remediation:
