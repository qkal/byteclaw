...
export interface WindowsAclSummary {
  ok: boolean;
...
const WORLD_PRINCIPALS = new Set([
  "everyone",
  "users",
  String.raw`builtin\users`,
  "authenticated users",
  String.raw`nt authority\authenticated users`,
]);
const TRUSTED_BASE = new Set([
  ...
  String.raw`autorite nt\système`,
  String.raw`nt-autorität\system`,
  String.raw`autoridad nt\system`,
  String.raw`autoridade nt\system`,
]);
const WORLD_SUFFIXES = [String.raw`\users`, String.raw`\authenticated users`];
const TRUSTED_SUFFIXES = [String.raw`\administrators`, String.raw`\system`, String.raw`\système`];
...
function parseAceEntry(entry: string): WindowsAclEntry | null {
  ...
  return { canRead, canWrite, principal, rawRights, rights };
}
...
export function summarizeWindowsAcl(
  ...
  return { trusted, untrustedGroup, untrustedWorld };
}
...
export async function inspectWindowsAcl(
  ...
  try {
    ...
    return { entries, ok: true, trusted, untrustedGroup, untrustedWorld };
  } catch (error) {
    return {
      entries: [],
      error: String(error),
