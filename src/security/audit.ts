...
interface AuditExecutionContext {
  cfg: OpenClawConfig;
  sourceConfig: OpenClawConfig;
  env: NodeJS.ProcessEnv;
  platform: NodeJS.Platform;
...
function countBySeverity(findings: SecurityAuditFinding[]): SecurityAuditSummary {
  ...
  return { critical, info, warn };
}

function normalizeAllowFromList(list: (string | number)[] | undefined | null): string[] {
        ...
        return [
          {
            ...
            detail: `${entry.pluginId}: ${String(error)}`,
...
async function maybeProbeGateway(params: {
  ...
  const res = await params
    .probe({ auth: authResolution.auth, timeoutMs: params.timeoutMs, url })
    .catch((error) => ({
      close: null,
      configSnapshot: null,
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  findings.push(...auditNonDeep.collectSyncedFolderFindings({ configPath, stateDir }));
  ...
  findings.push(...collectDeepProbeFindings({ authWarning: deepProbeResult?.authWarning, deep }));

  const summary = countBySeverity(findings);
  return { deep, findings, summary, ts: Date.now() };
}
