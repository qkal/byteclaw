...
const LEGACY_MODEL_PATTERNS: { id: string; re: RegExp; label: string }[] = [
  { id: "openai.gpt35", label: "GPT-3.5 family", re: /\bgpt-3\.5\b/i },
  { id: "anthropic.claude2", label: "Claude 2/Instant family", re: /\bclaude-(instant|2)\b/i },
  { id: "openai.gpt4_legacy", label: "Legacy GPT-4 snapshots", re: /\bgpt-4-(0314|0613)\b/i },
];

const WEAK_TIER_MODEL_PATTERNS: { id: string; re: RegExp; label: string }[] = [
  { id: "anthropic.haiku", label: "Haiku tier (smaller model)", re: /\bhaiku\b/i },
];
...
function collectRiskyToolExposureContexts(cfg: OpenClawConfig): {
  ...
  return { hasRuntimeRisk, riskyContexts };
}
...
export function collectGatewayHttpNoAuthFindings(
  ...
  const findings: SecurityAuditFinding[] = [];
  const tailscaleMode = cfg.gateway?.tailscale?.mode ?? "off";
  const auth = resolveGatewayAuth({ authConfig: cfg.gateway?.auth, env, tailscaleMode });
  if (auth.mode !== "none") {
    return findings;
  }
...
export function collectSandboxDangerousConfigFindings(cfg: OpenClawConfig): SecurityAuditFinding[] {
  ...
  const configs: { source: string; docker: Record<string, unknown> }[] = [];
...
export function collectNodeDenyCommandPatternFindings(cfg: OpenClawConfig): SecurityAuditFinding[] {
  ...
  const examples = [...knownCommands].slice(0, 8);
...
export function collectModelHygieneFindings(cfg: OpenClawConfig): SecurityAuditFinding[] {
  ...
  const addWeakMatch = (model: string, source: string, reason: string) => {
    ...
    if (!existing) {
      weakMatches.set(key, { model, reasons: [reason], source });
      return;
    }
  ...
  const matches: { model: string; source: string; reason: string }[] = [];
  for (const entry of models) {
    for (const pat of LEGACY_MODEL_PATTERNS) {
      if (pat.re.test(entry.id)) {
        matches.push({ model: entry.id, reason: pat.label, source: entry.source });
        break;
      }
    }
  }
  ...
  if (weakMatches.size > 0) {
    const lines = [...weakMatches.values()]
      .slice(0, 12)
