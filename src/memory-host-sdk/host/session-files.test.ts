...
describe("buildSessionEntry", () => {
  it("returns lineMap tracking original JSONL line numbers", async () => {
    ...
    // Line 7: user message
    const jsonlLines = [
      JSON.stringify({ customType: "model-snapshot", data: {}, type: "custom" }),
      JSON.stringify({ customType: "openclaw.cache-ttl", data: {}, type: "custom" }),
      JSON.stringify({ agentId: "test", type: "session-meta" }),
      JSON.stringify({ message: { content: "Hello world", role: "user" }, type: "message" }),
      JSON.stringify({ customType: "tool-result", data: {}, type: "custom" }),
      JSON.stringify({
        message: { content: "Hi there, how can I help?", role: "assistant" },
        type: "message",
      }),
      JSON.stringify({ message: { content: "Tell me a joke", role: "user" }, type: "message" }),
    ];
  ...
  it("skips blank lines and invalid JSON without breaking lineMap", async () => {
    const jsonlLines = [
      "",
      "not valid json",
      JSON.stringify({ message: { content: "First", role: "user" }, type: "message" }),
      "",
      JSON.stringify({ message: { content: "Second", role: "assistant" }, type: "message" }),
    ];
