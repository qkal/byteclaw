...
describe("bedrock embedding provider", () => {
  ...
  it("detects credentials via the AWS SDK default provider chain", async () => {
    ...
    expect(defaultProviderMock).toHaveBeenCalledWith({ maxRetries: 0, timeout: 1000 });
  });
  ...
  it("embeds with Titan V2", async () => {
    ...
    expect(reqBody()).toMatchObject({ dimensions: 1024, inputText: "test", normalize: true });
  });
  ...
  it("embeds Cohere V3 batch in single call", async () => {
    ...
    expect(reqBody()).toMatchObject({ input_type: "search_document", texts: ["a", "b"] });
  });
  ...
  it("embeds Nova with SINGLE_EMBEDDING format", async () => {
    sendMock.mockResolvedValue(
      enc({ embeddings: [{ embedding: [0.1, 0.2], embeddingType: "TEXT" }] }),
    );
    const { provider } = await createBedrockEmbeddingProvider({
      config: {} as never,
      fallback: "none",
  ...
  it("batches Nova concurrently", async () => {
    sendMock
      .mockResolvedValueOnce(enc({ embeddings: [{ embedding: [0.1], embeddingType: "TEXT" }] }))
      .mockResolvedValueOnce(enc({ embeddings: [{ embedding: [0.2], embeddingType: "TEXT" }] }));
    const { provider } = await createBedrockEmbeddingProvider({
      config: {} as never,
      fallback: "none",
      model: "amazon.nova-2-multimodal-embeddings-v1:0",
