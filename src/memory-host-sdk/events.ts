...
export interface MemoryHostRecallRecordedEvent {
...
export interface MemoryHostPromotionAppliedEvent {
...
export interface MemoryHostDreamCompletedEvent {
...
export async function readMemoryHostEvents(params: {
  ...
  const raw = await fs.readFile(eventLogPath, "utf8").catch((error: unknown) => {
