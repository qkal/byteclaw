...
function notifyActiveTaskWaiters(): void {
  const queueState = getQueueState();
  for (const waiter of [...queueState.activeTaskWaiters]) {
      ...
      while (state.activeTaskIds.size < state.maxConcurrent && state.queue.length > 0) {
        ...
        if (waitedMs >= entry.warnAfterMs) {
          try {
            ...
            diag.error(`lane onWait callback failed: lane=${lane} error="${String(error)}"`);
          }
          diag.warn(
        ...
        void (async () => {
          ...
          try {
            ...
            if (!isProbeLane && !isExpectedNonErrorLaneFailure(error)) {
              diag.error(
                `lane task error: lane=${lane} durationMs=${Date.now() - startTime} error="${String(error)}"`,
              );
            } else if (!isProbeLane) {
              diag.debug(
                `lane task interrupted: lane=${lane} durationMs=${Date.now() - startTime} reason="${String(error)}"`,
              );
            }
            if (completedCurrentGeneration) {
              notifyActiveTaskWaiters();
