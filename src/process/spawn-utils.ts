...
interface SpawnWithFallbackParams {
  argv: string[];
  options: SpawnOptions;
...
export async function spawnWithFallback(
  ...
  const attempts: { label?: string; options: SpawnOptions }[] = [
    { options: baseOptions },
  ...
  for (let index = 0; index < attempts.length; index += 1) {
    ...
    try {
      ...
      if (!nextFallback || !shouldRetry(error, retryCodes)) {
        throw error;
      }
      params.onFallback?.(error, nextFallback);
    }
  }
