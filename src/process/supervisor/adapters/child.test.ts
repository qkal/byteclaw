...
function createStubChild(pid = 1234) {
  ...
  Object.defineProperty(child, "signalCode", { configurable: true, value: null, writable: true });
  ...
  return { child, emitClose, emitExit, killMock };
}
...
describe("createChildAdapter", () => {
  ...
  it("uses process-tree kill for default SIGKILL", async () => {
    ...
    const spawnArgs = spawnWithFallbackMock.mock.calls[0]?.[0] as {
      ...
      fallbacks?: { options?: { detached?: boolean } }[];
    };
  ...
  it("passes explicit env overrides as strings", async () => {
    await createAdapterHarness({
      argv: ["node", "-e", "process.exit(0)"],
      env: { COUNT: "12", DROP_ME: undefined, FOO: "bar" },
      pid: 4444,
    });
    ...
    expect(spawnArgs.options?.env).toEqual({ COUNT: "12", FOO: "bar" });
  });
});
