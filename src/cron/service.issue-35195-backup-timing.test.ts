...
describe("cron backup timing for edit", () => {
  it("keeps .bak as the pre-edit store even after later normalization persists", async () => {
    ...
    await writeCronStoreSnapshot(store.storePath, [
      {
        ...
        schedule: { anchorMs: base, everyMs: 60_000, kind: "every" },
        sessionTarget: "main",
        state: {},
        updatedAtMs: base,
    ...
    const beforeEditRaw = await fs.readFile(store.storePath, "utf8");

    await service.update("job-35195", {
      payload: { kind: "systemEvent", text: "edited" },
    });

    const backupRaw = await fs.readFile(`${store.storePath}.bak`, "utf8");
    expect(JSON.parse(backupRaw)).toEqual(JSON.parse(beforeEditRaw));

    const diskAfterEdit = JSON.parse(await fs.readFile(store.storePath, "utf8"));
    const normalizedJob = {
    ...
    const backupAfterNormalize = await fs.readFile(`${store.storePath}.bak`, "utf8");
