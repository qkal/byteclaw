...
export async function sweepCronRunSessions(params: {
  ...
  try {
    ...
    params.log.warn({ err: String(error) }, "cron-reaper: failed to sweep session store");
    return { pruned: 0, swept: false };
  }
  ...
  if (prunedSessions.size > 0) {
    try {
      ...
      params.log.warn({ err: String(error) }, "cron-reaper: transcript cleanup failed");
    }
  }
