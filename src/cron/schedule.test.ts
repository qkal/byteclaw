...
describe("cron schedule", () => {
  ...
  it("computes next run for cron expression with timezone", () => {
    ...
    const next = computeNextRunAtMs(
      { expr: "0 9 * * 3", kind: "cron", tz: "America/Los_Angeles" },
      nowMs,
    );
  ...
  it("does not roll back year for Asia/Shanghai daily cron schedules (#30351)", () => {
    ...
    const next = computeNextRunAtMs(
      { expr: "0 8 * * *", kind: "cron", tz: "Asia/Shanghai" },
      nowMs,
    );
  ...
  it("computes next run for every schedule", () => {
    ...
    const next = computeNextRunAtMs({ anchorMs: anchor, everyMs: 30_000, kind: "every" }, now);
    expect(next).toBe(anchor + 30_000);
  });

  it("computes next run for every schedule when anchorMs is not provided", () => {
    const now = Date.parse("2025-12-13T00:00:00.000Z");
    const next = computeNextRunAtMs({ everyMs: 30_000, kind: "every" }, now);
  ...
  it("returns undefined for non-numeric string everyMs", () => {
    const now = Date.now();
    const next = computeNextRunAtMs({ everyMs: "abc" as unknown as number, kind: "every" }, now);
    expect(next).toBeUndefined();
  });

  it("advances when now matches anchor for every schedule", () => {
    const anchor = Date.parse("2025-12-13T00:00:00.000Z");
    const next = computeNextRunAtMs({ anchorMs: anchor, everyMs: 30_000, kind: "every" }, anchor);
    expect(next).toBe(anchor + 30_000);
  });
  ...
  it("reuses compiled cron evaluators for the same expression/timezone", () => {
    ...
    const third = computeNextRunAtMs({ expr: "0 8 * * *", kind: "cron", tz: "UTC" }, nowMs);
  ...
  describe("cron with specific seconds (6-field pattern)", () => {
    ...
    const dailyNoon = { expr: "0 0 12 * * *", kind: "cron" as const, tz: "UTC" };
