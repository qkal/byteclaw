...
describe("CronService - armTimer tight loop prevention", () => {
  function extractTimeoutDelays(timeoutSpy: ReturnType<typeof vi.spyOn>) {
    const calls = timeoutSpy.mock.calls as [unknown, unknown, ...unknown[]][];
    return calls
      ...
      jobs: [
        {
          ...
          updatedAtMs: now,
          wakeMode: "next-heartbeat" as const, // 10 seconds in the future
        },
      ],
      version: 1,
    };
  ...
  it("breaks the onTimer→armTimer hot-loop with stuck runningAtMs", async () => {
    ...
    expect(lastDelay).toBeGreaterThanOrEqual(2000);
