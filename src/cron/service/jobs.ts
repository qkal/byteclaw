...
function computeStaggeredCronPreviousRunAtMs(job: CronJob, nowMs: number) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    cursorMs = Math.max(0, basePrevious - 1000);
  }
  return undefined;
}
...
export function computeJobNextRunAtMs(job: CronJob, nowMs: number): number | undefined {
  ...
  if (job.schedule.kind === "every") {
    ...
    const next = computeNextRunAtMs({ ...job.schedule, anchorMs, everyMs }, nowMs);
...
export function recordScheduleComputeError(params: {
  ...
  if (errorCount >= MAX_SCHEDULE_ERRORS) {
    ...
    state.deps.log.error(
      { err: errText, errorCount, jobId: job.id, name: job.name },
...
function walkSchedulableJobs(
  ...
  for (const job of state.store.jobs) {
    const tick = normalizeJobTickState({ job, nowMs, state });
    if (tick.changed) {
      changed = true;
    }
    if (tick.skip) {
...
function recomputeJobNextRunAtMs(params: { state: CronServiceState; job: CronJob; nowMs: number }) {
  ...
  try {
    ...
    if (recordScheduleComputeError({ error, job: params.job, state: params.state })) {
      changed = true;
    }
  }
  return changed;
...
export function recomputeNextRuns(state: CronServiceState): boolean {
  return walkSchedulableJobs(state, ({ job, nowMs: now }) => {
    ...
    if (isDueOrMissing) {
      if (recomputeJobNextRunAtMs({ job, nowMs: now, state })) {
        changed = true;
      }
    }
    return changed;
