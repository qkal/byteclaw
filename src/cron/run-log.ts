...
export interface ReadCronRunLogPageOptions {
  limit?: number;
  offset?: number;
  jobId?: string;
...
export async function appendCronRunLog(
  ...
  const next = prev
    ...
    .then(async () => {
      ...
      await fs.mkdir(runDir, { mode: 0o700, recursive: true });
...
export async function readCronRunLogEntriesPage(
  ...
  const raw = await fs.readFile(path.resolve(filePath), "utf8").catch(() => "");
