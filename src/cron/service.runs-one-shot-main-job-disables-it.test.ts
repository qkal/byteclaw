...
function setFile(filePath: string, content: string) {
  ...
  fsState.entries.set(resolved, { content, kind: "file", mtimeMs: bumpMtimeMs() });
}
...
async function makeStorePath() {
  ...
  return { cleanup: async () => {}, storePath };
}
...
vi.mock("node:fs", async () => {
  ...
  const promises = {
    ...
    readFile: async (p: string) => {
      if (!isFixtureInMock(p)) {
        return await actual.promises.readFile(p, "utf8");
      }
      const entry = fsState.entries.get(absInMock(p));
      if (!entry || entry.kind !== "file") {
    ...
    writeFile: async (p: string, data: string | Uint8Array) => {
      if (!isFixtureInMock(p)) {
        return await actual.promises.writeFile(p, data, "utf8");
      }
...
vi.mock("node:fs/promises", async () => {
  ...
  const wrapped = {
    ...
    writeFile: async (p: string, data: string, _enc?: unknown) => {
      if (!isFixturePath(p)) {
        return await actual.writeFile(p, data, "utf8");
      }
      setFile(p, data);
    },
  };
...
function createCronEventHarness() {
  ...
  const waitFor = (predicate: (evt: CronEvent) => boolean) => {
    ...
    waiters.push({ deferred, predicate });
...
async function createCronHarness(options: CronHarnessOptions = {}) {
  ...
  await cron.start();
  return { cron, enqueueSystemEvent, events, requestHeartbeatNow, store };
}
...
async function addDefaultIsolatedAnnounceJob(cron: CronService, name: string) {
  ...
  const job = await cron.add({
    ...
    schedule: { at: runAt.toISOString(), kind: "at" },
...
async function addMainOneShotHelloJob(
  ...
  return cron.add({
    ...
    schedule: { at: new Date(params.atMs).toISOString(), kind: "at" },
    sessionTarget: "main",
    wakeMode: "now",
    payload: { kind: "systemEvent", text: "hello" },
  });
...
describe("CronService", () => {
  ...
  it("wakeMode now waits for heartbeat completion when available", async () => {
    ...
    if (typeof resolveHeartbeat === "function") {
      (resolveHeartbeat as (res: HeartbeatRunResult) => void)({ durationMs: 123, status: "ran" });
    }
    await runPromise;
  ...
  it("rejects sessionTarget main for non-default agents at creation time", async () => {
    const runHeartbeatOnce = vi.fn(async () => ({ durationMs: 1, status: "ran" as const }));
  ...
  it("rejects unsupported session/payload combinations", async () => {
    ...
    await expect(
      cron.add({
        ...
        schedule: { everyMs: 1000, kind: "every" },
        sessionTarget: "main",
