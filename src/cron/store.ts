...
export async function loadCronStore(storePath: string): Promise<CronStoreFile> {
  try {
    ...
    try {
      ...
      throw new Error(`Failed to parse cron store at ${storePath}: ${String(error)}`, {
        cause: error,
      });
    }
    const parsedRecord =
...
interface SaveCronStoreOptions {
  skipBackup?: boolean;
}
...
export async function saveCronStore(
  ...
  await fs.promises.mkdir(storeDir, { mode: 0o700, recursive: true });
  ...
  if (previous === null) {
    try {
      previous = await fs.promises.readFile(storePath, "utf8");
    } catch (error) {
      if ((error as { code?: unknown }).code !== "ENOENT") {
        throw error;
      }
