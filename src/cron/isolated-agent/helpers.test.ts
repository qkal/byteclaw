...
describe("pickSummaryFromPayloads", () => {
  ...
  it("treats isError: undefined as non-error", () => {
    const payloads = [
      { isError: undefined, text: "normal text" },
      { isError: true, text: "error text" },
    ];
...
describe("pickLastNonEmptyTextFromPayloads", () => {
  it("picks real text over error payload", () => {
    const payloads = [{ text: "Real output" }, { isError: true, text: "Service error" }];
    expect(pickLastNonEmptyTextFromPayloads(payloads)).toBe("Real output");
  });

  it("falls back to error payload when no real text exists", () => {
    const payloads = [{ isError: true, text: "Service error" }];
...
describe("pickLastDeliverablePayload", () => {
  it("picks real payload over error payload", () => {
    ...
    const error = { isError: true as const, text: "Error warning" };
  ...
  it("treats isError: undefined as non-error", () => {
    const normal = { isError: undefined, text: "ok" };
    const error = { isError: true as const, text: "bad" };
...
describe("pickDeliverablePayloads", () => {
  it("preserves all successful deliverable payloads", () => {
    const payloads = [
      { text: "line 1" },
      { isError: true as const, text: "temporary error" },
      { text: "line 2" },
    ];
  ...
  it("returns only the last error payload when all payloads are errors", () => {
    const payloads = [
      { isError: true as const, text: "first error" },
      { isError: true as const, text: "last error" },
    ];

    expect(pickDeliverablePayloads(payloads)).toEqual([{ isError: true, text: "last error" }]);
  });
});
...
describe("isHeartbeatOnlyResponse", () => {
  ...
  it("returns false when media is present even with HEARTBEAT_OK text", () => {
    expect(
      isHeartbeatOnlyResponse(
        [{ mediaUrl: "https://example.com/img.png", text: "HEARTBEAT_OK" }],
        ACK_MAX,
      ),
    ).toBe(false);
  });
  ...
  it("returns false when media is in a different payload than HEARTBEAT_OK", () => {
    expect(
      isHeartbeatOnlyResponse(
        [
          { text: "HEARTBEAT_OK" },
          { mediaUrl: "https://example.com/img.png", text: "Here's an image" },
        ],
        ACK_MAX,
      ),
    ).toBe(false);
