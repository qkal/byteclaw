...
describe("cron store", () => {
  it("returns empty store when file does not exist", async () => {
    ...
    expect(loaded).toEqual({ jobs: [], version: 1 });
  });

  it("throws when store contains invalid JSON", async () => {
    const store = await makeStorePath();
    await fs.mkdir(path.dirname(store.storePath), { recursive: true });
    await fs.writeFile(store.storePath, "{ not json", "utf8");
  ...
  it("backs up previous content before replacing the store", async () => {
    ...
    const currentRaw = await fs.readFile(store.storePath, "utf8");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      await fs.mkdir(storeDir, { mode: 0o755, recursive: true });
...
describe("saveCronStore", () => {
  const dummyStore: CronStoreFile = { jobs: [], version: 1 };
