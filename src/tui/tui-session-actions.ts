...
interface SessionActionBtwPresenter {
  clear: () => void;
}
...
export function createSessionActions(context: SessionActionContext) {
  ...
  const refreshAgents = async () => {
    try {
      ...
      chatLog.addSystem(`agents list failed: ${String(error)}`);
    }
  };
  ...
  const loadHistory = async () => {
    try {
      ...
      chatLog.addSystem(`history failed: ${String(error)}`);
    }
  ...
  const abortActive = async () => {
    ...
    try {
      ...
      chatLog.addSystem(`abort failed: ${String(error)}`);
