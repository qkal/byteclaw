...
describe("extractThinkingFromMessage", () => {
  it("collects only thinking blocks", () => {
    const text = extractThinkingFromMessage({
      content: [
        { thinking: "alpha", type: "thinking" },
        { text: "hello", type: "text" },
        { thinking: "beta", type: "thinking" },
      ],
      role: "assistant",
    });
...
describe("sanitizeRenderableText", () => {
  ...
  it.each([
    { input: "a".repeat(140), label: "very long" },
    { input: "b".repeat(90), label: "moderately long" },
