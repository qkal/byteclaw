...
describe("tui slash commands", () => {
  it("treats /elev as an alias for /elevated", () => {
    expect(parseCommand("/elev on")).toEqual({ args: "on", name: "elevated" });
  });
...
describe("resolveCtrlCAction", () => {
  it("clears input and arms exit on first ctrl+c when editor has text", () => {
    expect(resolveCtrlCAction({ hasInput: true, lastCtrlCAt: 0, now: 2000 })).toEqual({
      action: "clear",
  ...
  it("exits on second ctrl+c within the exit window", () => {
    expect(resolveCtrlCAction({ hasInput: false, lastCtrlCAt: 2000, now: 2800 })).toEqual({
      action: "exit",
      nextLastCtrlCAt: 2000,
    });
  });
  ...
  it("shows warning when exit window has elapsed", () => {
    expect(resolveCtrlCAction({ hasInput: false, lastCtrlCAt: 2000, now: 3501 })).toEqual({
      action: "warn",
