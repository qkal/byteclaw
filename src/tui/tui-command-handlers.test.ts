...
function createHarness(params?: {
  ...
  const { handleCommand } = createCommandHandlers({
    ...
    chatLog: { addSystem, addUser } as never,
    client: { getGatewayStatus, patchSession, resetSession, sendChat } as never,
    closeOverlay: vi.fn(),
  ...
  it("keeps gateway diagnostics on /gateway-status", async () => {
    const { handleCommand, getGatewayStatus, addSystem, addUser, sendChat } = createHarness({
      getGatewayStatus: vi.fn().mockResolvedValue({
        ...
        sessions: { count: 2, defaults: { contextTokens: 200_000, model: "gpt-5.4" } },
      }),
    });
