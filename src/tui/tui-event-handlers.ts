...
interface EventHandlerTui {
  requestRender: () => void;
}

interface EventHandlerBtwPresenter {
...
export function createEventHandlers(context: EventHandlerContext) {
  ...
  const handleChatEvent = (payload: unknown) => {
    ...
    if (evt.state === "final") {
      ...
      if (!evt.message) {
        ...
        finalizeRun({ runId: evt.runId, status: "idle", wasActiveRun });
        tui.requestRender();
        return;
      }
    ...
    if (evt.state === "aborted") {
      ...
      terminateRun({ runId: evt.runId, status: "aborted", wasActiveRun });
      maybeRefreshHistoryForRun(evt.runId);
    }
    if (evt.state === "error") {
      forgetLocalBtwRunId?.(evt.runId);
      ...
      terminateRun({ runId: evt.runId, status: "error", wasActiveRun });
      maybeRefreshHistoryForRun(evt.runId);
    }
    tui.requestRender();
  };
  ...
  return { handleAgentEvent, handleBtwEvent, handleChatEvent };
}
