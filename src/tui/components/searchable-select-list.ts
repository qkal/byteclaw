...
export class SearchableSelectList implements Component {
  ...
  private splitAnsiParts(text: string): { text: string; isAnsi: boolean }[] {
    ...
    while ((match = ANSI_SGR_REGEX.exec(text)) !== null) {
      if (match.index > lastIndex) {
        parts.push({ isAnsi: false, text: text.slice(lastIndex, match.index) });
      }
      parts.push({ isAnsi: true, text: match[0] });
      lastIndex = match.index + match[0].length;
    }
    if (lastIndex < text.length) {
      parts.push({ isAnsi: false, text: text.slice(lastIndex) });
    }
    return parts;
  }
  ...
  private highlightMatch(text: string, query: string): string {
    ...
    const uniqueTokens = [...new Set(tokens)].toSorted((a, b) => b.length - a.length);
    let parts = this.splitAnsiParts(text);
    for (const token of uniqueTokens) {
      const regex = this.getCachedRegex(token);
      const nextParts: { text: string; isAnsi: boolean }[] = [];
