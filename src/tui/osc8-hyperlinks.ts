...
const SGR_PATTERN = String.raw`\x1b\[[0-9;]*m`;
const OSC8_PATTERN = String.raw`\x1b\]8;;.*?(?:\x07|\x1b\\)`;
...
function findUrlRanges(
  ...
  if (pending) {
    ...
    if (matchLen > 0) {
      ...
      if (pending.consumed + matchLen < pending.url.length) {
        newPending = { consumed: pending.consumed + matchLen, url: pending.url };
      }
    }
  }
  ...
  while ((match = urlRe.exec(visibleText)) !== null) {
    ...
    ranges.push({ end: start + fragment.length, start, url: resolvedUrl });
    ...
    if (resolvedUrl.length > fragment.length && resolvedUrl.startsWith(fragment)) {
      newPending = { consumed: fragment.length, url: resolvedUrl };
    }
  }

  return { pending: newPending, ranges };
}
