...
function bindingMatchIdentityKey(match: AgentRouteBinding["match"]) {
  const roles = Array.isArray(match.roles)
    ? [...new Set(normalizeStringEntries(match.roles).toSorted())]
    : [];
  return [
    match.channel,
    match.peer?.kind ?? "",
...
export function applyAgentBindings(
  ...
  const conflicts: { binding: AgentRouteBinding; existingAgentId: string }[] = [];
  ...
  if (added.length === 0 && updated.length === 0) {
    return { added, config: cfg, conflicts, skipped, updated };
  }
...
export function removeAgentBindings(
  ...
  if (removeIndexes.size === 0) {
    return { config: cfg, conflicts, missing, removed };
  }
...
function resolveDefaultAccountId(cfg: OpenClawConfig, provider: ChannelId): string {
  ...
  return resolveChannelDefaultAccountId({ cfg, plugin });
}
...
export function buildChannelBindings(params: {
  ...
  for (const channel of params.selection) {
    ...
    bindings.push({ agentId, match, type: "route" });
  }
  return bindings;
}
