...
export interface GatewayStatusTarget {
  id: string;
  kind: TargetKind;
  url: string;
  active: boolean;
...
export function resolveTargets(cfg: OpenClawConfig, explicitUrl?: string): GatewayStatusTarget[] {
  ...
  if (explicit) {
    add({ active: true, id: "explicit", kind: "explicit", url: explicit });
  }
...
export async function resolveAuthForTarget(
  ...
  if (tokenOverride || passwordOverride) {
    return { password: passwordOverride, token: tokenOverride };
  }
...
export function extractConfigSummary(snapshotUnknown: unknown): GatewayConfigSummary {
  ...
  return {
    ...
    issues: issuesRaw
      ...
      .map((i) => ({ message: i.message, path: i.path })),
