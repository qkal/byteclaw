...
import { type SessionLockInspection, cleanStaleLockFiles } from "../agents/session-write-lock.js";
...
export async function noteSessionLockHealth(params?: { shouldRepair?: boolean; staleMs?: number }) {
  ...
  try {
    ...
    note(`- Failed to inspect session lock files: ${String(error)}`, "Session locks");
    return;
  }
