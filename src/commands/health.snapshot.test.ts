...
interface TelegramHealthAccount {
  accountId: string;
  token: string;
  configured: boolean;
  config: {
...
function stubTelegramFetchOk(calls: string[]) {
  vi.stubGlobal(
    ...
    vi.fn(async (url: string) => {
      ...
      return {
        json: async () => ({ description: "nope", ok: false }),
        ok: false,
        status: 404,
      } as unknown as Response;
    }),
  ...
  return {
    ...
    config: {
      ...
      resolveAccount: (cfg, accountId) =>
        resolveTelegramAccountForTest({ accountId, cfg: cfg as Record<string, unknown> }),
    },
    status: {
...
describe("getHealthSnapshot", () => {
  ...
  beforeEach(() => {
    setActivePluginRegistry(
      createTestRegistry([
        { plugin: createTelegramHealthPlugin(), pluginId: "telegram", source: "test" },
      ]),
    );
  });
  ...
  it("returns a structured telegram probe error when getMe fails", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string) => {
        if (url.includes("/getMe")) {
          return {
            json: async () => ({ description: "unauthorized", ok: false }),
            ok: false,
            status: 401,
          } as unknown as Response;
        }
    ...
    testConfig = {
      agents: {
        ...
        list: [
          ...
          { heartbeat: { every: "1h", target: "whatsapp" }, id: "ops" },
        ],
      },
    };
    testStore = {};

    const snap = await getHealthSnapshot({ probe: false, timeoutMs: 10 });
