...
export interface ExecSafeBinCoverageHit {
  scopePath: string;
  bin: string;
...
export interface ExecSafeBinTrustedDirHintHit {
  scopePath: string;
  bin: string;
  resolvedPath: string;
}
...
export function maybeRepairExecSafeBinProfiles(cfg: OpenClawConfig): {
  ...
  if (changes.length === 0 && warnings.length === 0) {
    return { changes: [], config: cfg, warnings: [] };
  }
  return { changes, config: next, warnings };
}
