...
import { type AllowFromMode, resolveAllowFromMode } from "./allow-from-mode.js";
...
export async function maybeRepairAllowlistPolicyAllowFrom(cfg: OpenClawConfig): Promise<{
    ...
    if (params.mode === "topOrNested") {
      ...
      const dm =
        dmEntry && typeof dmEntry === "object" && !Array.isArray(dmEntry)
          ? (dmEntry as Record<string, unknown>)
          : undefined;
      const nestedAllowFrom = dm?.allowFrom as (string | number)[] | undefined;
  ...
  const recoverAllowFromForAccount = async (params: {
    ...
    const topAllowFrom = params.account.allowFrom as (string | number)[] | undefined;
    const nestedAllowFrom = dm?.allowFrom as (string | number)[] | undefined;
    ...
    const recovered = [...new Set(normalizeStringEntries(fromStore))];
