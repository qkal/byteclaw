...
interface ResolvedChannelAccountRowParams {
...
function summarizeSources(sources: (string | undefined)[]): {
  label: string;
  parts: string[];
} {
...
async function resolveChannelAccountRow(
  ...
  const enabled =
    selectedInspection?.enabled ?? resolveChannelAccountEnabled({ account, cfg, plugin });
  const configured =
  ...
  return { account, accountId, configured, enabled, snapshot };
}
...
const buildAccountNotes = (params: {
  ...
  const allowFrom =
    plugin.config.resolveAllowFrom?.({ accountId: snapshot.accountId, cfg }) ?? snapshot.allowFrom;
  if (allowFrom?.length) {
...
function resolveLinkFields(summary: unknown): {
  ...
  return { authAgeMs, linked, selfE164 };
}
...
function summarizeTokenConfig(params: {
  ...
  if (
    ...
    if (ready.length === 0) {
      return { detail: "no credentials (need bot+signing)", state: "setup" };
    }
  ...
  if (hasBotTokenField && hasAppTokenField) {
    ...
    if (ready.length === 0) {
      return { detail: "no tokens (need bot+app)", state: "setup" };
    }
  ...
  if (hasBotTokenField) {
    ...
    if (ready.length === 0) {
      return { detail: "no bot token", state: "setup" };
    }
  ...
  if (ready.length === 0) {
    return { detail: "no token", state: "setup" };
  }
    ...
    if (configuredAccounts.length > 0) {
      details.push({
        ...
        rows: configuredAccounts.map((entry) => {
          const notes = buildAccountNotes({ cfg, entry, plugin });
          return {
