...
function normalizeIssuePath(path: PropertyKey[]): (string | number)[] {
...
export function formatConfigPath(parts: (string | number)[]): string {
  if (parts.length === 0) {
    return "<root>";
  }
  let out = "";
...
export function resolveConfigPathTarget(root: unknown, path: (string | number)[]): unknown {
