...
export async function runNonInteractiveSetup(
  ...
  if (mode === "remote") {
    await runNonInteractiveRemoteSetup({ baseConfig, baseHash: snapshot.hash, opts, runtime });
    return;
  }

  await runNonInteractiveLocalSetup({ baseConfig, baseHash: snapshot.hash, opts, runtime });
}
