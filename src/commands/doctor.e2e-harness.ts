...
export const ensureAuthProfileStore = vi
  .fn()
  .mockReturnValue({ profiles: {}, version: 1 }) as unknown as MockFn;
...
export const auditGatewayServiceConfig = vi
  .fn()
  .mockResolvedValue({ issues: [], ok: true }) as unknown as MockFn;
...
vi.mock(buildBundledPluginModuleId("telegram", "api.js"), () => ({
  resolveTelegramToken: vi.fn(() => ({ source: "none", token: "" })),
}));
...
beforeEach(() => {
  ...
  runExec.mockReset().mockResolvedValue({ stderr: "", stdout: "" });
  runCommandWithTimeout.mockReset().mockResolvedValue(createCommandWithTimeoutResult());
  ensureAuthProfileStore.mockReset().mockReturnValue({ profiles: {}, version: 1 });
  ...
  auditGatewayServiceConfig.mockReset().mockResolvedValue({ issues: [], ok: true });
