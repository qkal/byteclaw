...
function registerMSTeamsSetupPlugin(pluginId = "@openclaw/msteams-plugin"): void {
  vi.mocked(loadChannelSetupPluginRegistrySnapshotForChannel).mockReturnValue(
    createTestRegistry([{ plugin: createMSTeamsSetupPlugin(), pluginId, source: "test" }]),
  );
}
...
async function runSignalAddCommand(afterAccountConfigWritten: SignalAfterAccountConfigWritten) {
  ...
  setActivePluginRegistry(createTestRegistry([{ plugin, pluginId: "signal", source: "test" }]));
  configMocks.readConfigFileSnapshot.mockResolvedValue({ ...baseConfigSnapshot });
  await channelsAddCommand(
    { account: "ops", channel: "signal", signalNumber: "+15550001" },
    runtime,
    { hasFlags: true },
  );
}
...
describe("channelsAddCommand", () => {
  ...
  it("clears telegram update offsets when the token changes", async () => {
    ...
    await channelsAddCommand(
      { account: "default", channel: "telegram", token: "new-token" },
      runtime,
      { hasFlags: true },
    );
  ...
  it("does not clear telegram update offsets when the token is unchanged", async () => {
    ...
    await channelsAddCommand(
      { account: "default", channel: "telegram", token: "same-token" },
      runtime,
      { hasFlags: true },
    );
