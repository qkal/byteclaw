...
interface LinuxMountInfoEntry {
  mountPoint: string;
  fsType: string;
  source: string;
}

export interface LinuxSdBackedStateDir {
  path: string;
  mountPoint: string;
  fsType: string;
  source: string;
...
function shouldRequireOAuthDir(cfg: OpenClawConfig, env: NodeJS.ProcessEnv): boolean {
  ...
  for (const channelId of listBundledChannelPluginIds()) {
    if (hasBundledChannelPersistedAuthState({ cfg, channelId, env })) {
      return true;
    }
  }
...
function shouldSuppressOrphanTranscriptWarning(cfg: OpenClawConfig, agentId: string): boolean {
  const backendConfig = resolveMemoryBackendConfig({ agentId, cfg });
...
export async function noteStateIntegrity(
  ...
  if (stateDirExists && !canWriteDir(stateDir)) {
    ...
    if (repair) {
      try {
        ...
        warnings.push(`- Failed to repair ${displayStateDir}: ${String(error)}`);
      }
    }
  }
  if (stateDirExists && process.platform !== "win32") {
    try {
      ...
      warnings.push(`- Failed to read ${displayStateDir} permissions: ${String(error)}`);
    }
  }
  ...
  if (configPath && existsFile(configPath) && process.platform !== "win32") {
    try {
      ...
      warnings.push(
        `- Failed to read config permissions (${displayConfigPath ?? configPath}): ${String(error)}`,
      );
    }
  }
      ...
      if (!canWriteDir(dir)) {
        ...
        if (repair) {
          try {
            ...
            warnings.push(`- Failed to repair ${displayDir}: ${String(error)}`);
          }
        }
      }
    }
  ...
  if (extraStateDirs.size > 0) {
    warnings.push(
      [
        ...
        ...[...extraStateDirs].map((dir) => `  - ${shortenHomePath(dir)}`),
      ...
      if (archiveOrphans) {
        ...
        for (const orphanPath of orphanTranscriptPaths) {
          ...
          try {
            ...
            warnings.push(
              `- Failed to archive orphan transcript ${shortenHomePath(orphanPath)}: ${String(error)}`,
            );
          }
        }
        if (archived > 0) {
