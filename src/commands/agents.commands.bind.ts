...
interface AgentsBindingsListOptions {
  agent?: string;
  json?: boolean;
}

interface AgentsBindOptions {
  agent?: string;
  bind?: string[];
  json?: boolean;
}

interface AgentsUnbindOptions {
  agent?: string;
  bind?: string[];
  all?: boolean;
  json?: boolean;
...
function formatBindingConflicts(
  conflicts: { binding: AgentRouteBinding; existingAgentId: string }[],
): string[] {
...
function resolveParsedBindingsOrExit(params: {
  ...
  const parsed = parseBindingSpecs({ agentId: params.agentId, config: params.cfg, specs });
...
async function resolveConfigAndTargetAgentIdOrExit(params: {
  ...
  return { agentId, baseHash: configSnapshot.hash, cfg };
}
...
export async function agentsBindCommand(
  ...
  if (
    emitJsonPayload({ conflictCount: result.conflicts.length, json: opts.json, payload, runtime })
  ) {
    return;
  }
...
export async function agentsUnbindCommand(
  ...
  if (opts.all) {
    ...
    if (emitJsonPayload({ json: opts.json, payload, runtime })) {
      return;
    }
