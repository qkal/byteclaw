...
export async function loadModelRegistry(
  ...
  try {
    ...
    if (!shouldFallbackToAuthHeuristics(error)) {
      throw error;
    }
    ...
    if (!availabilityErrorMessage) {
      availabilityErrorMessage = formatErrorWithStack(error);
    }
  }
  return { availabilityErrorMessage, availableKeys, models, registry };
}
