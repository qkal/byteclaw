...
export async function addFallbackCommand(
  ...
  const updated = await updateConfig((cfg) => {
    const resolved = resolveModelTarget({ cfg, raw: modelRaw });
    const nextModels = {
...
export async function removeFallbackCommand(
  ...
  const updated = await updateConfig((cfg) => {
    ...
    return patchDefaultsFallbacks(cfg, { fallbacks: filtered, key: params.key });
  });
...
export async function clearFallbacksCommand(
  ...
  await updateConfig((cfg) => patchDefaultsFallbacks(cfg, { fallbacks: [], key: params.key }));
