...
export async function modelsStatusCommand(
  ...
  const providerAuth = providers
    .map((provider) => resolveProviderAuthOverview({ cfg, modelsPath, provider, store }))
    .filter((entry) => {
      const hasAny = entry.profiles.count > 0 || Boolean(entry.env) || Boolean(entry.modelsJson);
      return hasAny;
    });
  const providerAuthMap = new Map(providerAuth.map((entry) => [entry.provider, entry]));
  const missingProvidersInUse = [...providersInUse]
  ...
  if (probeSummary) {
    ...
    if (probeSummary.results.length === 0) {
      ...
      runtime.log(
        renderTable({
          columns: [
            { header: "Model", key: "Model", minWidth: 18 },
            { header: "Profile", key: "Profile", minWidth: 24 },
