...
describe("upsertApiKeyProfile", () => {
  ...
  it("writes to OPENCLAW_AGENT_DIR when set", async () => {
    ...
    upsertApiKeyProfile({ input: "sk-minimax-test", provider: "minimax" });
    ...
    const next = applyAuthProfileConfig(
      {
        auth: {
          profiles: {
            "kilocode:legacy": { mode: "oauth", provider: "kilocode" },
          },
        },
      },
      {
    ...
    const next = applyAuthProfileConfig(
      {
        auth: {
          ...
          profiles: {
            "zai:default": { mode: "api_key", provider: "z.ai" },
          },
        },
      },
      {
    ...
    const next = applyAuthProfileConfig(
      {
        auth: {
          ...
          profiles: {
            "zai:backup": { mode: "token", provider: "z-ai" },
            "zai:default": { mode: "api_key", provider: "z.ai" },
          },
        },
      },
    ...
    const next = applyAuthProfileConfig(
      {
        auth: {
          profiles: {
            "kilocode:legacy": { mode: "api_key", provider: "kilocode" },
          },
        },
      },
      {
