...
interface SandboxRecreateOptions {
  all: boolean;
  session?: string;
  agent?: string;
  browser: boolean;
...
export async function sandboxListCommand(
  ...
  if (opts.json) {
    writeRuntimeJson(runtime, { browsers, containers });
    return;
  }
...
async function removeContainer(
  ...
  try {
    ...
    runtime.error(`✗ Failed to remove ${containerName}: ${String(error)}`);
