...
async function stopAndUninstallService(runtime: RuntimeEnv): Promise<boolean> {
  ...
  try {
    ...
    runtime.error(`Gateway uninstall failed: ${String(error)}`);
    return false;
  }
}
  ...
  if (!hadExplicit) {
    ...
    const selection = await multiselectStyled<UninstallScope>({
      ...
      options: [
        ...
        { hint: "~/.openclaw", label: "State + config", value: "state" },
        { hint: "agent files", label: "Workspace", value: "workspace" },
        {
