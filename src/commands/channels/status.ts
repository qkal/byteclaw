...
export function formatGatewayChannelsStatusLines(payload: Record<string, unknown>): string[] {
  ...
  const accountLines = (provider: ChatChannel, accounts: Record<string, unknown>[]) =>
  ...
  const accountPayloads: Partial<Record<string, Record<string, unknown>[]>> = {};
  for (const plugin of plugins) {
    const raw = accountsByChannel?.[plugin.id];
    if (Array.isArray(raw)) {
      accountPayloads[plugin.id] = raw as Record<string, unknown>[];
    }
  }
...
export async function channelsStatusCommand(
  ...
  try {
    ...
    runtime.error(`Gateway not reachable: ${String(error)}`);
