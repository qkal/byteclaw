...
interface RuntimeMemoryAuditContext {
  workspaceDir?: string;
  backend?: string;
  dbPath?: string;
...
export async function maybeRepairMemoryRecallHealth(params: {
  ...
  try {
    ...
    note(
      `Memory recall repair could not be completed: ${formatErrorMessage(error)}`,
      "Memory search",
    );
  }
}
...
export async function noteMemorySearchHealth(
  ...
  const backendConfig = resolveActiveMemoryBackendConfig({ agentId, cfg });
  if (!backendConfig) {
...
function formatMemoryProviderEnvVarList(providers: { envVars: string[] }[]): string {
