...
export function hasExplicitMemorySearchConfig(cfg: OpenClawConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
    Object.hasOwn(cfg.agents.defaults, "memorySearch")
  ) {
    return true;
  }
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  return agents.some(
    (agent) => agent?.id === agentId && Object.hasOwn(agent, "memorySearch"),
  );
}
...
export function resolveMemoryPluginStatus(cfg: OpenClawConfig): MemoryPluginStatus {
  ...
  if (!pluginsEnabled) {
    return { enabled: false, reason: "plugins disabled", slot: null };
  }
  const raw = normalizeOptionalString(cfg.plugins?.slots?.memory) ?? "";
  if (normalizeOptionalLowercaseString(raw) === "none") {
    return { enabled: false, reason: 'plugins.slots.memory="none"', slot: null };
  }
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  const { manager } = await params.getMemorySearchManager({ agentId, cfg, purpose: "status" });
  if (!manager) {
    return null;
  }
  try {
