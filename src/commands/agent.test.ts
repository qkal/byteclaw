...
function mockConfig(
  ...
  agentsList?: { id: string; default?: boolean }[],
) {
  const cfg = {
    agents: {
      defaults: {
...
function readSessionStore<T>(storePath: string): Record<string, T> {
  return JSON.parse(fs.readFileSync(storePath, "utf8")) as Record<string, T>;
}
...
async function withCrossAgentResumeFixture(
  ...
  await withTempHome(async (home) => {
    ...
    const cfg = mockConfig(home, storePattern, undefined, undefined, [
      { id: "dev" },
      { default: true, id: "exec" },
    ]);
    await run({ cfg, sessionId, sessionKey });
  });
}
...
function createTelegramOutboundPlugin() {
  const sendWithTelegram = async (
    ...
    return { channel: "telegram", chatId: result.chatId, messageId: result.messageId };
  };
...
describe("agentCommand", () => {
  it("persists thinking and verbose overrides", async () => {
    await withTempHome(async (home) => {
      ...
      await agentCommand({ message: "hi", thinking: "high", to: "+1222", verbose: "on" }, runtime);
  ...
  it.each([
    ...
    {
      args: { message: "hi", senderIsOwner: false, to: "+1555" },
      expected: false,
  ...
  it("honors explicit senderIsOwner for ingress runs", async () => {
    await withTempHome(async (home) => {
      ...
      await agentCommandFromIngress(
        { allowModelOverride: false, message: "hi", senderIsOwner: false, to: "+1555" },
        runtime,
      );
  ...
  it("uses the resumed session agent scope when sessionId resolves to another agent store", async () => {
    await withCrossAgentResumeFixture(async ({ sessionId, sessionKey, cfg }) => {
      ...
      const agentId = resolveSessionAgentId({ config: cfg, sessionKey: resolution.sessionKey });
  ...
  it("uses origin.provider for channel-specific session reset overrides", async () => {
    await withTempHome(async (home) => {
      ...
      cfg.session = {
        ...cfg.session,
        reset: { idleMinutes: 10, mode: "idle" },
        resetByChannel: {
          discord: { idleMinutes: 120, mode: "idle" },
        },
      };
  ...
  it("does not duplicate agent events from embedded runs", async () => {
    await withTempHome(async (home) => {
      ...
      const assistantEvents: { runId: string; text?: string }[] = [];
      ...
      vi.mocked(runEmbeddedPiAgent).mockImplementationOnce(async (params) => {
        ...
        const data = { delta: "hello", text: "hello" };
        (
          params as {
            onAgentEvent?: (evt: { stream: string; data: Record<string, unknown> }) => void;
          }
        ).onAgentEvent?.({ data, stream: "assistant" });
        emitAgentEvent({ data, runId, stream: "assistant" });
        return {
    ...
    await withTempHome(async (home) => {
      ...
      vi.mocked(runEmbeddedPiAgent)
        ...
        .mockResolvedValueOnce({
          meta: {
            agentMeta: { model: "gpt-5.4", provider: "openai", sessionId: "session-subagent" },
            durationMs: 5,
          },
          payloads: [{ text: "ok" }],
        });
      ...
      const attempts = vi
        ...
        .mock.calls.map((call) => ({ model: call[0]?.model, provider: call[0]?.provider }));
  ...
  it("rejects unknown agent overrides", async () => {
    await withTempHome(async (home) => {
      ...
      await expect(agentCommand({ agentId: "ghost", message: "hi" }, runtime)).rejects.toThrow(
    ...
    await withTempHome(async (home) => {
      vi.mocked(runEmbeddedPiAgent).mockResolvedValue(
        createDefaultAgentResult({
          durationMs: 42,
          payloads: [{ mediaUrl: "http://x.test/a.jpg", text: "json-reply" }],
        }),
      );
      const store = path.join(home, "sessions.json");
      mockConfig(home, store);

      await agentCommand({ json: true, message: "hi", to: "+1999" }, runtime);
  ...
  it("passes through telegram accountId when delivering", async () => {
    await withTempHome(async (home) => {
      ...
      setActivePluginRegistry(
        createTestRegistry([
          { plugin: createTelegramOutboundPlugin(), pluginId: "telegram", source: "test" },
        ]),
      );
      const deps = {
        ...
        sendMessageTelegram: vi.fn().mockResolvedValue({ chatId: "123", messageId: "t1" }),
  ...
  it("uses reply channel as the message channel context", async () => {
    const callArgs = await runEmbeddedWithTempConfig({
      agentsList: [{ id: "ops" }],
      args: { agentId: "ops", message: "hi", replyChannel: "slack" },
    });
  ...
  it("prefers runContext for embedded routing", async () => {
    const callArgs = await runEmbeddedWithTempConfig({
      args: {
        channel: "whatsapp",
        message: "hi",
        runContext: { accountId: "acct-2", messageChannel: "slack" },
        to: "+1555",
      },
    });
    expect(callArgs?.messageChannel).toBe("slack");
  ...
  it("forwards accountId to embedded runs", async () => {
    const callArgs = await runEmbeddedWithTempConfig({
      args: { accountId: "kev", message: "hi", to: "+1555" },
    });
