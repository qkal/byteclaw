...
export interface WriteOAuthCredentialsOptions {
...
function buildEnvSecretRef(id: string): SecretRef {
  return { id, provider: DEFAULT_SECRET_PROVIDER_ALIAS, source: "env" };
}
...
export function applyAuthProfileConfig(
  ...
  const configuredProviderProfiles = Object.entries(cfg.auth?.profiles ?? {})
    ...
    .map(([profileId, profile]) => ({ mode: profile.mode, profileId }));
