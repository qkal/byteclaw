...
function formatAjvErrors(errors: ErrorObject[] | null | undefined): JsonSchemaValidationError[] {
  if (!errors || errors.length === 0) {
    return [{ message: "invalid config", path: "<root>", text: "<root>: invalid config" }];
  }
...
export function validateJsonSchemaValue(params: {
  ...
  if (!cached || cached.schema !== params.schema) {
    ...
    cached = { schema: params.schema, validate };
  ...
  return { errors: formatAjvErrors(cached.validate.errors), ok: false };
}
