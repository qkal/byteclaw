...
describe("plugin runtime command execution", () => {
  ...
  it.each([
    ...
    const runtime = createPluginRuntime();
    await expectRunCommandOutcome({ commandResult, expected, runtime });
  ...
  it("keeps subagent unavailable by default even after gateway initialization", async () => {
    ...
    expectGatewaySubagentRunFailure(runtime, { message: "hello", sessionKey: "s-1" });
  });
  ...
  it("late-binds to the gateway subagent when explicitly enabled", async () => {
    ...
    await expect(
      expectRuntimeSubagentRun(runtime, { message: "hello", sessionKey: "s-2" }),
    ).resolves.toEqual({
      runId: "run-1",
    });
    expect(run).toHaveBeenCalledWith({ message: "hello", sessionKey: "s-2" });
  });
});
