...
export interface ApplyProviderAuthChoiceParams {
  authChoice: string;
...
export interface ApplyProviderAuthChoiceResult {
  config: OpenClawConfig;
  agentModelOverride?: string;
}

export interface PluginProviderAuthChoiceOptions {
  authChoice: string;
  pluginId: string;
  providerId: string;
  methodId?: string;
...
export async function applyAuthChoiceLoadedPluginProvider(
  ...
  return { agentModelOverride, config: nextConfig };
}
...
export async function applyAuthChoicePluginProvider(
  ...
  if (applied.defaultModel) {
    ...
    nextConfig = restoreConfiguredPrimaryModel(nextConfig, params.config);
    return { agentModelOverride: applied.defaultModel, config: nextConfig };
  }
