...
export interface ProviderWizardOption {
  value: string;
  label: string;
  hint?: string;
  groupId: string;
...
export interface ProviderModelPickerEntry {
  value: string;
  label: string;
  hint?: string;
}
...
function listMethodWizardSetups(provider: ProviderPlugin): {
...
export function resolveProviderPluginChoice(params: {
  ...
  if (choice.startsWith(PROVIDER_PLUGIN_CHOICE_PREFIX)) {
    ...
    const providerId = separator !== -1 ? payload.slice(0, separator) : payload;
    const methodId = separator !== -1 ? payload.slice(separator + 1) : undefined;
  ...
  for (const provider of params.providers) {
    ...
    if (setup) {
      ...
      if ((normalizeOptionalString(setupChoiceId) ?? "") === choice) {
        ...
        if (method) {
          return { method, provider, wizard: setup };
        }
      }
    }
    if (
      ...
      return { method: provider.auth[0], provider };
    }
  }
