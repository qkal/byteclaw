...
export interface BundlePluginManifest {
  id: string;
  name?: string;
  description?: string;
  version?: string;
...
export function normalizeBundlePathList(value: unknown): string[] {
  return [...new Set(normalizePathList(value))];
}
    ...
    return matchBoundaryFileOpenFailure(opened, {
      ...
      path: () => {
        if (params.allowMissing) {
          return { manifestPath, ok: true, raw: {} };
        }
        return { error: `plugin manifest not found: ${manifestPath}`, manifestPath, ok: false };
      },
    });
  }
  try {
    const raw = JSON5.parse(fs.readFileSync(opened.fd, "utf8")) as unknown;
    if (!isRecord(raw)) {
