...
interface InstallScanLogger {
  warn?: (message: string) => void;
}

interface InstallScanFinding {
  ruleId: string;
...
export interface InstallSecurityScanResult {
  blocked?: {
...
function buildCriticalDetails(params: {
  findings: { file: string; line: number; message: string; severity: string }[];
}) {
...
async function scanDirectoryTarget(params: {
  ...
  try {
    ...
    return buildBuiltinScanFromError(error);
  }
}
