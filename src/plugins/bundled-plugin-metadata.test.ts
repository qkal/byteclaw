...
describe("bundled plugin metadata", () => {
  ...
  it("captures setup-entry metadata for bundled channel plugins", () => {
    ...
    expect(discord?.source).toEqual({ built: "index.js", source: "./index.ts" });
    expect(discord?.setupSource).toEqual({ built: "setup-entry.js", source: "./setup-entry.ts" });
    ...
    writeJson(path.join(tempRoot, "extensions", "alpha", "openclaw.plugin.json"), {
      channelConfigs: {
        alpha: {
          ...
          schema: { properties: { stale: { type: "boolean" } }, type: "object" },
          uiHints: {
