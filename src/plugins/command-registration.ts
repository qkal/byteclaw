...
export function registerPluginCommand(
  ...
  if (isPluginCommandRegistryLocked()) {
    return { error: "Cannot register commands while processing is in progress", ok: false };
  }

  const definitionError = validatePluginCommandDefinition(command);
  if (definitionError) {
    return { error: definitionError, ok: false };
  }
  ...
  for (const invocationKey of invocationKeys) {
    const existing =
      ...
      [...pluginCommands.values()].find((candidate) =>
