...
interface ModifyingHookPolicy<K extends PluginHookName, TResult> {
  mergeResults?: (
...
export function createHookRunner(registry: PluginRegistry, options: HookRunnerOptions = {}) {
  ...
  async function runVoidHook<K extends PluginHookName>(
    ...
    const promises = hooks.map(async (hook) => {
      try {
        ...
        handleHookError({ error: error, hookName, pluginId: hook.pluginId });
      }
    });
  ...
  async function runClaimingHooksList<
    ...
    TResult extends { handled: boolean },
  >(
    hooks: (PluginHookRegistration<K> & { pluginId: string })[],
    hookName: K,
    event: Parameters<NonNullable<PluginHookRegistration<K>["handler"]>>[0],
    ctx: Parameters<NonNullable<PluginHookRegistration<K>["handler"]>>[1],
  ): Promise<TResult | undefined> {
  ...
  > {
    ...
    for (const hook of hooks) {
      try {
        ...
        if (handlerResult?.handled) {
          return { result: handlerResult, status: "handled" };
        }
      } catch (error) {
        firstError ??= sanitizeHookError(error);
        handleHookError({ error: error, hookName, pluginId: hook.pluginId });
      }
    }

    if (firstError) {
      return { error: firstError, status: "error" };
    }
  ...
  function runToolResultPersist(
    ...
    for (const hook of hooks) {
      try {
        ...
        const msg = `[hooks] tool_result_persist handler from ${hook.pluginId} failed: ${String(error)}`;
        if (shouldCatchHookErrors("tool_result_persist")) {
          logger?.error(msg);
        } else {
  ...
  function runBeforeMessageWrite(
    ...
    for (const hook of hooks) {
      try {
        ...
        const msg = `[hooks] before_message_write handler from ${hook.pluginId} failed: ${String(error)}`;
        if (shouldCatchHookErrors("before_message_write")) {
          logger?.error(msg);
        } else {
          throw new Error(msg, { cause: error });
