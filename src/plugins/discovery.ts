...
function discoverInDirectory(params: {
  ...
  try {
    ...
    params.diagnostics.push({
      level: "warn",
      message: `failed to read extensions dir: ${params.dir} (${String(error)})`,
      source: params.dir,
    });
    return;
  }
...
export function discoverOpenClawPlugins(params: {
  ...
  const roots = resolvePluginSourceRoots({ env, workspaceDir: workspaceRoot });
