...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    ...
    const handle = await startTrackingServices({
      config,
      services: [
        createTrackingService("service-a", { contexts, starts, stops }),
        createTrackingService("service-b", { contexts, starts }),
        createTrackingService("service-c", { contexts, starts, stops }),
      ],
      workspaceDir: "/tmp/workspace",
    });
    await handle.stop();

    expectServiceLifecycleState({ config, contexts, starts, stops });
  });
