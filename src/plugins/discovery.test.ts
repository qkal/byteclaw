...
function writePluginEntry(filePath: string) {
  fs.writeFileSync(filePath, "export default function () {}", "utf8");
}
...
function createPackagePlugin(params: {
  ...
  if (params.pluginId) {
    writePluginManifest({ id: params.pluginId, pluginDir: params.packageDir });
  }
}
...
function createBundleRoot(bundleDir: string, markerPath: string, manifest?: unknown) {
  ...
  if (manifest) {
    fs.writeFileSync(path.join(bundleDir, markerPath), JSON.stringify(manifest), "utf8");
    return;
  }
...
function expectCandidateSource(
  candidates: { idHint?: string; source?: string }[],
  idHint: string,
  source: string,
) {
...
function expectEscapesPackageDiagnostic(diagnostics: { message: string }[]) {
...
describe("discoverOpenClawPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    mkdirSafe(globalExt);
    fs.writeFileSync(path.join(globalExt, "alpha.ts"), "export default function () {}", "utf8");

    const workspaceExt = path.join(workspaceDir, ".openclaw", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "beta.ts"), "export default function () {}", "utf8");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf8");
  ...
  it("ignores backup and disabled plugin directories in scanned roots", async () => {
    ...
    mkdirSafe(backupDir);
    fs.writeFileSync(path.join(backupDir, "index.ts"), "export default function () {}", "utf8");

    const disabledDir = path.join(globalExt, "telegram.disabled.20260222");
    mkdirSafe(disabledDir);
    fs.writeFileSync(path.join(disabledDir, "index.ts"), "export default function () {}", "utf8");

    const bakDir = path.join(globalExt, "discord.bak");
    mkdirSafe(bakDir);
    fs.writeFileSync(path.join(bakDir, "index.ts"), "export default function () {}", "utf8");

    const liveDir = path.join(globalExt, "live");
    mkdirSafe(liveDir);
    fs.writeFileSync(path.join(liveDir, "index.ts"), "export default function () {}", "utf8");
  ...
  it("does not discover nested node_modules copies under installed plugins", async () => {
    ...
    writePluginManifest({ id: "opik-openclaw", pluginDir });
    fs.writeFileSync(
    ...
    writePluginManifest({ id: "diffs", pluginDir: nestedDiffsDir });
    fs.writeFileSync(
  ...
  it("skips dependency and build directories while scanning workspace roots", () => {
    ...
    writePluginManifest({ id: "dist-copy", pluginDir: nestedDistDir });
    fs.writeFileSync(
  ...
  it.each([
    ...
    expectCandidateIds(candidates, { excludes, includes });
  });
  ...
  it.each([
    {
      ...
      setup: (stateDir: string) => {
        ...
        fs.writeFileSync(path.join(pluginDir, ".claude-plugin", "plugin.json"), "{", "utf8");
        return {};
      },
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        fs.writeFileSync(path.join(pluginDir, "index.ts"), "export default {}", "utf8");
        fs.writeFileSync(path.join(pluginDir, ".codex-plugin", "plugin.json"), "{", "utf8");
  ...
  it.each([
    {
      ...
      setup: (stateDir: string) => {
        ...
        fs.writeFileSync(outside, "export default function () {}", "utf8");
      },
    },
    {
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        fs.writeFileSync(path.join(outsideDir, "escape.ts"), "export default {}", "utf8");
        try {
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        mkdirSafe(globalExt);
        mkdirSafe(outsideDir);
        fs.writeFileSync(outsideFile, "export default {}", "utf8");
        try {
  ...
  it("ignores package manifests that are hardlinked aliases", async () => {
    ...
    mkdirSafe(globalExt);
    mkdirSafe(outsideDir);
    fs.writeFileSync(path.join(globalExt, "entry.ts"), "export default {}", "utf8");
    fs.writeFileSync(
      outsideManifest,
      JSON.stringify({
        name: "@openclaw/pack",
  ...
  it.runIf(process.platform !== "win32")("blocks world-writable plugin paths", async () => {
    ...
    fs.writeFileSync(pluginPath, "export default function () {}", "utf8");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      mkdirSafe(packDir);
      fs.writeFileSync(path.join(packDir, "index.ts"), "export default function () {}", "utf8");
  ...
  it.each([
      ...
      setup: () => {
        ...
        return {
          assert: (
            ...
            expectCandidatePresence(first, { absent: ["beta"], present: ["alpha"] });
            expectCandidatePresence(second, { absent: ["alpha"], present: ["beta"] });
          },
    ...
    expectCachedDiscoveryPair({ assert, first, second });
  });
