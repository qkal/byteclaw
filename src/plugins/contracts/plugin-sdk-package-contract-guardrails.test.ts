...
function collectPluginSdkSubpathReferences() {
  const references: { file: string; subpath: string }[] = [];
...
function resolveNpmCommandInvocation(npmArgs: string[]): NpmCommandInvocation {
  ...
  if (typeof npmExecPath === "string" && npmExecPath.length > 0 && isNpmExecPath(npmExecPath)) {
    return { args: [npmExecPath, ...npmArgs], command: process.execPath };
  }
  ...
  if (npmCliPath) {
    return { args: [npmCliPath, ...npmArgs], command: process.execPath };
  }

  const npmExePath = resolve(nodeDir, "npm.exe");
  if (existsSync(npmExePath)) {
    return { args: npmArgs, command: npmExePath };
  }
...
function collectExtensionCoreImportLeaks(): { file: string; specifier: string }[] {
  const leaks: { file: string; specifier: string }[] = [];
