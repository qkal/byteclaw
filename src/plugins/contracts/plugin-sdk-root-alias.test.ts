...
const rootAliasSource = fs.readFileSync(rootAliasPath, "utf8");
...
describe("plugin-sdk root alias", () => {
  it("exposes the fast empty config schema helper", () => {
    ...
    expect(schema.safeParse(undefined)).toEqual({ data: undefined, success: true });
    expect(schema.safeParse({})).toEqual({ data: {}, success: true });
  ...
  it("does not load the monolithic sdk for fast helpers", () => {
    ...
    expect(typeof factory).toBe("function");
    expect(factory?.().safeParse({})).toEqual({ data: {}, success: true });
    expect(lazyModule.createJitiCalls).toBe(0);
    expect(lazyModule.jitiLoadCalls).toBe(0);
  });
