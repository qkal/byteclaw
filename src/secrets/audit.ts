...
import { type SecretRef, resolveSecretInputRef } from "../config/types.secrets.js";
...
async function collectUnresolvedRefFindings(params: {
  ...
  for (const refsForProvider of refsByProvider.values()) {
    ...
    try {
      ...
      if (provider && isProviderScopedSecretResolutionError(error)) {
