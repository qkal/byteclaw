...
interface SecretRefCredentialMatrix {
  entries: {
    id: string;
    configFile: "openclaw.json" | "auth-profiles.json";
    path: string;
...
function loadCoverageRegistryEntries(): SecretRegistryEntry[] {
  ...
  return matrix.entries.map((entry) => ((Object.assign({id:entry.id,configFile:entry.configFile,pathPattern:entry.path}, entry.refPath?{refPathPattern:entry.refPath}:{}, {secretShape:entry.secretShape,expectedResolvedValue:`string`}, entry.when?.type?{authProfileType:entry.when.type}:{}))));
}
