...
describe("secret ref resolver", () => {
  ...
  async function resolveExecSecret(
    ...
    return resolveSecretRefString(
      { id: "openai/api-key", provider: "execmain", source: "exec" },
      {
        config: {
          secrets: {
            providers: {
  ...
  itPosix("resolves file refs in json mode", async () => {
    ...
    const value = await resolveSecretRefString(
      { id: "/providers/openai/apiKey", provider: "filemain", source: "file" },
      {
        config: {
          secrets: {
            providers: {
  ...
  itPosix("uses timeoutMs as the default no-output timeout for exec providers", async () => {
    ...
    const value = await resolveSecretRefString(
      { id: "delayed", provider: "execmain", source: "exec" },
      {
        config: {
          secrets: {
            providers: {
  ...
  itPosix("supports file singleValue mode with id=value", async () => {
    ...
    const value = await resolveSecretRefString(
      { id: "value", provider: "rawfile", source: "file" },
      {
        config: {
          secrets: {
            providers: {
  ...
  it("rejects misconfigured provider source mismatches", async () => {
    await expect(
      resolveSecretRefValue(
        { id: "abc", provider: "default", source: "exec" },
        {
          config: {
            secrets: {
              providers: {
