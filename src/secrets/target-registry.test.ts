...
describe("secret target registry", () => {
  it("supports filtered discovery by target ids", () => {
    const config = {
      ...buildTalkTestProviderConfig({ id: "TALK_API_KEY", provider: "default", source: "env" }),
      gateway: {
        remote: {
          token: { id: "REMOTE_TOKEN", provider: "default", source: "env" },
        },
      },
    };

    const targets = runTargetRegistrySnippet<
      { entry?: { id?: string }; providerId?: string; path?: string }[]
    >(
