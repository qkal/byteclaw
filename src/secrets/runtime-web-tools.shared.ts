...
export interface SecretResolutionResult<TSource extends string> {
  value?: string;
  source: TSource;
...
export interface RuntimeWebProviderMetadataBase<TSource extends string> {
...
export interface RuntimeWebProviderSelectionParams<
  TProvider extends {
    id: string;
...
export interface RuntimeWebProviderSurface<TProvider extends { id: string }> {
  providers: TProvider[];
  configuredProvider?: string;
  enabled: boolean;
...
export interface ResolveRuntimeWebProviderSurfaceParams<
  TProvider extends {
    id: string;
    requiresCredential?: boolean;
  },
...
>(
  ...
  if (params.enabled) {
    const candidates = params.configuredProvider
      ...
      : params.providers;
    const unresolvedWithoutFallback: { provider: string; path: string; reason: string }[] = [];
