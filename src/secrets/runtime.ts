...
export interface PreparedSecretsRuntimeSnapshot {
  sourceConfig: OpenClawConfig;
  config: OpenClawConfig;
  authStores: { agentDir: string; store: AuthProfileStore }[];
  warnings: SecretResolverWarning[];
  webTools: RuntimeWebToolsMetadata;
}

interface SecretsRuntimeRefreshContext {
...
export async function prepareSecretsRuntimeSnapshot(params: {
  ...
  const authStores: { agentDir: string; store: AuthProfileStore }[] = [];
