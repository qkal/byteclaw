...
export function setPathExistingStrict(
  ...
  const cursor = traverseToLeafParent({ requireExistingSegment: true, root, segments });
...
export function deletePathStrict(root: Record<string, unknown>, segments: string[]): boolean {
  const cursor = traverseToLeafParent({ requireExistingSegment: false, root, segments });
