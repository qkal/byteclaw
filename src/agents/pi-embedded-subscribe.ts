...
import { type UsageLike, hasNonzeroUsage, normalizeUsage } from "./usage.js";
...
export function subscribeEmbeddedPiSession(params: SubscribeEmbeddedPiSessionParams) {
  ...
  const state: EmbeddedPiSubscribeState = {
    ...
    blockState: { final: false, inlineCode: createInlineCodeState(), thinking: false },
    partialBlockState: { final: false, inlineCode: createInlineCodeState(), thinking: false },
  ...
  const emitBlockReplySafely = (
    ...
    try {
      ...
      const task = Promise.resolve(maybeTask).catch((error) => {
  ...
  const ensureCompactionPromise = () => {
    if (!state.compactionRetryPromise) {
      ...
      state.compactionRetryPromise.catch((error) => {
        log.debug(`compaction promise rejected (no waiter): ${String(error)}`);
      });
    }
  };
  ...
  const flushBlockReplyBuffer = (options?: {
    ...
    if (blockChunker?.hasBuffered()) {
      blockChunker.drain({ emit: (text) => emitBlockChunk(text, options), force: true });
      blockChunker.reset();
  ...
  const unsubscribe = () => {
    ...
    if (params.session.isCompacting) {
      ...
      try {
        ...
        log.warn(`unsubscribe: compaction abort failed runId=${params.runId} err=${String(error)}`);
      }
    }
    sessionUnsubscribe();
  };
  ...
  return {
    ...
    getMessagingToolSentTexts: () => [...messagingToolSentTexts],
    getMessagingToolSentMediaUrls: () => [...messagingToolSentMediaUrls],
    getMessagingToolSentTargets: () => [...messagingToolSentTargets],
