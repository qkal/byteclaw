...
export interface ProviderRequestTlsOverride {
  ca?: string;
  cert?: string;
  key?: string;
  passphrase?: string;
...
export interface ProviderRequestTransportOverrides {
...
export interface ResolvedProviderRequestExtraHeadersConfig {
  configured: boolean;
  headers?: Record<string, string>;
}
...
interface ResolveProviderRequestPolicyConfigParams {
  provider?: string;
  api?: RequestApi;
  baseUrl?: string;
...
function sanitizeConfiguredRequestString(value: unknown, path: string): string | undefined {
  if (typeof value !== "string") {
    ...
    assertSecretInputResolved({ path, value });
    return undefined;
  }
...
export function mergeModelProviderRequestOverrides(
  ...overrides: (ModelProviderRequestTransportOverrides | undefined)[]
): ModelProviderRequestTransportOverrides | undefined {
...
export function buildProviderRequestDispatcherPolicy(
  ...
  if (!request.proxy.configured) {
    return targetTls ? { connect: targetTls, mode: "direct" } : undefined;
  }
...
interface ModelWithProviderRequestTransport {
