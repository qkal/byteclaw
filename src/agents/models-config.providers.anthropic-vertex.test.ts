  ...
  it("accepts ADC credentials when the file includes a project_id", async () => {
    ...
    try {
      ...
      expect(providers?.["anthropic-vertex"]?.models).toMatchObject([
        { contextWindow: 1_000_000, id: "claude-opus-4-6", maxTokens: 128_000 },
        { contextWindow: 1_000_000, id: "claude-sonnet-4-6", maxTokens: 128_000 },
      ]);
    } finally {
      rmSync(adcDir, { force: true, recursive: true });
    }
  });
