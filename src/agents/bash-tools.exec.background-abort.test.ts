...
const ABORT_WAIT_TIMEOUT_MS = process.platform === "win32" ? 1500 : 1200;
const POLL_INTERVAL_MS = 15;
const FINISHED_WAIT_TIMEOUT_MS = process.platform === "win32" ? 8000 : 3000;
...
async function expectBackgroundSessionSurvivesAbort(params: {
  ...
  await expect
    .poll(
      ...
      { interval: POLL_INTERVAL_MS, timeout: ABORT_WAIT_TIMEOUT_MS },
    )
    .toBe(true);
...
test("background exec is not killed when tool signal aborts", async () => {
  ...
  await expectBackgroundSessionSurvivesAbort({
    executeParams: { background: true, command: BACKGROUND_HOLD_CMD },
    tool,
  });
});
...
test("pty background exec is not killed when tool signal aborts", async () => {
  const tool = createTestExecTool({ allowBackground: true, backgroundMs: 0 });
  await expectBackgroundSessionSurvivesAbort({
    executeParams: { background: true, command: BACKGROUND_HOLD_CMD, pty: true },
    tool,
  });
});
