...
import { type BoundaryFileOpenResult, openBoundaryFile } from "../infra/boundary-file-read.js";
import {
  mkdirPathWithinRoot,
...
interface SandboxApplyPatchConfig {
  root: string;
...
function resolvePatchFileOps(options: ApplyPatchOptions): PatchFileOps {
  if (options.sandbox) {
    ...
    return {
      mkdirp: (dir) => bridge.mkdirp({ cwd: root, filePath: dir }),
      readFile: async (filePath) => {
        const buf = await bridge.readFile({ cwd: root, filePath });
        return buf.toString("utf8");
      },
      remove: (filePath) => bridge.remove({ cwd: root, filePath, force: false }),
      writeFile: (filePath, content) => bridge.writeFile({ cwd: root, data: content, filePath }),
    };
  }
  const workspaceOnly = options.workspaceOnly !== false;
  return {
...
function parseOneHunk(lines: string[], lineNumber: number): { hunk: Hunk; consumed: number } {
  ...
  if (firstLine.startsWith(ADD_FILE_MARKER)) {
    ...
    return {
      consumed,
      hunk: { contents, kind: "add", path: targetPath },
    };
  }
