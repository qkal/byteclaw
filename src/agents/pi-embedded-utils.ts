...
interface AssistantTextExtractionResult {
  text: string;
...
function extractAssistantTextForPhase(
  ...
  if (!Array.isArray(msg.content)) {
    return { hadRequestedPhase: false, text: "" };
  }
...
export function splitThinkingTaggedText(text: string): ThinkTaggedSplitBlock[] | null {
  ...
  const pushThinking = (value: string) => {
    ...
    blocks.push({ thinking: cleaned, type: "thinking" });
  };
  ...
  for (const block of message.content) {
    ...
    for (const part of split) {
      if (part.type === "thinking") {
        next.push({ thinking: part.thinking, type: "thinking" });
      } else if (part.type === "text") {
        const cleaned = part.text.trimStart();
        if (cleaned) {
          next.push({ text: cleaned, type: "text" });
        }
      }
    }
  }
...
export function inferToolMetaFromArgs(toolName: string, args: unknown): string | undefined {
  const display = resolveToolDisplay({ args, name: toolName });
