...
async function resumeOrphanedSession(params: {
  ...
  try {
    ...
    log.warn(`failed to resume orphaned session ${params.sessionKey}: ${String(error)}`);
    return false;
  }
}
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const result = { failed: 0, recovered: 0, skipped: 0 };
      ...
      try {
        ...
        if (resumed) {
          ...
          try {
            ...
            log.warn(
              `resume succeeded but failed to update session store for ${childSessionKey}: ${String(error)}`,
            );
          }
          result.recovered++;
        } else {
        ...
        log.warn(`error processing orphaned session ${childSessionKey}: ${String(error)}`);
        result.failed++;
      }
    }
  } catch (error) {
    log.warn(`orphan recovery scan failed: ${String(error)}`);
      ...
      void recoverOrphanedSubagentSessions({ ...params, resumedSessionKeys })
        ...
        .catch((error) => {
          if (attempt < maxRetries) {
            const nextDelay = delay * RETRY_BACKOFF_MULTIPLIER;
            log.warn(
              `scheduled orphan recovery failed: ${String(error)}; retrying in ${nextDelay}ms (attempt ${attempt + 1}/${maxRetries})`,
            );
            attemptRecovery(attempt + 1, nextDelay);
          } else {
            log.warn(
              `scheduled orphan recovery failed after ${maxRetries} retries: ${String(error)}`,
            );
          }
        });
    }, delay).unref?.();
