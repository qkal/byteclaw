...
const buildDuplicateIdCollisionInput = () =>
  castAgentMessages([
    {
      content: [
        { arguments: {}, id: "call_a|b", name: "read", type: "toolCall" },
        { arguments: {}, id: "call_a:b", name: "read", type: "toolCall" },
      ],
      role: "assistant",
    },
...
const buildRepeatedRawIdInput = () =>
  castAgentMessages([
    {
      content: [
        { arguments: {}, id: "edit:22", name: "edit", type: "toolCall" },
        { arguments: {}, id: "edit:22", name: "edit", type: "toolCall" },
      ],
      role: "assistant",
    },
...
describe("sanitizeToolCallIdsForCloudCodeAssist", () => {
  describe("strict mode (default)", () => {
    it("is a no-op for already-valid non-colliding IDs", () => {
      const input = castAgentMessages([
        {
          content: [{ arguments: {}, id: "call1", name: "read", type: "toolCall" }],
          role: "assistant",
        },
        {
    ...
    it("strips non-alphanumeric characters from tool call IDs", () => {
      const input = castAgentMessages([
        {
          content: [{ arguments: {}, id: "call|item:123", name: "read", type: "toolCall" }],
          role: "assistant",
        },
        {
          content: [{ text: "ok", type: "text" }],
    ...
    it("caps tool call IDs at 40 chars while preserving uniqueness", () => {
      ...
      const input = castAgentMessages([
        {
          content: [
            { arguments: {}, id: longA, name: "read", type: "toolCall" },
            { arguments: {}, id: longB, name: "read", type: "toolCall" },
          ],
          role: "assistant",
        },
        {
  ...
  describe("strict mode (alphanumeric only)", () => {
    ...
    it("preserves native anthropic ids while sanitizing mixed-provider ids when requested", () => {
      ...
      const input = castAgentMessages([
        {
          content: [
            { id: nativeId, input: { path: "IDENTITY.md" }, name: "read", type: "toolUse" },
            { id: nonNativeId, input: { path: "README.md" }, name: "read", type: "toolUse" },
          ],
          role: "assistant",
        },
        {
          content: [{ text: "identity", type: "text" }],
          role: "toolResult",
          toolCallId: nativeId,
          toolName: "read",
          toolUseId: nativeId,
      ...
      expect((out[0] as Extract<AgentMessage, { role: "assistant" }>).content).toEqual([
        { id: nativeId, input: { path: "IDENTITY.md" }, name: "read", type: "toolUse" },
        { id: "call123fc123", input: { path: "README.md" }, name: "read", type: "toolUse" },
      ]);
      expect(
  ...
  describe("strict9 mode (Mistral tool call IDs)", () => {
    it("is a no-op for already-valid 9-char alphanumeric IDs", () => {
      const input = castAgentMessages([
        {
          content: [{ arguments: {}, id: "abc123XYZ", name: "read", type: "toolCall" }],
          role: "assistant",
        },
        {
          content: [{ text: "ok", type: "text" }],
    ...
    it("enforces alphanumeric IDs with length 9", () => {
      const input = castAgentMessages([
        {
          content: [
            { arguments: {}, id: "call_abc|item:123", name: "read", type: "toolCall" },
            { arguments: {}, id: "call_abc|item:456", name: "read", type: "toolCall" },
          ],
          role: "assistant",
        },
        {
