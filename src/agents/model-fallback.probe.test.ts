...
async function loadModelFallbackProbeModules() {
  ...
  ({ runWithModelFallback } = modelFallbackModule);
  ({ _probeThrottleInternals } = modelFallbackModule);
  ({ registerLogTransport } = loggerModule);
  ({ resetLogger } = loggerModule);
...
function expectFallbackUsed(
  result: { result: unknown; attempts: { reason?: string }[] },
  run: {
...
describe("runWithModelFallback – probe logic", () => {
  ...
  it("prunes stale probe throttle entries before checking eligibility", () => {
    ...
    _probeThrottleInternals.lastProbeAttempt.set("fresh", NOW - 5000);
