...
export function consumePendingToolMediaReply(
  ...
  const payload: BlockReplyPayload = {
    ...
    mediaUrls: state.pendingToolMediaUrls.length
      ? [...new Set(state.pendingToolMediaUrls)]
      : undefined,
  };
  clearPendingToolMedia(state);
  return payload;
...
export function handleMessageUpdate(
  ...
  if (next) {
    ...
    let deltaText = "";
    let replace = false;
    if (!hasAssistantVisibleReply({ audioAsVoice: hasAudio, mediaUrls, text: cleanedText })) {
      shouldEmit = false;
    } else {
  ...
  if (
    ...
    ctx.blockChunker?.drain({ emit: ctx.emitBlockChunk, force: false });
  }
  ...
  if (
    ...
    void Promise.resolve()
      ...
      .catch((error) => {
        ctx.log.debug(`text_end block reply flush failed: ${String(error)}`);
      });
  }
}
...
export function handleMessageEnd(
  ...
  const text = resolveSilentReplyFallbackText({
    ...
    text: ctx.stripBlockTags(rawVisibleText, { final: false, thinking: false }),
  });
  const rawThinking =
  ...
  const maybeEmitReasoning = () => {
    ...
    ctx.emitBlockReply({ isReasoning: true, text: formattedReasoning });
  };
  ...
  const emitSplitResultAsBlockReply = (
    ...
    if (hasAssistantVisibleReply({ audioAsVoice, mediaUrls, text: cleanedText })) {
      ctx.emitBlockReply({
        audioAsVoice,
        mediaUrls: mediaUrls?.length ? mediaUrls : undefined,
        replyToCurrent,
  ...
  if (
    ...
    if (hasBufferedBlockReply && ctx.blockChunker?.hasBuffered()) {
      ctx.blockChunker.drain({ emit: ctx.emitBlockChunk, force: true });
      ctx.blockChunker.reset();
