...
export interface AnnounceQueueSettings {
  mode: QueueMode;
  debounceMs?: number;
  cap?: number;
    ...
    try {
      ...
      for (;;) {
        ...
        if (queue.mode === "collect") {
          ...
          const summary = previewQueueSummaryPrompt({ noun: "announce", state: queue });
        ...
        const summaryPrompt = previewQueueSummaryPrompt({ noun: "announce", state: queue });
        if (summaryPrompt) {
          if (
      ...
      defaultRuntime.error?.(
        `announce queue drain failed for ${key} (attempt ${queue.consecutiveFailures}, retry in ${Math.round(retryDelayMs / 1000)}s): ${String(error)}`,
      );
    } finally {
