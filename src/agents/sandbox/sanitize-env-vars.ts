const BLOCKED_ENV_VAR_PATTERNS: readonly RegExp[] = [
  /^ANTHROPIC_API_KEY$/i,
  /^OPENAI_API_KEY$/i,
  /^GEMINI_API_KEY$/i,
...
const ALLOWED_ENV_VAR_PATTERNS: readonly RegExp[] = [
  /^LANG$/,
  /^LC_.*$/i,
  /^PATH$/i,
  /^HOME$/i,
...
export interface EnvSanitizationOptions {
  strictMode?: boolean;
  customBlockedPatterns?: readonly RegExp[];
  customAllowedPatterns?: readonly RegExp[];
}
