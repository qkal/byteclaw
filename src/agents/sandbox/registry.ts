...
export interface SandboxBrowserRegistryEntry {
  containerName: string;
  sessionKey: string;
  createdAtMs: number;
  lastUsedAtMs: number;
...
interface SandboxBrowserRegistry {
...
interface RegistryFile<T extends RegistryEntry> {
  entries: T[];
}
...
async function withRegistryLock<T>(registryPath: string, fn: () => Promise<T>): Promise<T> {
  const lock = await acquireSessionWriteLock({ allowReentrant: false, sessionFile: registryPath });
  try {
    return await fn();
  } finally {
    await lock.release();
