...
describe("system prompt stability for cache hits", () => {
  ...
  it("returns consistent ordering across calls", async () => {
    const testFiles = [
      { content: "# Agents content", name: DEFAULT_AGENTS_FILENAME },
      { content: "# Tools content", name: DEFAULT_TOOLS_FILENAME },
      { content: "# Soul content", name: DEFAULT_SOUL_FILENAME },
    ];

    // Write all test files
    for (const file of testFiles) {
      await writeWorkspaceFile({ content: file.content, dir: workspaceDir, name: file.name });
    }
