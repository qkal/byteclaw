...
export function resolveSessionIdToSend(params: {
  ...
  if (mode === "none") {
    return { isNew: !existing, sessionId: undefined };
  }
  if (mode === "existing") {
    return { isNew: !existing, sessionId: existing };
  }
  if (existing) {
    return { isNew: false, sessionId: existing };
  }
  return { isNew: true, sessionId: crypto.randomUUID() };
}
...
export async function writeCliImages(params: {
  ...
  await fs.mkdir(imageRoot, { mode: 0o700, recursive: true });
  const paths: string[] = [];
