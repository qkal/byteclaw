...
interface PreparedCliBundleMcpConfig {
...
function decodeHeaderEnvPlaceholder(value: string): { envVar: string; bearer: boolean } | null {
  ...
  if (bearerMatch) {
    return { bearer: true, envVar: bearerMatch[1] };
  }
  const envMatch = /^\${([A-Z0-9_]+)}$/.exec(value);
  if (envMatch) {
    return { bearer: false, envVar: envMatch[1] };
  }
  return null;
}
...
async function prepareModeSpecificBundleMcpConfig(params: {
  ...
  await fs.writeFile(mcpConfigPath, serializedConfig, "utf8");
  return {
    backend: {
      ...params.backend,
