...
import { type EnvApiKeyResult, resolveEnvApiKey } from "./model-auth-env.js";
import {
...
interface ResolvedCustomProviderApiKey {
  apiKey: string;
  source: string;
}
...
export function hasUsableCustomProviderApiKey(
  ...
  return Boolean(resolveUsableCustomProviderApiKey({ cfg, env, provider }));
}
...
interface SyntheticProviderAuthResolution {
...
export async function resolveApiKeyForProvider(params: {
  ...
  if (profileId) {
    ...
    if (
      ...
      return resolveApiKeyForProvider({ ...params, lockedProfile: true, profileId: undefined }) //
        .catch(() => result);
    }
    return result;
  }
  ...
  const customKey = resolveUsableCustomProviderApiKey({ cfg, provider });
  if (customKey) {
    const result = { apiKey: customKey.apiKey, mode: "api-key" as const, source: customKey.source };
    return result;
  }
