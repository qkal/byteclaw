...
describe("installSessionToolResultGuard", () => {
  ...
  it("backfills blank toolResult names from pending tool calls", () => {
    ...
    const messages = expectPersistedRoles(sm, ["assistant", "toolResult"]) as {
      role: string;
      toolName?: string;
    }[];
  ...
  it("preserves ordering with multiple tool calls and partial results", () => {
    ...
    sm.appendMessage(
      asAppendMessage({
        content: [
          { arguments: {}, id: "call_a", name: "one", type: "toolCall" },
          { arguments: {}, id: "call_b", name: "two", type: "toolUse" },
        ],
        role: "assistant",
      }),
    );
    ...
    sm.appendMessage(
      asAppendMessage({
        content: [{ text: "after tools", type: "text" }],
        role: "assistant",
      }),
    );
  ...
  it("flushes pending on guard when no toolResult arrived", () => {
    ...
    sm.appendMessage(
      asAppendMessage({
        content: [{ text: "hard error", type: "text" }],
        role: "assistant",
        stopReason: "error",
      }),
    );
  ...
  it("handles toolUseId on toolResult", () => {
    ...
    sm.appendMessage(
      asAppendMessage({
        content: [{ arguments: {}, id: "use_1", name: "f", type: "toolUse" }],
        role: "assistant",
      }),
    );
    sm.appendMessage(
  ...
  it("drops older pending ids before new tool calls when synthetic results are disabled", () => {
    ...
    sm.appendMessage(
      asAppendMessage({
        content: [{ arguments: {}, id: "call_2", name: "read", type: "toolCall" }],
        role: "assistant",
      }),
    );
      ...
      beforeMessageWriteHook: ({ message }) => {
        ...
        return {
          message: castAgentMessage({
            ...
            content: [{ text: "rewritten by hook", type: "text" }],
          }),
        };
      },
    });
  ...
  it("does NOT create synthetic toolResult for aborted assistant messages with toolCalls", () => {
    ...
    sm.appendMessage(
      asAppendMessage({
        content: [{ arguments: {}, id: "call_aborted", name: "read", type: "toolCall" }],
        role: "assistant",
        stopReason: "aborted",
      }),
    );
