...
describe("openai transport stream", () => {
  ...
  it.each([
    ...
    const params = buildOpenAIResponsesParams(
      ...
      input?: { role?: string; id?: string; phase?: string }[];
    };
  ...
  it("flattens pure text content arrays for string-only completions backends when opted in", () => {
    ...
    expect(params.messages?.[0]).toMatchObject({ content: "system", role: "system" });
    expect(params.messages?.[1]).toMatchObject({ content: "What is 2 + 2?", role: "user" });
  });
  ...
  it("omits strict tool shaping for Z.ai default-route completions providers", () => {
    const params = buildOpenAICompletionsParams(
      ...
      undefined,
    ) as { tools?: { function?: { strict?: boolean } }[] };
