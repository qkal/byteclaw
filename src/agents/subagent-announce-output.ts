...
interface SubagentAnnounceOutputDeps {
...
interface SubagentOutputSnapshot {
...
function summarizeSubagentOutputHistory(messages: unknown[]): SubagentOutputSnapshot {
...
export async function readSubagentOutput(
  ...
  const history = await subagentAnnounceOutputDeps.callGateway<{ messages?: unknown[] }>({
    method: "chat.history",
    params: { limit: 100, sessionKey },
  });
...
export function applySubagentWaitOutcome(params: {
  ...
  if (params.wait?.status === "timeout") {
    ...
    next.outcome = { error: waitError, status: "error" };
