...
interface OpenClawReadToolOptions {
...
function withToolResultText(
  ...
  const textBlock = { text, type: "text" } as unknown as TextContentBlock;
  return {
    ...result,
...
async function executeReadWithAdaptivePaging(params: {
  ...
  for (let page = 0; page < MAX_ADAPTIVE_READ_PAGES; page += 1) {
    ...
    const nextBytes = Buffer.byteLength(`${delimiter}${pageText}`, "utf8");
...
interface MemoryFlushAppendOnlyWriteOptions {
  root: string;
  relativePath: string;
  containerWorkdir?: string;
  sandbox?: {
...
async function readOptionalUtf8File(params: {
  ...
  try {
    ...
    return await fs.readFile(params.absolutePath, "utf8");
  } catch (error) {
...
async function appendMemoryFlushContent(params: {
  ...
  await fs.writeFile(params.absolutePath, next, "utf8");
}
...
export function wrapToolMemoryFlushAppendOnlyWrite(
  ...
  return {
    ...
    execute: async (toolCallId, args, signal, onUpdate) => {
      ...
      return {
        content: [{ text: `Appended content to ${options.relativePath}.`, type: "text" }],
        details: {
          appendOnly: true,
  ...
  return {
    ...
    execute: async (toolCallId, args, signal, onUpdate) => {
      ...
      for (const key of pathParamKeys) {
        ...
        const sandboxResult = await assertSandboxPath({ cwd: root, filePath: sandboxPath, root });
...
export function createSandboxedEditTool(params: SandboxToolParams) {
  ...
  const withRecovery = wrapEditToolWithRecovery(base, {
    readFile: async (absolutePath: string) =>
      (await params.bridge.readFile({ cwd: params.root, filePath: absolutePath })).toString("utf8"),
    root: params.root,
  });
...
export function createHostWorkspaceEditTool(root: string, options?: { workspaceOnly?: boolean }) {
  ...
  const withRecovery = wrapEditToolWithRecovery(base, {
    readFile: (absolutePath: string) => fs.readFile(absolutePath, "utf8"),
    root,
  });
  return wrapToolParamValidation(withRecovery, REQUIRED_PARAM_GROUPS.edit);
}
...
function createSandboxReadOperations(params: SandboxToolParams) {
  return {
    access: async (absolutePath: string) => {
      const stat = await params.bridge.stat({ cwd: params.root, filePath: absolutePath });
      if (!stat) {
        throw createFsAccessError("ENOENT", absolutePath);
      }
    },
    detectImageMimeType: async (absolutePath: string) => {
      const buffer = await params.bridge.readFile({ cwd: params.root, filePath: absolutePath });
      const mime = await detectMime({ buffer, filePath: absolutePath });
      return mime && mime.startsWith("image/") ? mime : undefined;
    },
    readFile: (absolutePath: string) =>
      params.bridge.readFile({ cwd: params.root, filePath: absolutePath }),
  } as const;
}
...
function createSandboxWriteOperations(params: SandboxToolParams) {
  return {
    mkdir: async (dir: string) => {
      await params.bridge.mkdirp({ cwd: params.root, filePath: dir });
    },
    writeFile: async (absolutePath: string, content: string) => {
      await params.bridge.writeFile({ cwd: params.root, data: content, filePath: absolutePath });
    },
  } as const;
}
...
function createSandboxEditOperations(params: SandboxToolParams) {
  return {
    ...
    writeFile: (absolutePath: string, content: string) =>
      params.bridge.writeFile({ cwd: params.root, data: content, filePath: absolutePath }),
  } as const;
}
...
function createHostWriteOperations(root: string, options?: { workspaceOnly?: boolean }) {
  ...
  return {
    mkdir: async (dir: string) => {
      ...
      await assertSandboxPath({ cwd: root, filePath: resolved, root });
      await fs.mkdir(resolved, { recursive: true });
    },
    writeFile: async (absolutePath: string, content: string) => {
