...
export interface CliStreamingDelta {
  text: string;
  delta: string;
  sessionId?: string;
  usage?: CliUsage;
...
function parseClaudeCliJsonlResult(params: {
  ...
  if (
    ...
    if (resultText) {
      return { sessionId: params.sessionId, text: resultText, usage: params.usage };
    }
    ...
    return { sessionId: params.sessionId, text: "", usage: params.usage };
  }
  return null;
}
...
export function parseCliOutput(params: {
  ...
  if (outputMode === "text") {
    return { sessionId: params.fallbackSessionId, text: params.raw.trim() };
  }
  if (outputMode === "jsonl") {
    return (
