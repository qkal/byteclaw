...
interface GatewayAgentInternalEvent {
  status?: string;
  statusLabel?: string;
  result?: string;
}

interface GatewayAgentRequestParams {
  sessionKey?: string;
  inputProvenance?: {
...
let agentCallPlan: ("ok" | "throw")[] = [];
let chatHistoryBySessionKey = new Map<string, Record<string, unknown>[]>();
...
describe("subagent registry lifecycle error grace", () => {
  ...
  it("ignores transient lifecycle errors when run retries and then ends successfully", async () => {
    ...
    emitLifecycleEvent("run-transient-error", { phase: "start", startedAt: 1050 });
    await flushAsync();

    await vi.advanceTimersByTimeAsync(20_000);
    expect(getAgentCalls()).toHaveLength(0);

    emitLifecycleEvent("run-transient-error", { endedAt: 1250, phase: "end" });
    await flushAsync();
  ...
  it("freezes completion result at run termination across deferred announce retries", async () => {
    ...
    emitLifecycleEvent("run-freeze", { endedAt, phase: "end" });
    await flushAsync();
    ...
    emitLifecycleEvent("run-freeze", { endedAt: endedAt + 100, phase: "end" });
    await flushAsync();
  ...
  it("refreshes frozen completion output from later turns in the same session", async () => {
    ...
    const endedAt = Date.now();
    emitLifecycleEvent("run-refresh", { endedAt, phase: "end" });
    await flushAsync();
    ...
    emitLifecycleEvent("run-refresh", { endedAt: endedAt + 300, phase: "end" });
    await flushAsync();
  ...
  it("ignores silent follow-up turns when refreshing frozen completion output", async () => {
    ...
    const endedAt = Date.now();
    emitLifecycleEvent("run-refresh-silent", { endedAt, phase: "end" });
    await flushAsync();
    ...
    emitLifecycleEvent("run-refresh-silent", { endedAt: endedAt + 300, phase: "end" });
    await flushAsync();
  ...
  it("regression, captures frozen completion output with 100KB cap and retains it for keep-mode cleanup", async () => {
    ...
    emitLifecycleEvent("run-capped", { endedAt: Date.now(), phase: "end" });
    await flushAsync();
  ...
  it("keeps parallel child completion results frozen even when late traffic arrives", async () => {
    ...
    emitLifecycleEvent("run-parallel-a", { endedAt: parallelEndedAt, phase: "end" });
    emitLifecycleEvent("run-parallel-b", { endedAt: parallelEndedAt + 1, phase: "end" });
    await flushAsync();
    ...
    emitLifecycleEvent("run-parallel-a", { endedAt: parallelEndedAt + 100, phase: "end" });
    emitLifecycleEvent("run-parallel-b", { endedAt: parallelEndedAt + 101, phase: "end" });
    await flushAsync();
