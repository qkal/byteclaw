...
import { type Skill, formatSkillsForPrompt } from "./skill-contract.js";
import type {
...
function filterSkillEntries(
  ...
  let filtered = entries.filter((entry) => shouldIncludeSkill({ config, eligibility, entry }));
...
interface ResolvedSkillsLimits {
...
function unwrapLoadedSkills(loaded: unknown): Skill[] {
  ...
  if (loaded && typeof loaded === "object" && "skills" in loaded) {
    const {skills} = (loaded as { skills?: unknown });
...
function loadSkillEntries(
  ...
  const loadSkills = (params: { dir: string; source: string }): Skill[] => {
    ...
    if (fs.existsSync(rootSkillMd)) {
      ...
      try {
        const {size} = fs.statSync(rootSkillRealPath);
    ...
    for (const name of limitedChildren) {
      ...
      try {
        const {size} = fs.statSync(skillMdRealPath);
        if (size > limits.maxSkillFileBytes) {
  ...
  const skillEntries: SkillEntry[] = [...merged.values()].map((skill) => {
    const frontmatter =
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: OpenClawConfig }): {
  ...
  return { compact, skillsForPrompt, truncated };
}
...
interface WorkspaceSkillBuildOptions {
  config?: OpenClawConfig;
  managedSkillsDir?: string;
  bundledSkillsDir?: string;
  entries?: SkillEntry[];
...
export async function syncSkillsToWorkspace(params: {
  ...
  await serializeByKey(`syncSkills:${targetDir}`, async () => {
    ...
    await fsp.rm(targetSkillsDir, { force: true, recursive: true });
