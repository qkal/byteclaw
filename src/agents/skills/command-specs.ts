...
import type { SkillCommandSpec, SkillEligibilityContext, SkillEntry } from "./types.js";
import {
    ...
    const dispatch = (() => {
      ...
      if (!toolName) {
        debugSkillCommandOnce(
          ...
          { command: unique, skillName: rawName },
        );
        return undefined;
      }
      ...
      if (!argMode) {
        debugSkillCommandOnce(
          ...
          { argMode: argModeRaw, command: unique, skillName: rawName },
        );
      }

      return { argMode: "raw", kind: "tool", toolName } as const;
    })();
  ...
  for (const entry of bundleCommands) {
    ...
    if (unique !== base) {
      debugSkillCommandOnce(
        ...
        { deduped: `/${unique}`, rawName: entry.rawName },
      );
    }
    used.add(normalizeLowercaseStringOrEmpty(unique));
    const description =
