...
const STREAM_BUFFER_MAX_CHARS = 4000;
...
export function startAcpSpawnParentStreamRelay(params: {
  ...
  const maxRelayLifetimeMs =
    typeof params.maxRelayLifetimeMs === "number" && Number.isFinite(params.maxRelayLifetimeMs)
      ? Math.max(1000, Math.floor(params.maxRelayLifetimeMs))
  ...
  const unsubscribe = onAgentEvent((event) => {
    ...
    logEvent("lifecycle", { data: event.data, phase: phase ?? "unknown" });
    if (phase === "end") {
      flushPending();
...
export interface AcpSpawnParentRelayHandle {
  dispose: () => void;
  notifyStarted: () => void;
}
