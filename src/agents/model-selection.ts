...
function buildModelCatalogMetadata(params: {
  ...
  return { aliasByKey, configuredByKey };
}
...
export function resolveModelRefFromString(params: {
  ...
  if (!model.includes("/")) {
    ...
    if (aliasMatch) {
      return { alias: aliasMatch.alias, ref: aliasMatch.ref };
    }
  }
...
export function resolveConfiguredModelRef(params: {
  ...
  if (rawModel) {
    ...
    if (!trimmed.includes("/")) {
      ...
      if (inferredProvider) {
        return { model: trimmed, provider: inferredProvider };
      }
      ...
      return { model: trimmed, provider: params.defaultProvider };
    }
  ...
  return { model: params.defaultModel, provider: params.defaultProvider };
}
...
export function buildAllowedModelSet(params: {
  ...
  for (const raw of rawAllowlist) {
    ...
    if (!catalogKeys.has(key) && !syntheticCatalogEntries.has(key)) {
      syntheticCatalogEntries.set(key, buildSyntheticAllowedCatalogEntry({ metadata, parsed }));
    }
  }
...
export function resolveAllowedModelRef(params: {
  ...
  return { key: status.key, ref: resolved.ref };
}
