...
function makeAssistantToolCall(
  ...
  return makeAgentAssistantMessage({
    content: [
      { text, type: "text" },
      { arguments: {}, id: toolCallId, name: "test_tool", type: "toolCall" },
    ],
    model: "gpt-5.4",
    stopReason,
    timestamp,
...
function pruneLargeSimpleHistory() {
  ...
  return { maxContextTokens, messages, pruned };
}
...
describe("splitMessagesByTokenShare", () => {
  ...
  it("keeps multiple toolResults with their assistant in the same chunk", () => {
    const assistant = makeAgentAssistantMessage({
      content: [
        { text: "x".repeat(4000), type: "text" },
        { arguments: {}, id: "call_a", name: "tool_a", type: "toolCall" },
        { arguments: {}, id: "call_b", name: "tool_b", type: "toolCall" },
      ],
      model: "gpt-5.2",
      stopReason: "stop",
      timestamp: 2,
