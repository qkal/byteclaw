...
interface ApiKeyRetryParams {
  apiKey: string;
  error: unknown;
  attempt: number;
}

interface ExecuteWithApiKeyRotationOptions<T> {
  provider: string;
  apiKeys: string[];
  ...
  for (let attempt = 0; attempt < keys.length; attempt += 1) {
    ...
    try {
      ...
      const retryable = params.shouldRetry
        ? params.shouldRetry({ apiKey, attempt, error, message })
      ...
      params.onRetry?.({ apiKey, attempt, error, message });
    }
  }
