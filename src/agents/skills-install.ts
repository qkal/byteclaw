...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  const timeoutMs = Math.min(Math.max(params.timeoutMs ?? 300_000, 1000), 900_000);
  ...
  const uvInstallFailure = await ensureUvInstalled({ brewExe, spec, timeoutMs });
  if (uvInstallFailure) {
    return withWarnings(uvInstallFailure, warnings);
  }

  const goInstallFailure = await ensureGoInstalled({ brewExe, spec, timeoutMs });
  if (goInstallFailure) {
  ...
  return withWarnings(await executeInstallCommand({ argv, env, timeoutMs }), warnings);
}
