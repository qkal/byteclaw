...
function analyzeInterpreterHeuristicsFromUnquoted(raw: string): {
  ...
  return { hasComplexSyntax, hasNode, hasProcessSubstitution, hasPython, hasScriptHint };
}
...
async function validateScriptFileForShellBleed(params: {
  ...
  for (const relOrAbsPath of target.relOrAbsPaths) {
    ...
    try {
      ...
      content = safeRead.buffer.toString("utf8");
    } catch (error) {
...
interface ParsedExecApprovalCommand {
  approvalId: string;
  ...
  return {
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      if (host === "gateway" && !bypassApprovals) {
        ...
        ({ execCommandOverride } = gatewayResult);
