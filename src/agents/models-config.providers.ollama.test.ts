...
describe("Ollama provider", () => {
  ...
  async function resolveProvidersWithOllamaKey(agentDir: string) {
    return withOllamaApiKey(() =>
      resolveProvidersWithOllamaOnly({
        agentDir,
        env: { NODE_ENV: "development", VITEST: "" },
      }),
    );
  }
  ...
  const createTagModel = (name: string) => ({ digest: "", modified_at: "", name, size: 1 });
  ...
  it("discovers per-model context windows from /api/show", async () => {
    ...
    const fetchMock = vi.fn(async (input: unknown, init?: RequestInit) => {
      ...
      if (url.endsWith("/api/show")) {
        ...
        if (parsed.name === "qwen3:32b") {
          return {
            json: async () => ({ model_info: { "qwen3.context_length": 131_072 } }),
            ok: true,
          };
        }
    ...
    expect(qwen?.contextWindow).toBe(131_072);
    expect(llama?.contextWindow).toBe(65_536);
    expectDiscoveryCallCounts(fetchMock, { show: 2, tags: 1 });
  });
  ...
  it("falls back to default context window when /api/show fails", async () => {
    ...
    expectDiscoveryCallCounts(fetchMock, { show: 1, tags: 1 });
  });
  ...
  it("caps /api/show requests when /api/tags returns a very large model list", async () => {
    ...
    expectDiscoveryCallCounts(fetchMock, { show: 200, tags: 1 });
          ...
          models: {
            providers: {
              ollama: {
                ...
                models: explicitModels, // Pragma: allowlist secret
              },
            },
          },
        },
