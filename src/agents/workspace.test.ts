...
async function readWorkspaceState(dir: string): Promise<{
  ...
  const raw = await fs.readFile(path.join(dir, ...WORKSPACE_STATE_PATH_SEGMENTS), "utf8");
...
describe("ensureAgentWorkspace", () => {
  ...
  it("recovers partial initialization by creating BOOTSTRAP.md when marker is missing", async () => {
    const tempDir = await makeTempWorkspace("openclaw-workspace-");
    await writeWorkspaceFile({ content: "existing", dir: tempDir, name: DEFAULT_AGENTS_FILENAME });
  ...
  it("does not recreate BOOTSTRAP.md after completion, even when a core file is recreated", async () => {
    const tempDir = await makeTempWorkspace("openclaw-workspace-");
    await ensureAgentWorkspace({ dir: tempDir, ensureBootstrapFiles: true });
    await writeWorkspaceFile({ content: "custom", dir: tempDir, name: DEFAULT_IDENTITY_FILENAME });
    await writeWorkspaceFile({ content: "custom", dir: tempDir, name: DEFAULT_USER_FILENAME });
  ...
  it("treats memory-backed workspaces as existing even when template files are missing", async () => {
    ...
    const state = await readWorkspaceState(tempDir);
    expect(state.setupCompletedAt).toMatch(/\d{4}-\d{2}-\d{2}T/);
    const memoryContent = await fs.readFile(path.join(tempDir, "MEMORY.md"), "utf8");
  ...
  it("writes the current fenced HEARTBEAT template body into new workspaces", async () => {
    ...
    const heartbeat = await fs.readFile(path.join(tempDir, DEFAULT_HEARTBEAT_FILENAME), "utf8");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("includes MEMORY.md when present", async () => {
    const tempDir = await makeTempWorkspace("openclaw-workspace-");
    await writeWorkspaceFile({ content: "memory", dir: tempDir, name: "MEMORY.md" });
  ...
  it("includes memory.md when MEMORY.md is absent", async () => {
    const tempDir = await makeTempWorkspace("openclaw-workspace-");
    await writeWorkspaceFile({ content: "alt", dir: tempDir, name: "memory.md" });
...
describe("filterBootstrapFilesForSession", () => {
  const mockFiles: WorkspaceBootstrapFile[] = [
    { content: "", missing: false, name: "AGENTS.md", path: "/w/AGENTS.md" },
    { content: "", missing: false, name: "SOUL.md", path: "/w/SOUL.md" },
    { content: "", missing: false, name: "TOOLS.md", path: "/w/TOOLS.md" },
    { content: "", missing: false, name: "IDENTITY.md", path: "/w/IDENTITY.md" },
    { content: "", missing: false, name: "USER.md", path: "/w/USER.md" },
    { content: "", missing: false, name: "HEARTBEAT.md", path: "/w/HEARTBEAT.md" },
    { content: "", missing: false, name: "BOOTSTRAP.md", path: "/w/BOOTSTRAP.md" },
    { content: "", missing: false, name: "MEMORY.md", path: "/w/MEMORY.md" },
  ];
