...
describe("spawnSubagentDirect filename validation", () => {
  ...
  afterEach(() => {
    if (workspaceDirOverride) {
      fs.rmSync(workspaceDirOverride, { force: true, recursive: true });
  ...
  async function spawnWithName(name: string) {
    ...
    return spawnSubagentDirect(
      {
        attachments: [{ content: validContent, encoding: "base64", name }],
        task: "test",
      },
      ctx,
    );
  ...
  it("duplicate name returns attachments_duplicate_name", async () => {
    ...
    const result = await spawnSubagentDirect(
      {
        attachments: [
          { content: validContent, encoding: "base64", name: "file.txt" },
          { content: validContent, encoding: "base64", name: "file.txt" },
        ],
        task: "test",
      },
  ...
  it("removes materialized attachments when lineage patching fails", async () => {
    ...
    const result = await spawnSubagentDirect(
      {
        attachments: [{ content: validContent, encoding: "base64", name: "file.txt" }],
        task: "test",
      },
      ctx,
    );
