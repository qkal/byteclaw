...
describe("subagent registry persistence", () => {
  ...
  it("skips cleanup when cleanupHandled was persisted", async () => {
    ...
    const persisted = {
      runs: {
        "run-2": {
          ...
          runId: "run-2",
          startedAt: 1,
          task: "do the other thing", // Already handled - should be skipped
        },
      },
      version: 2,
    };
    ...
    const calls = (announceSpy.mock.calls as unknown as [unknown][]).map((call) => call[0]);
  ...
  it("prefers active runs and can resolve them from persisted registry snapshots", async () => {
    ...
    const resolved = withEnv({ NODE_ENV: "development", VITEST: undefined }, () =>
