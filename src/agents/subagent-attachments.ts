...
export interface SubagentInlineAttachment {
  name: string;
  content: string;
...
export interface SubagentAttachmentReceiptFile {
  name: string;
  bytes: number;
  sha256: string;
}

export interface SubagentAttachmentReceipt {
  count: number;
  totalBytes: number;
...
export async function materializeSubagentAttachments(params: {
  ...
  try {
    await fs.mkdir(absDir, { mode: 0o700, recursive: true });

    const seen = new Set<string>();
    const files: SubagentAttachmentReceiptFile[] = [];
    const writeJobs: { outPath: string; buf: Buffer }[] = [];
    let totalBytes = 0;
    ...
    for (const raw of requestedAttachments) {
      ...
      writeJobs.push({ buf, outPath });
      files.push({ bytes, name, sha256 });
    }

    await Promise.all(
      writeJobs.map(({ outPath, buf }) => fs.writeFile(outPath, buf, { flag: "wx", mode: 0o600 })),
    );
    ...
    try {
      await fs.rm(absDir, { force: true, recursive: true });
    } catch {
      // Best-effort cleanup only.
    }
    return {
      error: error instanceof Error ? error.message : "attachments_materialization_failed",
      status: "error",
    };
  }
}
