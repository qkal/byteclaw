...
describe("failover-error", () => {
  ...
  it("401/403 with generic message still returns auth (backward compat)", () => {
    expect(resolveFailoverReasonFromError({ message: "Unauthorized", status: 401 })).toBe("auth");
    expect(resolveFailoverReasonFromError({ message: "Forbidden", status: 403 })).toBe("auth");
  });

  it("401 with ambiguous auth message returns auth", () => {
    expect(resolveFailoverReasonFromError({ message: "invalid_api_key", status: 401 })).toBe(
      "auth",
    );
  });

  it("403 with revoked key message returns auth_permanent", () => {
    expect(resolveFailoverReasonFromError({ message: "api key revoked", status: 403 })).toBe(
      "auth_permanent",
    );
  });
  ...
  it("coerces ambiguous auth error into the short auth lane", () => {
    const err = coerceToFailoverError(
      { message: "invalid_api_key", status: 401 },
  ...
  it("403 bare permission_error returns auth", () => {
    expect(resolveFailoverReasonFromError({ message: "permission_error", status: 403 })).toBe(
      "auth",
    );
  });
