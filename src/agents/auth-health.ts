...
function resolveOAuthStatus(
  ...
  if (remainingMs <= 0) {
    return { remainingMs, status: "expired" };
  }
  if (remainingMs <= warnAfterMs) {
    return { remainingMs, status: "expiring" };
  }
  return { remainingMs, status: "ok" };
}
...
export function buildAuthHealthSummary(params: {
  ...
  const providers = [...providersMap.values()].toSorted((a, b) =>
    a.provider.localeCompare(b.provider),
  );

  return { now, profiles, providers, warnAfterMs };
}
