...
class LocalAssistantMessageEventStream implements AssistantMessageEventStreamLike {
  ...
  push(event: AssistantMessageEvent): void {
    ...
    while (this.waiting.length > 0) {
      const waiter = this.waiting.shift();
      waiter?.({ done: true, value: undefined as unknown as AssistantMessageEvent });
    }
  }
...
export function hasWsSession(sessionId: string): boolean {
  ...
  return Boolean(s && !s.broken && s.manager.isConnected());
}
      ...
      while (true) {
        ...
        if (!session.manager.isConnected() && !session.broken) {
          try {
            ...
            log.warn(
              `[ws-stream] WebSocket connect failed for session=${sessionId}; falling back to HTTP. error=${String(error)}`,
            );
            return fallbackToHttp(model, context, options, apiKey, eventStream, opts.signal, {
              suppressStart: emittedStart,
              turnState: resolveProviderTransportTurnState(model, {
        ...
        if (resolveWsWarmup(options) && !session.warmUpAttempted) {
          ...
          try {
            ...
            warmupFailed = true;
            log.warn(
              `[ws-stream] warm-up failed for session=${sessionId}; continuing without warm-up. error=${String(error)}`,
            );
          }
        ...
        try {
          ...
          const normalizedErr = buildRetryableSendError(error);
          if (
            transport !== "websocket" &&
            !signal?.aborted &&
        ...
        try {
          await new Promise<void>((resolve, reject) => {
            ...
            const unsubscribe = session.manager.onMessage((event) => {
              ...
              if (event.type === "response.completed") {
                ...
                eventStream.push({ message: assistantMsg, reason, type: "done" });
                resolve();
              } else if (event.type === "response.failed") {
                outputItemPhaseById.clear();
                outputTextByPart.clear();
          ...
          const normalizedErr = normalizeWsRunError(error);
          if (
            transport !== "websocket" &&
            !signal?.aborted &&
