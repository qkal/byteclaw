...
async function loadMarkerModules() {
  ...
  ({ listKnownProviderEnvApiKeyNames } = envVarsModule);
  ({ GCP_VERTEX_CREDENTIALS_MARKER } = markersModule);
  ({ NON_ENV_SECRETREF_MARKER } = markersModule);
  ({ isKnownEnvApiKeyMarker } = markersModule);
  ({ isNonSecretApiKeyMarker } = markersModule);
  ({ resolveOAuthApiKeyMarker } = markersModule);
}
