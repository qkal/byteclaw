...
const LIVE_HEARTBEAT_MS = Math.max(1000, toInt(process.env.OPENCLAW_LIVE_HEARTBEAT_MS, 30_000));
  ...
  it(
    ...
    async () => {
      ...
      for (const model of models) {
        ...
        if (!filter && useModern) {
          if (!isHighSignalLiveModelRef({ id: model.id, provider: model.provider })) {
            continue;
          }
        }
        try {
          ...
          candidates.push({ apiKeyInfo, model });
        } catch (error) {
          skipped.push({ model: id, reason: String(error) });
        }
      }
      ...
      const selectedCandidates = selectHighSignalLiveItems(
        ...
        (entry) => ({ id: entry.model.id, provider: entry.model.provider }),
      ...
      for (const [index, entry] of selectedCandidates.entries()) {
        ...
        for (let attempt = 0; attempt < attemptMax; attempt += 1) {
          ...
          try {
            ...
            if (
              (model.provider === "google" || model.provider === "google-gemini-cli") &&
              isGoogleModelNotFoundError(error)
            ) {
              skipped.push({ model: id, reason: message });
              logProgress(`${progressLabel}: skip (google model not found)`);
              break;
            ...
            failures.push({ error: message, model: id });
            break;
          }
        }
      }
