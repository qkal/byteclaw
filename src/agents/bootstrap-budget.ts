...
export interface BootstrapInjectionStat {
  name: string;
  path: string;
  missing: boolean;
  rawChars: number;
...
export interface BootstrapPromptWarning {
  signature?: string;
...
export interface BootstrapTruncationReportMeta {
...
export function analyzeBootstrapBudget(params: {
  ...
  const files = params.files.map((file) => {
    if (file.missing) {
      return { ...file, causes: [], nearLimit: false };
    }
