...
describe("buildWorkspaceSkillCommandSpecs", () => {
  ...
  it("includes tool-dispatch metadata from frontmatter", async () => {
    ...
    expect(cmd?.dispatch).toEqual({ argMode: "raw", kind: "tool", toolName: "sessions_send" });
  });
...
describe("applySkillEnvOverrides", () => {
  it("sets and restores env vars", async () => {
    ...
    withClearedEnv(["ENV_KEY"], () => {
      const restore = applySkillEnvOverrides({
        ...
        skills: entries, // Pragma: allowlist secret
      });
  ...
  it("keeps env keys tracked until all overlapping overrides restore", async () => {
    ...
    withClearedEnv(["ENV_KEY"], () => {
      ...
      const restoreFirst = applySkillEnvOverrides({ config, skills: entries });
      const restoreSecond = applySkillEnvOverrides({ config, skills: entries });
  ...
  it("applies env overrides from snapshots", async () => {
    ...
    withClearedEnv(["ENV_KEY"], () => {
      const restore = applySkillEnvOverridesFromSnapshot({
        ...
        snapshot, // Pragma: allowlist secret
      });
