...
function createTempCommandDir(
  tempDirs: string[],
  files: { name: string; executable?: boolean }[],
): string {
...
describe("resolveShellFromPath", () => {
  ...
  it("returns the first executable match from PATH", () => {
    const notExecutable = createTempCommandDir(tempDirs, [{ executable: false, name: "bash" }]);
    const executable = createTempCommandDir(tempDirs, [{ executable: true, name: "bash" }]);
