...
interface SubagentDeliveryTargetResult {
  origin?: {
    channel?: string;
    accountId?: string;
    to?: string;
...
describe("subagent announce formatting", () => {
  ...
  beforeEach(() => {
    ...
    callGatewaySpy.mockReset().mockImplementation(async (req: unknown) => {
      ...
      if (typed.method === "agent.wait") {
        return { endedAt: 20, error: "boom", startedAt: 10, status: "error" };
      }
    ...
    chatHistoryMock.mockReset().mockImplementation(async (sessionKey?: string) => {
      ...
      return {
        messages: [{ content: [{ text, type: "text" }], role: "assistant" }],
      };
    });
    sessionStore = {};
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    const call = agentSpy.mock.calls[0]?.[0] as {
      params?: {
        message?: string;
        sessionKey?: string;
        internalEvents?: { type?: string; taskLabel?: string }[];
      };
    };
  ...
  it("rechecks timed-out waits before announcing timeout when the run finishes immediately after", async () => {
    const waitStatuses = [
      { endedAt: 20, startedAt: 10, status: "timeout" },
      { endedAt: 30, startedAt: 10, status: "ok" },
    ];
    callGatewaySpy.mockImplementation(async (req: unknown) => {
      ...
      if (typed.method === "agent.wait") {
        return waitStatuses.shift() ?? { endedAt: 30, startedAt: 10, status: "ok" };
      }
      if (typed.method === "chat.history") {
        return {
          messages: [
            {
              content: [{ text: "Worker executed successfully", type: "text" }],
              role: "assistant",
            },
          ],
        };
    ...
    const call = agentSpy.mock.calls[0]?.[0] as {
      params?: {
        message?: string;
        internalEvents?: { status?: string; statusLabel?: string; result?: string }[];
      };
    };
  ...
  it.each([
    { childRunId: "run-tool-fallback-1", role: "toolResult", toolOutput: "tool output line 1" },
    { childRunId: "run-tool-fallback-2", role: "tool", toolOutput: "tool output line 2" },
  ] as const)(
    ...
    async (testCase) => {
      chatHistoryMock.mockResolvedValueOnce({
        messages: [
          ...
          {
            content: [{ text: testCase.toolOutput, type: "text" }],
            role: testCase.role,
          },
        ],
      });
  ...
  it("uses latest assistant text when it appears after a tool output", async () => {
    chatHistoryMock.mockResolvedValueOnce({
      messages: [
        {
          content: [{ text: "tool output line", type: "text" }],
          role: "tool",
        },
        {
          content: [{ text: "assistant final line", type: "text" }],
          role: "assistant",
        },
      ],
    });
  ...
  it("routes manual spawn completion through a parent-agent announce turn", async () => {
    ...
    chatHistoryMock.mockResolvedValueOnce({
      messages: [{ content: [{ text: "final answer: 2", type: "text" }], role: "assistant" }],
    });
    readLatestAssistantReplyMock.mockResolvedValue("");
    ...
    const didAnnounce = await runSubagentAnnounceFlow({
      ...
      requesterSessionKey: "agent:main:main",
      requesterDisplayKey: "main",
      requesterOrigin: { accountId: "acct-1", channel: "discord", to: "channel:12345" },
      ...defaultOutcomeAnnounce,
  ...
  it("keeps completion delivery enabled for extension channels captured from requester origin", async () => {
    const didAnnounce = await runSubagentAnnounceFlow({
      ...
      requesterSessionKey: "agent:main:main",
      requesterDisplayKey: "main",
      requesterOrigin: { accountId: "acct-bb", channel: "bluebubbles", to: "+1234567890" },
      ...defaultOutcomeAnnounce,
      expectsCompletionMessage: true,
    });
  ...
  it("keeps direct completion announce delivery immediate even when sibling counters are non-zero", async () => {
    ...
    chatHistoryMock.mockResolvedValueOnce({
      messages: [{ content: [{ text: "final answer: done", type: "text" }], role: "assistant" }],
    });
  ...
  it("suppresses completion delivery when subagent reply is NO_REPLY", async () => {
    const didAnnounce = await runSubagentAnnounceFlow({
      ...
      requesterSessionKey: "agent:main:main",
      requesterDisplayKey: "main",
      requesterOrigin: { accountId: "acct-1", channel: "slack", to: "channel:C123" },
      ...defaultOutcomeAnnounce,
      expectsCompletionMessage: true,
  ...
  it("retries completion direct agent announce on transient channel-unavailable errors", async () => {
    ...
    const didAnnounce = await runSubagentAnnounceFlow({
      ...
      requesterSessionKey: "agent:main:main",
      requesterDisplayKey: "main",
      requesterOrigin: { accountId: "default", channel: "whatsapp", to: "+15550000000" },
      ...defaultOutcomeAnnounce,
      expectsCompletionMessage: true,
  ...
  it("retries direct agent announce on transient channel-unavailable errors", async () => {
    ...
    const didAnnounce = await runSubagentAnnounceFlow({
      ...
      requesterSessionKey: "agent:main:main",
      requesterDisplayKey: "main",
      requesterOrigin: { accountId: "default", channel: "whatsapp", to: "+15551112222" },
      ...defaultOutcomeAnnounce,
  ...
  it("keeps session-mode completion delivery on the bound destination when sibling runs are active", async () => {
    ...
    chatHistoryMock.mockResolvedValueOnce({
      messages: [{ content: [{ text: "bound answer: 2", type: "text" }], role: "assistant" }],
    });
    subagentRegistryMock.countActiveDescendantRuns.mockImplementation((sessionKey: string) =>
      sessionKey === "agent:main:main" ? 1 : 0,
    );
  ...
  it("routes Matrix bound completion delivery to room targets", async () => {
    ...
    chatHistoryMock.mockResolvedValueOnce({
      messages: [{ content: [{ text: "matrix bound answer", type: "text" }], role: "assistant" }],
    });
    subagentRegistryMock.countActiveDescendantRuns.mockImplementation((sessionKey: string) =>
      sessionKey === "agent:main:main" ? 1 : 0,
    );
    ...
    const didAnnounce = await runSubagentAnnounceFlow({
      ...
      requesterSessionKey: "agent:main:main",
      requesterDisplayKey: "main",
      requesterOrigin: { accountId: "acct-matrix", channel: "matrix", to: "room:!room:example" },
      ...defaultOutcomeAnnounce,
      expectsCompletionMessage: true,
      spawnMode: "session",
    });
  ...
  it("includes completion status details for error and timeout outcomes", async () => {
    const cases = [
      {
        ...
        outcome: { error: "boom", status: "error" } as const,
    ...
    for (const testCase of cases) {
      ...
      chatHistoryMock.mockResolvedValueOnce({
        messages: [{ content: [{ text: testCase.replyText, type: "text" }], role: "assistant" }],
      });
      readLatestAssistantReplyMock.mockResolvedValue("");
  ...
  it("routes manual completion announce agent delivery using requester thread hints", async () => {
    ...
    for (const testCase of cases) {
      ...
      chatHistoryMock.mockResolvedValueOnce({
        messages: [{ content: [{ text: "done", type: "text" }], role: "assistant" }],
      });
  ...
  it("prefers direct delivery first for completion-mode and then queues on direct failure", async () => {
    ...
    expect(agentSpy.mock.calls[0]?.[0]).toMatchObject({
      method: "agent",
      params: { channel: "whatsapp", deliver: true, sessionKey: "agent:main:main", to: "+1555" },
    });
  ...
  it("uses assistant output for completion-mode when latest assistant text exists", async () => {
    chatHistoryMock.mockResolvedValueOnce({
      messages: [
        {
          content: [{ text: "old tool output", type: "text" }],
          role: "toolResult",
        },
        {
          content: [{ text: "assistant completion text", type: "text" }],
          role: "assistant",
        },
      ],
    });
  ...
  it("falls back to latest tool output for completion-mode when assistant output is empty", async () => {
    chatHistoryMock.mockResolvedValueOnce({
      messages: [
        ...
        {
          content: [{ text: "tool output only", type: "text" }],
          role: "toolResult",
        },
      ],
    });
  ...
  it("ignores user text when deriving fallback completion output", async () => {
    chatHistoryMock.mockResolvedValueOnce({
      messages: [
        {
          content: [{ text: "user prompt should not be announced", type: "text" }],
          role: "user",
        },
      ],
    });
  ...
  it("queues announce delivery back into requester subagent session", async () => {
    ...
    const didAnnounce = await runSubagentAnnounceFlow({
      ...
      requesterOrigin: { accountId: "acct", channel: "whatsapp", to: "+1555" },
  ...
  it.each([
    {
      ...
      requesterOrigin: { accountId: "acct-123", channel: "whatsapp" },
    ...
    },
    {
      ...
      expectedChannel: "whatsapp",
      requesterOrigin: { accountId: " acct-987 ", channel: " whatsapp " },
  ...
  it("injects direct announce into requester subagent session as a user-turn agent call", async () => {
    ...
    const didAnnounce = await runSubagentAnnounceFlow({
      ...
      childSessionKey: "agent:main:subagent:worker",
      requesterDisplayKey: "agent:main:subagent:orchestrator",
      requesterOrigin: { accountId: "acct-123", channel: "whatsapp", to: "+1555" },
      requesterSessionKey: "agent:main:subagent:orchestrator",
      ...defaultOutcomeAnnounce,
    });
  ...
  it("bubbles child announce to parent requester when requester subagent session is missing", async () => {
    ...
    subagentRegistryMock.resolveRequesterForChildSession.mockReturnValue({
      requesterOrigin: { accountId: "acct-main", channel: "whatsapp", to: "+1555" },
      requesterSessionKey: "agent:main:main",
    });
    sessionStore = {
  ...
  it("routes or falls back for ended parent subagent sessions (#18037)", async () => {
    ...
    for (const testCase of cases) {
      ...
      subagentRegistryMock.resolveRequesterForChildSession.mockReturnValue({
        requesterOrigin: { accountId: "jaris-account", channel: "discord" },
        requesterSessionKey: "agent:main:main",
      });
        ...
        sessionKey === "agent:main:subagent:parent-error"
          ? [
              makeChildCompletion({
                ...
                outcome: { error: "child exploded", status: "error" },
