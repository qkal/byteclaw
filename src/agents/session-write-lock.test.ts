...
async function expectCurrentPidOwnsLock(params: {
  ...
  const lock = await acquireSessionWriteLock({ sessionFile, staleMs, timeoutMs });
...
async function withTempSessionLockFile(
  ...
  try {
    ...
    await run({ lockPath: `${sessionFile}.lock`, root, sessionFile });
  } finally {
...
describe("acquireSessionWriteLock", () => {
  ...
  it("reclaims stale lock files", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        lockPath,
        JSON.stringify({ createdAt: new Date(Date.now() - 60_000).toISOString(), pid: 123_456 }),
        "utf8",
      );

      await expectCurrentPidOwnsLock({ sessionFile, staleMs: 10, timeoutMs: 500 });
    } finally {
      await fs.rm(root, { force: true, recursive: true });
    }
  });
  ...
  it("does not reclaim fresh malformed lock files during contention", async () => {
    ...
    try {
      ...
      await expect(
        acquireSessionWriteLock({ sessionFile, staleMs: 60_000, timeoutMs: 50 }),
      ).rejects.toThrow(/session file locked/);
  ...
  it("reclaims malformed lock files once they are old enough", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      ...
      const lock = await acquireSessionWriteLock({ sessionFile, staleMs: 10_000, timeoutMs: 500 });
      await lock.release();
  ...
  it("derives max hold from timeout plus grace", () => {
    ...
    expect(resolveSessionLockMaxHoldFromTimeout({ minMs: 5000, timeoutMs: 1000 })).toBe(121_000);
  });
