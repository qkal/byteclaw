...
function createAnthropicModelFixture(overrides: Partial<Model<Api>> = {}): Model<Api> {
  return {
    ...
    cost: { cacheRead: 0, cacheWrite: 0, input: 15, output: 75 },
    id: "claude-opus-4-5",
    input: ["text"] as const,
    maxTokens: 4096,
...
const createCompactionEvent = (params: { messageText: string; tokensBefore: number }) => ({
  ...
  preparation: {
    ...
    messagesToSummarize: [
      { content: params.messageText, role: "user", timestamp: Date.now() },
    ] as AgentMessage[],
    ...
    modelRegistry: {
      getApiKeyAndHeaders:
        ...
        vi.fn(async (model) => {
          ...
          return apiKey !== undefined ? { apiKey, ok: true } : { error: "missing auth", ok: false };
        }),
    },
...
async function runCompactionScenario(params: {
  ...
  const getApiKeyAndHeadersMock = vi
    ...
    .mockResolvedValue(
      params.apiKey !== null
        ? { apiKey: params.apiKey, ok: true }
        : { error: "missing auth", ok: false },
    );
  ...
  return { getApiKeyAndHeadersMock, result };
}
...
describe("compaction-safeguard tool failures", () => {
  it("formats tool failures with meta and summary", () => {
    const messages: AgentMessage[] = [
      {
        content: [{ text: "ENOENT: missing file", type: "text" }],
        details: { exitCode: 1, status: "failed" },
        isError: true,
        role: "toolResult",
        timestamp: Date.now(),
        toolCallId: "call-1",
  ...
  it("caps the number of failures and adds overflow line", () => {
    const messages: AgentMessage[] = Array.from({ length: 9 }, (_, idx) => ({
      content: [{ text: `error ${idx}`, type: "text" }],
      isError: true,
      role: "toolResult",
      timestamp: Date.now(),
      toolCallId: `call-${idx}`,
...
describe("computeAdaptiveChunkRatio", () => {
  ...
  it("returns BASE_CHUNK_RATIO for normal messages", () => {
    ...
    const messages: AgentMessage[] = [
      { content: "x".repeat(1000), role: "user", timestamp: Date.now() },
      castAgentMessage({
        content: [{ text: "y".repeat(1000), type: "text" }],
        role: "assistant",
        timestamp: Date.now(),
      }),
    ];
  ...
  it("reduces ratio when average message > 10% of context", () => {
    ...
    const messages: AgentMessage[] = [
      { content: "x".repeat(50_000 * 4), role: "user", timestamp: Date.now() },
      castAgentMessage({
        content: [{ text: "y".repeat(50_000 * 4), type: "text" }],
        role: "assistant",
        timestamp: Date.now(),
      }),
    ];
  ...
  it("respects MIN_CHUNK_RATIO floor", () => {
    ...
    const messages: AgentMessage[] = [
      { content: "x".repeat(150_000 * 4), role: "user", timestamp: Date.now() },
    ];
  ...
  it("handles single huge message", () => {
    ...
    const messages: AgentMessage[] = [
      { content: "x".repeat(180_000 * 4), role: "user", timestamp: Date.now() },
    ];
...
describe("compaction-safeguard recent-turn preservation", () => {
  it("preserves the most recent user/assistant messages", () => {
    const messages: AgentMessage[] = [
      { content: "older ask", role: "user", timestamp: 1 },
      {
        content: [{ text: "older answer", type: "text" }],
        role: "assistant",
        timestamp: 2,
      } as unknown as AgentMessage,
      { content: "recent ask", role: "user", timestamp: 3 },
      {
        content: [{ text: "recent answer", type: "text" }],
        role: "assistant",
        timestamp: 4,
      } as unknown as AgentMessage,
    ];
  ...
  it("drops orphaned tool results from preserved assistant turns", () => {
    const messages: AgentMessage[] = [
      { content: "older ask", role: "user", timestamp: 1 },
      {
        content: [{ arguments: {}, id: "call_old", name: "read", type: "toolCall" }],
        role: "assistant",
        timestamp: 2,
      } as unknown as AgentMessage,
      {
        content: [{ text: "old result", type: "text" }],
        role: "toolResult",
        timestamp: 3,
        toolCallId: "call_old",
        toolName: "read",
      } as unknown as AgentMessage,
      { content: "recent ask", role: "user", timestamp: 4 },
      {
        content: [{ arguments: {}, id: "call_recent", name: "read", type: "toolCall" }],
        role: "assistant",
        timestamp: 5,
      } as unknown as AgentMessage,
      {
        content: [{ text: "recent result", type: "text" }],
        role: "toolResult",
        timestamp: 6,
      ...
      {
        content: [{ text: "recent final answer", type: "text" }],
        role: "assistant",
        timestamp: 7,
      } as unknown as AgentMessage,
    ];
  ...
  it("includes preserved tool results in the preserved-turns section", () => {
    const split = splitPreservedRecentTurns({
      messages: [
        ...
        {
          content: [{ text: "recent raw output", type: "text" }],
          role: "toolResult",
          timestamp: 5,
          toolCallId: "call_recent",
          toolName: "read",
  ...
  it("does not add non-text placeholders for text-only content blocks", () => {
    const section = formatPreservedTurnsSection([
      {
        content: [{ text: "plain text reply", type: "text" }],
        role: "assistant",
        timestamp: 1,
      } as unknown as AgentMessage,
    ]);
  ...
  it("caps preserved tail when user turns are below preserve target", () => {
    const messages: AgentMessage[] = [
      { content: "single user prompt", role: "user", timestamp: 1 },
      {
        content: [{ text: "assistant-1", type: "text" }],
        role: "assistant",
        timestamp: 2,
      } as unknown as AgentMessage,
      {
        content: [{ text: "assistant-2", type: "text" }],
        role: "assistant",
        timestamp: 3,
      } as unknown as AgentMessage,
      {
        content: [{ text: "assistant-3", type: "text" }],
        role: "assistant",
        timestamp: 4,
      } as unknown as AgentMessage,
      {
        content: [{ text: "assistant-4", type: "text" }],
        role: "assistant",
        timestamp: 5,
      } as unknown as AgentMessage,
      {
        content: [{ text: "assistant-5", type: "text" }],
        role: "assistant",
        timestamp: 6,
      } as unknown as AgentMessage,
      {
        content: [{ text: "assistant-6", type: "text" }],
        role: "assistant",
        timestamp: 7,
      } as unknown as AgentMessage,
      {
        content: [{ text: "assistant-7", type: "text" }],
        role: "assistant",
        timestamp: 8,
      } as unknown as AgentMessage,
      {
        content: [{ text: "assistant-8", type: "text" }],
        role: "assistant",
        timestamp: 9,
      } as unknown as AgentMessage,
    ];
  ...
  it("does not retry summaries unless quality guard is explicitly enabled", async () => {
    ...
    const event = {
      ...
      preparation: {
        ...
        isSplitTurn: false,
        messagesToSummarize: [
          { content: "older context", role: "user", timestamp: 1 },
          { content: "older reply", role: "assistant", timestamp: 2 } as unknown as AgentMessage,
        ],
        previousSummary: undefined,
        settings: { reserveTokens: 4000 },
        tokensBefore: 1500,
    ...
    const event = {
      ...
      preparation: {
        ...
        messagesToSummarize: [
          { content: "older context", role: "user", timestamp: 1 },
          { content: "older reply", role: "assistant", timestamp: 2 } as unknown as AgentMessage,
          { content: "latest ask status", role: "user", timestamp: 3 },
          {
            content: [
              {
                type: "text",
  ...
  it("preserves split-turn and recent-turn suffixes when retry fallback is capped", async () => {
    ...
    const event = {
      ...
      preparation: {
        ...
        tokensBefore: 1500,
        turnPrefixMessages: [
          { content: "prefix request that was split out", role: "user", timestamp: 0 },
        ],
      },
      signal: new AbortController().signal,
    };
  ...
  it("keeps required headings when all turns are preserved and history is carried forward", async () => {
    ...
    const event = {
      ...
      preparation: {
        ...
        isSplitTurn: false,
        messagesToSummarize: [
          { content: "latest user ask", role: "user", timestamp: 1 },
          {
            content: [{ type: "text", text: "latest assistant reply" }],
            role: "assistant",
            timestamp: 2,
...
describe("compaction-safeguard double-compaction guard", () => {
  it("cancels compaction when there are no real messages to summarize", async () => {
    ...
    const { result, getApiKeyAndHeadersMock } = await runCompactionScenario({
      apiKey: "sk-test",
      event: mockEvent,
      sessionManager, // Pragma: allowlist secret
    });
    const compaction = expectCompactionResult(result);
  ...
  it("does not write boundary when turnPrefixMessages has real content (split-turn)", async () => {
    ...
    const mockEvent = {
      ...
      preparation: {
        ...
        tokensBefore: 2000,
        turnPrefixMessages: [
          { content: "real turn prefix content", role: "user" as const },
        ] as AgentMessage[],
      },
      signal: new AbortController().signal,
    };
  ...
  it("treats tool results as real conversation only when linked to a meaningful user ask", async () => {
    expect(
      __testing.isRealConversationMessage(
        ...
        [
          { content: "<b>HEARTBEAT_OK</b>", role: "user" } as AgentMessage,
          {
            content: [{ text: "done", type: "text" }],
            role: "toolResult",
            toolCallId: "t1",
    ...
    expect(
      __testing.isRealConversationMessage(
        ...
        [
          { content: "please inspect the repo", role: "user" } as AgentMessage,
          {
            content: [{ text: "done", type: "text" }],
            role: "toolResult",
            toolCallId: "t2",
  ...
  it("does not treat reasoning-only assistant blocks as meaningful conversation", () => {
    ...
    expect(
      __testing.hasMeaningfulConversationContent({
        content: [{ summary: [], type: "reasoning" }],
        role: "assistant",
      } as unknown as AgentMessage),
    ).toBe(false);
  });
