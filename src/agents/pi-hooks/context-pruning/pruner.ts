...
function collectTextSegments(content: readonly (TextContent | ImageContent)[]): string[] {
  const parts: string[] = [];
...
function hasImageBlocks(content: readonly (TextContent | ImageContent)[]): boolean {
  for (const block of content) {
    if (block.type === "image") {
      return true;
    }
...
function estimateTextAndImageChars(content: readonly (TextContent | ImageContent)[]): number {
  let chars = 0;
  for (const block of content) {
    if (block.type === "text") {
