import { type Api, type AssistantMessage, type Model, completeSimple } from "@mariozechner/pi-ai";
...
export async function withLiveCacheHeartbeat<T>(
  ...
  const timer = setInterval(() => {
    heartbeatCount += 1;
    logLiveCache(
      `${context}: still running (${Math.max(1, Math.round((Date.now() - startedAt) / 1000))}s)`,
    );
  }, heartbeatMs);
  timer.unref?.();
  try {
    ...
    if (heartbeatCount > 0) {
      logLiveCache(
        `${context}: completed (${Math.max(1, Math.round((Date.now() - startedAt) / 1000))}s)`,
      );
    }
  }
}
...
export async function completeSimpleWithLiveTimeout<TApi extends Api>(
  ...
  timeoutMs = Math.max(1000, toInt(process.env.OPENCLAW_LIVE_MODEL_TIMEOUT_MS, DEFAULT_TIMEOUT_MS)),
