...
const NOTIFY_EVENT_TIMEOUT_MS = isWin ? 12_000 : 5000;
...
const createDisallowedElevatedExecTool = (
  ...
  createTestExecTool({
    elevated: { allowed: false, defaultLevel, enabled: true },
    ...overrides,
  });
...
describe("exec tool backgrounding", () => {
  ...
  it(
    ...
    async () => {
      ...
      await expect
        .poll(async () => {
          const pollResult = await pollProcessSession({ sessionId, tool: processTool });
...
describe("exec notifyOnExit", () => {
  ...
  it("scopes notifyOnExit heartbeat wake to the exec session key", async () => {
    const tool = createNotifyOnExitExecTool();
    const wakeHandler = vi.fn().mockResolvedValue({ reason: "disabled", status: "skipped" });
...
describe("applyPathPrepend with case-insensitive PATH key", () => {
  it("prepends to Path key on Windows-style env (no uppercase PATH)", () => {
    ...
    applyPathPrepend(env, [String.raw`C:\custom\bin`]);
    // Should write back to the same `Path` key, not create a new `PATH`
    expect(env.Path).toContain(String.raw`C:\custom\bin`);
    expect(env.Path).toContain(String.raw`C:\Windows\System32`);
  ...
  it("respects requireExisting option with Path key", () => {
    ...
    applyPathPrepend(env, [String.raw`C:\custom\bin`], { requireExisting: true });
