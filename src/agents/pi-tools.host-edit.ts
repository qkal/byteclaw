...
interface EditToolRecoveryOptions {
  root: string;
...
function readEditReplacements(record: Record<string, unknown> | undefined): EditReplacement[] {
  ...
  return record.edits.flatMap((entry) => {
    ...
    return [{ newText: replacement.newText, oldText: replacement.oldText }];
  });
}
    ...
    execute: async (
      ...
      try {
        ...
        if (
          ...
          shouldAddMismatchHint(error)
        ) {
          throw appendMismatchHint(error, currentContent);
        }
