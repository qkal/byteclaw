...
export interface RequestExecApprovalDecisionParams {
  id: string;
  command?: string;
  commandArgv?: string[];
...
export interface ExecApprovalRegistration {
  id: string;
  expiresAtMs: number;
...
export async function registerExecApprovalRequest(
  ...
  if (decision.present) {
    return { expiresAtMs, finalDecision: decision.value, id };
  }
...
interface HostExecApprovalParams {
  approvalId: string;
  command?: string;
  commandArgv?: string[];
  systemRunPlan?: SystemRunApprovalPlan;
...
interface ExecApprovalRequesterContext {
  agentId?: string;
  sessionKey?: string;
}
...
interface ExecApprovalTurnSourceContext {
  turnSourceChannel?: string;
  turnSourceTo?: string;
  turnSourceAccountId?: string;
  turnSourceThreadId?: string | number;
...
export async function registerExecApprovalRequestForHostOrThrow(
  ...
  try {
    ...
    throw new Error(`Exec approval registration failed: ${String(error)}`, { cause: error });
  }
}
