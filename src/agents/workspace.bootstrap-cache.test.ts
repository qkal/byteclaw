...
describe("workspace bootstrap file caching", () => {
  ...
  it("invalidates cache when mtime changes", async () => {
    ...
    const bumpedTime = new Date(Date.now() + 1000);
  ...
  it("invalidates cache when inode changes with same mtime", async () => {
    ...
    await fs.writeFile(tempPath, content2, "utf8");
  ...
  it("handles file deletion gracefully", async () => {
    ...
    await writeWorkspaceFile({ content, dir: workspaceDir, name: DEFAULT_AGENTS_FILENAME });
  ...
  it("caches files independently by path", async () => {
    ...
    await writeWorkspaceFile({ content: content1, dir: workspace1, name: DEFAULT_AGENTS_FILENAME });
    await writeWorkspaceFile({ content: content2, dir: workspace2, name: DEFAULT_AGENTS_FILENAME });
