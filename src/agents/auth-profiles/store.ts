...
interface LoadAuthProfileStoreOptions {
  allowKeychainPrompt?: boolean;
  readOnly?: boolean;
  syncExternalCli?: boolean;
}

interface SaveAuthProfileStoreOptions {
...
export function replaceRuntimeAuthProfileStoreSnapshots(
  entries: { agentDir?: string; store: AuthProfileStore }[],
): void {
...
export function loadAuthProfileStore(): AuthProfileStore {
  ...
  const store: AuthProfileStore = { profiles: {}, version: AUTH_STORE_VERSION };
  syncExternalCliCredentialsTimed(store);
  return overlayExternalAuthProfiles(store);
}
...
function loadAuthProfileStoreForAgent(
  ...
  if (agentDir && !readOnly) {
    ...
    if (mainStore && Object.keys(mainStore.profiles).length > 0) {
      ...
      const inherited = { profiles: { ...mainStore.profiles }, version: mainStore.version };
      writeCachedAuthProfileStore({
        authMtimeMs: readAuthStoreMtimeMs(authPath),
        authPath,
        stateMtimeMs: readAuthStoreMtimeMs(statePath),
...
export function loadAuthProfileStoreForSecretsRuntime(agentDir?: string): AuthProfileStore {
  return loadAuthProfileStoreForRuntime(agentDir, { allowKeychainPrompt: false, readOnly: true });
}
