...
function makeStore(usageStats: AuthProfileStore["usageStats"]): AuthProfileStore {
  return {
    profiles: {
      "anthropic:default": { key: "sk-test", provider: "anthropic", type: "api_key" },
      "kilocode:default": { key: "sk-kc-test", provider: "kilocode", type: "api_key" },
      ...
      "openai:default": { key: "sk-test-2", provider: "openai", type: "api_key" },
      "openrouter:default": { key: "sk-or-test", provider: "openrouter", type: "api_key" },
    },
    usageStats,
    version: 1,
  };
...
describe("resolveProfilesUnavailableReason", () => {
  ...
  it("breaks ties by reason priority for equal active failure counts", () => {
    ...
    const store = makeStore({
      "anthropic:default": {
        cooldownUntil: now + 60_000,
        failureCounts: { auth: 2, timeout: 2 },
      },
    });
  ...
  it("handles independent expiry: cooldown expired but disabled still active", () => {
    ...
    const store = makeStore({
      "anthropic:default": {
        ...
        errorCount: 5,
        failureCounts: { billing: 2, rate_limit: 3 },
      },
    });
    ...
    expect(stats?.failureCounts).toEqual({ billing: 2, rate_limit: 3 });
  });
  ...
  it("resets errorCount only when both cooldown and disabled have expired", () => {
    const store = makeStore({
      "anthropic:default": {
        ...
        disabledUntil: Date.now() - 1000,
        errorCount: 4,
        failureCounts: { billing: 2, rate_limit: 2 },
      },
    });
...
describe("clearAuthProfileCooldown", () => {
  it("clears all error state fields including disabledUntil and failureCounts", async () => {
    ...
    await clearAuthProfileCooldown({ profileId: "anthropic:default", store });
  ...
  it("preserves lastUsed and lastFailureAt timestamps", async () => {
    ...
    const lastFailureAt = Date.now() - 5000;
    const store = makeStore({
      "anthropic:default": {
        cooldownUntil: Date.now() + 60_000,
        errorCount: 3,
  ...
  it("no-ops for unknown profile id", async () => {
    const store = makeStore(undefined);
    await clearAuthProfileCooldown({ profileId: "nonexistent", store });
...
describe("markAuthProfileFailure — WHAM-aware Codex cooldowns", () => {
  ...
  it.each([
    {
      ...
      response: {
        rate_limit: {
          limit_reached: false,
          primary_window: { reset_after_seconds: 9000, used_percent: 45 },
        },
      },
    },
    {
      ...
      response: {
        rate_limit: {
          limit_reached: true,
          primary_window: { reset_after_seconds: 7200, used_percent: 100 },
        },
      },
    },
    {
      ...
      response: {
        rate_limit: {
          limit_reached: true,
          primary_window: { reset_after_seconds: 7200, used_percent: 100 },
          secondary_window: { reset_after_seconds: 201_600, used_percent: 85 },
        },
      },
    },
    {
      ...
      response: {
        rate_limit: {
          limit_reached: true,
          primary_window: { reset_after_seconds: 7200, used_percent: 90 },
          secondary_window: { reset_after_seconds: 28_800, used_percent: 100 },
        },
      },
    },
    ...
    await markCodexFailureAt({ now, store });
  ...
  it("preserves a longer existing cooldown via max semantics", async () => {
    ...
    mockWhamResponse(200, {
      rate_limit: {
        limit_reached: false,
        primary_window: { reset_after_seconds: 300, used_percent: 25 },
      },
    });

    await markCodexFailureAt({ now, store, useLock: true });
  ...
  it("falls back to a 30s cooldown when the WHAM probe fails", async () => {
    ...
    await markCodexFailureAt({ now, reason: "unknown", store });
...
describe("markAuthProfileFailure — per-model cooldown metadata", () => {
  ...
  it("records cooldownModel on first rate_limit failure", async () => {
    ...
    await markFailure({ modelId: "claude-sonnet-4.6", now, store });
  ...
  it("widens cooldownModel to undefined when a different model fails during active cooldown", async () => {
    ...
    await markFailure({ modelId: "gpt-4.1", now, store });
    const stats = store.usageStats?.["github-copilot:github"];
  ...
  it("widens cooldownModel when rate_limit failure during active cooldown has no modelId", async () => {
    ...
    await markFailure({ modelId: undefined, now, store });
    const stats = store.usageStats?.["github-copilot:github"];
    expect(stats?.cooldownReason).toBe("rate_limit");
    expect(stats?.cooldownModel).toBeUndefined();
  });
