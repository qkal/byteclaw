...
export async function runExecProcess(opts: {
  ...
  const emitUpdate = () => {
    ...
    opts.onUpdate({
      content: [{ text: warningText + (tailText || ""), type: "text" }],
      details: {
        cwd: session.cwd,
  ...
  try {
    ...
    if (spawnSpec.mode === "pty") {
      const warning = `Warning: PTY spawn failed (${String(error)}); retrying without PTY for \`${opts.command}\`.`;
      logWarn(
        `exec: PTY spawn failed (${String(error)}); retrying without PTY for "${opts.command}".`,
      );
  ...
  const promise = managedRun
    ...
    .catch((error): ExecProcessOutcome => {
      markExited(session, null, null, "failed");
      maybeNotifyOnExit(session, "failed");
