...
const createLargeBootstrapFiles = (): WorkspaceBootstrapFile[] => [
  makeFile({ content: "a".repeat(10_000), name: "AGENTS.md" }),
  makeFile({ content: "b".repeat(10_000), name: "SOUL.md", path: "/tmp/SOUL.md" }),
  makeFile({ content: "c".repeat(10_000), name: "USER.md", path: "/tmp/USER.md" }),
];
...
describe("ensureSessionHeader", () => {
  it("creates transcript files with restrictive permissions", async () => {
    ...
    try {
      ...
      await ensureSessionHeader({ cwd: tempDir, sessionFile, sessionId: "session-1" });
...
describe("buildBootstrapContextFiles", () => {
  it("keeps missing markers", () => {
    const files = [makeFile({ content: undefined, missing: true })];
  ...
  it("truncates large bootstrap content", () => {
    ...
    const files = [makeFile({ content: long, name: "TOOLS.md" })];
  ...
  it("enforces strict total cap even when truncation markers are present", () => {
    const files = [
      makeFile({ content: "a".repeat(1000), name: "AGENTS.md" }),
      makeFile({ content: "b".repeat(1000), name: "SOUL.md", path: "/tmp/SOUL.md" }),
    ];
    const result = buildBootstrapContextFiles(files, {
      maxChars: 100,
      totalMaxChars: 150,
  ...
  it("skips bootstrap injection when remaining total budget is too small", () => {
    const files = [makeFile({ content: "a".repeat(1000), name: "AGENTS.md" })];
    const result = buildBootstrapContextFiles(files, {
      maxChars: 200,
      totalMaxChars: 40,
    });
...
interface BootstrapLimitResolverCase {
...
describe("bootstrap limit resolvers", () => {
  ...
  it("use configured values when valid", () => {
    for (const resolver of BOOTSTRAP_LIMIT_RESOLVERS) {
      const cfg = {
        agents: { defaults: { [resolver.name]: 12_345 } },
      } as OpenClawConfig;
      expect(resolver.resolve(cfg)).toBe(12_345);
    }
  });
