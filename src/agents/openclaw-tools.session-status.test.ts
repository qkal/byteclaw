  ...
  return {
    buildStatusText: async (params: {
      ...
      const configuredAgent = Array.isArray(
        (mockConfig as { agents?: { list?: Record<string, unknown>[] } }).agents?.list,
      )
        ? (mockConfig as { agents?: { list?: Record<string, unknown>[] } }).agents?.list?.find(
...
function mockSpawnedSessionList(
  resolveSessions: (spawnedBy: string | undefined) => Record<string, unknown>[],
) {
  callGatewayMock.mockImplementation(async (opts: unknown) => {
    const request = opts as { method?: string; params?: Record<string, unknown> };
    if (request.method === "sessions.list") {
