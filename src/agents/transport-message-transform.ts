...
function appendMissingToolResults(
  ...
  for (const toolCall of pendingToolCalls) {
    if (!existingToolResultIds.has(toolCall.id)) {
      result.push({
        content: [{ text: "No result provided", type: "text" }],
        isError: true,
        role: "toolResult",
        timestamp: Date.now(),
  ...
  const transformed = messages.map((msg) => {
    ...
    for (const block of msg.content) {
      if (block.type === "thinking") {
        ...
        content.push(isSameModel ? block : { text: block.thinking, type: "text" });
        continue;
      }
      if (block.type === "text") {
        content.push(isSameModel ? block : { text: block.text, type: "text" });
        continue;
      }
