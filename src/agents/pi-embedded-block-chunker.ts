...
export interface BlockReplyChunking {
  minChars: number;
  maxChars: number;
...
function findFenceCloseLineStart(buffer: string, fence: FenceSpan, offset = 0): number {
  ...
  return lastNewline !== -1 ? lastNewline + 1 : -1;
}
...
export class EmbeddedBlockChunker {
  ...
  #emitBreakResult(params: {
    ...
    if (fenceSplit) {
      return { reopenFence: fenceSplit.fence, start: absoluteBreakIdx };
    }
    ...
    const nextStart =
      absoluteBreakIdx < source.length && /\s/.test(source[absoluteBreakIdx])
        ? absoluteBreakIdx + 1
        : absoluteBreakIdx;
    return { reopenFence: undefined, start: skipLeadingNewlines(source, nextStart) };
  }
