...
async function emitToolResultOutput(params: {
  ...
  const shouldEmitOutput =
    ctx.shouldEmitToolOutput() || shouldEmitCompactToolOutput({ outputText, result, toolName });
  if (shouldEmitOutput) {
    if (outputText) {
...
export function handleToolExecutionStart(
  ...
  const continueToolExecutionStart = () => {
    ...
    toolStartData.set(buildToolStartKey(runId, toolCallId), { args, startTime: startedAt });
    ...
    void ctx.params.onAgentEvent?.({
      data: { name: toolName, phase: "start", toolCallId },
      stream: "tool",
    });
...
export async function handleToolExecutionEnd(
  ...
  ctx.state.toolMetas.push({ meta, toolName });
  ...
  await emitToolResultOutput({ ctx, isToolError, meta, result, sanitizedResult, toolName });
