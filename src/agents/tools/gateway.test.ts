...
describe("gateway tool defaults", () => {
  ...
  it("accepts allowlisted gatewayUrl overrides (SSRF hardening)", async () => {
    ...
    await callGatewayTool(
      "health",
      { gatewayToken: "t", gatewayUrl: "ws://127.0.0.1:18789", timeoutMs: 5000 },
      {},
    );
  ...
  it("rejects non-allowlisted overrides (SSRF hardening)", async () => {
    await expect(
      callGatewayTool("health", { gatewayToken: "t", gatewayUrl: "ws://127.0.0.1:8080" }, {}),
    ).rejects.toThrow(/gatewayUrl override rejected/i);
    await expect(
      callGatewayTool("health", { gatewayToken: "t", gatewayUrl: "ws://169.254.169.254" }, {}),
    ).rejects.toThrow(/gatewayUrl override rejected/i);
  });
});
