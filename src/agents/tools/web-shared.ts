...
export interface ReadResponseTextResult {
  text: string;
  truncated: boolean;
  bytesRead: number;
}
...
export async function readResponseText(
  ...
  if (
    ...
    return { bytesRead, text: parts.join(""), truncated };
  }

  try {
    const text = await res.text();
    return { bytesRead: text.length, text, truncated: false };
  } catch {
    return { bytesRead: 0, text: "", truncated: false };
  }
}
