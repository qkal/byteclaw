...
describe("cron tool", () => {
  ...
  it("normalizes cron.add job payloads", async () => {
    ...
    expect(params).toEqual({
      ...
      name: "wake-up",
      payload: { kind: "systemEvent", text: "hello" },
      schedule: { at: new Date(123).toISOString(), kind: "at" },
      sessionTarget: "main",
      wakeMode: "now",
    });
  });
  ...
  it("adds recent context for systemEvent reminders when contextMessages > 0", async () => {
    callGatewayMock
      .mockResolvedValueOnce({
        messages: [
          { content: [{ text: "Discussed Q2 budget", type: "text" }], role: "user" },
          {
            content: [{ text: "We agreed to review on Tuesday.", type: "text" }],
            role: "assistant",
          },
          { content: [{ text: "Remind me about the thing at 2pm", type: "text" }], role: "user" },
        ],
      })
  ...
  it("caps contextMessages at 10", async () => {
    const messages = Array.from({ length: 12 }, (_, idx) => ({
      content: [{ text: `Message ${idx + 1}`, type: "text" }],
      role: "user",
    }));
  ...
  it("recovers flat message shorthand as agentTurn payload", async () => {
    ...
    await tool.execute("call-msg-shorthand", {
      action: "add",
      message: "do stuff",
      schedule: { at: new Date(456).toISOString(), kind: "at" },
    });
