...
describe("web_search scoped config merge", () => {
  it("returns the original config when no plugin config exists", () => {
    const searchConfig = { grok: { model: "grok-4-1-fast" }, provider: "grok" };
  ...
  it("merges plugin config into the scoped provider object", () => {
    expect(
      mergeScopedSearchConfig({ grok: { model: "old-model" }, provider: "grok" }, "grok", {
        apiKey: "xai-test-key",
        model: "new-model",
      }),
    ).toEqual({
      grok: { apiKey: "xai-test-key", model: "new-model" },
      provider: "grok",
    });
  });
  ...
  it("can mirror the plugin apiKey to the top level config", () => {
    expect(
      ...
      apiKey: "brave-test-key",
      brave: { apiKey: "brave-test-key", count: 5 },
      provider: "brave",
    });
  });
});
