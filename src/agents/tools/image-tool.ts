  ...
  const result = await runWithImageModelFallback({
    ...
    run: async (provider, modelId) => {
      ...
      if (
        ...
        return { model: described.model ?? modelId, provider, text: described.text };
      }
      const describeImage =
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (imageInputs.length > maxImages) {
        return {
          ...
          details: { count: imageInputs.length, error: "too_many_images", max: maxImages },
        };
      }
