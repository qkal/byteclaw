...
function sanitizeHistoryContentBlock(block: unknown): {
  ...
  if (!block || typeof block !== "object") {
    return { block, redacted: false, truncated: false };
  }
  ...
  return { block: entry, redacted, truncated };
}
...
function sanitizeHistoryMessage(message: unknown): {
  ...
  if (!message || typeof message !== "object") {
    return { message, redacted: false, truncated: false };
  }
  ...
  return { message: entry, redacted, truncated };
}
...
function enforceSessionsHistoryHardCap(params: {
  ...
  if (params.bytes <= params.maxBytes) {
    return { bytes: params.bytes, hardCapped: false, items: params.items };
  }
  ...
  if (lastBytes <= params.maxBytes) {
    return { bytes: lastBytes, hardCapped: true, items: lastOnly };
  }
  ...
  return { bytes: jsonUtf8Bytes(placeholder), hardCapped: true, items: placeholder };
}
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (!resolvedSession.ok) {
        return jsonResult({ error: resolvedSession.error, status: resolvedSession.status });
      }
      ...
      const result = await gatewayCall<{ messages: unknown[] }>({
        method: "chat.history",
        params: { limit, sessionKey: resolvedKey },
      });
