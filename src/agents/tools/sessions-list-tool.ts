...
export function createSessionsListTool(opts?: {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      const list = await gatewayCall<{ sessions: SessionListRow[]; path: string }>({
      ...
      const historyTargets: { row: SessionListRow; resolvedKey: string }[] = [];
      ...
      for (const entry of sessions) {
        ...
        const kind = classifySessionKind({ alias, gatewayKind, key, mainKey });
        ...
        if (messageLimit > 0) {
          ...
          historyTargets.push({ resolvedKey, row });
        }
        rows.push(row);
      }
        ...
        const worker = async () => {
          while (true) {
            ...
            const history = await gatewayCall<{ messages: unknown[] }>({
              method: "chat.history",
              params: { limit: messageLimit, sessionKey: target.resolvedKey },
            });
