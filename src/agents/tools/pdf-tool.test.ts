...
async function withConfiguredPdfTool(
  ...
  await withTempAgentDir(async (agentDir) => {
    ...
    const tool = requirePdfTool((await loadCreatePdfTool())({ agentDir, config: cfg }));
...
describe("createPdfTool", () => {
  ...
  it("rejects too many PDFs", async () => {
    await withConfiguredPdfTool(async (tool) => {
      ...
      const result = await tool.execute("t1", { pdfs: manyPdfs, prompt: "test" });
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      try {
        ...
        await expect(tool.execute("t1", { pdf: outsidePdf, prompt: "test" })).rejects.toThrow(
  ...
  it("uses native PDF path without eager extraction", async () => {
    await withTempAgentDir(async (agentDir) => {
      await stubPdfToolInfra(agentDir, { input: ["text", "document"], provider: "anthropic" });
      ...
      expect(result).toMatchObject({
        content: [{ text: "native summary", type: "text" }],
        details: { model: ANTHROPIC_PDF_MODEL, native: true },
      });
    });
  });
  ...
  it("uses extraction fallback for non-native models", async () => {
    await withTempAgentDir(async (agentDir) => {
      await stubPdfToolInfra(agentDir, { input: ["text"], provider: "openai" });
      ...
      expect(result).toMatchObject({
        content: [{ text: "fallback summary", type: "text" }],
        details: { model: OPENAI_PDF_MODEL, native: false },
      });
    });
  });
