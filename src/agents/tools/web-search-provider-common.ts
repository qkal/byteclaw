...
export function readConfiguredSecretString(value: unknown, path: string): string | undefined {
  return normalizeSecretInput(normalizeResolvedSecretInputString({ path, value })) || undefined;
}
...
export function buildSearchCacheKey(parts: (string | number | boolean | undefined)[]): string {
