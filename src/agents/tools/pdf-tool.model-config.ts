...
function resolveBundledImageCandidateRefs(params: {
  ...
  return resolveBundledAutoMediaKeyProviders("image")
    ...
    .filter((providerId) => hasAuthForProvider({ agentDir: params.agentDir, provider: providerId }))
    .map((providerId) => {
      const modelId =
        resolveProviderVisionModelFromConfig({
          cfg: params.cfg,
...
export function resolvePdfModelConfigForTool(params: {
  ...
  const googleOk = hasAuthForProvider({ agentDir: params.agentDir, provider: "google" });
  ...
  const providerOk = hasAuthForProvider({ agentDir: params.agentDir, provider: primary.provider });
  ...
  if (params.cfg?.models?.providers && typeof params.cfg.models.providers === "object") {
    for (const [providerKey, providerCfg] of Object.entries(params.cfg.models.providers)) {
      ...
      if (!providerId || !hasAuthForProvider({ agentDir: params.agentDir, provider: providerId })) {
        continue;
      }
