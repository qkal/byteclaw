...
interface ToolResultTruncationOptions {
...
export interface ToolResultReductionPotential {
  maxChars: number;
...
function buildAggregateToolResultReplacements(params: {
  ...
  const replacements: { entryId: string; message: AgentMessage }[] = [];
...
function truncateOversizedToolResultsInExistingSessionManager(params: {
  ...
  if (branch.length === 0) {
    return { reason: "empty session", truncated: false, truncatedCount: 0 };
  }
...
export function truncateOversizedToolResultsInSessionManager(params: {
  ...
  try {
    ...
    return { reason: errMsg, truncated: false, truncatedCount: 0 };
  }
}
