...
describe("hardenManualCompactionBoundary", () => {
  it("turns manual compaction into a true checkpoint for rebuilt context", async () => {
    ...
    session.appendMessage({ content: "old question", role: "user", timestamp: 1 });
    ...
    session.appendMessage({ content: "new question", role: "user", timestamp: 3 });
    ...
    reopened.appendMessage({ content: "what was happening?", role: "user", timestamp: 5 });
  ...
  it("is a no-op when the latest leaf is not a compaction entry", async () => {
    const dir = await makeTmpDir();
    const session = SessionManager.create(dir, dir);
    session.appendMessage({ content: "hello", role: "user", timestamp: 1 });
