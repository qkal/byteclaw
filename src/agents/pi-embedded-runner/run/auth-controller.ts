...
import { type ResolvedProviderAuth, getApiKeyForModel } from "../../model-auth.js";
import {
  type FailoverReason,
...
export function createEmbeddedRunAuthController(params: {
  ...
  const refreshRuntimeAuth = async (reason: string): Promise<void> => {
    ...
    runtimeAuthState.refreshInFlight = (async () => {
      ...
      applyPreparedRuntimeRequestOverrides({ preparedAuth, runtimeModel });
      ...
      .catch((error) => {
        const runtimeModel = params.getRuntimeModel();
        params.log.warn(
          `Runtime auth refresh failed for ${runtimeModel.provider}: ${formatErrorMessage(error)}`,
        );
        throw error;
      })
      .finally(() => {
  ...
  const applyApiKeyInfo = async (candidate?: string): Promise<void> => {
    ...
    applyPreparedRuntimeRequestOverrides({ preparedAuth: preparedAuth ?? {}, runtimeModel });
    if (preparedAuth?.apiKey) {
      params.authStorage.setRuntimeApiKey(runtimeModel.provider, preparedAuth.apiKey);
      params.setRuntimeAuthState({
        authMode: apiKeyInfo.mode,
