...
const SESSIONS_YIELD_ABORT_SETTLE_TIMEOUT_MS = process.env.OPENCLAW_TEST_FAST === "1" ? 250 : 2000;
...
export async function waitForSessionsYieldAbortSettle(params: {
  ...
  const outcome = await Promise.race([
    params.settlePromise
      ...
      .catch((error) => {
        log.warn(
          `sessions_yield abort settle failed: runId=${params.runId} sessionId=${params.sessionId} err=${String(error)}`,
        );
  ...
  await activeSession.sendCustomMessage(
    {
      ...
      details: { message, source: "sessions_yield" },
      display: false,
    },
    { triggerTurn: false },
  );
...
export function stripSessionsYieldArtifacts(activeSession: {
  ...
  const strippedMessages = [...activeSession.messages];
