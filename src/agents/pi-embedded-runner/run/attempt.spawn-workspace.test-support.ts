...
interface AttemptSpawnWorkspaceHoisted {
  ...
  const subscribeEmbeddedPiSessionMock = vi.fn<SubscribeEmbeddedPiSessionFn>(
    (_params) =>
      ({
        ...
        getItemLifecycle: () => ({ activeCount: 0, completedCount: 0, startedCount: 0 }),
...
vi.mock("../../model-selection.js", () => ({
  ...
  resolveDefaultModelForAgent: () => ({ model: "gpt-test", provider: "openai" }),
}));
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const seedMessages: AgentMessage[] =
    params.sessionMessages ?? ([{ content: "seed", role: "user", timestamp: 1 }] as AgentMessage[]);
