...
const MEDIA_ATTACHED_PATH_REGEX_SOURCE =
  String.raw`^\s*(.+?\.(?:` + IMAGE_EXTENSION_PATTERN + String.raw`))\s*(?:\(|$|\|)`;
const MESSAGE_IMAGE_REGEX_SOURCE =
  String.raw`\[Image:\s*source:\s*([^\]]+\.(?:` + IMAGE_EXTENSION_PATTERN + String.raw`))\]`;
...
export function mergePromptAttachmentImages(params: {
  ...
  offloadedImages?: (ImageContent | null)[];
...
export function splitPromptAndAttachmentRefs(params: {
  ...
  if (offloadedCount === 0) {
    return { attachmentRefs: [], promptRefs: params.refs };
  }
  ...
  return { attachmentRefs, promptRefs };
}
...
export function detectImageReferences(prompt: string): DetectedImageRef[] {
  ...
  const addPathRef = (raw: string) => {
    ...
    refs.push({ raw: trimmed, resolved, type: "path" });
  };
  ...
  while ((match = mediaAttachedPattern.exec(prompt)) !== null) {
    ...
    if (mediaUriMatch) {
      ...
      if (!seen.has(dedupeKey)) {
        seen.add(dedupeKey);
        refs.push({ raw: uri, resolved: uri, type: "media-uri" });
      }
      continue;
    }
  ...
  while ((match = fileUrlPattern.exec(prompt)) !== null) {
    ...
    try {
      ...
      refs.push({ raw, resolved, type: "path" });
    } catch {
...
export async function loadImageFromRef(
  ...
  if (ref.type === "media-uri") {
    ...
    try {
      ...
      log.debug(
        `Native image: failed to load media-uri ${ref.resolved}: ${formatErrorMessage(error)}`,
      );
      return null;
    }
  }
  ...
  try {
    ...
    if (options?.sandbox) {
      try {
        ...
        log.debug(
          `Native image: sandbox validation failed for ${ref.resolved}: ${formatErrorMessage(error)}`,
        );
        return null;
      }
    ...
    log.debug(`Native image: failed to load ${ref.resolved}: ${formatErrorMessage(error)}`);
    return null;
  }
}
...
export async function detectAndLoadPromptImages(params: {
  ...
  const offloadedImages: (ImageContent | null)[] = [];
