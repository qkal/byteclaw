...
export async function truncateSessionAfterCompaction(params: {
  ...
  try {
    ...
    return { entriesRemoved: 0, reason, truncated: false };
  }

  const header = sm.getHeader();
  if (!header) {
    return { entriesRemoved: 0, reason: "missing session header", truncated: false };
  }

  const branch = sm.getBranch();
  if (branch.length === 0) {
    return { entriesRemoved: 0, reason: "empty session", truncated: false };
  }
  ...
  if (latestCompactionIdx < 0) {
    return { entriesRemoved: 0, reason: "no compaction entry found", truncated: false };
  }

  // Nothing to truncate if compaction is already at root
  if (latestCompactionIdx === 0) {
    return { entriesRemoved: 0, reason: "compaction already at root", truncated: false };
  }
  ...
  if (removedIds.size === 0) {
    return { entriesRemoved: 0, reason: "no entries to remove", truncated: false };
  }
  ...
  return { bytesAfter, bytesBefore, entriesRemoved, truncated: true };
}
