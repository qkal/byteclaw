...
function createSessionWithCompaction(sessionDir: string): string {
  ...
  sm.appendMessage({ content: "hello", role: "user", timestamp: 1 });
  sm.appendMessage(makeAssistant("hi there", 2));
  sm.appendMessage({ content: "do something", role: "user", timestamp: 3 });
  ...
  sm.appendMessage({ content: "next task", role: "user", timestamp: 5 });
...
describe("truncateSessionAfterCompaction", () => {
  ...
  it("skips truncation when no compaction entry exists", async () => {
    ...
    sm.appendMessage({ content: "bye", role: "user", timestamp: 3 });
  ...
  it("archives original file when archivePath is provided (#39953)", async () => {
    ...
    const result = await truncateSessionAfterCompaction({ archivePath, sessionFile });
  ...
  it("handles multiple compaction cycles (#39953)", async () => {
    ...
    sm.appendMessage({ content: "cycle 1 message 1", role: "user", timestamp: 1 });
    ...
    sm.appendMessage({ content: "cycle 2 message 1", role: "user", timestamp: 3 });
    ...
    sm.appendMessage({ content: "final question", role: "user", timestamp: 5 });
  ...
  it("preserves non-message session state during truncation", async () => {
    ...
    sm.appendMessage({ content: "do task", role: "user", timestamp: 3 });
    sm.appendMessage(makeAssistant("done", 4));
    ...
    // Post-compaction messages
    sm.appendMessage({ content: "next", role: "user", timestamp: 5 });
  ...
  it("preserves the firstKeptEntryId unsummarized tail", async () => {
    ...
    sm.appendMessage({ content: "msg1", role: "user", timestamp: 1 });
    sm.appendMessage(makeAssistant("resp1", 2));
    sm.appendMessage({ content: "msg2", role: "user", timestamp: 3 });
  ...
  it("preserves unsummarized sibling branches during truncation", async () => {
    ...
    sm.appendMessage({ content: "do task A", role: "user", timestamp: 3 });
    sm.appendMessage(makeAssistant("done A", 4));

    // Create a sibling branch from the earlier point
    sm.branch(branchFromId);
    sm.appendMessage({ content: "do task B instead", role: "user", timestamp: 5 });
    ...
    sm.appendMessage({ content: "next", role: "user", timestamp: 8 });
