...
export interface EmbeddedPiQueueHandle {
  kind?: "embedded";
...
export interface ActiveEmbeddedRunSnapshot {
  transcriptLeafId: string | null;
  messages?: unknown[];
  inFlightPrompt?: string;
}

interface EmbeddedRunWaiter {
  resolve: (ended: boolean) => void;
  timer: NodeJS.Timeout;
}

export interface EmbeddedRunModelSwitchRequest {
  provider: string;
  model: string;
...
export function abortEmbeddedPiRun(
  ...
  if (typeof sessionId === "string" && sessionId.length > 0) {
    ...
    try {
      ...
      diag.warn(`abort failed: sessionId=${sessionId} err=${String(error)}`);
      return false;
    }
    return true;
  }
  ...
  if (mode === "compacting") {
    ...
    for (const [id, handle] of ACTIVE_EMBEDDED_RUNS) {
      ...
      try {
        ...
        diag.warn(`abort failed: sessionId=${id} err=${String(error)}`);
      }
    }
...
export function clearActiveEmbeddedRun(
  ...
  if (ACTIVE_EMBEDDED_RUNS.get(sessionId) === handle) {
    ...
    logSessionStateChange({ reason: "run_completed", sessionId, sessionKey, state: "idle" });
    if (!sessionId.startsWith("probe-")) {
