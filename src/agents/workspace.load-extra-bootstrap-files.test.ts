...
describe("loadExtraBootstrapFiles", () => {
  ...
  it("loads recognized bootstrap files from glob patterns", async () => {
    ...
    await fs.writeFile(path.join(packageDir, "TOOLS.md"), "tools", "utf8");
    await fs.writeFile(path.join(packageDir, "README.md"), "not bootstrap", "utf8");
  ...
  it("keeps path-traversal attempts outside workspace excluded", async () => {
    ...
    await fs.writeFile(path.join(outsideDir, "AGENTS.md"), "outside", "utf8");
  ...
  it("supports symlinked workspace roots with realpath checks", async () => {
    ...
    await fs.writeFile(path.join(realWorkspace, "AGENTS.md"), "linked agents", "utf8");
  ...
  it("skips oversized bootstrap files and reports diagnostics", async () => {
    ...
    await fs.writeFile(path.join(workspaceDir, "AGENTS.md"), payload, "utf8");
