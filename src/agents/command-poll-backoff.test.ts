...
describe("command-poll-backoff", () => {
  describe("calculateBackoffMs", () => {
    ...
    it("returns 60s for fourth and subsequent polls (capped)", () => {
      expect(calculateBackoffMs(3)).toBe(60_000);
      expect(calculateBackoffMs(4)).toBe(60_000);
      expect(calculateBackoffMs(10)).toBe(60_000);
      expect(calculateBackoffMs(100)).toBe(60_000);
    });
  });
  ...
  describe("recordCommandPoll", () => {
    ...
    it("increments count and increases backoff on consecutive no-output polls", () => {
      ...
      expect(recordCommandPoll(state, "cmd-123", false)).toBe(10_000); // Count=1 -> 10s
      expect(recordCommandPoll(state, "cmd-123", false)).toBe(30_000); // Count=2 -> 30s
      expect(recordCommandPoll(state, "cmd-123", false)).toBe(60_000); // Count=3 -> 60s
      expect(recordCommandPoll(state, "cmd-123", false)).toBe(60_000); // Count=4 -> 60s (capped)
  ...
  describe("getCommandPollSuggestion", () => {
    ...
    it("returns current backoff for tracked command", () => {
      ...
      expect(getCommandPollSuggestion(state, "cmd-123")).toBe(10_000);
    });
  });
    ...
    it("removes polls older than maxAge", () => {
      const state: SessionState = {
        commandPollCounts: new Map([
          ["cmd-old", { count: 5, lastPollAt: Date.now() - 7_200_000 }], // 2 hours ago
      ...
      pruneStaleCommandPolls(state, 3_600_000); // 1 hour max age
