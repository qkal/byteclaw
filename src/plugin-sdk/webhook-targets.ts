...
export interface RegisteredWebhookTarget<T> {
  target: T;
  unregister: () => void;
}

export interface RegisterWebhookTargetOptions<T extends { path: string }> {
...
export async function withResolvedWebhookRequestPipeline<T>(params: {
  ...
  const inFlightKey =
    typeof params.inFlightKey === "function"
      ? params.inFlightKey({ path: resolved.path, req: params.req, targets: resolved.targets })
