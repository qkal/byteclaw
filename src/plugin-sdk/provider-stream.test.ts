...
describe("buildProviderStreamFamilyHooks", () => {
  it("covers the stream family matrix", async () => {
    ...
    )({ id: "openai/gpt-5.4", provider: "kilocode" } as never, {} as never, {});
    expect(capturedPayload).toMatchObject({
    ...
    )({ id: "kilo/auto", provider: "kilocode" } as never, {} as never, {});
    expect(capturedPayload).toMatchObject({
      config: { thinkingConfig: { thinkingBudget: -1 } },
    });
    ...
    )({ id: "openai/gpt-5.4", provider: "openrouter" } as never, {} as never, {});
    expect(capturedPayload).toMatchObject({
      config: { thinkingConfig: { thinkingBudget: -1 } },
      reasoning: { effort: "high" },
    });
    ...
    )({ id: "x-ai/grok-3", provider: "openrouter" } as never, {} as never, {});
    expect(capturedPayload).toMatchObject({
      config: { thinkingConfig: { thinkingBudget: -1 } },
    });
    expect(capturedPayload).not.toHaveProperty("reasoning");
