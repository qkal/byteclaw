...
interface BundledEntryModuleRef {
  specifier: string;
  exportName?: string;
}

interface DefineBundledChannelEntryOptions<TPlugin = ChannelPlugin> {
  id: string;
  name: string;
  description: string;
...
interface DefineBundledChannelSetupEntryOptions {
  importMetaUrl: string;
  plugin: BundledEntryModuleRef;
  secrets?: BundledEntryModuleRef;
}
...
export interface BundledChannelSetupEntryContract<TPlugin = ChannelPlugin> {
...
interface BundledEntryModuleCandidate {
  path: string;
  boundaryRoot: string;
}
...
function addBundledEntryCandidates(
  ...
  for (const candidate of resolveSpecifierCandidates(basePath)) {
    ...
    candidates.push({ boundaryRoot, path: candidate });
  }
}
