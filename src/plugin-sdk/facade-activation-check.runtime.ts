...
let cachedBoundaryResolvedConfig:
  ...
  | undefined;
const cachedManifestRegistryByKey = new Map<string, readonly PluginManifestRecord[]>();
...
interface FacadeModuleLocation {
  modulePath: string;
  boundaryRoot: string;
}
...
function readFacadeBoundaryConfigSafely(): {
  ...
  try {
    ...
    if (!fs.existsSync(configPath)) {
      return { cacheKey: `missing:${configPath}`, rawConfig: EMPTY_FACADE_BOUNDARY_CONFIG };
    }
...
export function resolveRegistryPluginModuleLocation(params: {
  ...
  const tiers: ((plugin: RegistryRecord) => boolean)[] = [
  ...
  for (const matchFn of tiers) {
    for (const record of registry.filter(matchFn)) {
      ...
      if (fs.existsSync(builtCandidate)) {
        return { boundaryRoot: rootDir, modulePath: builtCandidate };
      }
      for (const ext of PUBLIC_SURFACE_SOURCE_EXTENSIONS) {
        const sourceCandidate = path.join(rootDir, `${sourceBaseName}${ext}`);
        if (fs.existsSync(sourceCandidate)) {
          return { boundaryRoot: rootDir, modulePath: sourceCandidate };
        }
      }
    }
  }
