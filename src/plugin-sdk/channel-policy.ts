...
export function normalizeAllowFromList(list: (string | number)[] | undefined | null): string[] {
...
export interface ChannelMutableAllowlistCandidate {
  pathLabel: string;
  list: unknown;
}

interface ChannelMutableAllowlistHit {
  path: string;
  entry: string;
...
function collectMutableAllowlistWarningLines(
  ...
  const flagPaths = [...new Set(hits.map((hit) => hit.dangerousFlagPath))];
  const flagHint =
    flagPaths.length === 1
