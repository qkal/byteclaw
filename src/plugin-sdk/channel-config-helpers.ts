...
interface ChannelConfigAccessorParams<Config extends OpenClawConfig = OpenClawConfig> {
  cfg: Config;
  accountId?: string | null;
}

interface MultiAccountChannelConfigAdapterParams<
  ResolvedAccount,
...
> {
  ...
  resolveAllowFrom: (account: AccessorAccount) => (string | number)[] | null | undefined;
  formatAllowFrom: (allowFrom: (string | number)[]) => string[];
...
export function formatTrimmedAllowFromEntries(allowFrom: (string | number)[]): string[] {
...
export function adaptScopedAccountAccessor<Result, Config extends OpenClawConfig = OpenClawConfig>(
  ...
  return (cfg, accountId) => accessor({ accountId, cfg });
}
...
>(params: {
  ...
  resolveAllowFrom: (account: ResolvedAccount) => (string | number)[] | null | undefined;
  formatAllowFrom: (allowFrom: (string | number)[]) => string[];
  ...
  const base = {
    ...
    resolveAllowFrom({ cfg, accountId }: { cfg: OpenClawConfig; accountId?: string | null }) {
      return mapAllowFromEntries(
        params.resolveAllowFrom(params.resolveAccount({ accountId, cfg: cfg as Config })),
      );
    },
  };
  ...
  return {
    ...
    resolveDefaultTo({ cfg, accountId }) {
      return resolveOptionalConfigString(
        params.resolveDefaultTo?.(params.resolveAccount({ accountId, cfg: cfg as Config })),
      );
    },
  };
}
