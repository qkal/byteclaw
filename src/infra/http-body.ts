...
interface RequestBodyLimitErrorInit {
...
interface RequestBodyLimitValues {
  maxBytes: number;
  timeoutMs: number;
}

interface RequestBodyChunkProgress {
  buffer: Buffer;
  totalBytes: number;
  exceeded: boolean;
}
  ...
  try {
    ...
    if (!trimmed) {
      if (options.emptyObjectOnEmpty === false) {
        return { code: "INVALID_JSON", error: "empty payload", ok: false };
      }
    ...
    if (isRequestBodyLimitError(error)) {
      return { code: error.code, error: requestBodyErrorToText(error.code), ok: false };
    }
    return {
      code: "INVALID_JSON",
      error: formatErrorMessage(error),
...
export interface RequestBodyLimitGuard {
  dispose: () => void;
  isTripped: () => boolean;
  code: () => RequestBodyLimitErrorCode | null;
}

export interface RequestBodyLimitGuardOptions {
  maxBytes: number;
  timeoutMs?: number;
