...
interface EnsureOpenClawPathOpts {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureOpenClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  return { append: append.filter(isDirectory), prepend: prepend.filter(isDirectory) };
}
...
export function ensureOpenClawCliOnPath(opts: EnsureOpenClawPathOpts = {}) {
  ...
  const merged = mergePath({ append, existing, prepend });
  if (merged) {
