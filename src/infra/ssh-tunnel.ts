...
export interface SshParsedTarget {
  user?: string;
  host: string;
  port: number;
}
...
export function parseSshTarget(raw: string): SshParsedTarget | null {
  ...
  if (colonIdx > 0 && colonIdx < hostPart.length - 1) {
    ...
    return { host, port, user: userPart };
  }
  ...
  return { host: hostPart, port: 22, user: userPart };
}
...
export async function startSshPortForward(opts: {
  ...
  try {
    ...
    throw new Error(`${formatErrorMessage(error)}${suffix}`, { cause: error });
  }
