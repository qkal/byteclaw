...
export interface ShellChainPart {
  part: string;
...
function splitShellPipeline(command: string): { ok: boolean; reason?: string; segments: string[] } {
  ...
  for (let i = 0; i < command.length; i += 1) {
    ...
    if (ch === "<" && next === "<") {
      ...
      if (parsed) {
        pendingHeredocs.push({ delimiter: parsed.delimiter, quoted: parsed.quoted, stripTabs });
...
export function splitCommandChainWithOperators(command: string): ShellChainPart[] | null {
  ...
  const pushPart = (opToNext: ShellChainOperator | null) => {
    ...
    parts.push({ opToNext, part: trimmed });
    return true;
  };
  ...
  parts.push({ opToNext: null, part: trimmed });
...
function rebuildWindowsShellCommandFromSource(params: {
  ...
  return { command: `& ${rendered.rendered}`, ok: true, segmentCount: 1 };
}
...
function rebuildShellCommandFromSource(params: {
  ...
  const chainParts: ShellChainPart[] = chain ?? [{ opToNext: null, part: source }];
  let segmentCount = 0;
  let out = "";
  ...
  return { command: out, ok: true, segmentCount };
}
...
function finalizeRebuiltShellCommand(
  ...
  return { command: rebuilt.command, ok: true };
}
...
export function analyzeShellCommand(params: {
  ...
  if (chainParts) {
    ...
    return { chains, ok: true, segments: allSegments };
  }
