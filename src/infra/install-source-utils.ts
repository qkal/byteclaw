...
export interface NpmSpecResolution {
  name?: string;
  version?: string;
  resolvedSpec?: string;
  integrity?: string;
...
export interface NpmResolutionFields {
  resolvedName?: string;
...
export interface NpmIntegrityDrift {
...
> {
  ...
  if (!(await fileExists(resolved))) {
    return { error: `archive not found: ${resolved}`, ok: false };
  }

  if (!resolveArchiveKind(resolved)) {
    return { error: `unsupported archive: ${resolved}`, ok: false };
  }
...
> {
  ...
  if (res.code !== 0) {
    ...
    return { error: `npm pack failed: ${raw}`, ok: false };
  }
  ...
  if (!packed) {
    return { error: "npm pack produced no archive", ok: false };
  }
