...
const activeForwarders: ReturnType<typeof createExecApprovalForwarder>[] = [];
...
function shouldSuppressTelegramExecApprovalForwardingFallbackForTest(params: {
  ...
  return isTelegramExecApprovalClientEnabledForTest({ accountId, cfg: params.cfg });
}
  ...
  return {
    channelData: {
      ...
      telegram: {
        buttons: [
          [
            { callback_data: `/approve ${params.request.id} allow-once`, text: "Allow Once" },
            { callback_data: `/approve ${params.request.id} allow-always`, text: "Allow Always" },
          ],
          [{ callback_data: `/approve ${params.request.id} deny`, text: "Deny" }],
        ],
      },
    },
    interactive: {
  ...
  approvalCapability: {
    delivery: {
      shouldSuppressForwardingFallback: ({
        ...
        isDiscordExecApprovalClientEnabledForTest({ accountId: target.accountId, cfg }),
    },
  },
};
...
function makeTargetsCfg(targets: { channel: string; to: string }[]): OpenClawConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
describe("exec approval forwarder", () => {
  ...
  it("skips telegram forwarding when telegram exec approvals handler is enabled", async () => {
    ...
    const { deliver, forwarder } = createForwarder({
      cfg,
      resolveSessionTarget: () => ({ channel: "telegram", threadId: 77, to: "-100999" }),
    });
