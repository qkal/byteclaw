...
const mockResolveGatewayPort = vi.hoisted(() => vi.fn(() => 18_789));
...
const mockReadWindowsProcessArgsResult = vi.hoisted(() =>
  vi.fn<(_pid: number, _timeoutMs?: number) => MockWindowsProcessArgsResult>(
    (_pid: number, _timeoutMs?: number) => ({ args: null, ok: true }),
  ),
);
...
function lsofOutput(entries: { pid: number; cmd: string }[]): string {
  return entries.map(({ pid, cmd }) => `p${pid}\nc${cmd}`).join("\n") + "\n";
}

interface MockLsofResult {
  error: Error | null;
  status: number | null;
  stdout: string;
  stderr: string;
...
function createOpenClawBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ cmd: "openclaw-gateway", pid }]),
    ...overrides,
  });
}
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  beforeEach(() => {
    ...
    mockResolveGatewayPort.mockReturnValue(18_789);
    ...
    mockReadWindowsProcessArgsResult.mockReturnValue({ args: null, ok: true });
  ...
  describe("findGatewayPidsOnPortSync", () => {
    it("returns [] when lsof exits with non-zero status", () => {
      mockSpawnSync.mockReturnValue({ error: null, status: 1, stderr: "", stdout: "" });
      expect(findGatewayPidsOnPortSync(18_789)).toEqual([]);
    });

    it("logs warning when initial lsof scan exits with status > 1", () => {
      mockSpawnSync.mockReturnValue({ error: null, status: 2, stderr: "lsof error", stdout: "" });
      expect(findGatewayPidsOnPortSync(18_789)).toEqual([]);
    ...
    it("parses openclaw-gateway pids and excludes the current process", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        stderr: "",
        stdout: lsofOutput([
          { cmd: "openclaw-gateway", pid: stalePid },
          { cmd: "openclaw-gateway", pid: process.pid },
        ]),
      });
      const pids = findGatewayPidsOnPortSync(18_789);
    ...
    it("excludes pids whose command does not include 'openclaw'", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stderr: "",
        stdout: lsofOutput([{ cmd: "nginx", pid: otherPid }]),
      });
      expect(findGatewayPidsOnPortSync(18_789)).toEqual([]);
    });

    it("forwards the spawnTimeoutMs argument to spawnSync", () => {
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stderr: "", stdout: "" });
      findGatewayPidsOnPortSync(18_789, 400);
    ...
    it("deduplicates pids from dual-stack listeners (IPv4+IPv6 emit same pid twice)", () => {
      ...
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stderr: "", stdout });
      const result = findGatewayPidsOnPortSync(18_789);
    ...
    it("delegates to Windows port helpers on win32 and skips lsof", () => {
      ...
      try {
        mockReadWindowsListeningPids.mockReturnValue([]);
        expect(findGatewayPidsOnPortSync(18_789)).toEqual([]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(18_789, undefined);
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        expect(findGatewayPidsOnPortSync(18_789)).toEqual([stalePid]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(18_789, undefined);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple openclaw pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        stderr: "",
        stdout: lsofOutput([
          { cmd: "openclaw-gateway", pid: pid1 },
          { cmd: "openclaw-gateway", pid: pid2 },
        ]),
      });
      const result = findGatewayPidsOnPortSync(18_789);
    ...
    it("returns [] when status 0 but only non-openclaw pids present", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stderr: "",
        stdout: lsofOutput([{ cmd: "caddy", pid: otherPid }]),
      });
      expect(findGatewayPidsOnPortSync(18_789)).toEqual([]);
    });
  });
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementation(() => ({ error: null, status: 1, stderr: "", stdout: "" }));
  ...
  describe("cleanStaleGatewayProcessesSync", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call <= 5) {
          return {
            ...
            status: 0,
            stderr: "",
            stdout: lsofOutput([{ cmd: "openclaw-gateway", pid: stalePid }]),
          };
        }
    ...
    it("treats failed Windows port probes as inconclusive, not free", () => {
      ...
      try {
        ...
        expect(mockReadWindowsListeningPidsResult).toHaveBeenCalledWith(18_789, 400);
        expect(mockRestartWarn).toHaveBeenCalledWith(
