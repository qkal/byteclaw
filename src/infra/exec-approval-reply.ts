...
export interface ExecApprovalActionDescriptor {
...
export interface ExecApprovalPendingReplyParams {
  warningText?: string;
  approvalId: string;
  approvalSlug: string;
...
export interface ExecApprovalUnavailableReplyParams {
  warningText?: string;
  channel?: string;
  channelLabel?: string;
  accountId?: string;
...
export function buildApprovalInteractiveReplyFromActionDescriptors(
  ...
  return buttons.length > 0 ? { blocks: [{ buttons, type: "buttons" }] } : undefined;
}
