...
const DEFAULT_PROGRAM_FILES = String.raw`C:\Program Files`;
const DEFAULT_PROGRAM_FILES_X86 = String.raw`C:\Program Files (x86)`;
const WINDOWS_NT_CURRENT_VERSION_KEY = String.raw`HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion`;
const WINDOWS_CURRENT_VERSION_KEY = String.raw`HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion`;
...
interface WindowsInstallRootsTestOverrides {
  queryRegistryValue?: QueryRegistryValue;
  isReadableFile?: IsReadableFile;
}

export interface WindowsInstallRoots {
  systemRoot: string;
  programFiles: string;
...
export function normalizeWindowsInstallRoot(raw: string | undefined): string | null {
  ...
  if (!path.win32.isAbsolute(normalized) || normalized.startsWith(String.raw`\\`)) {
    return null;
  }
