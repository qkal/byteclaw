...
beforeAll(async () => {
  ...
  const telegramPlugin = createOutboundTestPlugin({
    ...
    outbound: {
      ...
      sendMedia: async ({ to, text, mediaUrl, deps, accountId }) => {
        ...
        return { channel: "telegram", chatId: res.chatId, messageId: res.messageId };
      },
  ...
  testRegistry = createTestRegistry([
    { plugin: whatsappPlugin, pluginId: "whatsapp", source: "test" },
...
describe("resolveHeartbeatPrompt", () => {
  it.each([
    { cfg: {} as OpenClawConfig, expected: HEARTBEAT_PROMPT, name: "default prompt" },
    {
      cfg: {
...
describe("resolveHeartbeatDeliveryTarget", () => {
  ...
  it("resolves target variants across route and allowlist rules", () => {
    const cases: {
      ...
      {
        cfg: { agents: { defaults: { heartbeat: { directPolicy: "block", target: "last" } } } },
        entry: { ...baseEntry, lastChannel: "telegram", lastTo: "5232990709" },
        expected: {
          accountId: undefined,
          channel: "none",
  ...
  it.each([
    { expectedThreadId: 42, expectedTo: "-100111", name: "topic suffix", to: "-100111:topic:42" },
    { expectedThreadId: undefined, expectedTo: "-100111", name: "plain chat id", to: "-100111" },
  ])(
  ...
  it.each([
    ...
    ({ accountId, expected }) => {
      const cfg: OpenClawConfig = {
        agents: {
          defaults: {
            heartbeat: { accountId, target: "telegram", to: "-100123" },
          },
        },
...
describe("resolveHeartbeatSenderContext", () => {
  it("prefers delivery accountId for allowFrom resolution", () => {
    ...
    const ctx = resolveHeartbeatSenderContext({ cfg, delivery, entry });
...
describe("runHeartbeatOnce", () => {
  ...
  it("skips when agent heartbeat is not enabled", async () => {
    ...
    const res = await runHeartbeatOnce({ agentId: "main", cfg });
  ...
  it("skips outside active hours", async () => {
    const cfg: OpenClawConfig = {
      agents: {
        defaults: {
          heartbeat: {
            activeHours: { end: "24:00", start: "08:00", timezone: "user" },
            every: "30m",
          },
          userTimezone: "UTC",
        },
  ...
  it("uses per-agent heartbeat overrides and session keys", async () => {
    ...
    try {
      const cfg: OpenClawConfig = {
        agents: {
          ...
          list: [
            ...
            {
              heartbeat: { every: "5m", prompt: "Ops check", target: "whatsapp" },
              id: "ops",
              workspace: tmpDir,
            },
          ],
      ...
      const sessionKey = resolveAgentMainSessionKey({ agentId: "ops", cfg });
  ...
  it("reuses non-default agent sessionFile from templated stores", async () => {
    ...
    try {
      ...
      const sessionKey = resolveAgentMainSessionKey({ agentId, cfg });
  ...
  it("loads the default agent session from templated stores", async () => {
    ...
    try {
      const cfg: OpenClawConfig = {
        agents: {
          defaults: { heartbeat: { every: "5m", target: "whatsapp" }, workspace: tmpDir },
          list: [{ default: true, id: "work" }],
        },
        channels: { whatsapp: { allowFrom: ["*"] } },
        session: { store: storeTemplate },
      };
...
`,
    ...
    return { replySpy, res, sendWhatsApp, workspaceDir };
  }
