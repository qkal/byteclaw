...
function probeLoginShellEnv(params: {
  ...
  try {
    const stdout = execLoginShellEnvZero({ env: execEnv, exec, shell, timeoutMs });
...
export function loadShellEnvFallback(opts: ShellEnvFallbackOptions): ShellEnvFallbackResult {
  ...
  if (!opts.enabled) {
    lastAppliedKeys = [];
    return { applied: [], ok: true, skippedReason: "disabled" };
  }

  const hasAnyKey = opts.expectedKeys.some((key) => Boolean(opts.env[key]?.trim()));
  if (hasAnyKey) {
    lastAppliedKeys = [];
    return { applied: [], ok: true, skippedReason: "already-has-keys" };
  }
  ...
  if (!probe.ok) {
    ...
    lastAppliedKeys = [];
    return { applied: [], error: probe.error, ok: false };
  }
