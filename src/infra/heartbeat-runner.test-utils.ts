...
export interface HeartbeatSessionSeed {
  sessionId?: string;
  updatedAt?: number;
  lastChannel: string;
  lastProvider: string;
...
export async function seedSessionStore(
  ...
  try {
    existingStore = JSON.parse(await fs.readFile(storePath, "utf8")) as Record<string, unknown>;
  } catch {
    existingStore = {};
  }
  await fs.writeFile(
...
export async function withTempHeartbeatSandbox<T>(
  ...
  try {
    return await fn({ replySpy, storePath, tmpDir });
  } finally {
    replySpy.mockReset();
