...
const envSchema = z.object({
  ...
  // Feature flags
  OPENCLAW_SKIP_CHANNELS: z.string().transform((v) => v === "1").optional(),
  OPENCLAW_LOAD_SHELL_ENV: z.string().transform((v) => v === "1").optional(),
  OPENCLAW_SHELL_ENV_TIMEOUT_MS: z.string().regex(/^\d+$/).transform(Number).optional(),
  
  // Security
  NODE_DISABLE_COMPILE_CACHE: z.string().transform((v) => v === "1").optional(),
  OPENCLAW_AUTH_STORE_READONLY: z.string().transform((v) => v === "1").optional(),
  
  // Node.js Permission Model support
  OPENCLAW_ENABLE_PERMISSIONS: z.string().transform((v) => v === "1").optional(),
  OPENCLAW_ALLOW_FS_READ: z.string().optional(),
  OPENCLAW_ALLOW_FS_WRITE: z.string().optional(),
  OPENCLAW_ALLOW_CHILD_PROCESS: z.string().transform((v) => v === "1").optional(),
  OPENCLAW_ALLOW_WORKER_THREADS: z.string().transform((v) => v === "1").optional(),
});
...
export function validateEnv(): ValidationError[] {
  ...
  try {
    ...
    if (gatewayHost && gatewayHost !== "127.0.0.1" && gatewayHost !== "localhost" && !hasGatewayToken && !hasGatewayPassword) {
      errors.push({
        key: "GATEWAY_SECURITY",
        message: "Gateway exposed beyond localhost requires authentication (OPENCLAW_GATEWAY_TOKEN or OPENCLAW_GATEWAY_PASSWORD).",
        severity: "error",
      });
    }
    
    // Check port validity
    const port = process.env.OPENCLAW_GATEWAY_PORT ? Number.parseInt(process.env.OPENCLAW_GATEWAY_PORT, 10) : null;
    ...
    const shellTimeout = process.env.OPENCLAW_SHELL_ENV_TIMEOUT_MS ? Number.parseInt(process.env.OPENCLAW_SHELL_ENV_TIMEOUT_MS, 10) : null;
    ...
    if (process.env.OPENCLAW_ENABLE_PERMISSIONS === "1") {
      if (!process.execArgv.includes("--permission")) {
        errors.push({
          ...
          message: "OPENCLAW_ENABLE_PERMISSIONS is set but Node.js was not started with --permission flag.",
          severity: "warning",
        });
      }
    }
