...
describe("drainPendingDeliveries for WhatsApp reconnect", () => {
  ...
  it("drains entries that failed with 'no listener' error", async () => {
    ...
    const id = await enqueueDelivery(
      { accountId: "acct1", channel: "whatsapp", payloads: [{ text: "hi" }], to: "+1555" },
      tmpDir,
    );
    ...
    expect(deliver).toHaveBeenCalledWith(
      expect.objectContaining({ channel: "whatsapp", skipQueue: true, to: "+1555" }),
    );
  });
  ...
  it("skips entries from other accounts", async () => {
    ...
    const id = await enqueueDelivery(
      { accountId: "other", channel: "whatsapp", payloads: [{ text: "hi" }], to: "+1555" },
      tmpDir,
    );
    await failDelivery(id, "No active WhatsApp Web listener", tmpDir);
  ...
  it("retries immediately without resetting retry history", async () => {
    ...
    const before = JSON.parse(fs.readFileSync(filePath, "utf8")) as {
      retryCount: number;
  ...
  it("second concurrent call is skipped (concurrency guard)", async () => {
    ...
    const opts = { accountId: "acct1", deliver, log, stateDir: tmpDir };
  ...
  it("does not re-deliver an entry already being recovered at startup", async () => {
    ...
    const entry = JSON.parse(fs.readFileSync(queuePath, "utf8")) as {
      id: string;
      enqueuedAt: number;
      channel: string;
      to: string;
  ...
  it("does not re-deliver a stale startup snapshot after reconnect already acked it", async () => {
    ...
    const blockerId = await enqueueDelivery(
      { channel: "demo-channel-a", payloads: [{ text: "blocker" }], to: "+1000" },
      tmpDir,
    );
    ...
    const blockerEntry = JSON.parse(fs.readFileSync(blockerPath, "utf8")) as {
      enqueuedAt: number;
    };
    const whatsappEntry = JSON.parse(fs.readFileSync(whatsappPath, "utf8")) as {
      enqueuedAt: number;
    };
  ...
  it("ignores non-WhatsApp entries even when reconnect drain runs", async () => {
    ...
    await enqueueDelivery(
      { accountId: "acct1", channel: "telegram", payloads: [{ text: "hi" }], to: "+1555" },
      tmpDir,
    );
