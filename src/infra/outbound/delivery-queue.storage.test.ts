...
describe("delivery-queue storage", () => {
    ...
    it.each([
      {
        ...
        payload: { channel: "whatsapp", payloads: [{ text: "stale-marker" }], to: "+1" },
        prepareDeliveredMarker: true,
      },
      {
        action: (id: string) => ackDelivery(id, tmpDir()),
        name: "ack removes .delivered marker so recovery does not replay",
        payload: { channel: "whatsapp", payloads: [{ text: "ack-test" }], to: "+1" },
      },
      {
        ...
        payload: { channel: "telegram", payloads: [{ text: "stale" }], to: "99" },
        prepareDeliveredMarker: true,
      },
  ...
  describe("loadPendingDeliveries", () => {
    ...
    it("loads multiple entries", async () => {
      await enqueueTextDelivery({ channel: "whatsapp", payloads: [{ text: "a" }], to: "+1" });
      await enqueueTextDelivery({ channel: "telegram", payloads: [{ text: "b" }], to: "2" });
    ...
    it("backfills lastAttemptAt for legacy retry entries during load", async () => {
      ...
      fs.writeFileSync(filePath, JSON.stringify(legacyEntry), "utf8");
