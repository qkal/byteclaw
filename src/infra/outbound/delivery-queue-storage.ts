...
export interface QueuedDeliveryPayload {
...
async function readQueueEntry(filePath: string): Promise<QueuedDelivery> {
  return JSON.parse(await fs.promises.readFile(filePath, "utf8")) as QueuedDelivery;
}
...
export async function ensureQueueDir(stateDir?: string): Promise<string> {
  const queueDir = resolveQueueDir(stateDir);
  await fs.promises.mkdir(queueDir, { mode: 0o700, recursive: true });
  await fs.promises.mkdir(resolveFailedDir(stateDir), { mode: 0o700, recursive: true });
  return queueDir;
}
...
export async function moveToFailed(id: string, stateDir?: string): Promise<void> {
  ...
  await fs.promises.mkdir(failedDir, { mode: 0o700, recursive: true });
