...
function expectParserCases<T>(
  ...
  cases: { value: unknown; expected: T | undefined }[],
) {
...
describe("parseFiniteNumber", () => {
  it("parses finite values and rejects invalid inputs", () => {
    expectParserCases(parseFiniteNumber, [
      ...
      { expected: 3.14, value: "3.14" },
      { expected: 3.14, value: " 3.14ms" },
      { expected: 7, value: "+7" },
      { expected: 1000, value: "1e3" },
      { expected: undefined, value: Number.NaN },
      { expected: undefined, value: Number.POSITIVE_INFINITY },
      { expected: undefined, value: "not-a-number" },
...
describe("parseStrictInteger", () => {
  it("parses strict integers and rejects non-integers", () => {
    expectParserCases(parseStrictInteger, [
      ...
      { expected: 9, value: "+9" },
      { expected: undefined, value: "42ms" },
      { expected: undefined, value: "0abc" },
      { expected: undefined, value: "1.5" },
      { expected: undefined, value: "1e3" },
      { expected: undefined, value: " " },
      { expected: undefined, value: Number.MAX_SAFE_INTEGER + 1 },
    ]);
  });
});
