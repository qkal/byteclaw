...
describe("startHeartbeatRunner", () => {
  ...
  function createRequestsInFlightRunSpy(skipCount: number) {
    ...
    return vi.fn().mockImplementation(async () => {
      ...
      if (callCount <= skipCount) {
        return { reason: "requests-in-flight", status: "skipped" } as const;
      }
      return { durationMs: 1, status: "ran" } as const;
    });
  }
  ...
  it("updates scheduling when config changes without restart", async () => {
    ...
    const runSpy = vi.fn().mockResolvedValue({ durationMs: 1, status: "ran" });
    ...
    runner.updateConfig({
      agents: {
        defaults: { heartbeat: { every: "30m" } },
        list: [
          { heartbeat: { every: "10m" }, id: "main" },
          { heartbeat: { every: "15m" }, id: "ops" },
        ],
      },
    } as OpenClawConfig);
  ...
  it("continues scheduling after runOnce throws an unhandled error", async () => {
    ...
    const runSpy = vi.fn().mockImplementation(async () => {
      ...
      return { durationMs: 1, status: "ran" };
    });
  ...
  it("cleanup is idempotent and does not clear a newer runner's handler", async () => {
    ...
    const runSpy1 = vi.fn().mockResolvedValue({ durationMs: 1, status: "ran" });
    const runSpy2 = vi.fn().mockResolvedValue({ durationMs: 1, status: "ran" });
  ...
  it("does not push nextDueMs forward on repeated requests-in-flight skips", async () => {
    ...
    for (let i = 0; i < 4; i++) {
      requestHeartbeatNow({ coalesceMs: 0, reason: "retry" });
      await vi.advanceTimersByTimeAsync(1000);
    }
  ...
  it("does not fan out to unrelated agents for session-scoped exec wakes", async () => {
    ...
    const runner = await expectWakeDispatch({
      cfg: {
        ...heartbeatConfig([
          { heartbeat: { every: "30m" }, id: "main" },
          { heartbeat: { every: "30m" }, id: "finance" },
        ]),
      } as OpenClawConfig,
      expectedCall: {
        agentId: "main",
