...
export interface SafeLocalReadResult {
  buffer: Buffer;
  realPath: string;
  stat: Stats;
}

export interface FsSafeTestHooks {
...
async function openVerifiedLocalFile(
  ...
  try {
    ...
    if (isSymlinkOpenError(error)) {
      throw new SafeOpenError("symlink", "symlink open blocked", { cause: error });
    }
...
async function resolvePathWithinRoot(params: {
  ...
  return { resolved, rootReal, rootWithSep };
}
...
export async function readFileWithinRoot(params: {
  ...
  try {
    return await readOpenedFileSafely({ maxBytes: params.maxBytes, opened });
  } finally {
    await opened.handle.close().catch(() => {});
  }
}
...
export interface SafeWritableOpenResult {
  handle: FileHandle;
...
export async function openWritableFileWithinRoot(params: {
  ...
  try {
    ...
    throw new SafeOpenError("invalid-path", "path alias escape blocked", { cause: error });
  }
  ...
  try {
    ...
    if (isSymlinkOpenError(error)) {
      throw new SafeOpenError("invalid-path", "symlink open blocked", { cause: error });
    }
    throw error;
  }
  ...
  try {
    ...
    const cleanupCreatedPath = createdForWrite && error instanceof SafeOpenError;
...
export async function copyFileWithinRoot(params: {
  ...
  try {
    ...
    try {
      ...
      emitWriteBoundaryWarning(`post-copy verification failed: ${String(error)}`);
      throw error;
    }
  } finally {
    await source.handle.close().catch(() => {});
...
async function resolvePinnedPathWithinRoot(params: {
  ...
  if ((relativeResolved === "" || relativeResolved === ".") && params.allowRoot === true) {
    return { relativePosix: "", resolved: resolved.canonicalPath, rootReal: resolved.rootReal };
  }
  if (
    relativeResolved === "" ||
    relativeResolved === "." ||
