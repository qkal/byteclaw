...
describe("ssh-config", () => {
  ...
  it("resolves ssh config via ssh -G", async () => {
    const config = await resolveSshConfig({ host: "alias", port: 22, user: "me" });
  ...
  it("returns null when ssh -G fails", async () => {
    ...
    const config = await resolveSshConfig({ host: "bad-host", port: 22, user: "me" });
  ...
  it("returns null when the ssh process emits an error", async () => {
    ...
    await expect(resolveSshConfig({ host: "bad-host", port: 22, user: "me" })).resolves.toBeNull();
  });
});
