...
export interface RestartSentinelLog {
  stdoutTail?: string | null;
  stderrTail?: string | null;
  exitCode?: number | null;
}

export interface RestartSentinelStep {
  name: string;
  command: string;
  cwd?: string | null;
...
export interface RestartSentinelStats {
  mode?: string;
  root?: string;
...
export async function writeRestartSentinel(
  ...
  const data: RestartSentinel = { payload, version: 1 };
