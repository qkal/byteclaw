...
export interface DirectApnsRegistration {
  nodeId: string;
  transport: "direct";
  token: string;
  topic: string;
...
export interface RelayApnsRegistration {
  nodeId: string;
  transport: "relay";
  relayHandle: string;
  sendGrant: string;
...
interface ApnsRegistrationState {
  registrationsByNodeId: Record<string, ApnsRegistration>;
}

interface RegisterDirectApnsParams {
  nodeId: string;
  transport?: "direct";
  token: string;
  topic: string;
...
interface RegisterRelayApnsParams {
  nodeId: string;
  transport: "relay";
  relayHandle: string;
  sendGrant: string;
...
function getApnsBearerToken(auth: ApnsAuthConfig, nowMs: number = Date.now()): string {
  ...
  const payload = toBase64UrlJson({ iat, iss: auth.teamId });
...
async function sendApnsRequest(params: {
  ...
  return await new Promise((resolve, reject) => {
    ...
    req.on("end", () => {
      finish({ apnsId, body: responseBody, status: statusCode });
    });
...
interface ApnsAlertCommonParams {
  nodeId: string;
  title: string;
  body: string;
  timeoutMs?: number;
...
interface ApnsBackgroundWakeCommonParams {
  nodeId: string;
  wakeReason?: string;
  timeoutMs?: number;
}
...
interface ApnsExecApprovalAlertCommonParams {
  nodeId: string;
  approvalId: string;
  timeoutMs?: number;
}
...
interface ApnsExecApprovalResolvedCommonParams {
  nodeId: string;
  approvalId: string;
  timeoutMs?: number;
}
