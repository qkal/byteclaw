...
export interface ControlUiDistIndexHealth {
  ...
  for (const startDir of fallbackStartDirs) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (controlUiFsRuntime.existsSync(pkgJsonPath)) {
        try {
          const raw = controlUiFsRuntime.readFileSync(pkgJsonPath, "utf8");
...
export interface ControlUiRootResolveOptions {
  argv1?: string;
  moduleUrl?: string;
  cwd?: string;
  execPath?: string;
...
export interface EnsureControlUiAssetsResult {
  ok: boolean;
  built: boolean;
  message?: string;
}
