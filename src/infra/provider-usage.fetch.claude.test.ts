...
describe("fetchClaudeUsage", () => {
  ...
  it("parses oauth usage windows", async () => {
    ...
    const mockFetch = createProviderUsageFetch(async (_url, init) => {
      ...
      return makeResponse(200, {
        five_hour: { resets_at: fiveHourReset, utilization: 18 },
        seven_day: { resets_at: weekReset, utilization: 54 },
    ...
    expect(result.windows).toEqual([
      { label: "5h", resetAt: new Date(fiveHourReset).getTime(), usedPercent: 18 },
      { label: "Week", resetAt: new Date(weekReset).getTime(), usedPercent: 54 },
  ...
  it("clamps oauth usage windows and prefers sonnet over opus when both exist", async () => {
    ...
    expect(result.windows).toEqual([
      { label: "5h", resetAt: undefined, usedPercent: 0 },
      { label: "Week", resetAt: undefined, usedPercent: 100 },
  ...
  it("falls back to claude web usage when oauth scope is missing", async () => {
    ...
    expect(result.windows).toEqual([{ label: "5h", resetAt: undefined, usedPercent: 12 }]);
  });
  ...
  it("parses sessionKey from Cookie-prefixed CLAUDE_WEB_COOKIE headers", async () => {
    ...
    const result = await fetchClaudeUsage("token", 5000, mockFetch);
    expect(result.error).toBeUndefined();
    expect(result.windows).toEqual([{ label: "5h", resetAt: undefined, usedPercent: 9 }]);
