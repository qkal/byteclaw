...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { expected: "openclaw.internal.", value: "openclaw.internal" },
    { expected: "openclaw.internal.", value: "openclaw.internal." },
    { expected: "openclaw.internal.", value: "  openclaw.internal  " },
    { expected: null, value: "" },
    { expected: null, value: "   " },
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    const existing = renderWideAreaGatewayZoneText({ ...makeZoneOpts(), serial: 2_026_031_301 });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    vi.spyOn(fs, "readFileSync").mockReturnValue(
      renderWideAreaGatewayZoneText({ ...makeZoneOpts(), serial: 2_026_031_304 }),
    );
    const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation(() => undefined);
