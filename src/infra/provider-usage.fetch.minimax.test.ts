...
async function expectMinimaxUsageResult(params: {
  ...
  expected: {
    plan?: string;
    windows: { label: string; usedPercent: number; resetAt?: number }[];
  };
}) {
...
describe("fetchMinimaxUsage", () => {
  ...
  it.each([
    {
      expected: {
        plan: "Pro Max",
        windows: [{ label: "3h", resetAt: 1_700_000_000_000, usedPercent: 35 }],
      },
    ...
    },
    {
      expected: {
        plan: "Starter",
        windows: [
          { label: "30m", resetAt: new Date("2026-01-08T00:00:00Z").getTime(), usedPercent: 25 },
        ],
      },
    ...
    },
    {
      expected: {
        plan: "Team",
        windows: [{ label: "5h", resetAt: 1_700_000_000_000, usedPercent: 75 }],
      },
    ...
    },
    {
      expected: {
        plan: "Coding Plan",
        windows: [{ label: "5h", resetAt: undefined, usedPercent: 2 }],
      },
    ...
    },
    {
      expected: {
        plan: "Payload Plan",
        windows: [{ label: "2h", resetAt: 1_700_000_100_000, usedPercent: 40 }],
      },
    ...
    },
    {
      expected: {
        ...
        windows: [{ label: "4h", resetAt: 1_774_195_200_000, usedPercent: 0.8333333333333334 }],
      },
    ...
    },
    {
      expected: {
        ...
        windows: [{ label: "4h", resetAt: 1_774_195_200_000, usedPercent: 25 }],
      },
    ...
    await expectMinimaxUsageResult({ expected, payload });
  });
  ...
  it("handles repeated nested records while scanning usage candidates", async () => {
    ...
    const result = await fetchMinimaxUsage("key", 5000, mockFetch);
    expect(result.windows).toEqual([{ label: "1h", resetAt: undefined, usedPercent: 20 }]);
  });
});
