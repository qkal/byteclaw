...
export interface ShellParserParityFixtureCase {
  id: string;
  command: string;
  ok: boolean;
  executables: string[];
...
interface ShellParserParityFixture {
  cases: ShellParserParityFixtureCase[];
}

export interface WrapperResolutionParityFixtureCase {
  id: string;
  argv: string[];
  expectedRawExecutable: string | null;
}

interface WrapperResolutionParityFixture {
