...
async function* readJsonlRecords(filePath: string): AsyncGenerator<Record<string, unknown>> {
  ...
  const rl = readline.createInterface({ crlfDelay: Infinity, input: fileStream });
  try {
...
async function scanTranscriptFile(params: {
  ...
  for await (const parsed of readJsonlRecords(params.filePath)) {
    ...
    if (entry.usage && entry.costTotal === undefined) {
      ...
      entry.costTotal = estimateUsageCost({ cost, usage: entry.usage });
    }
...
export async function discoverAllSessions(params?: {
  ...
  return [...discovered.values()].toSorted((a, b) => b.mtime - a.mtime);
}
...
export async function loadSessionCostSummary(params: {
  ...
  await scanTranscriptFile({
    ...
    onEntry: (entry) => {
      ...
      if (entry.timestamp) {
        ...
        const existing = dailyMap.get(dayKey) ?? { cost: 0, tokens: 0 };
        dailyMap.set(dayKey, {
  ...
  const dailyBreakdown: SessionDailyUsage[] = [...dailyMap.entries()]
  ...
  const toolUsage: SessionToolUsage | undefined = toolUsageMap.size
    ? {
        tools: [...toolUsageMap.entries()]
          .map(([name, count]) => ({ count, name }))
          .toSorted((a, b) => b.count - a.count),
        totalCalls: [...toolUsageMap.values()].reduce((sum, count) => sum + count, 0),
        uniqueTools: toolUsageMap.size,
      }
    : undefined;

  const modelUsage = modelUsageMap.size
    ? [...modelUsageMap.values()].toSorted((a, b) => {
  ...
  return {
    activityDates: [...activityDatesSet].toSorted(),
    dailyBreakdown,
...
export async function loadSessionUsageTimeSeries(params: {
  ...
  if (sortedPoints.length > maxPoints) {
    ...
    return { points: downsampled, sessionId: params.sessionId };
  }

  return { points: sortedPoints, sessionId: params.sessionId };
}
      ...
      if (role === "assistant") {
        ...
        if (usage) {
          ...
          if (breakdown?.total !== undefined) {
            ...
            cost = estimateUsageCost({ cost: costConfig, usage });
          }
        }
      }
