...
describe("isGatewayArgv", () => {
  ...
  it("matches known entrypoints across slash and case variants", () => {
    expect(isGatewayArgv(["NODE", String.raw`C:\OpenClaw\DIST\ENTRY.JS`, "gateway"])).toBe(true);
  ...
  it("matches the openclaw executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv([String.raw`C:\bin\openclaw.cmd`, "gateway"])).toBe(true);
    ...
    expect(
      isGatewayArgv([String.raw`C:\bin\openclaw-gateway.EXE`, "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
