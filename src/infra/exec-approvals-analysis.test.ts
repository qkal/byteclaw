...
describe("exec approvals shell analysis", () => {
  ...
  describe("shell parsing", () => {
    ...
    it.each([String.raw`echo "output: \$(whoami)"`, "echo 'output: $(whoami)'"])(
    ...
    it("unwraps powershell -Command when a flag value contains spaces (quoted)", () => {
      ...
      const cases = [
        String.raw`powershell -WorkingDirectory "C:\Users\Jane Doe\proj" -Command "node a.js"`,
        String.raw`powershell -WorkingDirectory 'C:\Users\Jane Doe\proj' -Command "node a.js"`,
        String.raw`pwsh -ExecutionPolicy Bypass -WorkingDirectory "C:\My Projects\app" -Command "node a.js"`,
      ];
      for (const command of cases) {
        const res = analyzeShellCommand({ command, platform: "win32" });
        expect(res.ok).toBe(true);
  ...
  describe("shell allowlist (chained commands)", () => {
    ...
    it.each(['/usr/bin/echo "foo && bar"', String.raw`/usr/bin/echo "foo\" && bar"`])(
...
describe("windowsEscapeArg", () => {
  it("returns empty string quoted", () => {
    expect(windowsEscapeArg("")).toEqual({ escaped: '""', ok: true });
  });

  it("returns safe values as-is", () => {
    expect(windowsEscapeArg("foo.exe")).toEqual({ escaped: "foo.exe", ok: true });
    expect(windowsEscapeArg("C:/Program/bin")).toEqual({ escaped: "C:/Program/bin", ok: true });
  });

  it("double-quotes values with spaces", () => {
    expect(windowsEscapeArg("hello world")).toEqual({ escaped: '"hello world"', ok: true });
  });

  it("escapes embedded double quotes", () => {
    expect(windowsEscapeArg('say "hi"')).toEqual({ escaped: '"say ""hi"""', ok: true });
  });
  ...
  it("allows ! in double-quoted args (PowerShell does not treat ! as special)", () => {
    expect(windowsEscapeArg("hello!")).toEqual({ escaped: '"hello!"', ok: true });
  });
...
describe("matchAllowlist with argPattern", () => {
  ...
  it("matches argPattern with regex", () => {
    const entries: ExecAllowlistEntry[] = [{ argPattern: "^a\\.py$", pattern: "/usr/bin/python3" }];
    expect(matchAllowlist(entries, resolution, ["python3", "a.py"])).toBeTruthy();
  ...
  it("prefers argPattern match over path-only match", () => {
    const entries: ExecAllowlistEntry[] = [
      ...
      { argPattern: "^a\\.py$", pattern: "/usr/bin/python3" },
    ];
    const match = matchAllowlist(entries, resolution, ["python3", "a.py"]);
    expect(match).toBeTruthy();
    expect(match!.argPattern).toBe(String.raw`^a\.py$`);
  });
  ...
  it("handles invalid regex gracefully", () => {
    const entries: ExecAllowlistEntry[] = [{ argPattern: "[invalid", pattern: "/usr/bin/python3" }];
  ...
  it("rejects split-arg bypass against single-arg auto-generated argPattern", () => {
    ...
    const entries: ExecAllowlistEntry[] = [
      { argPattern: "^hello world\x00$", pattern: "/usr/bin/python3" },
    ];
  ...
  it("supports regex alternation in argPattern", () => {
    const entries: ExecAllowlistEntry[] = [
      { argPattern: "^(a|b)\\.py$", pattern: "/usr/bin/python3" },
    ];
    expect(matchAllowlist(entries, resolution, ["python3", "a.py"])).toBeTruthy();
    expect(matchAllowlist(entries, resolution, ["python3", "b.py"])).toBeTruthy();
  ...
  it("distinguishes zero-arg pattern from one-empty-string-arg pattern", () => {
    ...
    const zeroArgEntries: ExecAllowlistEntry[] = [
      { argPattern: "^\x00\x00$", pattern: "/usr/bin/python3" },
    ];
    const emptyArgEntries: ExecAllowlistEntry[] = [
      { argPattern: "^\x00$", pattern: "/usr/bin/python3" },
    ];
