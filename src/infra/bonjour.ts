...
export interface GatewayBonjourAdvertiser {
  stop: () => Promise<void>;
}

export interface GatewayBonjourAdvertiseOpts {
  instanceName?: string;
  gatewayPort: number;
  sshPort?: number;
...
interface BonjourCycle {
  responder: BonjourResponder;
  services: { label: string; svc: BonjourService }[];
  cleanupUnhandledRejection?: () => void;
}

interface ServiceStateTracker {
...
const STUCK_ANNOUNCING_MS = 8000;
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    function createCycle(): BonjourCycle {
      ...
      const services: { label: string; svc: BonjourService }[] = [];
      ...
      return { cleanupUnhandledRejection, responder, services };
    }
    ...
    function attachConflictListeners(services: { label: string; svc: BonjourService }[]) {
      for (const { label, svc } of services) {
        try {
          ...
          logDebug(`bonjour: failed to attach listeners for ${label}: ${String(error)}`);
        }
      }
    }

    function startAdvertising(services: { label: string; svc: BonjourService }[]) {
      for (const { label, svc } of services) {
        try {
          void svc
            .advertise()
            ...
            .catch((error) => {
              logWarn(
                `bonjour: advertise failed (${serviceSummary(label, svc)}): ${formatBonjourError(error)}`,
              );
            });
        } catch (error) {
          logWarn(
            `bonjour: advertise threw (${serviceSummary(label, svc)}): ${formatBonjourError(error)}`,
          );
        }
      }
    }
    ...
    const updateStateTrackers = (services: { label: string; svc: BonjourService }[]) => {
      const now = Date.now();
      for (const { label, svc } of services) {
        ...
        if (!current || current.state !== nextState || current.sinceMs !== nextEnteredAt) {
          stateTracker.set(label, { sinceMs: nextEnteredAt, state: nextState });
        }
      }
    };
    ...
    const watchdog = setInterval(() => {
      ...
      for (const { label, svc } of cycle.services) {
        ...
        try {
          void svc.advertise().catch((error) => {
            logWarn(
              `bonjour: watchdog advertise failed (${serviceSummary(label, svc)}): ${formatBonjourError(error)}`,
            );
          });
        } catch (error) {
          logWarn(
            `bonjour: watchdog advertise threw (${serviceSummary(label, svc)}): ${formatBonjourError(error)}`,
          );
        }
      }
