...
vi.mock("../agents/auth-profiles.js", () => {
  ...
  const resolveApiKeyForProfile = async (params: {
    ...
    if (cred.type === "token") {
      ...
      return cred.token
        ? { apiKey: cred.token, email: cred.email, provider: cred.provider ?? profileProvider }
        : null;
    }
    if (cred.type === "oauth") {
      if (typeof cred.expires === "number" && cred.expires <= Date.now()) {
      ...
      return token
        ? { apiKey: token, email: cred.email, provider: cred.provider ?? profileProvider }
        : null;
    }
    return null;
  };
    ...
    resolveProviderUsageAuthWithPlugin: vi.fn(async (params) => {
      ...
      if (params.provider === "google-gemini-cli") {
        ...
        try {
          ...
          return resolved.accountId ? { accountId: resolved.accountId, token } : { token };
        } catch {
          return resolved.accountId
            ? { accountId: resolved.accountId, token: resolved.token }
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    nodeFs.writeFileSync(
      ...
      `${JSON.stringify({ profiles: {}, version: 1 }, null, 2)}\n`,
      "utf8",
    );
    ...
    await expectResolvedAuthsFromSuiteHome({
      ...
      setup: async (home) => {
        await writeAuthProfiles(home, {
          "minimax:default": { provider: "minimax", token: "mini-\r\nmax", type: "token" },
          "xiaomi:default": { key: "xiao-\r\nmi", provider: "xiaomi", type: "api_key" },
        });
      },
    });
  });
  ...
  it("returns injected auth values unchanged", async () => {
    const auths = await resolveProviderAuths({
      auth: [{ accountId: "acc-1", provider: "anthropic", token: "token-1" }],
      providers: ["anthropic"],
    });
    expect(auths).toEqual([{ accountId: "acc-1", provider: "anthropic", token: "token-1" }]);
  });
      ...
      models: {
        providers: {
          minimax: {
            ...
            baseUrl: "https://api.minimaxi.com",
            models: [createTestModelDefinition()], // Pragma: allowlist secret
          },
          xiaomi: {
  ...
  it("uses zai api_key auth profiles when env and config are missing", async () => {
    await expectResolvedAuthsFromSuiteHome({
      ...
      setup: async (home) => {
        await writeAuthProfiles(home, {
          "zai:default": { key: "profile-zai-key", provider: "zai", type: "api_key" },
        });
      },
    });
  });
  ...
  it("skips oauth profiles that resolve without an api key and uses later profiles", async () => {
    await withSuiteHome(async (home) => {
      await writeAuthProfiles(home, {
        ...
        "anthropic:valid": { provider: "anthropic", token: "anthropic-token", type: "token" },
      });
  ...
  it("skips api_key entries in oauth token resolution order", async () => {
    await withSuiteHome(async (home) => {
      await writeAuthProfiles(home, {
        "anthropic:api": { key: "api-key-1", provider: "anthropic", type: "api_key" },
        "anthropic:token": { provider: "anthropic", token: "token-1", type: "token" },
      });
