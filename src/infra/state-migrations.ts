...
interface LegacySessionSurface {
...
async function runLegacyMigrationPlans(
  ...
  for (const plan of plans) {
    ...
    try {
      ...
      warnings.push(`Failed migrating ${plan.label} (${plan.sourcePath}): ${String(error)}`);
    }
  }
...
function canonicalizeSessionKeyForAgent(params: {
  ...
  const canonicalMain = canonicalizeMainSessionAlias({
    agentId,
    cfg: { session: { mainKey: params.mainKey, scope: params.scope } },
    sessionKey: raw,
  });
...
function canonicalizeSessionStore(params: {
  ...
  return { legacyKeys, store: canonical };
}
...
interface StateDirMigrationResult {
  migrated: boolean;
  skipped: boolean;
  changes: string[];
  warnings: string[];
...
export async function autoMigrateLegacyStateDir(params: {
  ...
  if (autoMigrateStateDirChecked) {
    return { changes: [], migrated: false, skipped: true, warnings: [] };
  }
  ...
  if (!legacyStat) {
    return { changes, migrated: false, skipped: false, warnings };
  }
  ...
  try {
    ...
    warnings.push(
      `Failed to move legacy state dir (${legacyDir ?? "unknown"} → ${targetDir}): ${String(error)}`,
    );
    return { changes, migrated: false, skipped: false, warnings };
  }
  ...
  try {
    ...
    try {
      if (process.platform === "win32") {
        if (!legacyDir) {
          throw new Error("Legacy state dir not found", { cause: error });
        }
      ...
      try {
        ...
        warnings.push(
          `State dir migration rolled back (failed to link legacy path): ${String(error)}`,
        );
        return { changes: [], migrated: false, skipped: false, warnings };
      } catch (error) {
        warnings.push(
          `State dir moved but failed to link legacy path (${legacyDir ?? "unknown"} → ${targetDir}): ${String(error)}`,
        );
        warnings.push(
          `Rollback failed; set OPENCLAW_STATE_DIR=${targetDir} to avoid split state: ${String(error)}`,
        );
  ...
  return { changes, migrated: changes.length > 0, skipped: false, warnings };
}
...
async function migrateLegacySessions(
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      warnings.push(`Failed moving ${from}: ${String(error)}`);
    }
  }
...
export async function migrateLegacyAgentDir(
  ...
  if (!emptyDirOrMissing(detected.agentDir.legacyDir)) {
    ...
    try {
      ...
      warnings.push(`Failed relocating legacy agent dir: ${String(error)}`);
    }
  }
...
export async function migrateOrphanedSessionKeys(params: {
  ...
  for (const [storePath, storeAgentIds] of storeMap) {
    ...
    try {
      ...
      warnings.push(`Could not read ${storePath}: ${String(error)}`);
      continue;
    }
    if (!parsed.ok) {
      continue;
    ...
    try {
      ...
      warnings.push(`Failed to write canonicalized store ${storePath}: ${String(error)}`);
    }
  }
