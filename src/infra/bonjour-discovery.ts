...
export interface GatewayBonjourBeacon {
  instanceName: string;
  domain?: string;
  displayName?: string;
  host?: string;
...
export interface GatewayDiscoveryResolvedEndpoint {
  host: string;
  port: number;
  gatewayTls: boolean;
  gatewayTlsFingerprintSha256?: string;
...
export interface GatewayBonjourDiscoverOpts {
  timeoutMs?: number;
  domains?: string[];
...
function parseAvahiBrowse(stdout: string): GatewayBonjourBeacon[] {
  ...
  for (const raw of stdout.split("\n")) {
    ...
    if (line.startsWith("=") && line.includes(GATEWAY_SERVICE_TYPE)) {
      ...
      const left = idx !== -1 ? line.slice(0, idx).trim() : line;
