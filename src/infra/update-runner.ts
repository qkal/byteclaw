...
async function removePathRecursive(target: string) {
  await fs
    .rm(target, { force: true, maxRetries: 3, recursive: true, retryDelay: 200 })
    .catch(() => {});
}
...
async function repairWindowsPreflightCleanup(worktreeDir: string, preflightRoot: string) {
  ...
  try {
    await fs.rm(worktreeDir, { force: true, maxRetries: 3, recursive: true, retryDelay: 200 });
    await fs.rm(preflightRoot, { force: true, maxRetries: 3, recursive: true, retryDelay: 200 });
    return true;
  } catch {
    return false;
  }
...
async function runStep(opts: RunStepOptions): Promise<UpdateStepResult> {
  ...
  const result = await runCommand(argv, { cwd, env, timeoutMs });
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const manager = await resolveUpdateBuildManager(
        (argv, options) => runCommand(argv, { env: options.env, timeoutMs: options.timeoutMs }),
        gitRoot,
        timeoutMs,
        defaultCommandEnv,
        "require-preferred",
