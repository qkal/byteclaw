...
async function makeFakeGitRepo(
  ...
  await fs.mkdir(gitdir, { recursive: true });
  await fs.writeFile(path.join(gitdir, "HEAD"), options.head, "utf8");
  ...
  if (options.commondir) {
    await fs.writeFile(path.join(gitdir, "commondir"), options.commondir, "utf8");
  }
...
describe("git commit resolution", () => {
  ...
  it("treats invalid moduleUrl inputs as a fallback hint instead of throwing", async () => {
    ...
    expect(() =>
      resolveCommitHash({ cwd: repoRoot, env: {}, moduleUrl: "not-a-file-url" }),
    ).not.toThrow();
    expect(resolveCommitHash({ cwd: repoRoot, env: {}, moduleUrl: "not-a-file-url" })).toBe(
      repoHead,
    );
  });
