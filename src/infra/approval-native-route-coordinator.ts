...
interface ApprovalRouteRuntimeRecord {
  runtimeId: string;
...
interface PendingApprovalRouteNotice {
  request: ApprovalRequest;
...
export function clearApprovalNativeRouteStateForTest(): void {
  for (const approvalId of [...pendingApprovalRouteNotices.keys()]) {
    clearPendingApprovalRouteNotice(approvalId);
  }
