...
async function setupSymlinkWriteRaceFixture(options?: { seedInsideTarget?: boolean }): Promise<{
  ...
  return { outside, outsideTarget, root, slot };
}
...
describe("fs-safe", () => {
  ...
  it("rejects directories", async () => {
    ...
    const err = await readLocalFileSafely({ filePath: dir }).catch((error: unknown) => error);
  ...
  it("rejects directory path within root without leaking EISDIR (issue #31186)", async () => {
    ...
    await expect(
      openFileWithinRoot({ relativePath: "memory", rootDir: root }),
