...
export function resolvePreferredOpenClawTmpDir(
  ...
  const isTrustedTmpDir = (st: {
    ...
    mode?: number;
    uid?: number;
  }): boolean => st.isDirectory() && !st.isSymbolicLink() && isSecureDirForUser(st);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    try {
      mkdirSync(fallbackPath, { mode: 0o700, recursive: true });
  ...
  try {
    ...
    mkdirSync(POSIX_OPENCLAW_TMP_DIR, { mode: 0o700, recursive: true });
