...
interface ShellWrapperSpec {
  kind: ShellWrapperKind;
  names: ReadonlySet<string>;
}

const SHELL_WRAPPER_SPECS: readonly ShellWrapperSpec[] = [
...
export interface ShellWrapperCommand {
  isWrapper: boolean;
  command: string | null;
}
...
export function unwrapKnownShellMultiplexerInvocation(
  ...
  return { argv: unwrapped, kind: "unwrapped", wrapper };
}
...
function extractShellWrapperCommandInternal(
  ...
  if (!resolved) {
    return { command: null, isWrapper: false };
  }

  return { command: rawCommand ?? resolved.payload, isWrapper: true };
}
