...
describe("update global helpers", () => {
  ...
  it("resolves global roots and package roots from runner output", async () => {
    const runCommand: CommandRunner = async (argv) => {
      if (argv[0] === "npm") {
        return { code: 0, stderr: "", stdout: "/tmp/npm-root\n" };
      }
      if (argv[0] === "pnpm") {
  ...
  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "openclaw-update-global-" }, async (base) => {
      ...
      const runCommand: CommandRunner = async (argv) => {
        if (argv[0] === "npm") {
          return { code: 0, stderr: "", stdout: `${npmRoot}\n` };
        }
        if (argv[0] === "pnpm") {
          return { code: 0, stderr: "", stdout: `${pnpmRoot}\n` };
        }
        throw new Error(`unexpected command: ${argv.join(" ")}`);
      };
      ...
      await fs.rm(path.join(npmRoot, "openclaw"), { force: true, recursive: true });
      await fs.rm(path.join(pnpmRoot, "openclaw"), { force: true, recursive: true });
  ...
  it("prefers the owning npm prefix when PATH npm points at a different global root", async () => {
    ...
    try {
      await withTempDir({ prefix: "openclaw-update-npm-prefix-" }, async (base) => {
        ...
        const runCommand: CommandRunner = async (argv) => {
          if (argv[0] === "npm") {
            return { code: 0, stderr: "", stdout: `${pathNpmRoot}\n` };
          }
          if (argv[0] === brewNpm) {
            return { code: 0, stderr: "", stdout: `${brewRoot}\n` };
          }
          if (argv[0] === "pnpm") {
            return { code: 1, stderr: "", stdout: "" };
          }
  ...
  it("builds install argv and npm fallback argv", () => {
    ...
    expect(
      globalInstallArgs({ command: "/opt/homebrew/bin/pnpm", manager: "pnpm" }, "openclaw@latest"),
