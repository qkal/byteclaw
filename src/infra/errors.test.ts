...
describe("error helpers", () => {
  it.each([
    { expected: "EADDRINUSE", value: { code: "EADDRINUSE" } },
    { expected: "429", value: { code: 429 } },
    { expected: undefined, value: { code: false } },
    { expected: undefined, value: "boom" },
  ])("extracts error codes from %j", ({ value, expected }) => {
    expect(extractErrorCode(value)).toBe(expected);
  });

  it.each([
    { expected: "AbortError", value: { name: "AbortError" } },
    { expected: "", value: { name: 42 } },
    { expected: "", value: null },
  ...
  it("walks nested error graphs once in breadth-first order", () => {
    ...
    const root = { cause: child, errors: [leaf, child], name: "root" };
    child.cause = root;
  ...
  it.each([
    { expected: "123", value: 123n },
    { expected: "false", value: false },
    { expected: "[object Object]", value: createCircularObject() },
