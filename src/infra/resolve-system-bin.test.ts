...
describe("resolveSystemBin", () => {
  ...
  if (process.platform === "darwin") {
    ...
    it("extraDirs composes with standard trust", () => {
      ...
      executables.add(`${customDir}/mytool`);
      expect(resolveSystemBin("mytool", { extraDirs: [customDir], trust: "standard" })).toBe(
        `${customDir}/mytool`,
      );
    });
  }
...
describe("trusted directory list", () => {
  ...
  if (process.platform === "win32") {
    ...
    it("uses validated Windows install roots from HKLM values", () => {
      _resetWindowsInstallRootsForTests({
        queryRegistryValue: (key, valueName) => {
          ...
          if (
            ...
            return String.raw`D:\Program Files`;
          }
          if (
            key === String.raw`HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion` &&
          ...
          if (
            ...
            return String.raw`E:\Program Files (x86)`;
          }
          return null;
        },
      });
      ...
      _resetResolveSystemBin((p: string) => executables.has(path.resolve(p)));
      const dirs = _getTrustedDirs();
      expect(dirs).toContain(path.win32.join(String.raw`D:\Windows`, "System32"));
      expect(dirs).toContain(path.win32.join(String.raw`D:\Program Files`, "OpenSSL-Win64", "bin"));
      expect(dirs).toContain(path.win32.join(String.raw`E:\Program Files (x86)`, "OpenSSL", "bin"));
    });
    ...
    it("falls back safely when HKLM values are unavailable", () => {
      ...
      expectDirsContainAll(normalizedDirs, [
        path.win32.join(String.raw`C:\Windows`, "System32").toLowerCase(),
        path.win32.join(String.raw`C:\Program Files`, "OpenSSL-Win64", "bin").toLowerCase(),
        path.win32.join(String.raw`C:\Program Files (x86)`, "OpenSSL", "bin").toLowerCase(),
      ]);
    });
