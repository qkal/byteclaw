...
export interface WideAreaGatewayZoneOpts {
  domain: string;
  gatewayPort: number;
  displayName: string;
  tailnetIPv4: string;
...
export async function writeWideAreaGatewayZone(
  ...
  const existing = (() => {
    try {
      return fs.readFileSync(zonePath, "utf8");
    } catch {
      return null;
    }
  })();
  ...
  fs.writeFileSync(zonePath, next, "utf8");
  return { changed: true, zonePath };
}
