...
export function readExecApprovalsSnapshot(): ExecApprovalsSnapshot {
  ...
  if (!fs.existsSync(filePath)) {
    const file = normalizeExecApprovals({ agents: {}, version: 1 });
    return {
      exists: false,
      file,
  ...
  const file =
    parsed?.version === 1
      ...
      : normalizeExecApprovals({ agents: {}, version: 1 });
  return {
    exists: true,
    file,
...
export function loadExecApprovals(): ExecApprovalsFile {
  ...
  try {
    if (!fs.existsSync(filePath)) {
      return normalizeExecApprovals({ agents: {}, version: 1 });
    }
    const raw = fs.readFileSync(filePath, "utf8");
...
function writeExecApprovalsRaw(filePath: string, raw: string) {
  ...
  try {
    fs.writeFileSync(tempPath, raw, { flag: "wx", mode: 0o600 });
    tempWritten = true;
...
interface ResolvedExecPolicyField<TValue extends ExecSecurity | ExecAsk> {
  value: TValue;
  source: string | null;
}
...
export function minSecurity(a: ExecSecurity, b: ExecSecurity): ExecSecurity {
  const order: Record<ExecSecurity, number> = { allowlist: 1, deny: 0, full: 2 };
  return order[a] <= order[b] ? a : b;
}

export function maxAsk(a: ExecAsk, b: ExecAsk): ExecAsk {
  const order: Record<ExecAsk, number> = { always: 2, off: 0, "on-miss": 1 };
  return order[a] >= order[b] ? a : b;
}
