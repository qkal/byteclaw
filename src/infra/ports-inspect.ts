...
async function runCommandSafe(argv: string[], timeoutMs = 5000): Promise<CommandResult> {
  try {
...
async function readUnixListenersFromSs(
  ...
  if (res.code === 0) {
    ...
    return { detail: res.stdout.trim() || undefined, errors, listeners };
  }
  const stderr = res.stderr.trim();
  if (res.code === 1 && !res.error && !stderr) {
    return { detail: undefined, errors, listeners: [] };
  }
  if (res.error) {
...
async function tryListenOnHost(port: number, host: string): Promise<PortUsageStatus | "skip"> {
  try {
    await tryListenOnPort({ exclusive: true, host, port });
    return "free";
  } catch (error) {
    if (isErrno(error) && error.code === "EADDRINUSE") {
      return "busy";
    }
    if (isErrno(error) && (error.code === "EADDRNOTAVAIL" || error.code === "EAFNOSUPPORT")) {
      return "skip";
    }
    return "unknown";
  }
