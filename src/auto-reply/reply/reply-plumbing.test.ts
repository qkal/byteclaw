...
describe("buildThreadingToolContext", () => {
  ...
  it("uses Slack plugin threading context when the plugin registry is active", () => {
    setActivePluginRegistry(
      createTestRegistry([
        { plugin: createSlackThreadingPlugin(), pluginId: "slack", source: "test" },
      ]),
    );
    const sessionCtx = {
      MessageThreadId: "123.456",
...
describe("subagents utils", () => {
  ...
  it("sorts subagent runs by newest start/created time", () => {
    const runs: SubagentRunRecord[] = [
      { ...baseRun, createdAt: 1000, runId: "run-1", startedAt: 1000 },
      { ...baseRun, createdAt: 1200, runId: "run-2", startedAt: 1200 },
      { ...baseRun, createdAt: 900, runId: "run-3" },
    ];
