...
function mockCompactionRun(params: {
  ...
  runEmbeddedPiAgentMock.mockImplementationOnce(
    async (args: {
      ...
      args.onAgentEvent?.({
        data: { completed: true, phase: "end", willRetry: params.willRetry },
        stream: "compaction",
      });
...
describe("createFollowupRunner compaction", () => {
  it("adds verbose auto-compaction notice and tracks count", async () => {
    ...
    mockCompactionRun({
      result: { meta: {}, payloads: [{ text: "final" }] },
      willRetry: true,
    });
    ...
    const firstCall = (onBlockReply.mock.calls as unknown as { text?: string }[][])[0];
  ...
  it("tracks auto-compaction from embedded result metadata even when no compaction event is emitted", async () => {
    ...
    runEmbeddedPiAgentMock.mockResolvedValueOnce({
      meta: {
        agentMeta: {
          compactionCount: 2,
          lastCallUsage: { input: 10_000, output: 3000, total: 13_000 },
  ...
  it("does not count failed compaction end events in followup runs", async () => {
    ...
    runEmbeddedPiAgentMock.mockImplementationOnce(async (args) => {
      args.onAgentEvent?.({
        data: { completed: false, phase: "end", willRetry: false },
        stream: "compaction",
      });
      return {
        meta: {
  ...
  it("injects the post-compaction refresh prompt before followup runs after preflight compaction", async () => {
    ...
    const embeddedCalls: { extraSystemPrompt?: string }[] = [];
    runEmbeddedPiAgentMock.mockImplementationOnce(
...
describe("createFollowupRunner messaging tool dedupe", () => {
  ...
  it("persists usage even when replies are suppressed", async () => {
    ...
    expect(sessionStore[sessionKey]?.inputTokens).toBe(1000);
