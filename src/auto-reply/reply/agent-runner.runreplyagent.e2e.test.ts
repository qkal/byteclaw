...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  it("suppresses NO_REPLY partials but allows normal No-prefix partials", async () => {
    ...
    for (const testCase of cases) {
      ...
      state.runEmbeddedPiAgentMock.mockImplementationOnce(async (params: AgentRunParams) => {
        ...
        return { meta: {}, payloads: [{ text: testCase.finalText }] };
      });
  ...
  it("suppresses narrated silent-turn partials, block replies, and final payloads", async () => {
    ...
    state.runEmbeddedPiAgentMock.mockImplementationOnce(async (params: AgentRunParams) => {
      ...
      return { meta: {}, payloads: [{ text: "I am trying to send NO_REPLY now." }] };
    });

    const { run } = createMinimalRun({
      blockStreamingEnabled: true,
      opts: { isHeartbeat: false, onBlockReply, onPartialReply, onReasoningStream },
  ...
  it("suppresses bare NO_REPLY silent-turn payloads", async () => {
    ...
    state.runEmbeddedPiAgentMock.mockImplementationOnce(async (params: AgentRunParams) => {
      ...
      return { meta: { finalAssistantText: "NO_REPLY" }, payloads: [{ text: "NO_REPLY" }] };
    });
  ...
  it("keeps assistant partial streaming enabled when reasoning mode is stream", async () => {
    ...
    expect(onPartialReply).toHaveBeenCalledWith({ mediaUrls: undefined, text: "answer chunk" });
  });
  ...
  it("signals typing on normalized block replies", async () => {
    const onBlockReply = vi.fn();
    state.runEmbeddedPiAgentMock.mockImplementationOnce(async (params: AgentRunParams) => {
      await params.onBlockReply?.({ mediaUrls: [], text: "\n\nchunk" });
      return { meta: {}, payloads: [{ text: "final" }] };
    });
    ...
    expect(blockPayload).toMatchObject({ audioAsVoice: false, text: "chunk" });
  ...
  it("handles typing for normal and silent tool results", async () => {
    ...
    for (const testCase of cases) {
      ...
      state.runEmbeddedPiAgentMock.mockImplementationOnce(async (params: AgentRunParams) => {
        await params.onToolResult?.({ mediaUrls: [], text: testCase.toolText });
        return { meta: {}, payloads: [{ text: "final" }] };
      });
  ...
  it("announces model fallback only when verbose mode is enabled", async () => {
    const cases = [
      { expectNotice: true, name: "verbose on", verbose: "on" as const },
      { expectNotice: false, name: "verbose off", verbose: "off" as const },
    ] as const;
    for (const testCase of cases) {
  ...
  it("announces model fallback only once per active fallback state", async () => {
    ...
    try {
      ...
      const fallbackEvents: Record<string, unknown>[] = [];
      const off = onAgentEvent((evt) => {
