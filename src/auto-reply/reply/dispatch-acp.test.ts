...
function createDispatcher(): {
  ...
  const counts = { block: 0, final: 0, tool: 0 };
...
function mockVisibleTextTurn(text = "visible") {
  managerMocks.runTurn.mockImplementationOnce(
    async ({ onEvent }: { onEvent: (event: unknown) => Promise<void> }) => {
      await onEvent({ tag: "agent_message_chunk", text, type: "text_delta" });
      await onEvent({ type: "done" });
    },
  );
}
...
describe("tryDispatchAcpReply", () => {
  beforeEach(async () => {
    ...
    routeMocks.routeReply.mockResolvedValue({ messageId: "mock", ok: true });
  ...
  it("edits ACP tool lifecycle updates in place when supported", async () => {
    ...
    routeMocks.routeReply.mockResolvedValueOnce({ messageId: "tool-msg-1", ok: true });
  ...
  it("falls back to new tool message when edit fails", async () => {
    ...
    routeMocks.routeReply
      .mockResolvedValueOnce({ messageId: "tool-msg-2", ok: true })
      .mockResolvedValueOnce({ messageId: "tool-msg-2-fallback", ok: true });
  ...
  it("falls back to final text when a later telegram ACP block delivery fails", async () => {
    ...
    managerMocks.runTurn.mockImplementation(
      async ({ onEvent }: { onEvent: (event: unknown) => Promise<void> }) => {
        await onEvent({ tag: "agent_message_chunk", text: "First chunk. ", type: "text_delta" });
        await onEvent({ tag: "agent_message_chunk", text: "Second chunk.", type: "text_delta" });
        await onEvent({ type: "done" });
      },
    );
  ...
  it("honors the configured default account for ACP projector chunking when AccountId is omitted", async () => {
    ...
    managerMocks.runTurn.mockImplementation(
      async ({ onEvent }: { onEvent: (event: unknown) => Promise<void> }) => {
        await onEvent({ tag: "agent_message_chunk", text: "abcdef", type: "text_delta" });
        await onEvent({ type: "done" });
      },
    );
