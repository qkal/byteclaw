...
export interface ReplyBackendHandle {
...
export interface ReplyRunRegistry {
  begin(params: {
    sessionKey: string;
    sessionId: string;
...
export function createReplyOperation(params: {
  ...
  const abortWithReason = (
    ...
    if (opts?.abortedCode && !result) {
      result = { code: opts.abortedCode, kind: "aborted" };
    }
    phase = "aborted";
