...
function estimateSessionRunCostUsd(params: {
  ...
  return resolveNonNegativeNumber(estimateUsageCost({ cost, usage: params.usage }));
}
...
export async function persistSessionUsageUpdate(params: {
  ...
  if (hasUsage || hasFreshContextSnapshot) {
    try {
      ...
      logVerbose(`failed to persist ${label}usage update: ${String(error)}`);
    }
    return;
  }
  ...
  if (params.modelUsed || params.contextTokensUsed) {
    try {
      ...
      logVerbose(`failed to persist ${label}model/context update: ${String(error)}`);
    }
  }
}
