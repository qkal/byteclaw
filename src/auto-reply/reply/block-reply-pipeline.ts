...
export interface BlockReplyPipeline {
...
export interface BlockReplyBuffer {
...
export function createBlockReplyContentKey(payload: ReplyPayload): string {
  ...
  return JSON.stringify({ mediaList: reply.mediaUrls, text: reply.trimmedText });
}
  ...
  const sendPayload = (payload: ReplyPayload, bypassSeenCheck: boolean = false) => {
    ...
    sendChain = sendChain
      ...
      .catch((error) => {
        ...
        logVerbose(`block reply delivery failed: ${String(error)}`);
      })
      .finally(() => {
