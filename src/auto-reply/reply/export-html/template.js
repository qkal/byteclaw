(function () {
      ...
      if (Array.isArray(content)) {
        for (const block of content) {
          if (block.type === "toolCall") {
            toolCallMap.set(block.id, { arguments: block.arguments, name: block.name });
          }
        }
      }
    }
  ...
  function buildShareUrl(entryId) {
    ...
    const gistId = [...url.searchParams.keys()].find((k) => !url.searchParams.get(k));
  ...
  function computeStats(entryList) {
    ...
    let customMessages = 0,
      ...
      branchSummaries = 0,
      toolCalls = 0;
    const tokens = { cacheRead: 0, cacheWrite: 0, input: 0, output: 0 };
