...
export interface RouteReplyParams {
...
export async function routeReply(params: RouteReplyParams): Promise<RouteReplyResult> {
  ...
  if (!channelId) {
    return { error: `Unknown channel: ${String(channel)}`, ok: false };
  }
  if (abortSignal?.aborted) {
    return { error: "Reply routing aborted", ok: false };
  }
  ...
  try {
    ...
    return { messageId: last?.messageId, ok: true };
  } catch (error) {
