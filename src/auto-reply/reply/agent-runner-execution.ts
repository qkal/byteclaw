...
const FALLBACK_SELECTION_STATE_KEYS = [
  ...
  "authProfileOverrideSource",
  "authProfileOverrideCompactionCount",
] as const satisfies readonly (keyof FallbackSelectionState)[];
    ...
    try {
      const normalizeStreamingText = (payload: ReplyPayload): { text?: string; skip: boolean } => {
        ...
        if (!text) {
          ...
          if (reply.hasMedia) {
            return { skip: false, text: undefined };
          }
          return { skip: true };
        }
        ...
        return { skip: false, text: sanitized };
      };
                    ...
                    if (phase === "start") {
                      ...
                      if (params.opts?.onCompactionStart) {
                        ...
                        try {
                          ...
                          logVerbose(
                            `compaction start notice delivery failed (non-fatal): ${String(error)}`,
                          );
                        }
                      }
                    }
                      ...
                      return (payload: ReplyPayload) => {
                        toolResultChain = toolResultChain
                          ...
                          .catch((error) => {
                            ...
                            logVerbose(`tool result delivery failed: ${String(error)}`);
                          });
      ...
      if (error instanceof LiveSessionModelSwitchError) {
        ...
        if (liveModelSwitchRetries > MAX_LIVE_SWITCH_RETRIES) {
          ...
          defaultRuntime.error(
            `Live model switch failed after ${MAX_LIVE_SWITCH_RETRIES} retries ` +
              `(${sanitizeForLog(error.provider)}/${sanitizeForLog(error.model)}). The requested model may be unavailable.`,
          );
          ...
          params.replyOperation?.fail("run_failed", error);
          return {
            kind: "final",
            payload: {
        ...
        params.followupRun.run.provider = error.provider;
        params.followupRun.run.model = error.model;
        params.followupRun.run.authProfileId = error.authProfileId;
        params.followupRun.run.authProfileIdSource = error.authProfileId
      ...
      const restartLifecycleError = resolveRestartLifecycleError(error);
      ...
      if (
        ...
        try {
          ...
          defaultRuntime.error(
            `Failed to reset corrupted session ${params.sessionKey}: ${String(error)}`,
          );
        }

        params.replyOperation?.fail("session_corruption_reset", error);
        return {
          kind: "final",
          payload: {
      ...
      const isRateLimit = isFallbackSummaryError(error)
        ? isPureTransientRateLimitSummary(error)
      ...
      const fallbackText = isBilling
        ? BILLING_ERROR_USER_MESSAGE
        : isRateLimit
          ? buildRateLimitCooldownMessage(error)
          : isContextOverflow
