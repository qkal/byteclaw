...
import { SafeOpenError, copyFileWithinRoot } from "../../infra/fs-safe.js";
...
export async function stageSandboxMedia(params: {
  ...
  for (const raw of rawPaths) {
    ...
    try {
      ...
      if (error instanceof SafeOpenError && error.code === "too-large") {
        logVerbose(
          `Blocking inbound media staging above ${STAGED_MEDIA_MAX_BYTES} bytes: ${source}`,
        );
      } else {
        logVerbose(`Failed to stage inbound media path ${source}: ${String(error)}`);
      }
      continue;
    }
