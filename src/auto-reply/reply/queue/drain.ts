    ...
    try {
      ...
      while (queue.items.length > 0 || queue.droppedCount > 0) {
        ...
        if (queue.mode === "collect") {
          ...
          const summary = previewQueueSummaryPrompt({ noun: "message", state: queue });
        ...
        const summaryPrompt = previewQueueSummaryPrompt({ noun: "message", state: queue });
        if (summaryPrompt) {
      ...
      defaultRuntime.error?.(`followup queue drain failed for ${key}: ${String(error)}`);
    } finally {
