...
describe("typing controller", () => {
  ...
  it("stops only after both run completion and dispatcher idle are set (any order)", async () => {
    vi.useFakeTimers();
    const cases = [
      { first: "run", name: "run-complete first", second: "idle" },
      { first: "idle", name: "dispatch-idle first", second: "run" },
    ] as const;
...
describe("resolveResponsePrefixTemplate", () => {
  ...
  it("preserves unresolved/unknown placeholders and handles static inputs", () => {
    const cases = [
      { expected: undefined, name: "undefined template", template: undefined, values: {} },
      { expected: "[Claude]", name: "no variables", template: "[Claude]", values: {} },
      {
        expected: "[{model}]",
...
describe("createTypingSignaler", () => {
  it("gates run-start typing by mode", async () => {
    const cases = [
      { expectedStartCalls: 1, mode: "instant" as const, name: "instant" },
      { expectedStartCalls: 0, mode: "message" as const, name: "message" },
      { expectedStartCalls: 0, mode: "thinking" as const, name: "thinking" },
    ] as const;
    for (const testCase of cases) {
...
describe("block reply coalescer", () => {
  ...
  it("does not coalesce reasoning blocks into visible reply text", async () => {
    const flushes: { text?: string; isReasoning?: boolean }[] = [];
    const coalescer = createBlockReplyCoalescer({
      config: { idleMs: 0, joiner: "\n\n", maxChars: 200, minChars: 1 },
      onFlush: (payload) => {
        flushes.push({
    ...
    coalescer.enqueue({ isReasoning: true, text: "Reasoning:\n_hidden_" });
    coalescer.enqueue({ text: "Visible answer" });
    await coalescer.flush({ force: true });

    expect(flushes).toEqual([
      { isReasoning: true, text: "Reasoning:\n_hidden_" },
      { isReasoning: undefined, text: "Visible answer" },
    ]);
    coalescer.stop();
  });

  it("preserves compaction notice markers across flushes", async () => {
    const flushes: { text?: string; isCompactionNotice?: boolean }[] = [];
    const coalescer = createBlockReplyCoalescer({
      config: { idleMs: 0, joiner: "\n\n", maxChars: 200, minChars: 1 },
      onFlush: (payload) => {
        flushes.push({
    ...
    coalescer.enqueue({ isCompactionNotice: true, text: "Compacting context..." });
    await coalescer.flush({ force: true });

    expect(flushes).toEqual([{ isCompactionNotice: true, text: "Compacting context..." }]);
    coalescer.stop();
  });
  ...
  it("flushes immediately per enqueue when flushOnEnqueue is set", async () => {
    const cases = [
      {
        config: { flushOnEnqueue: true, idleMs: 50, joiner: "\n\n", maxChars: 200, minChars: 10 },
        expected: ["Hi"],
        inputs: ["Hi"],
      },
      {
        config: { flushOnEnqueue: true, idleMs: 100, joiner: "\n\n", maxChars: 30, minChars: 1 },
  ...
  it("flushes buffered text before media payloads", () => {
    const flushes: { text?: string; mediaUrls?: string[] }[] = [];
    const coalescer = createBlockReplyCoalescer({
      config: { idleMs: 0, joiner: " ", maxChars: 200, minChars: 1 },
      onFlush: (payload) => {
        flushes.push({
