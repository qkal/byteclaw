...
export function estimatePromptTokensForMemoryFlush(prompt?: string): number | undefined {
  ...
  const message: AgentMessage = { content: trimmed, role: "user", timestamp: Date.now() };
...
export interface SessionTranscriptUsageSnapshot {
  promptTokens?: number;
  outputTokens?: number;
}
...
export async function runPreflightCompactionIfNeeded(params: {
  ...
  const reserveTokensFloor =
    ...
    20_000;
  const softThresholdTokens = memoryFlushPlan?.softThresholdTokens ?? 4000;
  ...
  if (entry && shouldPersistTranscriptPromptTokens) {
    ...
    if (params.storePath && params.sessionKey) {
      try {
        ...
        logVerbose(`failed to persist derived prompt totalTokens: ${String(error)}`);
      }
    }
  }
  ...
  try {
    ...
    if (params.storePath && params.sessionKey) {
      try {
        ...
        logVerbose(`failed to persist memory flush metadata: ${String(error)}`);
      }
    }
  } catch (error) {
