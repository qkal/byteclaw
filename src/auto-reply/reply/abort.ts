...
function normalizeRequesterSessionKey(
  ...
  return resolveInternalSessionKey({ alias, key: cleaned, mainKey });
}
...
export function stopSubagentsForRequester(params: {
  ...
  const runs = [...dedupedRunsByChildKey.values()];
  if (runs.length === 0) {
    return { stopped: 0 };
  }
...
export async function tryFastAbortFromMessage(params: {
  ...
  if (!abortRequested) {
    return { aborted: false, handled: false };
  }
  ...
  if (targetKey) {
    ...
    return { aborted, handled: true, stoppedSubagents: stopped };
  }
  ...
  const { stopped } = stopSubagentsForRequester({ cfg, requesterSessionKey });
  return { aborted: false, handled: true, stoppedSubagents: stopped };
}
