...
export interface ModelDirectiveSelection {
  provider: string;
  model: string;
  isDefault: boolean;
  alias?: string;
...
function scoreFuzzyMatch(params: {
  ...
  let score = 0;
  score += scoreFragment(haystack, { exact: 220, includes: 110, starts: 140 });
...
export async function createModelSelectionState(params: {
  ...
  if (storedOverride?.model && !skipStoredOverride) {
    ...
    if (allowedModelKeys.size === 0 || allowedModelKeys.has(key)) {
      ({ provider } = normalizedStoredOverride);
      ({ model } = normalizedStoredOverride);
    }
  }
...
export function resolveModelDirectiveSelection(params: {
  ...
  const resolveFuzzy = (params: {
    ...
    if (!params.provider) {
      const aliasMatches: { provider: string; model: string }[] = [];
  ...
  if (rawLower.includes("/")) {
    ...
    const fuzzy = resolveFuzzy({ fragment, provider });
    if (fuzzy.selection || fuzzy.error) {
      return fuzzy;
    }
  }
