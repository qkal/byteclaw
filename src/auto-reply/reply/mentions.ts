...
import { type ConfigRegexRejectReason, compileConfigRegexes } from "../../security/config-regex.js";
import {
...
function normalizeMentionPattern(pattern: string): string {
  ...
  return pattern.split(BACKSPACE_CHAR).join(String.raw`\b`);
}
...
export interface ExplicitMentionSignal {
...
export function stripMentions(
  ...
  const providerRegexes =
    providerMentions?.stripRegexes?.({ agentId, cfg, ctx }) ??
    compileMentionPatternsCached({
      cache: mentionStripRegexCompileCache,
      flags: "gi",
      patterns: normalizeMentionPatterns(
        providerMentions?.stripPatterns?.({ agentId, cfg, ctx }) ?? [],
      ),
      warnRejected: false,
    });
