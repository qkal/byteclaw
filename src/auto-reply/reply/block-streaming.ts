...
import { type TextChunkProvider, resolveChunkMode, resolveTextChunkLimit } from "../chunk.js";
...
function resolveProviderChunkContext(
  ...
  return { providerId, providerKey, textLimit };
}

interface ProviderBlockStreamingConfig {
...
export interface BlockStreamingCoalescing {
  minChars: number;
  maxChars: number;
  idleMs: number;
  joiner: string;
...
export interface BlockStreamingChunking {
  minChars: number;
  maxChars: number;
