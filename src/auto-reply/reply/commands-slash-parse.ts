...
export function parseSlashCommandActionArgs(raw: string, slash: string): SlashCommandParseResult {
  ...
  return { action, args, kind: "parsed" };
}
...
export function parseSlashCommandOrNull(
  ...
  if (parsed.kind === "invalid") {
    return { message: opts.invalidMessage, ok: false };
  }
  if (parsed.kind === "empty") {
    return { action: opts.defaultAction ?? "show", args: "", ok: true };
  }
  return { action: parsed.action, args: parsed.args, ok: true };
}
