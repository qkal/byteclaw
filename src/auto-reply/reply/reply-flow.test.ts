...
describe("createReplyDispatcher", () => {
  ...
  it("uses custom bounds for humanDelay and clamps when max <= min", async () => {
    ...
    const dispatcher = createReplyDispatcher({
      deliver,
      humanDelay: { maxMs: 400, minMs: 1200, mode: "custom" },
    });
...
describe("createReplyToModeFilter", () => {
  ...
  it("keeps only the first replyToId when mode is first", () => {
    ...
    expect(filter({ replyToId: "1", text: "hi" }).replyToId).toBe("1");
    expect(filter({ replyToId: "1", text: "next" }).replyToId).toBeUndefined();
  });
});
