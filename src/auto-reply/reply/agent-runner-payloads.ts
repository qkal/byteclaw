...
import { type BlockReplyPipeline, createBlockReplyContentKey } from "./block-reply-pipeline.js";
import {
...
async function normalizeReplyPayloadMedia(params: {
  ...
  try {
    ...
    logVerbose(`reply payload media normalization failed: ${String(error)}`);
    return {
      ...params.payload,
      audioAsVoice: false,
      mediaUrl: undefined,
...
async function normalizeSentMediaUrlsForDedupe(params: {
  ...
  for (const raw of params.sentMediaUrls) {
    ...
    try {
      ...
      logVerbose(`messaging tool sent-media normalization failed: ${String(error)}`);
    }
  }
