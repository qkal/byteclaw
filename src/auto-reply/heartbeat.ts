...
function stripTokenAtEdges(raw: string): { text: string; didStrip: boolean } {
  ...
  return { didStrip, text: collapsed };
}
...
export function stripHeartbeatToken(
  ...
  if (!raw) {
    return { didStrip: false, shouldSkip: true, text: "" };
  }
  ...
  if (!hasToken) {
    return { didStrip: false, shouldSkip: false, text: trimmed };
  }
  ...
  if (!picked.text) {
    return { didStrip: true, shouldSkip: true, text: "" };
  }
  ...
  return { didStrip: true, shouldSkip: false, text: rest };
}
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.startsWith("- name:")) {
      ...
      if (name && interval && prompt) {
        tasks.push({ interval, name, prompt });
      }
    }
  }
