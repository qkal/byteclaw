...
const modelCatalogMocks = getSharedMocks("openclaw.trigger-handling.model-catalog-mocks", () => ({
  loadModelCatalog: vi.fn().mockResolvedValue([
    ...
    { id: "gpt-5.4", name: "GPT-5.2 (Codex)", provider: "openai-codex" },
...
afterAll(async () => {
  ...
  try {
    rmSync(suiteTempHomeRoot, { force: true, recursive: true });
  } catch {
...
export async function readSessionStore(cfg: {
  ...
  const storeRaw = await fs.readFile(requireSessionStorePath(cfg), "utf8");
...
export function createBlockReplyCollector() {
  const blockReplies: { text?: string }[] = [];
  return {
    blockReplies,
    handlers: {
