...
describe("commands registry", () => {
  ...
  it("filters commands based on config flags", () => {
    const disabled = listChatCommandsForConfig({
      commands: { config: false, debug: false, plugins: false },
    });
    ...
    const enabled = listChatCommandsForConfig({
      commands: { config: true, debug: true, plugins: true },
    });
    ...
    const nativeDisabled = listNativeCommandSpecsForConfig({
      commands: { config: false, debug: false, native: true, plugins: false },
    });
...
describe("commands registry args", () => {
  ...
  it("parses positional args and captureRemaining", () => {
    const command: ChatCommandDefinition = {
      args: [
        { description: "action", name: "action", type: "string" },
        { description: "path", name: "path", type: "string" },
        { captureRemaining: true, description: "value", name: "value", type: "string" },
      ],
  ...
  it("serializes args via raw first, then values", () => {
    const command: ChatCommandDefinition = {
      args: [{ captureRemaining: true, description: "model", name: "model", type: "string" }],
      argsParsing: "positional",
      description: "model",
      key: "model",
      scope: "both",
  ...
  it("resolves auto arg menus when missing a choice arg", () => {
    ...
    const menu = resolveCommandArgMenu({ args: undefined, cfg: {} as never, command });
