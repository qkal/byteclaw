...
function resolveMessageText(content: unknown): { text: string; hasNonTextContent: boolean } {
  if (typeof content === "string") {
    return { hasNonTextContent: false, text: content };
  }
  if (!Array.isArray(content)) {
    return { hasNonTextContent: content != null, text: "" };
  }
...
export function isHeartbeatOkResponse(
  ...
  return stripHeartbeatToken(text, { maxAckChars: ackMaxChars, mode: "heartbeat" }).shouldSkip;
}
