  ...
  for (const command of commands) {
    ...
    for (const alias of command.textAliases) {
      ...
      if (!map.has(normalized)) {
        map.set(normalized, { acceptsArgs, canonical, key: command.key });
      }
    }
  }
...
export function resolveCommandArgMenu(params: {
  ...
  const argName =
    argSpec === "auto"
      ? command.args.find((arg) => resolveCommandArgChoices({ arg, cfg, command }).length > 0)?.name
      : argSpec.arg;
  if (!argName) {
    return null;
  }
  ...
  const choices = resolveCommandArgChoices({ arg, cfg, command });
  if (choices.length === 0) {
    return null;
  }
...
export function resolveTextCommand(
  ...
  return { args: args || undefined, command };
}
