...
describe("resolveThinkingDefaultForModel", () => {
  it("uses provider runtime hooks for default thinking levels", () => {
    ...
    expect(resolveThinkingDefaultForModel({ model: "demo-model", provider: "demo" })).toBe(
      "adaptive",
    );
  });
  ...
  it("uses provider-advertised adaptive defaults", () => {
    ...
    expect(
      resolveThinkingDefaultForModel({ model: "claude-opus-4-6", provider: "anthropic" }),
    ).toBe("adaptive");
  });
  ...
  it("uses provider-advertised adaptive defaults for Bedrock aliases", () => {
    ...
    expect(
      resolveThinkingDefaultForModel({ model: "claude-sonnet-4-6", provider: "aws-bedrock" }),
    ).toBe("adaptive");
  });
  ...
  it("defaults reasoning-capable catalog models to low", () => {
    expect(
      resolveThinkingDefaultForModel({
        catalog: [{ id: "gpt-5.4", provider: "openai", reasoning: true }],
        model: "gpt-5.4",
        provider: "openai",
      }),
    ).toBe("low");
  ...
  it("defaults to off when no adaptive or reasoning hint is present", () => {
    expect(
      resolveThinkingDefaultForModel({
        catalog: [{ id: "gpt-4.1-mini", provider: "openai", reasoning: false }],
        model: "gpt-4.1-mini",
        provider: "openai",
      }),
    ).toBe("off");
