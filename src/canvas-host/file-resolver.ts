...
import { SafeOpenError, type SafeOpenResult, openFileWithinRoot } from "../infra/fs-safe.js";
...
export async function resolveFileWithinRoot(
  ...
  const tryOpen = async (relative: string) => {
    try {
      return await openFileWithinRoot({ relativePath: relative, rootDir: rootReal });
    } catch (error) {
