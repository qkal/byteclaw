...
export interface PairingSetupPayload {
  url: string;
  bootstrapToken: string;
}

export interface PairingSetupCommandResult {
  code: number | null;
  stdout: string;
  stderr?: string;
}
...
export interface ResolvePairingSetupOptions {
...
interface ResolveAuthLabelResult {
...
async function resolveGatewayUrl(
  ...
  if (typeof opts.publicUrl === "string" && opts.publicUrl.trim()) {
    ...
    if (url) {
      return { source: "plugins.entries.device-pair.config.publicUrl", url };
    }
  ...
  if (opts.preferRemoteUrl && remoteUrl) {
    return { source: "gateway.remote.url", url: remoteUrl };
  }
  ...
  if (tailscaleMode === "serve" || tailscaleMode === "funnel") {
    ...
    return { source: `gateway.tailscale.mode=${tailscaleMode}`, url: `wss://${host}` };
  }
...
export async function resolvePairingSetupFromConfig(
  ...
  if (authLabel.error) {
    return { error: authLabel.error, ok: false };
  }
  ...
  if (!urlResult.url) {
    return { error: urlResult.error ?? "Gateway URL unavailable.", ok: false };
  }
  const mobilePairingUrlError = validateMobilePairingUrl(urlResult.url, urlResult.source);
  if (mobilePairingUrlError) {
    return { error: mobilePairingUrlError, ok: false };
  }

  if (!authLabel.label) {
    return { error: "Gateway auth is not configured (no token or password).", ok: false };
  }
