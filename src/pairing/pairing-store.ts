...
interface AllowFromReadCacheEntry {
  exists: boolean;
  mtimeMs: number | null;
  size: number | null;
  entries: string[];
...
async function withFileLock<T>(
  ...
  return await withPathLock(filePath, PAIRING_STORE_LOCK_OPTIONS, async () => await fn());
}
...
function pruneExcessRequestsByAccount(reqs: PairingRequest[], maxPending: number) {
  if (maxPending <= 0 || reqs.length <= maxPending) {
    return { removed: false, requests: reqs };
  }
...
async function updateAllowFromStoreEntry(params: {
  ...
  return await withFileLock(
    filePath,
    { allowFrom: [], version: 1 } satisfies AllowFromStore,
    async () => {
      ...
      if (!normalized) {
        return { allowFrom: current, changed: false };
      }
      ...
      return { allowFrom: next, changed: true };
    },
  );
}
...
interface AllowFromStoreEntryUpdateParams {
  channel: PairingChannel;
  entry: string | number;
  accountId?: string;
  env?: NodeJS.ProcessEnv;
...
export async function listChannelPairingRequests(
  ...
  return await withFileLock(
    filePath,
    { requests: [], version: 1 } satisfies PairingStore,
    async () => {
