...
describe("boot-md startup hook integration", () => {
  ...
  it("dispatches gateway:startup through internal hooks and runs BOOT for each configured agent scope", async () => {
    const cfg = {
      agents: {
        list: [
          { default: true, id: "main", workspace: "/ws/main" },
    ...
    expect(runBootOnce).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ agentId: "main", cfg, deps, workspaceDir: mainWorkspaceDir }),
    );
    expect(runBootOnce).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ agentId: "ops", cfg, deps, workspaceDir: opsWorkspaceDir }),
    );
  });
});
