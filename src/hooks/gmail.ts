...
export interface GmailHookOverrides {
  account?: string;
  label?: string;
  topic?: string;
  subscription?: string;
...
export function resolveGmailHookRuntimeConfig(
  ...
  if (!hookToken) {
    return { error: "hooks.token missing (needed for gmail hook)", ok: false };
  }

  const account = overrides.account ?? gmail?.account ?? "";
  if (!account) {
    return { error: "gmail account required", ok: false };
  }

  const topic = overrides.topic ?? gmail?.topic ?? "";
  if (!topic) {
    return { error: "gmail topic required", ok: false };
  }
  ...
  if (!pushToken) {
    return { error: "gmail push token required", ok: false };
  }
