...
import { DEFAULT_GATEWAY_PORT, type OpenClawConfig } from "../config/config.js";
import {
  buildDefaultHookUrl,
  buildTopicPath,
  parseTopicPath,
...
describe("gmail hook config", () => {
  ...
  it("defaults serve path to / when tailscale is enabled", () => {
    ...
    expectResolvedPaths(result, { publicPath: "/gmail-pubsub", servePath: "/" });
  });
  ...
  it("keeps custom public path when serve path is set", () => {
    ...
    expectResolvedPaths(result, { publicPath: "/custom", servePath: "/" });
  });
  ...
  it("keeps serve path when tailscale target is set", () => {
    ...
    expectResolvedPaths(result, { publicPath: "/custom", servePath: "/custom", target });
  });
});
