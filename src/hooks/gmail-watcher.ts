...
async function startGmailWatch(
  ...
  try {
    ...
    log.error(`watch start error: ${String(error)}`);
    return false;
  }
}
...
export async function startGmailWatcher(cfg: OpenClawConfig): Promise<GmailWatcherStartResult> {
  ...
  if (!cfg.hooks?.enabled) {
    return { reason: "hooks not enabled", started: false };
  }

  if (!cfg.hooks?.gmail?.account) {
    return { reason: "no gmail account configured", started: false };
  }

  // Check if gog is available
  const gogAvailable = isGogAvailable();
  if (!gogAvailable) {
    return { reason: "gog binary not found", started: false };
  }

  // Resolve the full runtime config
  const resolved = resolveGmailHookRuntimeConfig(cfg, {});
  if (!resolved.ok) {
    return { reason: resolved.error, started: false };
  }
  ...
  if (runtimeConfig.tailscale.mode !== "off") {
    try {
      ...
      log.error(`tailscale setup failed: ${String(error)}`);
      return {
        reason: `tailscale setup failed: ${String(error)}`,
        started: false,
      };
    }
  }
