...
const loadedHookRegistrations = resolveGlobalSingleton<
  { event: string; handler: InternalHookHandler }[]
...
export async function loadInternalHooks(
  ...
  try {
    ...
    const eligible = hookEntries.filter((entry) => shouldIncludeHook({ config: cfg, entry }));
    ...
    for (const entry of eligible) {
      try {
        ...
        log.error(
          `Failed to load hook ${safeLogValue(entry.hook.name)}: ${safeLogValue(formatErrorMessage(error))}`,
        );
      }
    }
  } catch (error) {
    log.error(`Failed to load directory-based hooks: ${safeLogValue(formatErrorMessage(error))}`);
  }
  ...
  for (const handlerConfig of handlers) {
    try {
      ...
      log.error(
        `Failed to load hook handler from ${safeLogValue(handlerConfig.module)}: ${safeLogValue(formatErrorMessage(error))}`,
      );
    }
  }
