      ...
      .action(async (opts: NodesRpcOpts) => {
        await runNodesCommand("status", async () => {
          ...
          if (opts.json) {
            ...
            defaultRuntime.writeJson({ ...obj, nodes: filtered, ts });
            return;
          }
          ...
          defaultRuntime.log(
            renderTable({
              columns: [
                ...
                { flex: true, header: "Detail", key: "Detail", minWidth: 18 },
                { header: "Status", key: "Status", minWidth: 18 },
                { flex: true, header: "Caps", key: "Caps", minWidth: 12 },
              ],
              rows,
              width: tableWidth,
            }).trimEnd(),
    ...
    nodes
      ...
      .action(async (opts: NodesRpcOpts) => {
        await runNodesCommand("list", async () => {
          ...
          if (opts.json) {
            defaultRuntime.writeJson({ paired: filteredPaired, pending: pendingRows });
            return;
          }
            ...
            defaultRuntime.log(
              renderTable({
                columns: [
                  ...
                  { header: "ID", key: "Id", minWidth: 10 },
                  { header: "IP", key: "IP", minWidth: 10 },
                  { header: "Last Connect", key: "LastConnect", minWidth: 14 },
                ],
                rows: pairedRows,
                width: tableWidth,
              }).trimEnd(),
