...
const maybeRunCliInContainerMock = vi.hoisted(() =>
  ...
  >((argv: string[]) => ({ argv, handled: false })),
);
...
vi.mock("./container-target.js", () => ({
  ...
  parseCliContainerArgs: (argv: string[]) => ({ argv, container: null, ok: true }),
}));
...
describe("runCli exit behavior", () => {
  ...
  it("returns after a handled container-target invocation", async () => {
    maybeRunCliInContainerMock.mockReturnValueOnce({ exitCode: 0, handled: true });
