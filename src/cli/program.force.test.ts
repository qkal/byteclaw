...
describe("gateway --force helpers", () => {
  ...
  it("returns empty list when lsof finds nothing", () => {
    ...
    expect(listPortListeners(18_789)).toEqual([]);
  });
  ...
  it("throws when lsof missing", () => {
    ...
    expect(() => listPortListeners(18_789)).toThrow(/lsof not found/);
  });
  ...
  it("kills each listener and returns metadata", () => {
    ...
    const killed = forceFreePort(18_789);
  ...
  it("retries until the port is free", async () => {
    ...
    const promise = forceFreePortAndWait(18_789, {
      intervalMs: 100,
    ...
    expect(killMock).toHaveBeenCalledWith(42, "SIGTERM");
    expect(res.killed).toEqual<PortProcess[]>([{ command: "node", pid: 42 }]);
  ...
  it("falls back to fuser when lsof is permission denied", async () => {
    ...
    const result = await forceFreePortAndWait(18_789, { intervalMs: 100, timeoutMs: 500 });
  ...
  it("throws when lsof is unavailable and fuser is missing", async () => {
    ...
    await expect(forceFreePortAndWait(18_789, { intervalMs: 100, timeoutMs: 200 })).rejects.toThrow(
      /fuser not found/i,
    );
  });
});
...
describe("gateway --force helpers (Windows netstat path)", () => {
  ...
  it("parses PIDs from netstat output correctly", () => {
    (execFileSync as unknown as Mock).mockReturnValue(makeNetstatOutput(18_789, 42, 99));
    expect(listPortListeners(18_789)).toEqual<PortProcess[]>([{ pid: 42 }, { pid: 99 }]);
  });
  ...
  it("deduplicates PIDs that appear multiple times", () => {
    (execFileSync as unknown as Mock).mockReturnValue(makeNetstatOutput(18_789, 42, 42));
    expect(listPortListeners(18_789)).toEqual<PortProcess[]>([{ pid: 42 }]);
  });
  ...
  it("throws a descriptive error when netstat fails", () => {
    ...
    expect(() => listPortListeners(18_789)).toThrow(/netstat failed/);
  });
