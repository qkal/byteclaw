...
function setSnapshot(resolved: OpenClawConfig, config: OpenClawConfig) {
  mockReadConfigFileSnapshot.mockResolvedValueOnce(buildSnapshot({ config, resolved }));
}
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      expect(written.gateway?.port).toBe(18_789);
      ...
      const resolved: OpenClawConfig = {
        gateway: {
          auth: {
            ...
            token: "token-drop", // Pragma: allowlist secret
          },
        },
      };
      setSnapshot(resolved, resolved);
      ...
      const resolved: OpenClawConfig = {
        gateway: {
          auth: {
            ...
            token: "token-keep", // Pragma: allowlist secret
          },
        },
      };
      setSnapshot(resolved, resolved);
  ...
  describe("config validate", () => {
    ...
    it("returns machine-readable JSON with --json for invalid config", async () => {
      setSnapshotOnce(
        makeInvalidSnapshot({
          issues: [{ message: "Invalid enum value", path: "gateway.bind" }],
        }),
      );
      ...
      expect(payload.issues).toEqual([{ message: "Invalid enum value", path: "gateway.bind" }]);
      expect(mockError).not.toHaveBeenCalled();
    });
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: OpenClawConfig = { gateway: { port: 18_789 } };
      setSnapshot(resolved, resolved);
  ...
  describe("config set builders and dry-run", () => {
    ...
    it("does not duplicate policy errors in --dry-run --json mode for parent-object writes", async () => {
      ...
      const payload = JSON.parse(String(raw)) as {
        ...
        errors?: { kind: string; message: string; ref?: string }[];
      };
    ...
    it("fails dry-run when provider updates make existing refs unresolvable", async () => {
      const resolved: OpenClawConfig = {
        ...
        secrets: {
          providers: {
            vaultfile: { mode: "json", path: "/tmp/secrets.json", source: "file" },
          },
        },
        tools: {
          web: {
  ...
  describe("config file", () => {
    ...
    it("handles config file path with home directory", async () => {
      const resolved: OpenClawConfig = { gateway: { port: 18_789 } };
      const snapshot = buildSnapshot({ config: resolved, resolved });
