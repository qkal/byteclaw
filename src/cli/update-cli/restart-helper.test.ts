...
describe("restart-helper", () => {
  ...
  async function prepareAndReadScript(env: Record<string, string>, gatewayPort = 18_789) {
    const scriptPath = await prepareRestartScript(env, gatewayPort);
    expect(scriptPath).toBeTruthy();
    const content = await fs.readFile(scriptPath!, "utf8");
    return { content, scriptPath: scriptPath! };
  }
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 18_789) {
  ...
  describe("prepareRestartScript", () => {
    ...
    it("escapes single quotes in profile names for shell scripts", async () => {
      ...
      expect(content).toContain(String.raw`it'\''s`);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      expect(content).toContain(String.raw`ai.openclaw.it'\''s-a-test.plist`);
      await cleanupScript(scriptPath);
    });
  ...
  describe("runRestartScript", () => {
    ...
    it("uses cmd.exe on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      const scriptPath = String.raw`C:\Temp\fake-script.bat`;
      const mockChild = { unref: vi.fn() };
      vi.mocked(spawn).mockReturnValue(mockChild as unknown as ChildProcess);
    ...
    it("quotes cmd.exe /c paths with metacharacters on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      const scriptPath = String.raw`C:\Temp\me&(ow)\fake-script.bat`;
      const mockChild = { unref: vi.fn() };
      vi.mocked(spawn).mockReturnValue(mockChild as unknown as ChildProcess);
