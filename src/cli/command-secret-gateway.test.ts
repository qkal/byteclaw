...
describe("resolveCommandSecretRefsViaGateway", () => {
  function makeTalkProviderApiKeySecretRefConfig(envKey: string): OpenClawConfig {
    return buildTalkTestProviderConfig({ id: envKey, provider: "default", source: "env" });
  }
        ...
        list: [
          {
            ...
            memorySearch: {
              enabled: false,
              remote: {
                apiKey: { id: "AGENT_MEMORY_API_KEY", provider: "default", source: "env" },
              },
            },
          },
        ],
        ...
        [
          {
            ...
            path: "channels.discord.accounts.ops.token",
            pathSegments: ["channels", "discord", "accounts", "ops", "token"],
            value: { id: "DISCORD_OPS_TOKEN", provider: "default", source: "env" },
          },
          {
            entry: { expectedResolvedValue: "string" },
            path: "channels.discord.accounts.chat.token",
            pathSegments: ["channels", "discord", "accounts", "chat", "token"],
            value: { id: "DISCORD_CHAT_TOKEN", provider: "default", source: "env" },
          },
        ] as never,
    });
    callGateway.mockResolvedValueOnce({
        ...
        config: {
          channels: {
            discord: {
              accounts: {
                chat: {
                  token: { id: "DISCORD_CHAT_TOKEN", provider: "default", source: "env" },
                },
                ops: {
                  token: { id: "DISCORD_OPS_TOKEN", provider: "default", source: "env" },
                },
              },
            },
          },
            ...
            plugins: {
              entries: {
                google: {
                  config: {
                    webSearch: {
                      apiKey: { id: envKey, provider: "default", source: "env" },
                    },
                  },
                },
              },
  ...
  it("fails when configured refs remain unresolved after gateway assignments are applied", async () => {
    ...
    await withEnvValue(envKey, undefined, async () => {
      await expect(resolveTalkProviderApiKey({ envKey })).rejects.toThrow(
        new RegExp(
          `${TALK_TEST_PROVIDER_API_KEY_PATH.replaceAll(".", String.raw`\.`)} is unresolved in the active runtime snapshot`,
          "i",
        ),
      );
    });
        ...
        list: [
          {
            ...
            memorySearch: {
              remote: {
                apiKey: { id: "MISSING_MEMORY_API_KEY", provider: "default", source: "env" },
              },
            },
          },
        ],
    ...
    try {
      const result = await resolveCommandSecretRefsViaGateway({
        ...
        config: {
          ...
          gateway: {
            auth: {
              password: { id: gatewayEnvKey, provider: "default", source: "env" },
            },
          },
        } as unknown as OpenClawConfig,
        mode: "read_only_status",
