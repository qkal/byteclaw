...
interface ResolveCommandSecretsResult {
...
interface GatewaySecretsResolveResult {
  ok?: boolean;
  assignments?: {
    path?: string;
    pathSegments: string[];
...
interface CommandSecretGatewayDeps {
...
function parseGatewaySecretsResolveResult(payload: unknown): {
  assignments: { path?: string; pathSegments: string[]; value: unknown }[];
  diagnostics: string[];
  inactiveRefPaths: string[];
} {
...
export async function resolveCommandSecretRefsViaGateway(params: {
  ...
  try {
    ...
    try {
      ...
      return {
        diagnostics: dedupeDiagnostics([
          ...
          `${params.commandName}: gateway secrets.resolve unavailable (${formatErrorMessage(error)}); ${fallbackMessage}`,
        ]),
    ...
    if (isUnsupportedSecretsResolveError(error)) {
      throw new Error(
        `${params.commandName}: active gateway does not support secrets.resolve (${formatErrorMessage(error)}). Update the gateway or run without SecretRefs.`,
        { cause: error },
      );
    }
    throw new Error(
      `${params.commandName}: failed to resolve secrets from the active gateway snapshot (${formatErrorMessage(error)}). Start the gateway and retry.`,
      { cause: error },
    );
  }
  ...
  for (const assignment of parsed.assignments) {
    ...
    try {
      ...
      throw new Error(
        `${params.commandName}: failed to apply resolved secret assignment at ${path} (${formatErrorMessage(error)}).`,
        { cause: error },
      );
    }
  }
