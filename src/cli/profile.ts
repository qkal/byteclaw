...
export function parseCliProfileArgs(argv: string[]): CliProfileParseResult {
  if (argv.length < 2) {
    return { argv, ok: true, profile: null };
  }
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--dev") {
      ...
      if (profile && profile !== "dev") {
        return { error: "Cannot combine --dev with --profile", ok: false };
      }
      sawDev = true;
      profile = "dev";
      continue;
    ...
    if (arg === "--profile" || arg.startsWith("--profile=")) {
      ...
      if (!value) {
        return { error: "--profile requires a value", ok: false };
      }
  ...
  return { argv: out, ok: true, profile };
}
