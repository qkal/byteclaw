...
export interface NamedCommandDescriptor {
  name: string;
  description: string;
  hasSubcommands: boolean;
}

export interface CommandGroupDescriptorSpec<TRegister> {
  commandNames: readonly string[];
  register: TRegister;
}

export interface ImportedCommandGroupDefinition<TRegisterArgs, TModule> {
  commandNames: readonly string[];
  loadModule: () => Promise<TModule>;
  register: (module: TModule, args: TRegisterArgs) => Promise<void> | void;
}

export interface ResolvedCommandGroupEntry<TDescriptor extends NamedCommandDescriptor, TRegister> {
...
interface AnyImportedProgramCommandGroupDefinition {
  commandNames: readonly string[];
  loadModule: () => Promise<Record<string, unknown>>;
  exportName: string;
}

export interface ImportedProgramCommandGroupDefinition<
