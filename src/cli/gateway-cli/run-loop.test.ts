...
function addedSignalListener(
  ...
  const listeners = process.listeners(signal) as ((...args: unknown[]) => void)[];
...
async function createSignaledLoopHarness(exitCallOrder?: string[]) {
  ...
  const { loopPromise } = await runLoopWithStart({ runtime, start });
  await waitForStart(started);
  return { close, exited, loopPromise, runtime, start };
}
...
describe("runGatewayLoop", () => {
  ...
  it("restarts after SIGUSR1 even when drain times out, and resets lanes for the new iteration", async () => {
    ...
    await withIsolatedSignals(async ({ captureSignal }) => {
      ...
      expect(waitForActiveTasks).toHaveBeenCalledWith(1234);
      expect(waitForActiveEmbeddedRuns).toHaveBeenCalledWith(1234);
  ...
  it("forwards lockPort to initial and restart lock acquisitions", async () => {
    ...
    await withIsolatedSignals(async ({ captureSignal }) => {
      ...
      await new Promise<void>((resolve) => setImmediate(resolve));
      expect(acquireGatewayLock).toHaveBeenNthCalledWith(1, { port: 18_789 });
      expect(acquireGatewayLock).toHaveBeenNthCalledWith(2, { port: 18_789 });
      expect(acquireGatewayLock).toHaveBeenNthCalledWith(3, { port: 18_789 });
...
describe("gateway discover routing helpers", () => {
  ...
  it("prefers resolved service port over TXT gatewayPort", () => {
    ...
    expect(pickGatewayPort(beacon)).toBe(18_789);
  });
