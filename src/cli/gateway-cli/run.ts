...
async function runGatewayCommand(opts: GatewayRunOpts) {
  ...
  const { startGatewayServer } = await withProgress(
    { indeterminate: true, label: "Loading gateway modules…" },
  ...
  if (opts.force) {
    try {
      ...
      defaultRuntime.error(`Force: ${String(error)}`);
      defaultRuntime.exit(1);
      return;
    }
  }
  ...
  try {
    ...
    while (true) {
      try {
        ...
        const isGatewayAlreadyRunning =
          ...
          error.message.includes("gateway already running");
    ...
    if (isGatewayLockError(error)) {
      ...
      defaultRuntime.exit(isHealthyGatewayLockError(error) ? 0 : 1);
      return;
    }
    defaultRuntime.error(`Gateway failed to start: ${String(error)}`);
    defaultRuntime.exit(1);
  }
}
