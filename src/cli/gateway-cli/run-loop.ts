...
export async function runGatewayLoop(params: {
  ...
  const reacquireLockForInProcessRestart = async (): Promise<boolean> => {
    try {
      ...
      gatewayLog.error(`failed to reacquire gateway lock for in-process restart: ${String(error)}`);
      exitProcess(1);
      return false;
    }
  };
  ...
  const SHUTDOWN_TIMEOUT_MS = SUPERVISOR_STOP_TIMEOUT_MS - 5000;
  ...
  const request = (action: GatewayRunSignalAction, signal: string) => {
    ...
    void (async () => {
      try {
        ...
        gatewayLog.error(`shutdown error: ${String(error)}`);
      } finally {
