...
import { type OpenClawConfig, readBestEffortConfig } from "../config/config.js";
...
interface EffectivePolicyReport {
...
async function loadSnapshotTarget(opts: ExecApprovalsCliOpts): Promise<{
  ...
  if (!opts.gateway && !opts.node) {
    return { nodeId: null, snapshot: loadSnapshotLocal(), source: "local" };
  }
  const nodeId = await resolveTargetNodeId(opts);
  const snapshot = await loadSnapshot(opts, nodeId);
  return { nodeId, snapshot, source: nodeId ? "node" : "gateway" };
}
...
async function loadWritableSnapshotTarget(opts: ExecApprovalsCliOpts): Promise<{
  ...
  return { baseHash, nodeId, snapshot, source, targetLabel };
}
...
function renderEffectivePolicy(params: { report: EffectivePolicyReport }) {
  ...
  defaultRuntime.log(
    renderTable({
      columns: [
        { header: "Scope", key: "Scope", minWidth: 12 },
        { flex: true, header: "Requested", key: "Requested", minWidth: 24 },
        { flex: true, header: "Host", key: "Host", minWidth: 24 },
        { header: "Effective", key: "Effective", minWidth: 16 },
        { flex: true, header: "Notes", key: "Notes", minWidth: 20 },
      ],
      rows,
...
function renderApprovalsSnapshot(snapshot: ExecApprovalsSnapshot, targetLabel: string) {
  ...
  defaultRuntime.log(
    renderTable({
      columns: [
        { header: "Field", key: "Field", minWidth: 8 },
  ...
  defaultRuntime.log(
    renderTable({
      columns: [
        { header: "Target", key: "Target", minWidth: 10 },
        { header: "Agent", key: "Agent", minWidth: 8 },
        { flex: true, header: "Pattern", key: "Pattern", minWidth: 20 },
        { header: "Last Used", key: "LastUsed", minWidth: 10 },
      ],
      rows: allowlistRows,
      width: tableWidth,
    }).trimEnd(),
...
async function saveSnapshot(
  ...
  const params = nodeId ? { baseHash, file, nodeId } : { baseHash, file };
  const snapshot = (await callGatewayFromCli(method, opts, params)) as ExecApprovalsSnapshot;
  return snapshot;
}
...
async function loadWritableAllowlistAgent(opts: ExecApprovalsCliOpts): Promise<{
  ...
  return { agent, agentKey, allowlistEntries, baseHash, file, nodeId, source, targetLabel };
}
...
async function runAllowlistMutation(
  ...
  try {
    ...
    defaultRuntime.error(formatCliError(error));
    defaultRuntime.exit(1);
  }
}
...
export function registerExecApprovalsCli(program: Command) {
    ...
    .action(async (opts: ExecApprovalsCliOpts) => {
      try {
        ...
        try {
          ...
          exitWithError(`Failed to parse approvals JSON: ${String(error)}`);
        }
        file.version = 1;
        await saveSnapshotTargeted({ baseHash, file, nodeId, opts, source, targetLabel });
      } catch (error) {
        defaultRuntime.error(formatCliError(error));
        defaultRuntime.exit(1);
      }
  ...
  registerAllowlistMutationCommand({
    ...
    mutate: ({ trimmedPattern, file, agent, agentKey, allowlistEntries }) => {
      ...
      allowlistEntries.push({ lastUsedAt: Date.now(), pattern: trimmedPattern });
