...
const onSpecificDates =
  (dates: [number, number, number][], durationDays = 1): HolidayRule =>
  (date) => {
    const parts = utcParts(date);
...
const HOLIDAY_RULES = new Map<string, HolidayRule>([
  ...
  [
    HOLIDAY_TAGLINES.hanukkah,
    inYearWindow([
      { day: 15, duration: 8, month: 11, year: 2025 },
      { day: 5, duration: 8, month: 11, year: 2026 },
      { day: 25, duration: 8, month: 11, year: 2027 },
    ]),
  ],
