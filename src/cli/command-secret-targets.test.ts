...
vi.mock("../secrets/target-registry.js", () => ({
  discoverConfigSecretTargetsByIds: vi.fn((config: unknown, targetIds?: Iterable<string>) => {
    ...
    const out: { path: string; pathSegments: string[] }[] = [];
      ...
      config: {
        channels: {
          discord: {
            accounts: {
              ops: {
                token: { id: "DISCORD_OPS", provider: "default", source: "env" },
              },
            },
            defaultAccount: "ops",
          },
      ...
      config: {
        channels: {
          discord: {
            accounts: {
              chat: {
                token: { id: "DISCORD_CHAT", provider: "default", source: "env" },
              },
              ops: {
                token: { id: "DISCORD_OPS", provider: "default", source: "env" },
              },
            },
            token: { id: "DISCORD_DEFAULT", provider: "default", source: "env" },
          },
        },
      } as never,
    });
