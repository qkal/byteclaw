...
export interface PluginInstallRequestContext {
  rawSpec: string;
...
function resolvePluginInstallArgvRequest(commandPath: string[], argv: string[]) {
  ...
  return rawSpec ? { marketplace, rawSpec } : null;
}
...
export function resolvePluginInstallPreactionRequest(params: {
  ...
  const request = resolvePluginInstallRequestContext({ marketplace, rawSpec });
