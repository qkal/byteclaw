...
async function notifyApproved(channel: PairingChannel, id: string) {
  ...
  await notifyPairingApproved({ cfg, channelId: channel, id });
}
...
export function registerPairingCli(program: Command) {
  ...
  pairing
    ...
    .action(async (channelArg, opts) => {
      ...
      defaultRuntime.log(
        renderTable({
          columns: [
            { header: "Code", key: "Code", minWidth: 10 },
            { flex: true, header: idLabel, key: "ID", minWidth: 12 },
            { flex: true, header: "Meta", key: "Meta", minWidth: 8 },
  ...
  pairing
    ...
    .action(async (codeOrChannel, code, opts) => {
      ...
      await notifyApproved(channel, approved.id).catch((error) => {
        defaultRuntime.log(theme.warn(`Failed to notify requester: ${String(error)}`));
      });
    });
}
