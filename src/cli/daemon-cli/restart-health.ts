...
export interface GatewayPortHealthSnapshot {
  portUsage: PortUsage;
  healthy: boolean;
}
...
async function confirmGatewayReachable(port: number): Promise<boolean> {
  ...
  const probe = await probeGateway({
    auth: token || password ? { password, token } : undefined,
...
export async function inspectGatewayRestart(params: {
  ...
  try {
    ...
    runtime = { detail: String(error), status: "unknown" };
  }
...
function withWaitContext(
  ...
  return { ...snapshot, elapsedMs, waitOutcome };
}
