...
interface ResolvedGatewayStatus {
...
export interface DaemonStatus {
  ...
  extraServices: { label: string; detail: string; scope: string }[];
}
...
async function loadDaemonConfigContext(
  ...
  const cliIO = createConfigIO({ configPath: cliConfigPath, env: process.env });
...
export async function gatherDaemonStatus(
  ...
  const configAudit = command
    ...
    : { issues: [] satisfies ServiceConfigAudit["issues"], ok: true };
  const {
    mergedDaemonEnv,
    cliCfg,
    daemonCfg,
