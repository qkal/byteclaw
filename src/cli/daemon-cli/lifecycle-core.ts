...
interface DaemonLifecycleOptions {
  json?: boolean;
}
...
interface ServiceRecoveryResult {
...
interface ServiceRecoveryContext {
  json: boolean;
  stdout: Writable;
  fail: (message: string, hints?: string[]) => void;
}
...
async function maybeAugmentSystemdHints(hints: string[]): Promise<string[]> {
  ...
  return [
    ...hints,
    ...renderSystemdUnavailableHints({ kind: "generic_unavailable", wsl: await isWSL() }),
  ];
}
...
async function resolveServiceLoadedOrFail(params: {
  ...
  try {
    ...
    params.fail(`${params.serviceNoun} service check failed: ${String(error)}`);
    return null;
  }
}
...
export async function runServiceUninstall(params: {
  ...
  try {
    ...
    fail(`${params.serviceNoun} uninstall failed: ${String(error)}`);
    return;
  }
...
export async function runServiceStart(params: {
  ...
  if (!loaded) {
    try {
      const handled = await params.onNotLoaded?.({ fail, json, stdout });
      if (handled) {
        emit({
      ...
      fail(`${params.serviceNoun} start failed: ${String(error)}`, hints);
      return;
    }
  }
  try {
...
export async function runServiceStop(params: {
  ...
  if (!loaded) {
    try {
      ...
      fail(`${params.serviceNoun} stop failed: ${String(error)}`);
      return;
    }
    emit({
      message: `${params.serviceNoun} service ${params.service.notLoadedText}.`,
...
export async function runServiceRestart(params: {
  ...
  if (!loaded) {
    try {
      handledRecovery = (await params.onNotLoaded?.({ fail, json, stdout })) ?? null;
    } catch (error) {
      fail(`${params.serviceNoun} restart failed: ${String(error)}`);
      return false;
    }
    if (!handledRecovery) {
      await handleServiceNotLoaded({
  ...
  if (loaded && params.checkTokenDrift) {
    ...
    try {
      ...
      const driftIssue = checkTokenDrift({ configToken, serviceToken });
      if (driftIssue) {
  ...
  try {
    ...
    if (params.postRestartCheck) {
      const postRestartResult = await params.postRestartCheck({ fail, json, stdout, warnings });
    ...
    const hints = params.renderStartHints();
    fail(`${params.serviceNoun} restart failed: ${String(error)}`, hints);
    return false;
  }
}
