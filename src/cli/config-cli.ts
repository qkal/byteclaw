...
interface ConfigSetParseOpts {
  strictJson?: boolean;
}
type ConfigSetInputMode = ConfigSetDryRunInputMode;
interface ConfigSetOperation {
...
function parseValue(raw: string, opts: ConfigSetParseOpts): unknown {
  ...
  if (opts.strictJson) {
    try {
      ...
      throw new Error(`Failed to parse JSON value: ${String(error)}`, { cause: error });
    }
  }
...
export async function runConfigSet(opts: {
  ...
  try {
    ...
    if (
      ...
      error instanceof ConfigSetDryRunValidationError
    ) {
      writeRuntimeJson(runtime, error.result);
      runtime.exit(1);
      return;
    }
    runtime.error(danger(String(error)));
    runtime.exit(1);
  }
}
...
export async function runConfigSchema(opts: { runtime?: RuntimeEnv } = {}) {
  ...
  try {
    ...
    runtime.error(danger(`Config schema error: ${String(error)}`));
    runtime.exit(1);
  }
}
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  ...
  try {
    ...
    if (!snapshot.exists) {
      if (opts.json) {
        writeRuntimeJson(runtime, { error: "file not found", path: outputPath, valid: false }, 0);
      } else {
    ...
    if (!snapshot.valid) {
      ...
      if (opts.json) {
        writeRuntimeJson(runtime, { issues, path: outputPath, valid: false });
      } else {
    ...
    if (opts.json) {
      writeRuntimeJson(runtime, { path: outputPath, valid: true }, 0);
    } else {
      runtime.log(success(`Config valid: ${shortPath}`));
    }
  } catch (error) {
    if (opts.json) {
      writeRuntimeJson(runtime, { error: String(error), path: outputPath, valid: false }, 0);
    } else {
      runtime.error(danger(`Config validation error: ${String(error)}`));
    }
    runtime.exit(1);
  }
}
...
export function registerConfigCli(program: Command) {
  ...
  cmd
    ...
    .action(async (path: string, opts) => {
      await runConfigGet({ json: Boolean(opts.json), path });
    });
