...
export function formatHooksList(report: HookStatusReport, opts: HooksListOptions): string {
  ...
  const columns = [
    ...
    { flex: true, header: "Hook", key: "Hook", minWidth: 18 },
    { flex: true, header: "Description", key: "Description", minWidth: 24 },
    { flex: true, header: "Source", key: "Source", minWidth: 12 },
  ];
  if (opts.verbose) {
    columns.push({ flex: true, header: "Missing", key: "Missing", minWidth: 18 });
  }
...
export async function disableHook(hookName: string): Promise<void> {
  ...
  const nextConfig = buildConfigWithHookEnabled({ config, enabled: false, hookName });
