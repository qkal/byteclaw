...
function killPortWithFuser(port: number, signal: "SIGTERM" | "SIGKILL"): PortProcess[] {
  ...
  try {
    ...
    if (code === "EACCES" || code === "EPERM") {
      throw withErrnoCode("fuser permission denied while forcing gateway port", code, error);
    }
...
export function listPortListeners(port: number): PortProcess[] {
  if (process.platform === "win32") {
    try {
      ...
      throw new Error(`netstat failed: ${String(error)}`, { cause: error });
    }
  }
  ...
  try {
    ...
    if (code === "EACCES" || code === "EPERM") {
      throw withErrnoCode("lsof permission denied while inspecting gateway port", code, error);
    }
    if (status === 1) {
...
export function forceFreePort(port: number): PortProcess[] {
  ...
  for (const proc of listeners) {
    try {
      ...
      throw new Error(
        `failed to kill pid ${proc.pid}${proc.command ? ` (${proc.command})` : ""}: ${String(error)}`,
        { cause: error },
      );
    }
  }
...
export async function forceFreePortAndWait(
  ...
  try {
    ...
    if (!isRecoverableLsofError(error)) {
      throw error;
    }
  ...
  if (!(await checkBusy())) {
    return { escalatedToSigkill: false, killed, waitedMs: 0 };
  }
  ...
  for (let i = 0; i < triesSigterm; i++) {
    if (!(await checkBusy())) {
      return { escalatedToSigkill: false, killed, waitedMs };
    }
  ...
  for (let i = 0; i < triesSigkill; i++) {
    if (!(await checkBusy())) {
      return { escalatedToSigkill: true, killed, waitedMs };
    }
    await sleep(intervalMs);
    waitedMs += intervalMs;
  }
