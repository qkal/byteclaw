...
function createRemoteQrConfig(params?: { withTailscale?: boolean }) {
  return {
    gateway: {
      ...
      remote: { token: "remote-tok", url: "wss://remote.example.com:444" },
    },
    plugins: {
      entries: {
        "device-pair": {
...
function createLocalGatewayPasswordRefAuth(secretId: string) {
  return {
    mode: "password",
    password: { id: secretId, provider: "default", source: "env" },
  };
}
...
describe("registerQrCli", () => {
  ...
  it("fails when token and password SecretRefs are both configured with inferred mode", async () => {
    ...
    loadConfig.mockReturnValue({
      gateway: {
        auth: {
          password: { id: "MISSING_LOCAL_GATEWAY_PASSWORD", provider: "default", source: "env" },
          token: { id: "QR_INFERRED_GATEWAY_TOKEN", provider: "default", source: "env" },
        },
        bind: "custom",
        customBindHost: "gateway.local",
      },
