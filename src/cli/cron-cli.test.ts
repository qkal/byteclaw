...
describe("cron cli", () => {
  it.each([
    ...
    const { exitSpy } = await runCronRunAndCaptureExit({ enqueued, ran });
  ...
  it.each([
    { expectedBestEffort: true, flag: "--best-effort-deliver" },
    { expectedBestEffort: false, flag: "--no-best-effort-deliver" },
  ...
  it("applies --exact to existing cron job without requiring --cron on edit", async () => {
    const patch = await runCronEditWithScheduleLookup(
      { expr: "0 */2 * * *", kind: "cron", staggerMs: 300_000, tz: "UTC" },
      ["--exact"],
    );
  ...
  it("rejects --exact on edit when existing job is not cron", async () => {
    await expectCronEditWithScheduleLookupExit({ everyMs: 60_000, kind: "every" }, ["--exact"]);
  });
