...
const getApiKeyForModelMock = vi.hoisted(() =>
  vi.fn(async () => ({ apiKey: "test-key", mode: "api-key", source: "test" })),
);
...
describe("applyMediaUnderstanding – echo transcript", () => {
  beforeAll(async () => {
    ...
    ({ applyMediaUnderstanding } = mod);
    const runner = await import("./runner.js");
    ({ clearMediaUnderstandingBinaryCacheForTests } = runner);
  });
  ...
  it("does NOT echo when echoTranscript is false (default)", async () => {
    ...
    await applyMediaUnderstanding({ cfg, ctx, providers });
