...
function resolveEntryRunOptions(params: {
  ...
  const maxBytes = resolveMaxBytes({ capability, cfg, config: params.config, entry });
  const maxChars = resolveMaxChars({ capability, cfg, config: params.config, entry });
  ...
  return { maxBytes, maxChars, prompt, timeoutMs };
}
...
export async function runProviderEntry(params: {
  ...
  if (capability === "audio") {
    ...
    assertMinAudioSize({ attachmentIndex: params.attachmentIndex, size: media.size });
...
export async function runCliEntry(params: {
  ...
  if (capability === "audio") {
    ...
    assertMinAudioSize({ attachmentIndex: params.attachmentIndex, size: stat.size });
  }
