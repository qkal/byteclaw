...
export interface MediaAttachmentCacheOptions {
...
export class MediaAttachmentCache {
  ...
  async getBuffer(params: {
    ...
    try {
      ...
      const fetched = await fetchRemoteMedia({ fetchImpl, maxBytes: params.maxBytes, url });
  ...
  async getPath(params: {
    ...
    if (entry.tempPath) {
      ...
      return { cleanup: entry.tempCleanup, path: entry.tempPath };
    }
    ...
    return { cleanup: entry.tempCleanup, path: tmpPath };
  }
  ...
  private async ensureLocalStat(entry: AttachmentCacheEntry): Promise<number | undefined> {
    ...
    try {
      ...
      entry.resolvedPath = undefined;
      if (shouldLogVerbose()) {
        logVerbose(`Failed to read attachment ${entry.attachment.index + 1}: ${String(error)}`);
      }
      return undefined;
    }
  }
