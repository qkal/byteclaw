...
describe("sessions", () => {
  ...
  async function createSessionStoreFixture(params: {
    ...
    await fs.writeFile(storePath, JSON.stringify(params.entries), "utf8");
    return { storePath };
  }
  ...
  async function createAgentSessionsLayout(label: string): Promise<{
    ...
    await fs.writeFile(mainStorePath, "{}", "utf8");

    const bot2SessionPath = path.join(bot2SessionsDir, "sess-1.jsonl");
    await fs.writeFile(bot2SessionPath, "{}", "utf8");
    ...
    return { bot2SessionPath, mainStorePath, outsidePath, stateDir };
  }
  ...
  const deriveSessionKeyCases = [
    ...
    {
      ctx: { ChatType: "group", From: "12345-678@g.us", Provider: "whatsapp" },
  ...
  const resolveSessionKeyCases = [
    {
      ctx: { ChatType: "group", From: "discord:group:12345" },
  ...
  it("updateSessionStoreEntry re-reads disk inside lock instead of using stale cache", async () => {
    ...
    const externalStore = JSON.parse(await fs.readFile(storePath, "utf8")) as Record<
      string,
    ...
    await fs.writeFile(storePath, JSON.stringify(externalStore), "utf8");
