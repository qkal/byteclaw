...
export interface ObserveRecoveryDeps {
  fs: {
    promises: {
...
function createConfigHealthFingerprint(params: {
  ...
  return {
    hash: params.hash,
    bytes: Buffer.byteLength(params.raw, "utf8"),
...
function readConfigHealthStateSync(deps: ObserveRecoveryDeps): ConfigHealthState {
  try {
    const raw = deps.fs.readFileSync(resolveConfigHealthStatePath(deps.env, deps.homedir), "utf8");
    const parsed = deps.json5.parse(raw);
    return isRecord(parsed) ? (parsed as ConfigHealthState) : {};
  } catch {
    return {};
...
export async function maybeRecoverSuspiciousConfigRead(params: {
  ...
  if (!suspicious.includes("update-channel-only-root")) {
    return { parsed: params.parsed, raw: params.raw };
  }
  ...
  const backupRaw = await params.deps.fs.promises.readFile(backupPath, "utf8").catch(() => null);
  if (!backupRaw) {
    return { parsed: params.parsed, raw: params.raw };
  }
  ...
  return { parsed: backupParsed, raw: backupRaw };
}
...
export function maybeRecoverSuspiciousConfigReadSync(params: {
  ...
  try {
    backupRaw = params.deps.fs.readFileSync(backupPath, "utf8");
  } catch {
    return { parsed: params.parsed, raw: params.raw };
  }
  let backupParsed: unknown;
