...
export interface IncludeFileReadParams {
  includePath: string;
  resolvedPath: string;
  rootRealDir: string;
  ioFs?: typeof fs;
...
class IncludeProcessor {
  ...
  private resolvePath(includePath: string): string {
    ...
    try {
      ...
      if (error instanceof ConfigIncludeError) {
        throw error;
      }
...
export function readConfigIncludeFileWithGuards(params: IncludeFileReadParams): string {
  ...
  if (!canUseBoundaryFileOpen(ioFs)) {
    return ioFs.readFileSync(params.resolvedPath, "utf8");
  }
  ...
  try {
    return ioFs.readFileSync(opened.fd, "utf8");
  } finally {
