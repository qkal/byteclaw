...
function makeStore(entries: [string, SessionEntry][]): Record<string, SessionEntry> {
...
describe("rotateSessionFile", () => {
  ...
  it("file over maxBytes: renamed to .bak.{timestamp}, returns true", async () => {
    ...
    await fs.writeFile(storePath, bigContent, "utf8");
    ...
    const bakContent = await fs.readFile(path.join(testDir, bakFiles[0]), "utf8");
  ...
  it("multiple rotations: only keeps 3 most recent .bak files", async () => {
    ...
    try {
      ...
      for (let i = 0; i < 4; i++) {
        await fs.writeFile(storePath, `data-${i}-${"x".repeat(100)}`, "utf8");
