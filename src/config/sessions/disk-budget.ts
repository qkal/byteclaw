...
export interface SessionDiskBudgetConfig {
  maxDiskBytes: number | null;
  highWaterBytes: number | null;
}

export interface SessionDiskBudgetSweepResult {
...
export interface SessionDiskBudgetLogger {
...
export async function enforceSessionDiskBudget(params: {
  ...
  if (total > highWaterBytes) {
    ...
    for (const key of keys) {
      ...
      const transcriptPath = resolveSessionTranscriptPathForEntry({ entry, sessionsDir });
      if (!transcriptPath) {
        continue;
      }
      const deletedBytes = await removeFileForBudget({
