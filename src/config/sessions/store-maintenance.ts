...
export interface SessionMaintenanceWarning {
...
export interface ResolvedSessionMaintenanceConfig {
...
export function pruneStaleEntries(
  ...
  for (const [key, entry] of Object.entries(store)) {
    if (entry?.updatedAt != null && entry.updatedAt < cutoffMs) {
      opts.onPruned?.({ entry, key });
      delete store[key];
      pruned++;
    }
  }
  if (pruned > 0 && opts.log !== false) {
    log.info("pruned stale session entries", { maxAgeMs, pruned });
  }
  return pruned;
}
...
export function capEntryCount(
  ...
  for (const key of toRemove) {
    ...
    if (entry) {
      opts.onCapped?.({ entry, key });
    }
    delete store[key];
  }
  if (opts.log !== false) {
    log.info("capped session entry count", { maxEntries, removed: toRemove.length });
  }
