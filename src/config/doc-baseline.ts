...
export interface ConfigDocBaselineEntry {
  path: string;
...
export interface ConfigDocBaseline {
...
export interface ConfigDocBaselineKindBaseline {
  generatedBy: "scripts/generate-config-doc-baseline.ts";
  kind: ConfigDocBaselineKind;
  entries: ConfigDocBaselineEntry[];
}

export interface ConfigDocBaselineArtifacts {
  combined: string;
  core: string;
  channel: string;
  plugin: string;
...
export interface ConfigDocBaselineArtifactsRender {
  baseline: ConfigDocBaseline;
  json: ConfigDocBaselineArtifacts;
}

export interface ConfigDocBaselineArtifactPaths {
  combined: string;
  core: string;
  channel: string;
  plugin: string;
...
export interface ConfigDocBaselineArtifactsWriteResult {
  changed: boolean;
  wrote: boolean;
...
function resolveUiHintMatch(
  ...
  if (!index) {
    ...
    for (const [hintPath, hint] of Object.entries(uiHints)) {
      ...
      const entry = { hint, parts, path: hintPath };
      if (bucket) {
        bucket.push(entry);
      } else {
...
export function splitConfigDocBaselineEntries(entries: ConfigDocBaselineEntry[]): {
  ...
  return { channelEntries, coreEntries, pluginEntries };
}
...
export async function writeConfigDocBaselineArtifacts(params?: {
  ...
  for (const key of Object.keys(jsonPaths) as (keyof ConfigDocBaselineArtifacts)[]) {
