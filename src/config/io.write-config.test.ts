...
vi.mock("../plugins/doctor-contract-registry.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    applyPluginDoctorCompatibilityMigrations: () => ({ changes: [], next: null }),
...
describe("config io write", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withSuiteHome(async (home) => {
        ...
        await fs.mkdir(stateDir, { mode: 0o777, recursive: true });
  ...
  it("keeps writes inside an OPENCLAW_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        liveConfigPath,
        `${JSON.stringify({ gateway: { mode: "local", port: 18_789 } }, null, 2)}\n`,
        "utf8",
      );
      ...
      const livePersisted = JSON.parse(await fs.readFile(liveConfigPath, "utf8")) as {
        gateway?: { mode?: unknown; port?: unknown };
      };
      expect(livePersisted.gateway).toEqual({ mode: "local", port: 18_789 });

      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "openclaw.json"), "utf8"),
      ) as {
