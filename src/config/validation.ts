...
interface AllowedValuesCollection {
  values: unknown[];
  incomplete: boolean;
  hasValues: boolean;
}
...
function collectAllowedValuesFromJsonSchemaNode(schema: unknown): AllowedValuesCollection {
  ...
  if (Object.hasOwn(node, "const")) {
    return { hasValues: true, incomplete: false, values: [node.const] };
  }
...
function collectAllowedValuesFromCustomIssue(record: UnknownIssueRecord): AllowedValuesCollection {
  ...
  if (expectedMatch?.[1]) {
    ...
    return { hasValues: values.length > 0, incomplete: false, values };
  }
...
function collectAllowedValuesFromIssueList(
  ...
  return { hasValues, incomplete: false, values: collected };
}
...
function extractBindingsSpecificUnionIssue(
  ...
  return { message: subMessage, path: fullPath };
}
...
export function validateConfigObjectRaw(
  ...
  if (policyIssues.length > 0) {
    return { issues: policyIssues, ok: false };
  }
  ...
  if (avatarIssues.length > 0) {
    return { issues: avatarIssues, ok: false };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validatedConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { issues: gatewayTailscaleBindIssues, ok: false };
  }
  return {
...
function validateConfigObjectWithPluginsBase(
  ...
  if (config.channels && isRecord(config.channels)) {
    for (const key of Object.keys(config.channels)) {
      ...
      const result = validateJsonSchemaValue({
        ...
        value: config.channels[trimmed], // Always apply defaults for AJV schema validation;
  ...
  const validateHeartbeatTarget = (target: string | undefined, path: string) => {
    ...
    if (!trimmed) {
      issues.push({ message: "heartbeat target must not be empty", path });
      return;
    }
    ...
    issues.push({ message: `unknown heartbeat target: ${target}`, path });
  };
  ...
  if (!hasExplicitPluginsConfig) {
    ...
    return { config: mutatedConfig, ok: true, warnings };
  }
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        const res = validateJsonSchemaValue({
          ...
          value: entry?.config ?? {}, // Always apply defaults for AJV schema validation;
          // WriteConfigFile persists persistCandidate, not validated.config (#61841)
        });
        if (!res.ok) {
