...
import { type ConfigUiHints, buildConfigSchema } from "./schema.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact maxTokens-style fields", () => {
    ...
    const result = redactConfigSnapshot(snapshot);
    expect((result.config as Record<string, unknown>).maxTokens).toBe(16_384);
    ...
    expect(providerList[0]?.maxTokens).toBe(65_536);
    expect(providerList[0]?.contextTokens).toBe(200_000);
    ...
    expect(providers.openai.contextTokens).toBe(128_000);
    expect(providers.openai.tokenCount).toBe(500);
    expect(providers.openai.tokenLimit).toBe(100_000);
    expect(providers.openai.tokenBudget).toBe(50_000);
  ...
  it("handles overlap fallback and SecretRef in the same snapshot", () => {
    const config = {
      gateway: { auth: { password: "default" }, mode: "default" }, // Pragma: allowlist secret
      models: {
        providers: {
          default: {
            apiKey: { id: "OPENAI_API_KEY", provider: "default", source: "env" },
  ...
  it.each([
    ...
    {
      assert: (config: Record<string, unknown>) => {
        expect((config.compaction as Record<string, unknown>).softThresholdTokens).toBe(50_000);
      },
      name: "does not redact softThresholdTokens",
      snapshot: makeSnapshot({ compaction: { softThresholdTokens: 50_000 } }),
    },
    {
      assert: (config: Record<string, unknown>) => {
