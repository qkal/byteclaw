...
import { type ConfigUiHints, isSensitiveConfigPath } from "./schema.hints.js";
...
export function restoreRedactedValues(
  ...
  if (incoming === null || incoming === undefined) {
    return { error: "no input", ok: false };
  }
  if (typeof incoming !== "object") {
    return { error: "input not an object", ok: false };
  }
  try {
    ...
    if (error instanceof RedactionError) {
      return {
        humanReadableMessage: error.humanReadableMessage,
        ok: false,
      };
    }
    throw error; // Some coding error, pass through
  }
}
  ...
  for (const [key, value] of Object.entries(incoming as Record<string, unknown>)) {
    ...
    for (const candidate of [path, wildcardPath]) {
      if (lookup.has(candidate)) {
        ...
        if (
          ...
          result[key] = restoreOriginalValueOrThrow({ key, original: orig, path: candidate });
        } else if (typeof value === "object" && value !== null) {
    ...
    if (!matched) {
      ...
      if (
        ...
        result[key] = restoreOriginalValueOrThrow({ key, original: orig, path });
      } else if (typeof value === "object" && value !== null) {
