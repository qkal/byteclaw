...
describe("restoreRedactedValues", () => {
  ...
  it("preserves non-sensitive fields unchanged", () => {
    const incoming = {
      gateway: { auth: { token: REDACTED_SENTINEL }, port: 9999 },
      ui: { seamColor: "#ff0000" },
    };
    const original = {
      gateway: { auth: { token: "real-secret" }, port: 18_789 },
      ui: { seamColor: "#0088cc" },
    };
    const result = restoreRedactedValues(incoming, original) as typeof incoming;
  ...
  it("round-trips config through redact → restore", () => {
    const originalConfig = {
      ...
      gateway: { auth: { token: "gateway-auth-secret-token-value" }, port: 18_789 },
      models: {
        providers: {
          openai: {
  ...
  it("round-trips with uiHints for custom sensitive fields", () => {
    ...
    const originalConfig = {
      custom: { displayName: "My Bot", myApiKey: "secret-custom-api-key-value" },
    };
    const snapshot = makeSnapshot(originalConfig);
