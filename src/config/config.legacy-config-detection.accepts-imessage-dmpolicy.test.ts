...
async function expectLoadRejectionPreservesField(params: {
  ...
  await withTempHome(async (home) => {
    ...
    await fs.writeFile(configPath, JSON.stringify(params.config, null, 2), "utf8");
...
async function withSnapshotForConfig(
  ...
  await withTempHome(async (home) => {
    ...
    const parsed = JSON.parse(await fs.readFile(configPath, "utf8")) as unknown;
    await run({ configPath, parsed, snapshot });
  });
}
...
describe("legacy config detection", () => {
  it('accepts imessage.dmPolicy="open" with allowFrom "*"', async () => {
    const res = validateConfigObject({
      channels: { imessage: { allowFrom: ["*"], dmPolicy: "open" } },
    });
    expect(res.ok).toBe(true);
    if (res.ok) {
  ...
  it.each([
    ...
    expectSchemaConfigValue({ config, expectedValue, readValue, schema });
  });
  ...
  it.each([
    {
      config: {
        channels: { discord: { allowFrom: ["*"], dm: { allowFrom: ["123"], policy: "open" } } },
      },
      name: 'accepts discord dm.allowFrom="*" with top-level allowFrom alias',
    },
    {
      config: {
        channels: { slack: { allowFrom: ["*"], dm: { allowFrom: ["U123"], policy: "open" } } },
      },
        ...
        JSON.stringify(
          {
            auth: {
              profiles: {
                "anthropic:claude-cli": { mode: "token", provider: "anthropic" },
              },
            },
          },
          null,
  ...
  it("rejects bindings[].match.provider on load", async () => {
    await expectLoadRejectionPreservesField({
      ...
      expectedValue: "slack",
      readValue: (parsed) =>
        (parsed as { bindings?: { match?: { provider?: string } }[] }).bindings?.[0]?.match
          ?.provider,
    });
  });
  it("rejects bindings[].match.accountID on load", async () => {
    await expectLoadRejectionPreservesField({
      ...
      expectedValue: "work",
      readValue: (parsed) =>
        (parsed as { bindings?: { match?: { accountID?: string } }[] }).bindings?.[0]?.match
          ?.accountID,
    });
  });
  it("accepts bindings[].comment on load", () => {
    expectValidConfigValue({
      ...
      readValue: (config) =>
        (config as { bindings?: { comment?: string }[] }).bindings?.[0]?.comment,
    });
  });
    ...
    await withSnapshotForConfig(
      ...
      async (ctx) => {
        ...
        const parsed = ctx.parsed as {
          session?: { sendPolicy?: { rules?: { match?: { provider?: string } }[] } };
        };
