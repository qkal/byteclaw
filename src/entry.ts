...
const ENTRY_WRAPPER_PAIRS = [
  ...
  { entryBasename: 'entry.js', wrapperBasename: 'openclaw.js' },
] as const;
...
if (
  ...
  const { installGaxiosFetchCompat } =
    await import('./infra/gaxios-fetch-compat.js');
  ...
  const { validateEnvOrThrow } = await import('./infra/env-validation.js');
  try {
    validateEnvOrThrow();
  } catch (error) {
    console.error(
      '[openclaw] Environment validation failed:',
  ...
  if (!ensureCliRespawnReady()) {
    ...
    if (containerTargetName && parsed.profile) {
      console.error(
        '[openclaw] --container cannot be combined with --profile/--dev',
      );
      process.exit(2);
    }
