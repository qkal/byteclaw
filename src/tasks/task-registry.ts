...
export async function maybeDeliverTaskTerminalUpdate(taskId: string): Promise<TaskRecord | null> {
  ...
  try {
    ...
    const preferred = latest.runId
      ...
      : undefined;
    if (
      shouldSuppressDuplicateTerminalDelivery({ preferredTaskId: preferred?.taskId, task: latest })
    ) {
...
export async function cancelTaskById(params: {
  ...
  if (!task) {
    return { cancelled: false, found: false, reason: "Task not found." };
  }
  if (
