...
describe("logger helpers", () => {
  ...
  it("formats messages through runtime log/error", () => {
    const log = vi.fn();
    const error = vi.fn();
    const runtime: RuntimeEnv = { error, exit: vi.fn(), log };
  ...
  it("filters messages below configured level", () => {
    withTempDirSync({ prefix: "openclaw-log-test-" }, (dir) => {
      const logPath = path.join(dir, "openclaw.log");
      setLoggerOverride({ file: logPath, level: "warn" });
      logInfo("info-only");
      logWarn("warn-only");
      const content = fs.readFileSync(logPath, "utf8");
  ...
  it("uses daily rolling default log file and prunes old ones", () => {
    ...
    expect(fs.readFileSync(todayPath, "utf8")).toContain("roll-me");
...
describe("stripRedundantSubsystemPrefixForConsole", () => {
  it.each([
    { expected: "hello", input: "discord: hello", subsystem: "discord" },
    { expected: "hello", input: "WhatsApp: hello", subsystem: "whatsapp" },
    { expected: "gateway: closed", input: "discord gateway: closed", subsystem: "discord" },
    {
