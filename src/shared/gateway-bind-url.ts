...
export function resolveGatewayBindUrl(params: {
  ...
  if (bind === "custom") {
    ...
    if (host) {
      return { source: "gateway.bind=custom", url: `${params.scheme}://${host}:${params.port}` };
    }
  ...
  if (bind === "tailnet") {
    ...
    if (host) {
      return { source: "gateway.bind=tailnet", url: `${params.scheme}://${host}:${params.port}` };
    }
  ...
  if (bind === "lan") {
    ...
    if (host) {
      return { source: "gateway.bind=lan", url: `${params.scheme}://${host}:${params.port}` };
    }
