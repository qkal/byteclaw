...
describe("avatar policy", () => {
  it("classifies avatar URI and path helpers directly", () => {
    ...
    expect(isWindowsAbsolutePath(String.raw`C:\\avatars\\openclaw.png`)).toBe(true);
  });
  ...
  it("accepts workspace-relative avatar paths and rejects URI schemes", () => {
    ...
    expect(isWorkspaceRelativeAvatarPath(String.raw`C:\\avatars\\openclaw.png`)).toBe(true);
