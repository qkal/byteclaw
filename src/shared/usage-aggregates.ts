...
>(params: {
  ...
  return {
    byChannel: [...params.byChannelMap.entries()]
      .map(([channel, totals]) => ({ channel, totals }))
      .toSorted((a, b) => b.totals.totalCost - a.totals.totalCost),
    daily: [...params.dailyMap.values()].toSorted((a, b) => a.date.localeCompare(b.date)),
    dailyLatency: [...params.dailyLatencyMap.values()]
      .map((entry) => ({
    ...
    latency:
      params.latencyTotals.count > 0
        ...
        : undefined,
    modelDaily: [...params.modelDailyMap.values()].toSorted(
