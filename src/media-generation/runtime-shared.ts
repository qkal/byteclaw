...
export interface ParsedProviderModelRef {
  provider: string;
  model: string;
}
...
export interface MediaGenerationNormalizationMetadataInput {
...
interface CapabilityProviderCandidate {
  id: string;
...
export function buildNoCapabilityModelConfiguredMessage(params: {
  ...
  providers: { id: string; defaultModel?: string | null }[];
