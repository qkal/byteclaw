...
function createHarness(sessions: { sessionId: string; sessionKey: string }[]): Harness {
  ...
  it("projects gateway thinking blocks into hidden ACP thought chunks", async () => {
      ...
      createChatEvent({
        message: {
          content: [
            ...
            { text: "Final visible reply", type: "text" },
          ],
        },
        runId: pending.runId,
        seq: 1,
    ...
    expect(harness.sessionUpdateSpy).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        sessionId: "session-1",
        update: expect.objectContaining({
          content: { text: "Final visible reply", type: "text" },
