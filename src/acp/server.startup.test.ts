...
interface GatewayClientCallbacks {
  onHelloOk?: () => void;
  onConnectError?: (err: Error) => void;
  onClose?: (code: number, reason: string) => void;
}

interface GatewayClientAuth {
  token?: string;
  password?: string;
}
...
class MockGatewayClient {
  ...
  constructor(opts: GatewayClientCallbacks & GatewayClientAuth) {
    this.callbacks = opts;
    mockState.gatewayAuth.push({ password: opts.password, token: opts.token });
