...
export function classifyAcpToolApproval(params: {
  ...
  if (!toolName) {
    return { approvalClass: "unknown", autoApprove: false, toolName: undefined };
  }
  ...
  if (SAFE_SEARCH_TOOL_IDS.has(toolName) && isTrustedToolId) {
    return { approvalClass: "readonly_search", autoApprove: true, toolName };
  }
  const ownerOnlyApprovalClass = resolveOwnerOnlyToolApprovalClass(toolName);
  if (ownerOnlyApprovalClass) {
    return { approvalClass: ownerOnlyApprovalClass, autoApprove: false, toolName };
  }
  if (EXEC_CAPABLE_TOOL_IDS.has(toolName)) {
    return { approvalClass: "exec_capable", autoApprove: false, toolName };
  }
  if (CONTROL_PLANE_TOOL_IDS.has(toolName)) {
    return { approvalClass: "control_plane", autoApprove: false, toolName };
  }
  if (isMutatingToolCall(toolName, params.toolCall?.rawInput)) {
    return { approvalClass: "mutating", autoApprove: false, toolName };
  }
  return { approvalClass: "other", autoApprove: false, toolName };
}
