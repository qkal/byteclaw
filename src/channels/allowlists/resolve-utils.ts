...
export interface AllowlistUserResolutionLike {
  input: string;
  resolved: boolean;
  id?: string;
}
...
export function buildAllowlistResolutionSummary<T extends AllowlistUserResolutionLike>(
  ...
  return { additions, mapping, resolvedMap, unresolved };
}
...
>(params: { existing?: (string | number)[]; resolvedMap: Map<string, T> }): string[] {
