...
export interface CreateTypingCallbacksParams {
  start: () => Promise<void>;
  stop?: () => Promise<void>;
...
export function createTypingCallbacks(params: CreateTypingCallbacksParams): TypingCallbacks {
  ...
  const fireStop = () => {
    ...
    stopSent = true;
    void stop().catch((error) => (params.onStopError ?? params.onStartError)(error));
  };

  return { onCleanup: fireStop, onIdle: fireStop, onReplyStart };
}
