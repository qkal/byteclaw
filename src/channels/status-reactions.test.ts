  ...
  return {
    adapter: {
      removeReaction: vi.fn(async (emoji: string) => {
        calls.push({ emoji, method: "remove" });
      }),
      setReaction: vi.fn(async (emoji: string) => {
        calls.push({ emoji, method: "set" });
      }),
...
function expectSetEmojiCall(calls: { method: string; emoji: string }[], emoji: string) {
  expect(calls).toContainEqual({ emoji, method: "set" });
}
...
describe("resolveToolEmoji", () => {
  it.each([
    { expected: DEFAULT_EMOJIS.coding, name: "returns coding emoji for exec tool", tool: "exec" },
    {
    ...
    { expected: DEFAULT_EMOJIS.web, name: "returns web emoji for browser tool", tool: "browser" },
    {
      expected: DEFAULT_EMOJIS.tool,
      name: "returns tool emoji for unknown tool",
      tool: "unknown_tool",
    },
    { expected: DEFAULT_EMOJIS.tool, name: "returns tool emoji for empty string", tool: "" },
    { expected: DEFAULT_EMOJIS.tool, name: "returns tool emoji for undefined", tool: undefined },
    { expected: DEFAULT_EMOJIS.coding, name: "is case-insensitive", tool: "EXEC" },
    {
      expected: DEFAULT_EMOJIS.coding,
      name: "matches tokens within tool names",
      tool: "my_exec_wrapper",
    },
  ] satisfies { name: string; tool: string | undefined; expected: string }[])(
    "should $name",
    ({ tool, expected }) => {
...
describe("createStatusReactionController", () => {
  ...
  it("should call removeReaction when adapter supports it and emoji changes", async () => {
    ...
    expectSetEmojiCall(calls, DEFAULT_EMOJIS.thinking);
    expect(calls).toContainEqual({ emoji: "👀", method: "remove" });
  });
