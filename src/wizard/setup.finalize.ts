...
interface FinalizeOnboardingOptions {
  flow: WizardFlow;
  opts: OnboardOptions;
...
export async function finalizeSetupWizard(
  ...
  if (installDaemon) {
    ...
    if (
      ...
      try {
        ...
        installError = formatErrorMessage(error);
      } finally {
        progress.stop(
  ...
  if (!opts.skipHealth) {
    ...
    if (gatewayProbe.ok) {
      try {
        ...
        runtime.error(formatHealthCheckFailure(error));
        await prompter.note(
          [
            "Docs:",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    hatchChoice = await prompter.select({
      ...
      options: [
        { label: "Hatch in TUI (recommended)", value: "tui" },
        { label: "Open the Web UI", value: "web" },
