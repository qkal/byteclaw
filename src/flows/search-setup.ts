...
function buildSearchEnvRef(config: OpenClawConfig, provider: SearchProvider): SecretRef {
  ...
  return { id: resolvedEnvVar, provider: DEFAULT_SECRET_PROVIDER_ALIAS, source: "env" };
}
...
export function applySearchKey(
  ...
  const search: MutableSearchConfig = { ...config.tools?.web?.search, enabled: true, provider };
